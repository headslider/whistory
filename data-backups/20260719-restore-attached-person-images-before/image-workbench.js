...
const historyData = clone(window.historyContentData || {});
  ...
  function modalPersonRecords() {
    const map = new Map();
    for (const person of peopleData.people || []) if (personName(person) && !map.has(personName(person))) map.set(personName(person), person);
    for (const person of Object.values(peopleData.peopleByName || {})) if (personName(person) && !map.has(personName(person))) map.set(personName(person), person);
    return [...map.values()];
  }
  ...
  function syncModalPersonVisuals() {
    ...
    for (const person of peopleData.people || []) collect(person);
    for (const person of Object.values(peopleData.peopleByName || {})) collect(person);
