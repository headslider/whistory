...
const forbiddenStandaloneRubyTerms = ["農業", "文明", "文字", "仏教", "鉄道", "戦争", "王", "村", "道", "核", "港", "海", "銀"];
...
const fivePassTooltipOnlyTerms = fivePassRequiredTooltipTerms.filter((term) => !["封建的特権", "水力紡績機"].includes(term));
...
for (const term of broadAuditRequiredTooltipTerms) {
  ...
  if (learningTerms.terms[term]?.reading || learningTerms.terms[term]?.ruby) fail(`${term} should be tooltip-only after broad audit`);
}
