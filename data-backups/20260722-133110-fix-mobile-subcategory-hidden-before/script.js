...
function syncTimelinePlacement() {
  ...
  if (isMobileTimelineMode()) {
    if (timeline.parentElement !== timelineDialogBody) timelineDialogBody.appendChild(timeline);
    return;
  }
...
function navigateToEraGroup(groupId) {
  if (isMobileTimelineMode()) openMobileTimelineDialog();
  ...
  const scrollTarget = isMobileTimelineMode() ? timelineDialogBody : target;
  target.scrollIntoView({ behavior: motionShouldReduce() ? "auto" : "smooth", block: "start" });
  if (isMobileTimelineMode() && scrollTarget) {
    window.requestAnimationFrame(() => {
      const containerRect = timelineDialogBody.getBoundingClientRect();
      const targetRect = target.getBoundingClientRect();
      timelineDialogBody.scrollTo({
        top: timelineDialogBody.scrollTop + targetRect.top - containerRect.top - 12,
        behavior: motionShouldReduce() ? "auto" : "smooth"
      });
    });
  }
