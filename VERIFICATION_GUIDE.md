...
- 人物図鑑に表示される271件以外にも、本文リンクや勢力カードから開ける人物がある。
...
## 2026-07-22 画像管理画面の人物対象チェック

- 画像管理画面の人物一覧は `people-data.json` の `people` 配列だけを対象にする。
- `peopleByName` だけに存在する補助人物、例: アル・フワーリズミー、を人物画像管理一覧に出してはいけない。
- 保存処理は人物画像更新対象を正式 `people` 配列から探し、保存後に同名の `peopleByName` へ画像情報を同期する。
- 検証では `people.length` と管理対象人物数が一致し、人物画像の重複参照が0件であることを確認する。