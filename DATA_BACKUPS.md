...
## 20260722-regional-timeline-label-ruby-chip-before
- 保存先: data-backups/20260722-regional-timeline-label-ruby-chip-before/
- 内容: 地域勢力タイムラインの勢力名ラベルでルビと白い背景チップが重なり、表示が崩れる問題を修正する前の styles.css / index.html / DATA_BACKUPS.md。
- 注意: 表示CSSとCSSキャッシュ更新のみ。history-content と people-data は変更していない。子カテゴリー本文を短文化・復元しないこと。

## 2026-07-22 mobile-year-chip-safe-inset-before
- 保存先: data-backups/20260722-mobile-year-chip-safe-inset-before/
- 内容: 携帯表示の左年代チップ安全余白修正前の styles.css / index.html / DATA_BACKUPS.md。
- 注意: 子カテゴリー本文データは変更していない。


## 2026-07-23 mobile-year-chip-dom-before
- 保存先: data-backups/20260723-mobile-year-chip-dom-before/
- 内容: 携帯用年代チップを実体DOM化する前の styles.css / script.js / index.html / DATA_BACKUPS.md。
- 注意: 子カテゴリー本文データは変更していない。


## 2026-07-23 mobile-year-chip-restore-before
- 保存先: data-backups/20260723-mobile-year-chip-restore-before/
- 内容: 携帯用年代チップの大カテゴリー/時代カード両表示復元前の styles.css / index.html / DATA_BACKUPS.md。
- 注意: 子カテゴリー本文データは変更していない。


## 2026-07-27 heading-tooltip-link-removal-before
- 保存先: `data-backups/20260727-heading-tooltip-link-removal-before/`
- 対象: `script.js`, `index.html`, `DATA_BACKUPS.md`
- 内容: 大カテゴリー・時代カード・子カテゴリー見出し/補足領域からツールチップとカードリンクを排除する前のバックアップ。
- 復元注意: `history-content` 本文データは変更していない。復元時も子カテゴリー本文の短文化禁止条件を必ず検証する。

## 2026-07-27 action-cards-strictly-revised-v2-before
- 保存先: data-backups/20260727-action-cards-strictly-revised-v2-before/
- 対象: data/action-cards.json, data/action-cards.js, DATA_BACKUPS.md
- 内容: C:\Users\tamak\OneDrive\Desktop\action-cards_world-history_strictly_revised_v2.json へ置き換える前のバックアップ。
- 復元注意: 復元後は data/action-cards.json と data/action-cards.js の同期、history-content 本文保護条件、人物画像重複なしを再検証する。

## 2026-07-27 short-tooltip-compound-cover-before
- 保存先: data-backups/20260727-short-tooltip-compound-cover-before/
- 対象: data/learning-terms.json, data/learning-terms.js, DATA_BACKUPS.md
- 内容: 短い専門語ツールチップが長い漢字複合語を分割する監査警告へ対応する前のバックアップ。
- 復元注意: 復元後は scripts/verify-static.js で短語分割警告が再発しないか必ず確認する。
