...
| `allRecords()` | 人物271件、アクション111件、子カテゴリー53件(全435件)を一覧化する。 |
...
## 2026-07-22 スマホUI・子カテゴリー開閉・ツールチップ

- script.js の setupTermTooltips() はPCのホバー/フォーカスとスマホのタップ/長押しを統合して扱う。.term-tooltip-layer は常に document.body 直下へ置き、モーダル内テキストでも下に隠れないようにする。
- enderActionSubcategories() は .event-subcategory-card と .subcategory-description-toggle を出力する。スマホではシェブロンボタンだけが本文開閉を担当する。
- styles.css 末尾の 2026-07-22 mobile UI, subcategory disclosure, modal, and tooltip interaction port ブロックが、日本史準拠のスマホトップバー、子カテゴリー開閉、モーダル圧縮、ツールチップ表示を上書きする。
- 王国カードの3Dマップ用 .kingdom-detail 基本構造は変更せず、人物・アクション系モーダルのスマホ表示だけを主対象にする。

## 2026-07-22 日本史UI引き継ぎメモ

- 上部メニューは `index.html` の `menuButton` / `eraDrawer` / `eraLinks` をページ目次、`optionOpenButton` / `optionDrawer` / `optionLinks` をオプションメニューとして分離する。旧 `rubyToggle`、`motionToggle`、`soundToggle` は本体HTMLから外し、`script.js` でも直接参照しない。
- `script.js` の `renderEraLinks()` は目次リンクHTMLを `eraLinks` と `optionLinks` の両方へ同期する。目次内の大カテゴリーリンクは `navigateToEraGroup()` で開閉とスクロールを制御する。
- `openEraDetail()` はPCでは従来通り `.fact-grid` 末尾へ `.inline-detail` を入れ、スマホでは選択した `くらし` / `できごと` / `大きな力` カードの直下へ入れる。
- 各地域勢力タイムラインはPCでは本文内に埋め込み表示し、スマホでは `.regional-timeline-open` ボタンから `regionalTimelineDialog` へ複製表示する。モーダル本体は縦スクロールのみ、横スクロールは `.regional-timeline` 自身だけが担当する。

## 2026-07-22 画像管理画面の人物対象

- 人物画像管理画面の一覧対象は `data/people-data.json` の正式 `people` 配列のみ。
- `peopleByName` は検索・リンク補助用の参照データであり、画像管理画面の人物候補としては扱わない。
- 保存時は正式 `people` 側の画像情報を同名の `peopleByName` に同期するが、`peopleByName` だけに残る人物を新規保存対象にしない。