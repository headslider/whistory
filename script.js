...
const timelineHome = document.getElementById("timelineHome");
const menuButton = document.getElementById("menuButton");
const eraDrawer = document.getElementById("eraDrawer");
const closeDrawer = document.getElementById("closeDrawer");
const eraLinks = document.getElementById("eraLinks");
const optionOpenButton = document.getElementById("optionOpenButton");
const optionDrawer = document.getElementById("optionDrawer");
const closeOptionDrawer = document.getElementById("closeOptionDrawer");
const optionLinks = document.getElementById("optionLinks");
const openTimelineDialogButton = document.getElementById("openTimelineDialog");
const timelineDialog = document.getElementById("timelineDialog");
const closeTimelineDialogButton = document.getElementById("closeTimelineDialog");
const timelineDialogBody = document.getElementById("timelineDialogBody");
const regionalTimelineDialog = document.getElementById("regionalTimelineDialog");
const closeRegionalTimelineDialogButton = document.getElementById("closeRegionalTimelineDialog");
const regionalTimelineDialogBody = document.getElementById("regionalTimelineDialogBody");
const regionalTimelineDialogTitle = document.getElementById("regionalTimelineDialogTitle");
const mobileTimelineMedia = window.matchMedia("(max-width: 760px)");
...
function normalizeLearningTermsData(data) {
  ...
  Object.entries(terms).forEach(([term, value]) => {
    ...
    const rubyConfig = value.ruby && typeof value.ruby === "object" ? value.ruby : null;
    const rubyBase = typeof rubyConfig?.base === "string" && rubyConfig.base ? rubyConfig.base : term;
    const reading = typeof rubyConfig?.reading === "string" && rubyConfig.reading ? rubyConfig.reading : (typeof value.reading === "string" ? value.reading : "");
    const scopes = Array.isArray(value.scopes) ? value.scopes : [];
    const targetScopes = scopes.length ? scopes : ["study", "kingdom"];
    if (reading && reading !== rubyBase) {
      if (targetScopes.includes("study")) manualStudyRuby[rubyBase] = reading;
      if (targetScopes.includes("kingdom")) manualKingdomRuby[rubyBase] = reading;
      if (targetScopes.includes("person")) manualPersonRuby[rubyBase] = reading;
    }
...
function termTooltipHtml(word, innerHtml, options = {}) {
  ...
  return `<span class="term-tooltip" tabindex="0" role="button" aria-expanded="false" data-term="${escapeHtml(word)}" data-title="${escapeHtml(title)}" data-body="${escapeHtml(body)}">${innerHtml}</span>`;
}
...
function termTooltipOwner(trigger) {
  return trigger?.closest?.("dialog[open]") || document.body;
}

function ensureTermTooltipLayer(owner = null) {
  if (!termTooltipLayer) {
  ...
  const targetOwner = owner || termTooltipLayer.parentElement || document.body;
  if (termTooltipLayer.parentElement !== targetOwner) targetOwner.appendChild(termTooltipLayer);
  return termTooltipLayer;
}

function closestTermTooltip(target) {
  return target instanceof Element ? target.closest(".term-tooltip") : null;
}

function positionTermTooltip(trigger) {
  if (!trigger) return;
  const layer = ensureTermTooltipLayer(termTooltipOwner(trigger));
  const margin = 12;
  const gap = 12;
  const rect = trigger.getBoundingClientRect();
  const layerRect = layer.getBoundingClientRect();
  const width = layerRect.width || Math.min(360, window.innerWidth - 24);
  const height = layerRect.height || 120;
...
function showTermTooltip(trigger) {
  ...
  const layer = ensureTermTooltipLayer(termTooltipOwner(trigger));
  if (activeTermTooltip && activeTermTooltip !== trigger) {
    activeTermTooltip.setAttribute("aria-expanded", "false");
  }
  activeTermTooltip = trigger;
  trigger.setAttribute("aria-expanded", "true");
  layer.innerHTML = `<strong>${escapeHtml(title)}</strong><span>${applyStudyRuby(body, { disableTooltips: true })}</span>`;
  layer.setAttribute("aria-hidden", "false");
  positionTermTooltip(trigger);
  layer.classList.add("visible");
}

function hideTermTooltip(trigger) {
  if (trigger && activeTermTooltip && trigger !== activeTermTooltip) return;
  const layer = ensureTermTooltipLayer();
  if (activeTermTooltip) activeTermTooltip.setAttribute("aria-expanded", "false");
  activeTermTooltip = null;
  layer.classList.remove("visible", "above");
  layer.setAttribute("aria-hidden", "true");
}

function toggleTermTooltip(trigger) {
  if (!trigger) return;
  if (activeTermTooltip === trigger && ensureTermTooltipLayer().classList.contains("visible")) {
    hideTermTooltip(trigger);
    return;
  }
...
function setupTermTooltips() {
  ...
  document.addEventListener("pointerout", (event) => {
    ...
    if (related && (trigger.contains(related) || ensureTermTooltipLayer().contains(related))) return;
    hideTermTooltip(trigger);
  });
  ...
  document.addEventListener("focusout", (event) => {
    const trigger = closestTermTooltip(event.target);
    if (trigger) hideTermTooltip(trigger);
  });
  document.addEventListener("pointerdown", (event) => {
    const trigger = closestTermTooltip(event.target);
    if (trigger || event.target.closest?.(".term-tooltip-layer")) event.stopPropagation();
  }, true);
  document.addEventListener("click", (event) => {
    ...
    if (activeTermTooltip) hideTermTooltip(activeTermTooltip);
  }, true);
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && activeTermTooltip) hideTermTooltip(activeTermTooltip);
  }, true);
...
function renderEraLinks() {
  ...
  const linksHtml = contentLinks.map((link) => {
  ...
  eraLinks.innerHTML = linksHtml;
  if (optionLinks) optionLinks.innerHTML = linksHtml;
}
...
function renderRegionalPowerTimeline(era, eraKingdoms, options = {}) {
  ...
  return `
    <button class="regional-timeline-open" type="button" aria-label="${escapeHtml(heading)}をモーダルで開く">${heading}を開く</button>
    ...
    <section class="action-subcategory-section" aria-label="${group.title}の地域別子カテゴリー">
      ...
      <div class="subcategory-list">
        ${subcategories.map((subcategory) => {
          ...
          return `
            <article class="action-subcategory-card event-subcategory-card">
              <div class="subcategory-card-body">
                ...
                <header>
                  ...
                  <h4><strong>${applyStudyRuby(name, { disableTooltips: true })}</strong></h4>
...
function renderEraCard(era) {
  ...
  return `
    <article class="era" id="era-${era.id}" data-era="${era.name}" data-icon="${era.icon}" data-western-year="${era.westernYear}" style="--era-a:${era.colors[0]};--era-b:${era.colors[1]};color:${era.colors[0]}">
      ...
      <div class="era-card">
        <header class="era-head"><p class="eyebrow">${era.years}</p><h2>${era.name}</h2><p>${applyStudyRuby(era.question, { disableTooltips: true })}</p></header>
        ...
        <div class="era-body">
          ...
          <details class="era-kingdoms" aria-label="${era.name}の王国・勢力・その他">
            <summary class="era-kingdoms-head">
              <h3>できた王国・勢力・その他</h3>
...
function isMobileTimelineMode() {
  return mobileTimelineMedia.matches;
}

function syncTimelinePlacement() {
  if (!timeline || !timelineHome || !timelineDialogBody) return;
  if (!isMobileTimelineMode() && timelineDialog?.open) timelineDialog.close();
  if (timeline.parentElement !== timelineHome) timelineHome.appendChild(timeline);
  if (!timelineDialog?.open) timelineDialogBody.replaceChildren();
}

function cloneTimelineForDialog() {
  if (!timeline || !timelineDialogBody) return null;
  const clone = timeline.cloneNode(true);
  clone.removeAttribute("id");
  clone.setAttribute("aria-label", "縦スクロール年表（モーダル表示）");
  ...
  timelineDialogBody.replaceChildren(clone);
  return clone;
}

function openMobileTimelineDialog() {
  if (!timelineDialog || !isMobileTimelineMode()) return false;
  syncTimelinePlacement();
  cloneTimelineForDialog();
  if (!timelineDialog.open) timelineDialog.showModal();
  return true;
}

function closeMobileTimelineDialog() {
  if (timelineDialog?.open) timelineDialog.close();
}

function openRegionalTimelineDialog(openButton) {
  if (!regionalTimelineDialog || !regionalTimelineDialogBody) return false;
  const timelineSection = openButton?.nextElementSibling?.classList?.contains("regional-timeline")
    ? openButton.nextElementSibling
    : openButton?.closest(".action-subcategory-card, .era-card")?.querySelector(".regional-timeline");
  if (!timelineSection) return false;
  const clone = timelineSection.cloneNode(true);
  clone.querySelectorAll("[id]").forEach((node) => node.removeAttribute("id"));
  regionalTimelineDialogBody.replaceChildren(clone);
  const heading = timelineSection.querySelector(".regional-timeline-head h3")?.textContent?.trim() || "地域の勢力を見る";
  if (regionalTimelineDialogTitle) regionalTimelineDialogTitle.textContent = heading;
  if (!regionalTimelineDialog.open) regionalTimelineDialog.showModal();
  regionalTimelineDialogBody.scrollTo({ top: 0, left: 0, behavior: "auto" });
  return true;
}

function closeRegionalTimelineDialog() {
  if (regionalTimelineDialog?.open) regionalTimelineDialog.close();
}

function setupRegionalTimelineDialog() {
  closeRegionalTimelineDialogButton?.addEventListener("click", closeRegionalTimelineDialog);
  regionalTimelineDialog?.addEventListener("click", (event) => {
    if (event.target === regionalTimelineDialog) closeRegionalTimelineDialog();
  });
  regionalTimelineDialog?.addEventListener("close", () => {
    regionalTimelineDialogBody?.replaceChildren();
  });
}

function setupMobileTimelineDialog() {
  syncTimelinePlacement();
  openTimelineDialogButton?.addEventListener("click", () => {
    openMobileTimelineDialog();
    timelineDialogBody?.scrollTo({ top: 0, behavior: "auto" });
  });
  closeTimelineDialogButton?.addEventListener("click", closeMobileTimelineDialog);
  timelineDialog?.addEventListener("click", (event) => {
    if (event.target === timelineDialog) closeMobileTimelineDialog();
  });
  timelineDialog?.addEventListener("close", () => {
    closeEraDetail();
  });
  mobileTimelineMedia.addEventListener?.("change", syncTimelinePlacement);
  window.addEventListener("resize", syncTimelinePlacement);
}
...
function openEraDetail(button) {
  ...
  panel.innerHTML = `<div class="detail-panel-head"><div><p class="eyebrow">${applyStudyRuby(era.name, { disableTooltips: true })}</p><h3>${applyStudyRuby(detail.title, { disableTooltips: true })}</h3></div></div><p>${enrichDetailLinks(detail.text)}</p>`;
  if (isMobileTimelineMode()) {
    item.after(panel);
  } else {
...
function setEraDrawerOpen(open) {
  if (!eraDrawer || !menuButton) return;
  eraDrawer.classList.toggle("open", open);
  menuButton.setAttribute("aria-expanded", open ? "true" : "false");
  menuButton.setAttribute("aria-label", open ? "ページ目次を閉じる" : "ページ目次を開く");
}

function setOptionDrawerOpen(open) {
  if (!optionDrawer || !optionOpenButton) return;
  optionDrawer.classList.toggle("open", open);
  optionOpenButton.setAttribute("aria-expanded", open ? "true" : "false");
}

function handleDrawerLinkClick(event, closeMenu) {
...
menuButton?.addEventListener("click", () => setEraDrawerOpen(!eraDrawer?.classList.contains("open")));
closeDrawer?.addEventListener("click", () => setEraDrawerOpen(false));
eraLinks?.addEventListener("click", (event) => handleDrawerLinkClick(event, () => setEraDrawerOpen(false)));
optionOpenButton?.addEventListener("click", () => setOptionDrawerOpen(!optionDrawer?.classList.contains("open")));
closeOptionDrawer?.addEventListener("click", () => setOptionDrawerOpen(false));
optionLinks?.addEventListener("click", (event) => handleDrawerLinkClick(event, () => setOptionDrawerOpen(false)));
closePerson.addEventListener("click", () => {
  cleanupKingdomGlobe();
  personDialog.close();
});
personDialog.addEventListener("close", () => { hideTermTooltip(activeTermTooltip); cleanupKingdomGlobe(); });
...
timeline.addEventListener("click", (event) => {
  const regionalTimelineButton = event.target.closest(".regional-timeline-open");
  if (regionalTimelineButton) {
    event.preventDefault();
    event.stopPropagation();
    openRegionalTimelineDialog(regionalTimelineButton);
    return;
  }
...
async function initApp() {
  ...
  renderEraLinks();
  renderTimeline();
  setupMobileTimelineDialog();
  setupRegionalTimelineDialog();
  setupTermTooltips();
  renderPeopleFilters();
  renderPeople();
  renderQuiz();
