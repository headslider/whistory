...
let subcategoryImageFocusUp = new Set();
...
const rubyGlossary = {
  "農業": "のうぎょう", "牧畜": "ぼくちく", "文明": "ぶんめい", "都市": "とし", "文字": "もじ", "法律": "ほうりつ", "王朝": "おうちょう", "帝国": "ていこく", "交易": "こうえき", "宗教": "しゅうきょう", "仏教": "ぶっきょう", "キリスト教": "きりすときょう", "イスラム教": "いすらむきょう", "植民地": "しょくみんち", "奴隷貿易": "どれいぼうえき", "革命": "かくめい", "産業革命": "さんぎょうかくめい", "帝国主義": "ていこくしゅぎ", "国際連合": "こくさいれんごう", "人権": "じんけん", "冷戦": "れいせん", "環境": "かんきょう", "技術": "ぎじゅつ", "先住民": "せんじゅうみん", "公民権運動": "こうみんけんうんどう", "感染症": "かんせんしょう"
};
  ...
  groups.forEach((group) => {
    (group.eras || []).forEach((era) => {
      (era.subcategories || []).forEach((subcategory) => {
        ...
        if (subcategory.imageFocus === "up") focusUpNames.push(subcategory.title);
      });
    });
  });
  subcategoryImageFocusUp = new Set(focusUpNames);
}
...
function applyStudyRuby(text) {
  ...
  const readings = new Map([...Object.entries(rubyGlossary), ...people.map((person) => [person[0], person[1]])]);
  const words = [...readings.keys()]
    .filter((word) => /[\u3400-\u9fff]/.test(word) && !/[\u30a0-\u30ff]/.test(word) && readings.get(word) !== word && source.includes(word))
...
function enrichDetailLinks(text) {
  ...
  for (const match of source.matchAll(pattern)) {
    ...
    if (shouldSkipInlineLink(source, name, match.index, item)) {
      ...
      const label = applyStudyRuby(name);
    ...
    <section class="action-subcategory-section" aria-label="${group.title}の地域別子カテゴリー">
      ...
      <div class="subcategory-list">
        ${subcategories.map((subcategory) => {
          ...
          const description = subcategoryDescription(subcategory);
          const imagePath = subcategoryImages[name];
          const imageClass = subcategoryImageFocusUp.has(name) ? "subcategory-image subcategory-image-up" : "subcategory-image";
          ...
          return `
            <article class="action-subcategory-card">
              ...
              ${imageHtml}
              <p class="subcategory-description">${enrichDetailLinks(description)}</p>
        ...
        <div class="era-body">
          ...
          <section class="era-kingdoms" aria-label="${era.name}の王国・勢力・その他">
            ...
            <div class="kingdom-list">
              ${eraKingdoms.map((card) => `<button class="kingdom-chip" type="button" onclick="openKingdom('${card.id}')" aria-label="${card.displayName}の王国カードを開く"><span aria-hidden="true">◆</span>${applyStudyRuby(card.displayName)}</button>`).join("") || "<p>この時代の王国・勢力・その他カードを準備中です。</p>"}
            </div>
          </section>
...
function openKingdom(id) {
  ...
  personDetail.innerHTML = `
    ...
    <h2>${applyStudyRuby(card.displayName)}</h2>
...
function openPerson(name) {
  ...
  const genreTags = getPersonGenres(person).map((genre) => `<span class="tag">${applyStudyRuby(genre.label)}</span>`).join("");
  personDetail.innerHTML = `<div class="detail-avatar">${person[6]}</div><p class="eyebrow">${applyStudyRuby(person[2])}</p><h2>${ruby(person[0], person[1])}</h2><p><strong>${applyStudyRuby(person[4])}</strong></p><p>${applyStudyRuby(person[5])}</p><div class="tag-row"><span class="tag">${applyStudyRuby(person[3])}</span><span class="tag">${applyStudyRuby(person[2])}</span>${genreTags}</div>`;
  showPersonDialog();
}
...
function openAction(name) {
  ...
  personDetail.className = "";
  personDetail.innerHTML = `<div class="detail-avatar">💡</div><p class="eyebrow">アクションカード</p><h2>${applyStudyRuby(name)}</h2><p><strong>${applyStudyRuby(summary)}</strong></p><p>${enrichDetailLinks(text)}</p><div class="tag-row">${tags.map((tag) => `<span class="tag">${applyStudyRuby(tag)}</span>`).join("")}<span class="tag">もっと知る</span></div>`;
  showPersonDialog();
}
...
personDialog.addEventListener("click", handleInlineCardLink);
