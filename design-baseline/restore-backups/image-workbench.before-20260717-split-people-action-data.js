...
const historyData = clone(window.historyContentData || {});
  ...
  function trimEmptyVisualMeta(target, index) {
    ...
    if (!meta) return;
    if (!meta.image && !meta.imageFocus && !meta.imageAlt) target.splice(index, 1);
  }
  function decorateVisualTarget(target, index) {
    if (!target || !Array.isArray(target) || target.__imageWorkbenchDecorated) return target;
    Object.defineProperties(target, {
      image: { configurable: true, get() { return maybeVisualMeta(this, index)?.image || ""; }, set(value) { if (value) visualMeta(this, index).image = value; else { const meta = maybeVisualMeta(this, index); if (meta) delete meta.image; trimEmptyVisualMeta(this, index); } } },
      imageFocus: { configurable: true, get() { return maybeVisualMeta(this, index)?.imageFocus || ""; }, set(value) { if (value) visualMeta(this, index).imageFocus = value; else { const meta = maybeVisualMeta(this, index); if (meta) delete meta.imageFocus; trimEmptyVisualMeta(this, index); } } },
      imageAlt: { configurable: true, get() { return maybeVisualMeta(this, index)?.imageAlt || ""; }, set(value) { if (value) visualMeta(this, index).imageAlt = value; else { const meta = maybeVisualMeta(this, index); if (meta) delete meta.imageAlt; trimEmptyVisualMeta(this, index); } } },
      __imageWorkbenchDecorated: { value: true, enumerable: false }
    });
    return target;
  }
  function modalPersonRecords() {
    const map = new Map();
    for (const person of modalData.people || []) if (Array.isArray(person) && person[0] && !map.has(person[0])) map.set(person[0], person);
    for (const person of Object.values(modalData.peopleByName || {})) if (Array.isArray(person) && person[0] && !map.has(person[0])) map.set(person[0], person);
    return [...map.values()];
  }
  function decorateModalVisualTargets() {
    for (const person of modalPersonRecords()) decorateVisualTarget(person, 7);
    for (const action of Object.values(modalData.actionCards || {})) decorateVisualTarget(action, 3);
  }
  ...
  function applyVisualFields(target, index, fields) {
    if (!target || !Array.isArray(target)) return;
    if (fields.image || fields.imageFocus || fields.imageAlt) {
      ...
      const meta = maybeVisualMeta(target, index);
      if (meta) target.splice(index, 1);
    }
    decorateVisualTarget(target, index);
  }
  function syncModalPersonVisuals() {
    const peopleByName = new Map();
    const collect = (person) => {
      if (!Array.isArray(person) || !person[0]) return;
      if (!peopleByName.has(person[0])) peopleByName.set(person[0], []);
      peopleByName.get(person[0]).push(person);
    };
    for (const person of modalData.people || []) collect(person);
    for (const person of Object.values(modalData.peopleByName || {})) collect(person);
    for (const copies of peopleByName.values()) {
      const fields = copies.map((person) => visualFields(person, 7)).find((item) => item.image || item.imageFocus || item.imageAlt) || { image: "", imageFocus: "", imageAlt: "" };
      for (const person of copies) applyVisualFields(person, 7, fields);
    }
  ...
  function allRecords() {
    ...
    const people = modalPersonRecords().map((person) => ({
      key: `person:${person[0]}`, type: "person", typeLabel: "人物カード", id: person[0], title: person[0],
      meta: [person[2], person[3], person[4]].filter(Boolean).join(" / "),
      body: [person[5]].filter(Boolean).join(" "),
      target: decorateVisualTarget(person, 7)
    }));
    const actions = Object.entries(modalData.actionCards || {}).map(([name, action]) => ({
      key: `action:${name}`, type: "action", typeLabel: "アクションカード", id: name, title: name,
      meta: [action[0], ...((Array.isArray(action[2]) ? action[2] : []))].filter(Boolean).join(" / "),
      body: [action[0], action[1]].filter(Boolean).join(" "),
  ...
  function effectiveVisual(record) {
    ...
    if (record.type === "action") {
      const tags = Array.isArray(record.target[2]) ? record.target[2] : [];
    ...
    if (record.type === "subcategory") {
      ...
      for (const actionName of actionNames) {
        const action = modalData.actionCards?.[actionName];
  ...
  function selectRecord(key) {
    ...
    els.webSearchLink.href = `https://www.google.com/search?tbm=isch&q=${encodeURIComponent(`${record.title} 日本史`)}`;
  ...
  function importDataObject(data, name = "") {
    if (data && data.historyContent && data.modalData) {
      replaceObject(historyData, data.historyContent);
      replaceObject(modalData, data.modalData);
      decorateModalVisualTargets();
      return ["履歴", "モーダル"];
    }
    ...
    if (data && Array.isArray(data.people) && data.peopleByName && data.actionCards) {
  ...
  async function applyToProject() {
    ...
    try {
      const response = await fetch("/api/apply-image-data", {
        ...
        body: JSON.stringify({ historyContent: historyData, modalData, patch })
      });
