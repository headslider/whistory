...
function findVisualForPerson(person) {
  ...
  const related = worldHistoryActionSubcategories.find((subcategory) => {
    ...
    return image && subcategoryDescription(subcategory).includes(name);
  });
  if (related) {
    const [, , title] = related;
...
function renderLearningModal({ type, icon, eyebrow, titleHtml, subtitle, visual, sections, tags }) {
  ...
  personDetail.innerHTML = `
    <article class="learning-modal-card modal-type-${escapeHtml(type)}">
      ...
      <div class="modal-section-grid">
        ${sections.map((section) => modalSectionHtml(section.icon, section.title, section.text)).join("")}
      </div>
    </article>
  `;
}
...
function personModalSections(person) {
  const [name, , eraName, field, title, description] = person;
  return [
    {
      icon: "人",
      title: "どんな人物？",
      text: compactJoin([`${name}は、${eraName}に${field}と深く関わって活躍した人物です。`, description])
    },
    {
      icon: "動",
      title: "何をした？",
      text: compactJoin([`${title}として知られ、政治、文化、社会、戦争や交流の動きに関わりました。`, description])
    },
    {
      icon: "光",
      title: "なぜ重要？",
      text: `${name}を知ると、${eraName}の${field}や世界史の大きな流れを、具体的な人物の行動から考えられます。`
    }
  ];
}
...
function actionModalSections(name, action) {
  ...
  const context = tags?.[0] || "世界史";
  return [
    {
      icon: "知",
      title: "どんな内容？",
      text: compactJoin([`${name}は、${context}に関わる「${summary}」です。`, text])
    },
    {
      icon: "動",
      title: "何が起きた？",
    ...
    },
    {
      icon: "光",
      title: "なぜ重要？",
      text: `${name}を知ると、${(tags || []).join("、")}のつながりが見えてきます。原因と結果を分けて考えることで、世界史の流れとして理解できます。`
    }
  ];
}
