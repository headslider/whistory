...
| `updateModalFavoriteButton(name)` | 人物モーダル内の「お気に入りに追加 / お気に入り済み」ボタン表示だけを更新する。 |
...
| `renderLearningModal(...)` | 人物・アクション共通モーダルを描画する。人物モーダルだけ `heroActions` でお気に入りボタンを差し込む。 |
...
| `openPerson(name)` | `personByName` から人物を探してモーダルを開き、現在のお気に入り状態に合わせたボタンを表示する。 |
