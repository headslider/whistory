...
| 領域 | 主ファイル | 役割 |
|---|---|---|
| 本番ページ | `index.html` | 教材本体のHTML。`styles.css`、`data/history-content.js`、`data/people-data.js`、`data/action-cards.js`、`data/modal-data.js`、`data/learning-terms.js`、`script.js` を読み込む。 |
...
| 正本データ | `data/*.json` | 編集・監査の正本。現在は `history-content.json`、`people-data.json`、`action-cards.json`、`modal-data.json`、`learning-terms.json`。 |
...
| `data/people-data.json` | `data/people-data.js` | `window.WORLD_HISTORY_PEOPLE_DATA` | 人物カード本体 `people` と全人物名引き `peopleByName`。各人物は日本史側を基準にした `name`、`kana`、`title`、`genre`、`modal.profile`、`modal.whatDid`、`modal.whyImportant` 構造。世界史側の `genre` は人物図鑑フィルターIDの配列。 |
...
| `data/learning-terms.json` | `data/learning-terms.js` | `window.WORLD_HISTORY_LEARNING_TERMS_DATA` | ルビ対象語の正本。`reading` と `scopes` で、本文・王国・人物のどこに適用するかを管理する。`tooltip` は任意で、未設定なら説明ポップアップは出さない。 |
...
ルビは `script.js` に直書きしない。難読語・人物名・王国名など、必要な語を `data/learning-terms.json` と `data/learning-terms.js` に個別登録する。広範囲の自動ルビ化や、カタカナ混じり語を辞書語として丸ごとルビ化する処理は入れない。

`script.js` の `normalizeLearningTermsData(data)` が `manualStudyRuby`、`manualKingdomRuby`、`manualPersonRuby` を組み立て、`applyStudyRuby()` と `applyKingdomRuby()` が表示時に使う。`tooltip` は任意項目で、存在する語だけ `.term-tooltip` が付く。フォールバック説明文は生成しない。
### `action-cards`
...
| 関数 | 役割 |
|---|---|
| `getPersonGenres(person)` / `getPersonGenre(person)` | 人物データの `genre` 配列を `personGenreGroups` の表示ラベルへ変換する。`setPersonGenreOverrides` とキーワード自動判定は廃止済み。 |
