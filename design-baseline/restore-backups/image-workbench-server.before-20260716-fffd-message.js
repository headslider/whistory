...
function validatePayload(payload) {
  ...
  if (!modal || !Array.isArray(modal.people)) throw new Error("modalData.people がありません");
  if (!modal.peopleByName || typeof modal.peopleByName !== "object") throw new Error("modalData.peopleByName がありません");
  if (!modal.actionCards || typeof modal.actionCards !== "object") throw new Error("modalData.actionCards がありません");
...
function materializeDataUrlImage(value, folder, id, stats) {
  ...
  const fileName = `${safeFileName(id)}-${hash}.${ext}`;
...
function materializeEmbeddedImages(history, modal) {
  const stats = { count: 0, bytes: 0 };
  for (const person of modal.people || []) {
    const meta = visualMeta(person, 7);
    if (meta) meta.image = materializeDataUrlImage(meta.image, "people", person[0], stats);
  }
  for (const person of Object.values(modal.peopleByName || {})) {
    const meta = visualMeta(person, 7);
    if (meta) meta.image = materializeDataUrlImage(meta.image, "people", person[0], stats);
  }
  for (const [name, action] of Object.entries(modal.actionCards || {})) {
    const meta = visualMeta(action, 3);
...
function loadCurrentDataSets() {
  ...
  validatePayload({ historyContent: history, modalData: modal });
  return { history, modal };
}
...
function targetForImageOperation(datasets, operation) {
  if (operation.type === "person") return datasets.modal.peopleByName?.[operation.id] || (datasets.modal.people || []).find((person) => person[0] === operation.id);
  if (operation.type === "action") return datasets.modal.actionCards?.[operation.id];
...
async function handleApply(req, res) {
  try {
    ...
    const { history, modal } = validatePayload(payload);
    ...
    const assetStats = materializeEmbeddedImages(history, modal);
    if (operations.length) assertAppliedImageOperations({ history, modal }, operations);
    ...
    sendJson(res, 200, {
      ...
      datasets: { historyContent: persisted.history, modalData: persisted.modal },
      backups: listBackups()
    });
  } catch (error) {
...
async function handleRollback(req, res) {
  try {
    ...
    const { history, modal } = loadCurrentDataSets();
    sendJson(res, 200, {
      ok: true,
      restoredFrom: backupDir,
      rollbackBackupDir,
      datasets: { historyContent: history, modalData: modal },
      backups: listBackups()
    });
  } catch (error) {
    sendJson(res, 400, { ok: false, error: error.message });
