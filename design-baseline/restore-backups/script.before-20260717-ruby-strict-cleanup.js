...
function normalizeLearningTermsData(data) {
  ...
  Object.entries(terms).forEach(([term, value]) => {
    ...
    if (!reading || reading === term) return;
    rubyGlossary[term] = reading;
...
function termTooltipHtml(word, innerHtml, options = {}) {
  if (options.disableTooltips || hasModalLinkTarget(word)) return innerHtml;
...
function applyRubyReadings(text, readingEntries, options = {}) {
  ...
  const words = [...readings.keys()]
    .filter((word) => readings.get(word) !== word && source.includes(word) && shouldApplyRuby(word, readings.get(word)))
  ...
  source.replace(pattern, (word, index) => {
    ...
    html += termTooltipHtml(word, ruby(word, readings.get(word)), options);
