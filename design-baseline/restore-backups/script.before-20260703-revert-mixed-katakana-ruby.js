...
function applyStudyRuby(text) {
  ...
  const glossaryWords = new Set(Object.keys(rubyGlossary));
  const words = [...readings.keys()]
    .filter((word) => /[\u3400-\u9fff]/.test(word) && (glossaryWords.has(word) || !/[\u30a0-\u30ff]/.test(word)) && readings.get(word) !== word && source.includes(word))
