...
- タグの下に人物モーダル専用の `お気に入りに追加` ボタンが表示される。
- ボタンを押すと `お気に入り済み` に変わり、人物図鑑の `お気に入り` フィルターに同じ人物が表示される。解除するとフィルターから消える。
- アクションモーダルと王国・勢力モーダルには `data-modal-favorite` が出ない。
...
- 人物画像の保存で `画像保存の照合に失敗しました: person/...` が出る場合は、保存前ペイロードの `modalData.people` と `modalData.peopleByName[人物名]` の両方に、同じ配列末尾メタ情報が入っているか確認する。サーバー側は人物名検索で `peopleByName` を優先するため、片方だけ更新されている状態は失敗扱いになる。
- 画像作業ページ側では、保存前に `syncModalPersonVisuals()` が走り、`buildPatch()` の前に人物データの複数コピーが同期されている必要がある。

## カード生成・データ管理の検証

世界史側は日本史側の `people-data.*` / `action-cards.*` 分離構成ではなく、`data/modal-data.*` 一体型で人物・アクション・王国カードを管理する。日本史側の検証手順を流用する場合は、必ず次の世界史仕様へ読み替える。
...
console.log(JSON.stringify({
  ...
  people: modal.people.length,
  peopleByName: Object.keys(modal.peopleByName || {}).length,
  actionCards: Object.keys(modal.actionCards || {}).length,
  kingdomCards: (modal.kingdomCards || []).length,
  kingdomPeople: Object.keys(modal.kingdomPeople || {}).length,
  replacementCharacterFiles: ['data/history-content.json','data/history-content.js','data/modal-data.json','data/modal-data.js','script.js']
...
- `normalizeModalData(data)` が `people`、`personByName`、`actionCards`、`kingdomCards`、`kingdomPeople` を作る。
...
- `renderLearningModal(...)`
- `data/modal-data.json` の `peopleByName` と `actionCards`
...
- 人物・アクション画像: `data/modal-data.json` / `data/modal-data.js`

人物・アクション画像は配列末尾メタ情報に保存する。
...
- `data/modal-data.json` / `.js` の人物・アクション・王国データだけを変更した。
