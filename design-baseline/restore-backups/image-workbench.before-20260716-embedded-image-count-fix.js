...
const historyData = clone(window.historyContentData || {});
  ...
  function syncModalPersonVisuals() {
    const fieldsByName = new Map(modalPersonRecords().map((person) => [person[0], visualFields(person, 7)]));
    for (const person of modalData.people || []) if (fieldsByName.has(person[0])) applyVisualFields(person, 7, fieldsByName.get(person[0]));
    for (const person of Object.values(modalData.peopleByName || {})) if (fieldsByName.has(person[0])) applyVisualFields(person, 7, fieldsByName.get(person[0]));
