window.WORLD_HISTORY_PEOPLE_DATA = {
  "schemaVersion": 2,
  "people": [
    {
      "name": "ハンムラビ",
      "kana": "はんむらび",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミア",
      "title": "法律を整えた王",
      "icon": "📜",
      "modal": {
        "profile": "古バビロニア王国の第6代王で、周辺都市を征服してメソポタミア南部を統一しました。各地の慣習法や裁判例を整理し、王が正義を守るという形で法を石碑に刻ませたものが「ハンムラビ法典」です。刑罰には身分差がありましたが、契約、相続、結婚、賃金など当時の社会生活を詳しく伝える史料として残っています。 王権と法の関係も読み取れます。",
        "whatDid": "メソポタミア南部を統一し、裁判の基準となる法律を「ハンムラビ法典」として石碑に刻ませました。",
        "whyImportant": "古代社会の法律、身分、家族、商業の仕組みを具体的に知ることができる代表的な法典を残したためです。"
      },
      "image": "assets/people/person-bd338fe558.webp",
      "imageAlt": "ハンムラビの画像"
    },
    {
      "name": "クフ王",
      "kana": "くふおう",
      "era": "最初の文明の時代",
      "genre": [
        "monarch"
      ],
      "field": "古代エジプト",
      "title": "大ピラミッドの時代の王",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト古王国時代の第4王朝の王で、ギザにある三大ピラミッドのうち最大のものを築かせた王と考えられています。巨大な石材を切り出して運び、職人や農民へ食料を配る大規模な国家事業には、王権の強さと行政組織の発達が表れています。ピラミッドは王の墓であると同時に、古代エジプトの技術と信仰を今に伝えます。",
        "whatDid": "ギザの大ピラミッドを築かせ、大量の石材・労働者・食料を動かす国家事業を指揮しました。",
        "whyImportant": "大ピラミッドが、古代エジプトの王権、宗教観、測量・建築技術を示す世界的遺産となったためです。"
      },
      "image": "assets/people/person-5119f42d3b.webp",
      "imageAlt": "クフ王の画像"
    },
    {
      "name": "孔子",
      "kana": "こうし",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "中国",
      "title": "人の道を説いた思想家",
      "icon": "📘",
      "modal": {
        "profile": "中国の春秋時代、諸国の争いで政治や社会の秩序が揺らぐなか、人を思いやる「仁」と、礼儀や社会の決まりである「礼」を重んじた思想家です。身分にかかわらず学ぶ意欲のある弟子を教え、政治を行う者は徳によって人々を導くべきだと説きました。死後、弟子たちが言葉や対話をまとめた書物が『論語』です。孔子の教えをもとに発達した儒教は、東アジアの政治、教育、家族観に長く影響しました。",
        "whatDid": "「仁」と「礼」を重んじる政治と生き方を説き、多くの弟子を育てました。その言葉は『論語』にまとめられました。",
        "whyImportant": "孔子の教えから儒教が発達し、中国・朝鮮・日本の政治、教育、道徳の基本となったためです。"
      },
      "image": "assets/people/person-3c08d403ad.webp",
      "imageAlt": "孔子の画像"
    },
    {
      "name": "始皇帝",
      "kana": "しこうてい",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "中国",
      "title": "中国を統一した皇帝",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代の秦王として六国を滅ぼし、紀元前221年に中国を初めて統一して「皇帝」を名乗りました。地域ごとに異なっていた文字、貨幣、度量衡、車輪の幅をそろえ、郡県制で全国を中央から支配しました。一方、万里の長城の建設や宮殿工事に多くの人々を動員し、厳しい法と思想統制への反発を招きました。短命に終わった秦の制度は、後の中国王朝の統治の土台になりました。",
        "whatDid": "六国を征服して中国を統一し、文字・貨幣・度量衡を統一して郡県制による中央集権を進めました。",
        "whyImportant": "中国を一つの皇帝国家として治める制度と標準を整え、後の歴代王朝の統治モデルを築いたためです。"
      },
      "image": "assets/people/person-6ce46faf8f.webp",
      "imageAlt": "始皇帝の画像"
    },
    {
      "name": "アレクサンドロス大王",
      "kana": "あれくさんどろすだいおう",
      "era": "古代の大国の時代",
      "genre": [
        "monarch"
      ],
      "field": "ギリシャ・西アジア",
      "title": "東西を結んだ征服者",
      "icon": "🛡️",
      "modal": {
        "profile": "マケドニア王フィリッポス2世の子として即位し、ギリシャ諸都市を従えてペルシャ帝国への遠征を始めました。小アジア、エジプト、メソポタミア、イランを征服し、軍はインド北西部に達しました。各地にギリシャ風都市を建設し、現地の支配層も取り入れたため、ギリシャ文化と西アジア・エジプトの文化が混ざるヘレニズム文化が広がりました。死後、帝国は後継者に分割されました。",
        "whatDid": "ペルシャ帝国を征服してインド北西部まで進軍し、各地にギリシャ風都市を建設しました。",
        "whyImportant": "征服地で東西の文化が交流し、地中海から西アジアに広がるヘレニズム世界が成立したためです。"
      },
      "image": "assets/people/person-6f8c27de1c.webp",
      "imageAlt": "アレクサンドロス大王の画像"
    },
    {
      "name": "アショーカ王",
      "kana": "あしょーかおう",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "インド",
      "title": "仏教を保護した王",
      "icon": "🕊️",
      "modal": {
        "profile": "インドのマウリヤ朝第3代の王で、紀元前3世紀にほぼ全インドを治めました。カリンガ征服で多数の死者や捕虜が出たことを悔い、武力による拡大よりも生命の尊重や寛容を重んじる「ダルマ」に基づく政治を掲げました。命令や教えを石柱・岩壁の碑文に刻み、道路、井戸、医療施設も整えました。仏教を保護し、使節を各地へ送り、その広がりを後押ししました。",
        "whatDid": "カリンガ戦争後に武力征服を反省し、ダルマによる政治を行って仏教を保護・普及しました。",
        "whyImportant": "王の政治に生命尊重と宗教的寛容を取り入れ、仏教がインド内外へ広がる契機をつくったためです。"
      },
      "image": "assets/people/person-3b5126d5ca.webp",
      "imageAlt": "アショーカ王の画像"
    },
    {
      "name": "イエス",
      "kana": "いえす",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "西アジア",
      "title": "キリスト教の中心となった人",
      "icon": "🕊️",
      "modal": {
        "profile": "ローマ帝国支配下のユダヤ地方で活動し、神への愛と隣人への愛、弱い立場の人への救いを説いた人物です。律法の形式だけでなく、人の心と行いを重視し、多くの弟子を集めました。エルサレムで捕らえられ、ローマ総督ポンティウス・ピラトゥスのもとで十字架刑に処されました。弟子たちはイエスが復活したと信じ、その教えと救いを広めたため、のちにキリスト教が成立しました。",
        "whatDid": "神と隣人への愛、罪のゆるし、弱い人への救いを説き、その教えのもとに弟子たちを集めました。",
        "whyImportant": "死後、弟子たちが教えと復活への信仰を広め、世界宗教の一つであるキリスト教が成立したためです。"
      },
      "image": "assets/people/person-83bd092dca.webp",
      "imageAlt": "イエスの画像"
    },
    {
      "name": "ムハンマド",
      "kana": "むはんまど",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "アラビア半島",
      "title": "イスラム教を始めた預言者",
      "icon": "☪️",
      "modal": {
        "profile": "アラビア半島の商業都市メッカに生まれ、唯一神アッラーから啓示を受けた預言者として人々に信仰を説きました。迫害を受けて622年にメディナへ移住し、信者の共同体を築きました。この移住はヒジュラと呼ばれ、イスラム暦の元年とされます。のちにメッカを支配下に入れ、アラビア半島の多くをまとめました。啓示は死後に『コーラン』として編まれ、イスラム教の教えと生活規範の基礎になりました。",
        "whatDid": "唯一神アッラーへの信仰を説き、メディナにイスラム共同体を築いてアラビア半島をまとめました。",
        "whyImportant": "『コーラン』に基づくイスラム教と共同体の基礎を築き、その後の西アジア世界を大きく形づくったためです。"
      },
      "image": "assets/people/person-920c89b7aa.webp",
      "imageAlt": "ムハンマドの画像"
    },
    {
      "name": "玄奘",
      "kana": "げんじょう",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "中国・インド",
      "title": "仏教を学んだ旅の僧",
      "icon": "📚",
      "modal": {
        "profile": "唐の僧で、中国に伝わった仏典の内容に疑問を持ち、原典を求めて629年ごろ長安を出発しました。砂漠や高山を越え、中央アジアを通ってインドへ入り、ナーランダー僧院などで仏教、とくに唯識思想を学びました。約17年後に多数の経典や仏像を持ち帰り、長安で正確な漢訳に取り組みました。旅の記録『大唐西域記』は、当時のインドや中央アジアを知る重要史料でもあります。",
        "whatDid": "仏教の原典を求めてインドへ旅し、帰国後に多くのサンスクリット経典を漢訳しました。",
        "whyImportant": "中国仏教の理解を深め、『大唐西域記』によって7世紀の中央アジアとインドの姿も伝えたためです。"
      },
      "image": "assets/people/person-077a351fc3.webp",
      "imageAlt": "玄奘の画像"
    },
    {
      "name": "クローヴィス",
      "kana": "くろーびす",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランク王国",
      "title": "フランク王国を建てた王",
      "icon": "👑",
      "modal": {
        "profile": "フランク人の一部族サリ族の王から勢力を広げ、ガリア北部のローマ系勢力や他のゲルマン諸部族を破ってフランク王国の基礎を築きました。496年ごろ、家臣とともにカトリックへ改宗したと伝えられます。当時の多くのゲルマン王がアリウス派だったなか、ローマ教会と同じ信仰を選んだことで、ガリアのローマ系住民や司教の支持を得ました。王国とカトリック教会の結びつきは西ヨーロッパの政治に長く影響しました。",
        "whatDid": "フランク諸部族をまとめてガリアを支配し、カトリックへ改宗してローマ教会と結びつきました。",
        "whyImportant": "フランク王国とカトリック教会の協力関係を築き、中世西ヨーロッパの政治秩序の出発点となったためです。"
      },
      "image": "assets/people/person-547e05b8b9.webp",
      "imageAlt": "クローヴィスの画像"
    },
    {
      "name": "カール大帝",
      "kana": "かーるたいてい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランク王国",
      "title": "西ヨーロッパをまとめた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "フランク王国のカロリング朝の王で、ザクセン、ランゴバルド王国などを征服し、西ヨーロッパの広い地域を支配しました。800年、ローマ教皇から皇帝の冠を授けられ、西ローマ帝国滅亡後の西欧に皇帝権を復活させました。各地に伯を置き、巡察使を送って統治を監督し、修道院や宮廷学校で古典学問とラテン語教育を保護しました。この文化復興はカロリング・ルネサンスと呼ばれます。",
        "whatDid": "西ヨーロッパの広域を統一し、皇帝に即位して統治制度と学問・教育の整備を進めました。",
        "whyImportant": "王権・ローマ教会・古典文化を結びつけ、中世西ヨーロッパの政治と文化の基礎を築いたためです。"
      },
      "image": "assets/people/person-5ba86ccd42.webp",
      "imageAlt": "カール大帝の画像"
    },
    {
      "name": "オットー1世",
      "kana": "おっとーいっせい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "神聖ローマ帝国",
      "title": "神聖ローマ帝国を始めた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "東フランク王国のザクセン朝第2代の王で、反乱する諸侯を抑え、司教や修道院長を統治に利用して王権を強めました。955年のレヒフェルトの戦いでマジャール人を破り、東方からの侵入を止めました。イタリアへ遠征して教皇を保護し、962年にローマ皇帝の冠を受けました。この戴冠は、後に神聖ローマ帝国と呼ばれるドイツ・イタリアを中心とした政治秩序の出発点とされています。",
        "whatDid": "諸侯を抑えて王権を強化し、マジャール人を破った後、962年にローマ皇帝として戴冠しました。",
        "whyImportant": "ドイツ王とローマ皇帝を結びつけ、神聖ローマ帝国へ続く中世ヨーロッパの政治秩序を築いたためです。"
      },
      "image": "assets/people/person-516f25fdf3.webp",
      "imageAlt": "オットー1世の画像"
    },
    {
      "name": "フリードリヒ・バルバロッサ",
      "kana": "ふりーどりひ・ばるばろっさ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "神聖ローマ帝国",
      "title": "イタリアへ遠征した皇帝",
      "icon": "👑",
      "modal": {
        "profile": "ホーエンシュタウフェン朝の神聖ローマ皇帝で、「赤ひげ」を意味するバルバロッサの名で知られます。ドイツ諸侯をまとめ、北イタリア都市に皇帝権を及ぼそうと何度も遠征しましたが、ロンバルディア同盟と教皇の抵抗を受け、1176年のレニャーノの戦いで敗れました。その後は和約を結び、第三回十字軍に参加しましたが、小アジアの川で溺死しました。皇帝と都市・教皇の対立を象徴する人物です。",
        "whatDid": "北イタリアへ遠征して皇帝権の拡大を図り、晩年には第三回十字軍を率いました。",
        "whyImportant": "皇帝・教皇・自治都市が競い合った中世ヨーロッパの複雑な権力関係を示す代表的な皇帝だからです。"
      },
      "image": "assets/people/person-dfb6ed9cfb.webp",
      "imageAlt": "フリードリヒ・バルバロッサの画像"
    },
    {
      "name": "エドワード1世",
      "kana": "えどわーどいっせい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "イングランド",
      "title": "議会と法を整えた王",
      "icon": "👑",
      "modal": {
        "profile": "13世紀後半のイングランド王で、法令と裁判制度を整え、王権による国内統治を強めました。ウェールズを征服して城を築き、王子に「プリンス・オブ・ウェールズ」の称号を与えました。スコットランドにも介入しましたが、ウィリアム・ウォレスらの抵抗を受けました。1295年には聖職者、貴族、騎士、都市代表を集めた模範議会を開き、後のイングランド議会の構成に大きな影響を与えました。",
        "whatDid": "法と裁判制度を整え、ウェールズを征服し、1295年に各身分の代表を集めた模範議会を開きました。",
        "whyImportant": "王の課税に各身分の代表が関わる議会の形を整え、後のイギリス議会政治の発達につながったためです。"
      },
      "image": "assets/people/person-2108b78bda.webp",
      "imageAlt": "エドワード1世の画像"
    },
    {
      "name": "ヘンリー5世",
      "kana": "へんりーごせい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "イングランド",
      "title": "百年戦争を戦った王",
      "icon": "👑",
      "modal": {
        "profile": "ランカスター朝のイングランド王で、国内の反乱を抑えた後、フランス王位への権利を主張して百年戦争を再開しました。1415年のアジャンクールの戦いでは、長弓兵を生かして数で勝るフランス軍に大勝しました。1420年のトロワ条約でフランス王シャルル6世の娘と結婚し、将来のフランス王位継承を認めさせましたが、若くして病死し、両国統一は実現しませんでした。",
        "whatDid": "百年戦争を再開し、アジャンクールで大勝してトロワ条約によりフランス王位継承権を得ました。",
        "whyImportant": "イングランドが百年戦争で最も優位に立った時期を築き、その後のフランス側の反撃を招いたためです。"
      },
      "image": "assets/people/person-9e2923c493.webp",
      "imageAlt": "ヘンリー5世の画像"
    },
    {
      "name": "ジャンヌ・ダルク",
      "kana": "じゃんぬ・だるく",
      "era": "交易と王国の時代",
      "genre": [
        "war"
      ],
      "field": "フランス",
      "title": "百年戦争で人々を励ました少女",
      "icon": "⚔️",
      "modal": {
        "profile": "フランス北東部の農村に生まれ、神の声を聞いたと信じて百年戦争中の王太子シャルルを訪ねました。1429年、包囲されたオルレアンへ入り、フランス軍の士気を高めて解放に導き、シャルル7世のランスでの戴冠を実現させました。翌年ブルゴーニュ軍に捕らえられ、イングランド側の宗教裁判で異端とされて火刑に処されました。後に裁判は無効とされ、フランス抵抗の象徴となりました。",
        "whatDid": "オルレアン解放を助け、王太子シャルルをランスへ導いてフランス王シャルル7世として戴冠させました。",
        "whyImportant": "百年戦争で劣勢だったフランスの士気と王権を立て直し、国民的結束の象徴となったためです。"
      },
      "image": "assets/people/person-9f857af814.webp",
      "imageAlt": "ジャンヌ・ダルクの画像"
    },
    {
      "name": "ジル・ド・レエ",
      "kana": "じる・ど・れえ",
      "era": "交易と王国の時代",
      "genre": [
        "war"
      ],
      "field": "フランス",
      "title": "ジャンヌとともに戦った貴族",
      "icon": "⚔️",
      "modal": {
        "profile": "フランス西部の有力貴族で、百年戦争ではジャンヌ・ダルクとともにオルレアン解放やシャルル7世の戴冠行軍に参加し、若くしてフランス元帥となりました。戦後は領地で豪華な生活や演劇に巨額を費やし、財産を失っていきました。1440年、聖職者への暴行や多数の子どもの殺害などの罪で教会と世俗の裁判にかけられ、処刑されました。軍事的栄誉と重大犯罪の記録が併存する人物です。",
        "whatDid": "百年戦争でジャンヌ・ダルクと戦いフランス元帥となりましたが、後に重大犯罪で裁かれ処刑されました。",
        "whyImportant": "英雄的な軍歴と犯罪者としての裁判記録が併存し、中世貴族社会と司法の暗部を示す人物だからです。"
      },
      "imageAlt": "ジル・ド・レエの画像",
      "image": "assets/people/person-f6d272d91c.webp"
    },
    {
      "name": "マクシミリアン1世",
      "kana": "まくしみりあんいっせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch"
      ],
      "field": "神聖ローマ帝国",
      "title": "結婚で領土を広げた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "ハプスブルク家の神聖ローマ皇帝で、ブルゴーニュ公女マリーとの結婚によってネーデルラントなどの領地を獲得しました。子どもたちをスペイン王家と結婚させ、さらに孫の婚姻でボヘミア・ハンガリーへの継承権も確保しました。「戦争は他国にさせよ、幸いなるオーストリアよ、結婚せよ」と表現される婚姻政策により、孫カール5世が広大な領土を受け継ぐ土台を築きました。",
        "whatDid": "自らと子孫の婚姻を利用して、ネーデルラント・スペイン・中欧へハプスブルク家の継承権を広げました。",
        "whyImportant": "戦争だけでなく婚姻によって領土を拡大するハプスブルク家の国際戦略を確立したためです。"
      },
      "image": "assets/people/person-6686946bab.webp",
      "imageAlt": "マクシミリアン1世の画像"
    },
    {
      "name": "カール5世",
      "kana": "かーるごせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch"
      ],
      "field": "スペイン・神聖ローマ帝国",
      "title": "広大な領土を治めた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "ハプスブルク家に生まれ、スペイン王カルロス1世と神聖ローマ皇帝を兼ね、ネーデルラント、イタリアの一部、アメリカ大陸の植民地を含む広大な領土を治めました。宗教改革ではルター派諸侯と対立し、フランス王フランソワ1世やオスマン帝国とも戦いました。しかし領土が広すぎて統一支配は難しく、晩年に退位してスペイン系とオーストリア系に領地を分けました。",
        "whatDid": "スペイン王と神聖ローマ皇帝を兼ね、宗教改革・フランス・オスマン帝国との戦いに対応しました。",
        "whyImportant": "ヨーロッパとアメリカにまたがるハプスブルク帝国を治め、16世紀国際政治の中心となったためです。"
      },
      "image": "assets/people/person-32c3a12328.webp",
      "imageAlt": "カール5世の画像"
    },
    {
      "name": "ヘンリー8世",
      "kana": "へんりーはっせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "イングランド",
      "title": "イギリス国教会をつくった王",
      "icon": "👑",
      "modal": {
        "profile": "テューダー朝のイングランド王で、王妃キャサリンとの結婚無効を教皇が認めなかったことからローマ教会と対立しました。1534年の国王至上法で国王をイングランド教会の最高首長とし、修道院を解散して土地と財産を没収しました。教義にはカトリック的要素を残しましたが、教皇から独立した国教会を成立させたことで、イングランドの宗教改革と王権強化を大きく進めました。",
        "whatDid": "国王至上法を制定してローマ教皇から離れ、国王を首長とするイングランド国教会を成立させました。",
        "whyImportant": "宗教と政治の中心を教皇から国王へ移し、イングランド宗教改革と近代国家形成を進めたためです。"
      },
      "image": "assets/people/person-f58f3d2792.webp",
      "imageAlt": "ヘンリー8世の画像"
    },
    {
      "name": "アンリ4世",
      "kana": "あんりよんせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランス",
      "title": "宗教戦争をしずめた王",
      "icon": "👑",
      "modal": {
        "profile": "フランスのブルボン朝最初の王で、もとはプロテスタントでした。ユグノー戦争のさなかに王位を継ぎ、国内多数派のカトリックへ改宗してパリに入りました。1598年のナントの勅令で、カトリックを国の宗教としながらプロテスタントにも一定の信仰と権利を認め、長い宗教戦争を終結させました。財政と農業の再建も進めましたが、1610年にカトリック過激派に暗殺されました。",
        "whatDid": "カトリックへ改宗して王位を固め、ナントの勅令でプロテスタントにも一定の信仰の自由を認めました。",
        "whyImportant": "宗教内戦を収めてフランスの統一と復興を進め、ブルボン朝による王権強化の基礎を築いたためです。"
      },
      "image": "assets/people/person-1ac2f10bef.webp",
      "imageAlt": "アンリ4世の画像"
    },
    {
      "name": "ルイ14世",
      "kana": "るいじゅうよんせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch"
      ],
      "field": "フランス",
      "title": "絶対王政の太陽王",
      "icon": "👑",
      "modal": {
        "profile": "ブルボン朝のフランス王で、幼少期の反乱フロンドの乱を経験した後、宰相に頼らず自ら政治を行いました。貴族をベルサイユ宮殿に集めて宮廷生活へ組み込み、官僚と常備軍を用いて絶対王政を強化しました。財務総監コルベールは重商主義政策を進めましたが、度重なる戦争と宮殿建設は財政を圧迫しました。ナントの勅令を廃止してユグノーを迫害したことも社会と経済に損失を与えました。",
        "whatDid": "貴族をベルサイユ宮殿に集め、官僚・常備軍・重商主義政策によって絶対王政を完成に近づけました。",
        "whyImportant": "フランス絶対王政の最盛期を築く一方、戦争と宗教迫害で後の財政危機も深めたためです。"
      },
      "image": "assets/people/person-4305b9c03d.webp",
      "imageAlt": "ルイ14世の画像"
    },
    {
      "name": "チャールズ1世",
      "kana": "ちゃーるずいっせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "イングランド",
      "title": "革命で処刑された王",
      "icon": "👑",
      "modal": {
        "profile": "ステュアート朝のイングランド王で、王権神授説を信じ、議会の同意を得ずに課税しようとして対立しました。権利の請願を受け入れた後も議会を長く開かず、宗教政策への反発も重なって清教徒革命が始まりました。王党派を率いて議会軍と戦いましたが敗北し、国家への反逆罪で公開裁判にかけられ、1649年に処刑されました。王を法の下で裁いた出来事は、王権の限界を示しました。",
        "whatDid": "議会を無視して統治しようとして内戦を招き、敗北後に国王として公開裁判を受け処刑されました。",
        "whyImportant": "国王も法と議会に制約されることを示し、イギリス立憲政治の発展につながったためです。"
      },
      "image": "assets/people/person-d3cc59b102.webp",
      "imageAlt": "チャールズ1世の画像"
    },
    {
      "name": "ルイ16世",
      "kana": "るいじゅうろくせい",
      "era": "革命と工場の時代",
      "genre": [
        "monarch"
      ],
      "field": "フランス",
      "title": "フランス革命で処刑された王",
      "icon": "👑",
      "modal": {
        "profile": "ブルボン朝末期のフランス王で、即位時から戦争による国の借金と不公平な税制度に直面しました。財政改革を試みましたが、特権身分の抵抗で進まず、1789年に三部会を招集したことがフランス革命の始まりにつながりました。王一家は国外逃亡を企てたヴァレンヌ逃亡事件で信用を失い、王政は廃止されました。外国と通じた反逆の罪で裁かれ、1793年に処刑されました。",
        "whatDid": "財政危機を解決できず三部会を招集し、革命後は国外逃亡に失敗して王政廃止と処刑に至りました。",
        "whyImportant": "絶対王政の崩壊と国民主権への転換を象徴し、フランス革命の進展に大きく関わったためです。"
      },
      "image": "assets/people/person-4fd5b57d17.webp",
      "imageAlt": "ルイ16世の画像"
    },
    {
      "name": "マリー・アントワネット",
      "kana": "まりー・あんとわねっと",
      "era": "革命と工場の時代",
      "genre": [
        "monarch"
      ],
      "field": "フランス",
      "title": "フランス革命で処刑された王妃",
      "icon": "👑",
      "modal": {
        "profile": "オーストリアのハプスブルク家からフランス王太子ルイへ嫁ぎ、のちに王妃となりました。フランスとオーストリアの同盟を支える政略結婚でしたが、外国出身であることや宮廷の浪費の象徴として批判され、実際以上の悪評も広まりました。革命後、王一家のヴァレンヌ逃亡に加わり、外国勢力との連絡を疑われました。王政廃止後に裁判へかけられ、1793年に処刑されました。",
        "whatDid": "フランス王妃として宮廷に入り、革命後は王一家の逃亡計画と外国勢力への働きかけに関わりました。",
        "whyImportant": "旧体制の特権と浪費の象徴として革命宣伝に利用され、王政崩壊の過程を示す人物となったためです。"
      },
      "image": "assets/people/person-11e8ea1d8a.webp",
      "imageAlt": "マリー・アントワネットの画像"
    },
    {
      "name": "マンサ・ムーサ",
      "kana": "まんさ・むーさ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "西アフリカ",
      "title": "マリ帝国の王",
      "icon": "💰",
      "modal": {
        "profile": "14世紀の西アフリカに栄えたマリ帝国の王で、金と塩の交易路を支配し、帝国の最盛期を築きました。1324年からのメッカ巡礼では多数の従者と大量の金を伴い、カイロなどで金を広く使ったため、その富が地中海世界に知られました。帰国後、トンブクトゥやガオにモスクを建て、イスラム学者や建築家を招きました。マリ帝国を交易とイスラム学問の中心として世界地図に刻んだ王です。",
        "whatDid": "金と塩の交易でマリ帝国を繁栄させ、メッカ巡礼と学者・建築家の招致でイスラム文化を発展させました。",
        "whyImportant": "サハラ交易で栄えた西アフリカの富と学問を地中海・イスラム世界へ広く知らせたためです。"
      },
      "image": "assets/people/person-827fa9b9b3.webp",
      "imageAlt": "マンサ・ムーサの画像"
    },
    {
      "name": "チンギス・ハン",
      "kana": "ちんぎす・はん",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "モンゴル高原",
      "title": "モンゴル帝国を築いた人",
      "icon": "🐎",
      "modal": {
        "profile": "モンゴル高原の有力者テムジンとして生まれ、部族間の争いを勝ち抜き、1206年に諸部族からチンギス・ハンに推戴されました。軍を十進法で編成し、能力を重んじる人事と厳しい軍律で統率して、金、西夏、中央アジアのホラズム朝へ征服を広げました。遠征では都市破壊と大量殺害も起こりました。彼が築いた帝国は子孫の時代にユーラシアの広域を結び、人・物・技術の交流を活発にしました。",
        "whatDid": "モンゴル諸部族を統一し、機動力の高い騎馬軍を組織して中央アジアまで大帝国を築きました。",
        "whyImportant": "征服の大被害を伴いながらも、ユーラシアを一つの広域支配圏へ結ぶモンゴル帝国の基礎を築いたためです。"
      },
      "image": "assets/people/person-94822eef6a.webp",
      "imageAlt": "チンギス・ハンの画像"
    },
    {
      "name": "イブン・バットゥータ",
      "kana": "いぶん・ばっとぅーた",
      "era": "交易と王国の時代",
      "genre": [
        "religion",
        "culture",
        "political-scholar"
      ],
      "field": "イスラム世界",
      "title": "広い世界を旅した学者",
      "icon": "🧭",
      "modal": {
        "profile": "モロッコのタンジールに生まれたイスラム法学者で、1325年にメッカ巡礼へ出発した後、約30年にわたり北アフリカ、西アジア、東アフリカ、インド、東南アジア、中国などを旅しました。各地で法官や使節として働き、イスラム圏に共通する信仰と、地域ごとに異なる政治・習慣を見聞しました。帰国後、口述した旅行記『大旅行記』は、14世紀の交易路、港町、宮廷、庶民生活を伝える重要史料です。",
        "whatDid": "約30年かけてアフリカ・西アジア・インド・中国を旅し、その見聞を『大旅行記』に残しました。",
        "whyImportant": "14世紀のイスラム世界とインド洋交易圏の広さ、多様な社会と人々の交流を具体的に伝えたためです。"
      },
      "image": "assets/people/person-ad4f3ff7e7.webp",
      "imageAlt": "イブン・バットゥータの画像"
    },
    {
      "name": "鄭和",
      "kana": "ていわ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "中国・インド洋",
      "title": "大船団を率いた航海者",
      "icon": "⛵",
      "modal": {
        "profile": "雲南出身のイスラム教徒の家に生まれ、明の永楽帝に仕えた宦官・武将です。1405年から1433年まで七回にわたり大船団を率い、東南アジア、インド、アラビア半島、東アフリカ沿岸まで航海しました。各地の支配者へ明皇帝の威信を示し、朝貢と交易を求め、使節や珍しい産物を中国へ連れ帰りました。領土征服を目的とした航海ではありませんが、明とインド洋世界の外交・交易を大きく広げました。",
        "whatDid": "七回の大航海で大船団を率い、東南アジアから東アフリカまで明の外交と朝貢貿易を広げました。",
        "whyImportant": "ヨーロッパの大航海に先立ち、中国とインド洋沿岸諸国を結ぶ大規模な海上交流を実現したためです。"
      },
      "image": "assets/people/person-be824ba0ed.webp",
      "imageAlt": "鄭和の画像"
    },
    {
      "name": "クリストファー・コロンブス",
      "kana": "くりすとふぁー・ころんぶす",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "大西洋",
      "title": "大西洋を渡った航海者",
      "icon": "⛵",
      "modal": {
        "profile": "ジェノヴァ出身の航海者で、地球が球形であることを前提に大西洋を西へ進めばアジアへ着けると考え、スペイン王室の支援を得ました。1492年にカリブ海の島へ到達しましたが、そこをアジア周辺と誤認したまま四回航海しました。この航海を契機にヨーロッパ人のアメリカ進出が本格化し、作物・家畜・病原体の交換が進みました。一方、先住民への暴力、強制労働、植民地支配も始まりました。",
        "whatDid": "スペイン王室の支援で大西洋を横断し、1492年にカリブ海へ到達してヨーロッパの進出を促しました。",
        "whyImportant": "旧大陸とアメリカ大陸の恒常的接触を始め、世界規模の交流と植民地支配の時代を開いたためです。"
      },
      "image": "assets/people/person-ad25b28545.webp",
      "imageAlt": "クリストファー・コロンブスの画像"
    },
    {
      "name": "バルトロメ・デ・ラス・カサス",
      "kana": "ばるとろめ・で・らす・かさす",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "スペイン領アメリカ",
      "title": "先住民の苦しみを訴えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "スペイン人植民者としてカリブ海へ渡り、当初は先住民を労働させるエンコミエンダ制の利益を受けました。しかし征服と強制労働による虐待を目にして土地と先住民を手放し、ドミニコ会修道士として保護運動に転じました。著書『インディアスの破壊についての簡潔な報告』で残虐行為を告発し、スペイン王室へ制度改革を求めました。ただし初期にはアフリカ人奴隷導入を提案したという限界もあり、後に悔いています。",
        "whatDid": "植民地支配に加担した立場を改め、先住民への虐待と強制労働を記録してスペイン王室へ改革を訴えました。",
        "whyImportant": "ヨーロッパの征服を内部から批判し、植民地支配と先住民の権利をめぐる議論を早くから起こしたためです。"
      },
      "image": "assets/people/person-96f97c1af2.webp",
      "imageAlt": "バルトロメ・デ・ラス・カサスの画像"
    },
    {
      "name": "ジョージ・ワシントン",
      "kana": "じょーじ・わしんとん",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "北アメリカ",
      "title": "アメリカ独立を導いた人",
      "icon": "🗽",
      "modal": {
        "profile": "バージニアの農園主・軍人で、イギリス本国と植民地の対立が深まると、大陸軍総司令官としてアメリカ独立戦争を指揮しました。フランスの支援を得てヨークタウンで英軍を降伏させ、独立を実現しました。1789年に初代大統領となり、連邦政府の制度と権威を整えました。二期で退任して平和的に政権を譲る慣例をつくりましたが、自身は奴隷を所有しており、自由の理念との矛盾も残しました。",
        "whatDid": "大陸軍を率いて独立戦争に勝利し、初代大統領として連邦政府の基礎と政権交代の慣例を築きました。",
        "whyImportant": "アメリカ独立と共和政国家の出発を導き、大統領制の実際の運用に先例を残したためです。"
      },
      "image": "assets/people/person-b128c0aae5.webp",
      "imageAlt": "ジョージ・ワシントンの画像"
    },
    {
      "name": "ナポレオン",
      "kana": "なぽれおん",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランス・ヨーロッパ",
      "title": "ヨーロッパを動かした指導者",
      "icon": "⚔️",
      "modal": {
        "profile": "コルシカ島出身の砲兵将校で、フランス革命戦争の勝利によって頭角を現し、1799年のクーデタで第一統領、1804年に皇帝となりました。行政・教育・税制を中央集権化し、法の下の平等や私有財産を定めたナポレオン法典を整えました。ヨーロッパ各地を征服して革命の制度を広げる一方、支配への抵抗と民族意識も高めました。ロシア遠征の失敗後に敗れ、ワーテルローで最終的に退位しました。",
        "whatDid": "革命後のフランスで皇帝となり、ナポレオン法典と中央集権制度を整え、ヨーロッパ各地へ進出しました。",
        "whyImportant": "革命の法と制度を欧州へ広げる一方、征服戦争が民族主義と国際秩序の再編を促したためです。"
      },
      "image": "assets/people/person-aecb5fefb9.webp",
      "imageAlt": "ナポレオンの画像"
    },
    {
      "name": "シモン・ボリバル",
      "kana": "しもん・ぼりばる",
      "era": "革命と工場の時代",
      "genre": [
        "politician"
      ],
      "field": "南アメリカ",
      "title": "独立運動を進めた人",
      "icon": "🗽",
      "modal": {
        "profile": "南米北部の裕福なクリオーリョに生まれ、啓蒙思想とアメリカ・フランス革命の影響を受けてスペインからの独立運動に参加しました。アンデス山脈を越える遠征などを指揮し、ベネズエラ、コロンビア、エクアドル、ペルーの解放に貢献しました。大コロンビアを中心に南米諸国の統合を目指しましたが、地域対立と権力争いで崩壊しました。独立を実現した一方、奴隷制や社会格差の解消は不十分でした。",
        "whatDid": "軍を率いて南米北部のスペイン支配を打破し、大コロンビアを建てて地域統合を目指しました。",
        "whyImportant": "ラテンアメリカ諸国の独立を実現した中心人物で、現在も「解放者」として国家形成の象徴だからです。"
      },
      "image": "assets/people/person-04de7d8241.webp",
      "imageAlt": "シモン・ボリバルの画像"
    },
    {
      "name": "ジェームズ・ワット",
      "kana": "じぇーむず・わっと",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "イギリス",
      "title": "蒸気機関を改良した発明家",
      "icon": "⚙️",
      "modal": {
        "profile": "スコットランド出身の器具製作者・技術者で、ニューコメン型蒸気機関の模型修理をきっかけに、熱が失われる原因を研究しました。蒸気をシリンダーとは別の復水器で冷やす方式を考案し、燃料消費を大きく減らしました。実業家ボールトンと協力し、回転運動を取り出せる機関へ改良したことで、鉱山の排水だけでなく紡績・製粉など工場機械にも利用が広がりました。蒸気動力の普及を支えた技術者です。",
        "whatDid": "分離復水器を考案して蒸気機関の燃料効率を高め、回転運動を利用できる工場動力へ改良しました。",
        "whyImportant": "蒸気機関を多くの工場で使える実用的な動力にし、産業革命の機械化を大きく進めたためです。"
      },
      "image": "assets/people/person-8e2dc5e8cf.webp",
      "imageAlt": "ジェームズ・ワットの画像"
    },
    {
      "name": "フローレンス・ナイチンゲール",
      "kana": "ふろーれんす・ないちんげーる",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war",
        "culture",
        "medicine"
      ],
      "field": "イギリス",
      "title": "看護を変えた人",
      "icon": "🩺",
      "modal": {
        "profile": "イギリスの裕福な家庭に生まれ、家族の反対を押して看護を学びました。クリミア戦争中、スクタリの陸軍病院で看護団を率い、清掃、換気、洗濯、食事、記録を改善しました。死亡率低下には戦争状況など複数の要因がありますが、衛生環境の重要性を統計図で政府へ示したことが改革を進めました。帰国後は看護学校を設立し、看護を訓練と知識を必要とする専門職へ発展させました。",
        "whatDid": "クリミア戦争の病院で衛生と記録を改善し、統計を用いて軍医療改革を求め、看護学校を設立しました。",
        "whyImportant": "近代看護の教育制度と専門性を確立し、統計を政策改善に用いる公衆衛生の発展にも貢献したためです。"
      },
      "image": "assets/people/person-2dab609327.webp",
      "imageAlt": "フローレンス・ナイチンゲールの画像"
    },
    {
      "name": "チャールズ・ダーウィン",
      "kana": "ちゃーるず・だーうぃん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "イギリス",
      "title": "進化論を示した科学者",
      "icon": "🔬",
      "modal": {
        "profile": "イギリスの博物学者で、測量船ビーグル号に乗って南米やガラパゴス諸島などの生物・化石・地層を観察しました。生物の個体差のうち環境に適した特徴をもつものが多く子孫を残し、その特徴が世代を重ねて広がる「自然選択」を進化の仕組みとして考えました。1859年の『種の起源』で発表し、生物は固定された存在ではなく共通祖先から変化したという見方を科学的研究の中心にしました。",
        "whatDid": "世界各地の生物と化石を調べ、自然選択によって生物が進化する仕組みを『種の起源』で示しました。",
        "whyImportant": "生物の多様性を自然の仕組みで説明し、現代生物学の基本となる進化論を確立したためです。"
      },
      "image": "assets/people/person-50808aa356.webp",
      "imageAlt": "チャールズ・ダーウィンの画像"
    },
    {
      "name": "マハトマ・ガンディー",
      "kana": "まはとま・がんでぃー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "インド",
      "title": "非暴力で独立を求めた人",
      "icon": "🕊️",
      "modal": {
        "profile": "インド出身の弁護士で、南アフリカでインド系住民への差別に直面し、真理に基づく非暴力抵抗「サティヤーグラハ」を実践しました。帰国後、イギリス植民地支配に対し、不買運動、非協力運動、塩の行進を指導し、農民や都市住民を独立運動へ参加させました。ヒンドゥー教徒とイスラム教徒の融和を求めましたが、独立時にインドとパキスタンは分離し、1948年にヒンドゥー至上主義者に暗殺されました。",
        "whatDid": "非暴力・不服従を掲げ、非協力運動や塩の行進を通じてインド独立運動を大衆運動へ発展させました。",
        "whyImportant": "武力に頼らない抵抗で植民地支配へ対抗し、世界各地の公民権運動や人権運動に影響したためです。"
      },
      "image": "assets/people/person-ee0e1c6ce3.webp",
      "imageAlt": "マハトマ・ガンディーの画像"
    },
    {
      "name": "孫文",
      "kana": "そんぶん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician"
      ],
      "field": "中国",
      "title": "近代中国の革命家",
      "icon": "📜",
      "modal": {
        "profile": "清末の中国に生まれ、医師となった後、外国支配と清朝の弱体化を批判して革命運動へ進みました。民族の独立、民権による政治、民生の安定を柱とする「三民主義」を掲げ、海外華僑から資金を集めて蜂起を支援しました。1911年の辛亥革命後、南京で中華民国臨時大総統となりましたが、袁世凱へ地位を譲りました。その後は国民党を再編し、中国統一を目指しましたが、実現前に病死しました。",
        "whatDid": "三民主義を掲げて清朝打倒の革命を進め、中華民国の臨時大総統となり、国民党を再編しました。",
        "whyImportant": "皇帝政治から共和政へ移る中国革命の理念と組織を築き、近代中国の国家構想に影響したためです。"
      },
      "image": "assets/people/person-5afa942243.webp",
      "imageAlt": "孫文の画像"
    },
    {
      "name": "アルベルト・アインシュタイン",
      "kana": "あるべると・あいんしゅたいん",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science"
      ],
      "field": "ヨーロッパ・アメリカ",
      "title": "物理学を変えた科学者",
      "icon": "🔬",
      "modal": {
        "profile": "ドイツ生まれの理論物理学者で、1905年に特殊相対性理論を発表し、時間と空間が観測者の運動によって関係づけられることを示しました。質量とエネルギーの等価関係も導き、1915年には重力を時空のゆがみとして説明する一般相対性理論を完成させました。光電効果の研究でノーベル物理学賞を受賞しました。ナチス政権を逃れてアメリカへ移住し、戦後は核兵器管理と平和を訴えました。",
        "whatDid": "特殊・一般相対性理論を築き、光電効果を説明して、時間・空間・重力・光の物理学を変えました。",
        "whyImportant": "現代物理学の理論的基礎を築き、宇宙研究、原子力、精密測定など広い分野に影響したためです。"
      },
      "image": "assets/people/person-48f6387587.webp",
      "imageAlt": "アルベルト・アインシュタインの画像"
    },
    {
      "name": "ネルソン・マンデラ",
      "kana": "ねるそん・まんでら",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "南アフリカ",
      "title": "差別とたたかった指導者",
      "icon": "🕊️",
      "modal": {
        "profile": "南アフリカの法律家で、アフリカ民族会議に加わり、人種隔離政策アパルトヘイトへ抵抗しました。平和的抗議が弾圧されると武装組織の創設にも関わり、1964年の裁判で終身刑となって27年間投獄されました。1990年の釈放後は白人政府との交渉を進め、報復ではなく多数派支配と少数派保護を両立する民主化を目指しました。1994年、全人種選挙で南アフリカ初の黒人大統領となりました。",
        "whatDid": "アパルトヘイトに抵抗して長期投獄され、釈放後は政府と交渉して全人種参加の民主選挙を実現しました。",
        "whyImportant": "人種隔離体制を終わらせ、内戦を避けながら民主的な南アフリカへ移行する中心的役割を果たしたためです。"
      },
      "image": "assets/people/person-1e015e3361.webp",
      "imageAlt": "ネルソン・マンデラの画像"
    },
    {
      "name": "マーティン・ルーサー・キング・ジュニア",
      "kana": "まーてぃん・るーさー・きんぐ・じゅにあ",
      "era": "今につながる時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "アメリカ",
      "title": "公民権運動の指導者",
      "icon": "🕊️",
      "modal": {
        "profile": "アメリカ南部のバプテスト派牧師で、黒人が公共施設や選挙で差別される制度をなくす公民権運動を指導しました。モンゴメリー・バス・ボイコット、バーミングハム運動、ワシントン大行進などで、非暴力の抗議と演説を用いました。運動は1964年公民権法と1965年投票権法の成立を後押ししました。晩年は貧困やベトナム戦争も批判し、1968年に暗殺されました。",
        "whatDid": "非暴力の抗議運動と演説で人種隔離を批判し、公民権法・投票権法の成立を後押ししました。",
        "whyImportant": "法制度として残っていた人種差別を撤廃する運動を全国化し、世界の人権運動へ影響したためです。"
      },
      "image": "assets/people/person-191db0da82.webp",
      "imageAlt": "マーティン・ルーサー・キング・ジュニアの画像"
    },
    {
      "name": "ワンガリ・マータイ",
      "kana": "わんがり・まーたい",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ケニア",
      "title": "木を植える環境運動家",
      "icon": "🌱",
      "modal": {
        "profile": "ケニアの生物学者で、東・中央アフリカの女性として初めて博士号を取得しました。農村女性が薪や水を得にくくなった原因に森林破壊があると考え、1977年にグリーンベルト運動を始めました。女性たちが地域で苗木を育て植樹する活動を、土壌保全、生活改善、女性の自立へ結びつけました。さらに森林の私物化や独裁政治にも反対し、環境保護・民主主義・平和を一体の問題として訴えました。",
        "whatDid": "グリーンベルト運動を創設し、女性による植樹を環境保全、生活改善、民主化運動へ発展させました。",
        "whyImportant": "環境問題を女性の権利・貧困・民主主義・平和と結びつけ、持続可能な開発の実践を示したためです。"
      },
      "image": "assets/people/person-57040e1942.webp",
      "imageAlt": "ワンガリ・マータイの画像"
    },
    {
      "name": "マララ・ユスフザイ",
      "kana": "まらら・ゆすふざい",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "パキスタン",
      "title": "教育の権利を訴える人",
      "icon": "📚",
      "modal": {
        "profile": "パキスタン北西部スワート渓谷に生まれ、武装勢力タリバンが女子教育を禁じるなか、11歳からBBCのブログで学校へ通えない生活を伝えました。教育を受ける権利を公に訴えたため、2012年に通学バスで銃撃されましたが、治療後も活動を続けました。マララ基金を通じて各国の女子教育を支援し、2014年に17歳でノーベル平和賞を受賞しました。教育を基本的人権として訴える世界的象徴です。",
        "whatDid": "タリバンによる女子教育禁止を告発し、銃撃後もマララ基金を通じてすべての子どもの教育権を訴えました。",
        "whyImportant": "女子が学ぶ権利への弾圧を世界へ知らせ、教育を基本的人権として守る国際運動を強めたためです。"
      },
      "image": "assets/people/person-7ca7f530e8.webp",
      "imageAlt": "マララ・ユスフザイの画像"
    },
    {
      "name": "釈迦",
      "kana": "しゃか",
      "era": "古代の大国の時代",
      "genre": [
        "religion"
      ],
      "field": "インド",
      "title": "仏教を開いた人",
      "icon": "📿",
      "modal": {
        "profile": "紀元前5世紀ごろ、現在のネパール南部に近い地域の釈迦族の王子として生まれたと伝えられ、本名をゴータマ・シッダールタといいます。老い、病、死の苦しみに向き合うため出家し、極端な苦行を退けて瞑想を続け、悟りを開きました。欲望や執着が苦しみを生むこと、正しい見方と行いによって苦しみを離れられることを説きました。その教えをもとに仏教教団が成立し、アジア各地へ広がりました。",
        "whatDid": "出家して悟りを開き、苦しみの原因と、それを離れるための生き方を説いて仏教教団を始めました。",
        "whyImportant": "仏教の出発点となる教えを示し、アジアの宗教、思想、芸術、社会に長く影響したためです。"
      },
      "image": "assets/people/person-5efae04b1c.webp",
      "imageAlt": "釈迦の画像"
    },
    {
      "name": "ハンニバル",
      "kana": "はんにばる",
      "era": "古代の大国の時代",
      "genre": [
        "war"
      ],
      "field": "カルタゴ（北アフリカ）",
      "title": "ローマと戦った将軍",
      "icon": "⚔️",
      "modal": {
        "profile": "北アフリカの都市国家カルタゴの名門に生まれた将軍で、第二次ポエニ戦争ではスペインから軍を率い、象を伴ってピレネー山脈とアルプス山脈を越えてイタリアへ侵入しました。カンナエの戦いで包囲戦術を用いてローマ軍を壊滅させましたが、ローマ市を攻略できず、補給と援軍も不足しました。ローマ軍が北アフリカへ攻め込むと帰国し、ザマの戦いでスキピオに敗れました。",
        "whatDid": "軍象を伴ってアルプスを越え、カンナエの戦いで包囲戦術によりローマ軍を大破しました。",
        "whyImportant": "強大なローマを滅亡寸前まで追い込み、包囲戦術の代表例を残した古代屈指の将軍だからです。"
      },
      "image": "assets/people/person-f420e3f10a.webp",
      "imageAlt": "ハンニバルの画像"
    },
    {
      "name": "カエサル",
      "kana": "かえさる",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマ",
      "title": "ローマを率いた英雄",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家・軍人で、民衆派として勢力を伸ばし、ガリア遠征で現在のフランス周辺を征服しました。元老院から軍の解散を命じられるとルビコン川を渡って内戦を始め、政敵ポンペイウスを破りました。終身独裁官となり、暦の改革、植民市建設、属州民の登用などを進めましたが、王を目指すと疑われ、紀元前44年に元老院議員らに暗殺されました。死後、共和政はさらに崩壊へ向かいました。",
        "whatDid": "ガリアを征服し、内戦で政敵を破って終身独裁官となり、暦や属州統治を改革しました。",
        "whyImportant": "共和政の政治均衡を崩し、ローマが皇帝による支配へ移る決定的な転換を引き起こしたためです。"
      },
      "image": "assets/people/person-de50729232.webp",
      "imageAlt": "カエサルの画像"
    },
    {
      "name": "クレオパトラ",
      "kana": "くれおぱとら",
      "era": "古代の大国の時代",
      "genre": [
        "monarch"
      ],
      "field": "エジプト",
      "title": "エジプト最後の女王",
      "icon": "👑",
      "modal": {
        "profile": "プトレマイオス朝エジプト最後の女王で、ギリシャ系王家に生まれながらエジプト語も用いたとされます。弟との王位争いで追放されましたが、ローマのカエサルの支援で復位しました。カエサル死後はアントニウスと同盟し、ローマの影響下でエジプトの独立を守ろうとしました。しかしオクタウィアヌスとのアクティウムの海戦に敗れ、自死しました。その後エジプトはローマの属州となりました。",
        "whatDid": "カエサルとアントニウスとの同盟を利用して王位とエジプトの独立を守ろうとしました。",
        "whyImportant": "ヘレニズム王国エジプトの終焉と、ローマが地中海世界を統一する過程を象徴する女王だからです。"
      },
      "image": "assets/people/person-c35825710f.webp",
      "imageAlt": "クレオパトラの画像"
    },
    {
      "name": "グーテンベルク",
      "kana": "ぐーてんべるく",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture"
      ],
      "field": "ドイツ",
      "title": "活版印刷を広めた人",
      "icon": "📖",
      "modal": {
        "profile": "15世紀ドイツのマインツで活動した金属加工職人・印刷業者で、金属活字、油性インク、圧搾機を組み合わせ、活字を繰り返し使える印刷方式を実用化しました。1450年代には美しいラテン語聖書を大量に印刷し、手書き写本より速く同じ文章を複製できることを示しました。技術はヨーロッパ各地へ急速に広まり、本の価格を下げ、宗教改革、学問、科学、読み書きの普及を支えました。",
        "whatDid": "金属活字・油性インク・圧搾機を組み合わせた活版印刷を実用化し、聖書を大量印刷しました。",
        "whyImportant": "同じ情報を速く正確に大量配布できるようにし、宗教改革、科学、教育、出版文化を発展させたためです。"
      },
      "image": "assets/people/person-521067b1b1.webp",
      "imageAlt": "グーテンベルクの画像"
    },
    {
      "name": "レオナルド・ダ・ヴィンチ",
      "kana": "れおなるど・だ・びんち",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture",
        "science"
      ],
      "field": "イタリア",
      "title": "ルネサンスの天才",
      "icon": "🎨",
      "modal": {
        "profile": "ルネサンス期イタリアの芸術家・技術者で、フィレンツェで絵画を学び、ミラノやローマ、フランスの宮廷で活動しました。『最後の晩餐』『モナ・リザ』では人体、光、空気遠近法を観察して写実的表現を深めました。同時に人体解剖、水流、機械、飛行などを多数の手稿に図解しました。多くの構想は当時実用化されませんでしたが、芸術と自然研究を観察によって結びつけたルネサンス的人間の代表です。",
        "whatDid": "『最後の晩餐』『モナ・リザ』を描き、人体・機械・水流・飛行を観察して多数の手稿に記録しました。",
        "whyImportant": "芸術と科学を分けず、観察と実験的発想によって人間と自然を探究したルネサンスの象徴だからです。"
      },
      "image": "assets/people/person-ad6e558560.webp",
      "imageAlt": "レオナルド・ダ・ヴィンチの画像"
    },
    {
      "name": "ミケランジェロ",
      "kana": "みけらんじぇろ",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture"
      ],
      "field": "イタリア",
      "title": "ダビデ像を作った芸術家",
      "icon": "🎨",
      "modal": {
        "profile": "イタリア・ルネサンスを代表する彫刻家・画家・建築家です。若いころから人体の骨格や筋肉を観察し、大理石から「ダビデ像」や「ピエタ」を生み出しました。教皇の命でシスティナ礼拝堂の天井に旧約聖書の場面を描き、晩年にはサン・ピエトロ大聖堂の設計にも携わりました。人間の肉体と感情を力強く表し、古代文化を学び直したルネサンスの理想を後世に伝えました。",
        "whatDid": "「ダビデ像」やシスティナ礼拝堂天井画を制作し、彫刻・絵画・建築でルネサンスを代表しました。",
        "whyImportant": "人体と感情を壮大に表した作品が、西洋美術の表現と芸術家の地位を大きく高めたためです。"
      },
      "image": "assets/people/person-1141366ba2.webp",
      "imageAlt": "ミケランジェロの画像"
    },
    {
      "name": "ラファエロ",
      "kana": "らふぁえろ",
      "era": "大航海と出会いの時代",
      "genre": [
        "other"
      ],
      "field": "イタリア",
      "title": "ルネサンスの三大巨匠の一人",
      "icon": "🎨",
      "modal": {
        "profile": "イタリア・ルネサンス盛期の画家で、調和の取れた人物配置と穏やかな表情で知られます。フィレンツェでレオナルドやミケランジェロの表現を学び、のちに教皇ユリウス2世に招かれてバチカン宮殿の壁画を制作しました。「アテネの学堂」では古代ギリシャの哲学者たちを一つの空間に描き、古典文化とキリスト教世界を結びつけました。短い生涯ながら、その均整の取れた作風は長く模範とされました。",
        "whatDid": "バチカン宮殿に「アテネの学堂」などを描き、調和と均整を重んじる絵画様式を完成させました。",
        "whyImportant": "古典文化とキリスト教文化を結ぶ構成が、ルネサンス美術の理想的な形として受け継がれたためです。"
      },
      "image": "assets/people/person-0011a7b938.webp",
      "imageAlt": "ラファエロの画像"
    },
    {
      "name": "マルティン・ルター",
      "kana": "まるてぃん・るたー",
      "era": "大航海と出会いの時代",
      "genre": [
        "religion"
      ],
      "field": "ドイツ",
      "title": "宗教改革を始めた人",
      "icon": "📖",
      "modal": {
        "profile": "神聖ローマ帝国の修道士・神学者で、教会が販売した贖宥状に反対して宗教改革を始めました。1517年に「九十五か条の論題」を示し、人は善行や金銭ではなく信仰によって救われると主張しました。教皇から破門された後も、聖書をドイツ語に訳して一般の人々が読めるようにしました。その運動からルター派が成立し、西ヨーロッパの宗教的統一が崩れる大きな転機となりました。",
        "whatDid": "贖宥状を批判して宗教改革を起こし、聖書をドイツ語に訳して信仰の考え方を広めました。",
        "whyImportant": "カトリック教会の権威を揺るがし、プロテスタント諸派の成立と近代ヨーロッパの形成を促したためです。"
      },
      "image": "assets/people/person-b1751a213c.webp",
      "imageAlt": "マルティン・ルターの画像"
    },
    {
      "name": "カルヴァン",
      "kana": "かるばん",
      "era": "大航海と出会いの時代",
      "genre": [
        "religion"
      ],
      "field": "スイス・フランス",
      "title": "宗教改革を進めた人",
      "icon": "📖",
      "modal": {
        "profile": "フランス生まれの宗教改革者で、迫害を逃れてスイスのジュネーヴで活動しました。著書『キリスト教綱要』で、救われる者は神によってあらかじめ定められているという予定説を体系化しました。市政と教会を結びつけ、質素な生活と厳しい道徳を信者に求めました。その教えはフランスのユグノー、イングランドのピューリタン、オランダの改革派へ広がり、各地の政治や社会にも影響しました。",
        "whatDid": "予定説を中心に宗教改革の教えを体系化し、ジュネーヴを拠点に改革派教会を広げました。",
        "whyImportant": "カルヴァン派が各国へ広がり、宗教対立だけでなく市民社会や政治の形成にも影響したためです。"
      },
      "image": "assets/people/person-575516c8d8.webp",
      "imageAlt": "カルヴァンの画像"
    },
    {
      "name": "コペルニクス",
      "kana": "こぺるにくす",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture",
        "science"
      ],
      "field": "ポーランド",
      "title": "地動説を唱えた学者",
      "icon": "🔭",
      "modal": {
        "profile": "ポーランド王国出身の聖職者・天文学者で、天体観測と古代の学説を検討し、地球が太陽の周りを回る地動説を唱えました。当時広く信じられていた天動説では惑星の複雑な動きを説明しにくいと考え、数学的により整った宇宙像を示しました。主著『天球の回転について』は死の直前に刊行されました。この考えはすぐには受け入れられませんでしたが、近代天文学と科学革命の出発点になりました。",
        "whatDid": "地球を含む惑星が太陽の周りを回る地動説を、数学的な宇宙モデルとして示しました。",
        "whyImportant": "宇宙の中心を地球とする従来の常識を問い直し、観測と計算にもとづく近代天文学を開いたためです。"
      },
      "image": "assets/people/person-e08a16a58c.webp",
      "imageAlt": "コペルニクスの画像"
    },
    {
      "name": "ガリレオ・ガリレイ",
      "kana": "がりれお・がりれい",
      "era": "大航海と出会いの時代",
      "genre": [
        "science"
      ],
      "field": "イタリア",
      "title": "近代科学の父",
      "icon": "🔭",
      "modal": {
        "profile": "イタリアの物理学者・天文学者で、望遠鏡を改良して月の表面、木星の衛星、金星の満ち欠けなどを観測しました。これらは、すべての天体が地球の周りを回るという天動説では説明しにくく、地動説を支える証拠になりました。また、落下運動を実験と数学で研究しました。教会の裁判で地動説を捨てるよう命じられましたが、観測・実験・計算を組み合わせる研究方法は近代科学の基礎となりました。",
        "whatDid": "望遠鏡で木星の衛星や金星の満ち欠けを観測し、実験と数学によって地動説を支えました。",
        "whyImportant": "権威ある学説より観測と実験を重視する姿勢を示し、近代科学の研究方法を形づくったためです。"
      },
      "image": "assets/people/person-ee0d81f4a1.webp",
      "imageAlt": "ガリレオ・ガリレイの画像"
    },
    {
      "name": "ニュートン",
      "kana": "にゅーとん",
      "era": "大航海と出会いの時代",
      "genre": [
        "science"
      ],
      "field": "イギリス",
      "title": "万有引力を発見した科学者",
      "icon": "🔬",
      "modal": {
        "profile": "イングランドの数学者・物理学者で、地上の物体の落下と月や惑星の運動を同じ法則で説明しました。主著『プリンキピア』で運動の三法則と万有引力の法則を示し、自然界の動きを数学で予測できる体系を築きました。光をプリズムに通して白色光が多くの色から成ることも確かめ、微積分の発展にも貢献しました。宇宙と地上を統一的に理解する考えは、その後の科学と技術の基準になりました。",
        "whatDid": "運動の三法則と万有引力の法則を示し、天体と地上の運動を数学で統一的に説明しました。",
        "whyImportant": "自然現象を数式で予測する近代物理学の体系を築き、科学と技術の発展の土台を作ったためです。"
      },
      "image": "assets/people/person-4ae32d11c2.webp",
      "imageAlt": "ニュートンの画像"
    },
    {
      "name": "デカルト",
      "kana": "でかると",
      "era": "大航海と出会いの時代",
      "genre": [
        "philosopher",
        "science"
      ],
      "field": "フランス",
      "title": "近代哲学を切り開いた人",
      "icon": "🧠",
      "modal": {
        "profile": "フランスの哲学者・数学者で、確実な知識を得るため、疑うことのできるものをすべて疑う方法的懐疑を用いました。その結果、疑っている自分の存在は確実だとして「我思う、ゆえに我あり」と表しました。著書『方法序説』では理性を使い、問題を順序立てて考える方法を説きました。また、座標を用いて図形と方程式を結びつけ、解析幾何学を発展させました。近代哲学と数学の双方に新しい出発点を与えました。",
        "whatDid": "方法的懐疑から「我思う、ゆえに我あり」を示し、理性にもとづく近代哲学を始めました。",
        "whyImportant": "伝統や権威ではなく、自分の理性から確実な知識を築く近代的な思考法を示したためです。"
      },
      "image": "assets/people/person-638347cdb2.webp",
      "imageAlt": "デカルトの画像"
    },
    {
      "name": "スピノザ",
      "kana": "すぴのざ",
      "era": "大航海と出会いの時代",
      "genre": [
        "philosopher"
      ],
      "field": "オランダ",
      "title": "自然と神を考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "オランダのユダヤ人共同体に生まれた哲学者で、伝統的な宗教解釈に疑問を示したため共同体から追放されました。神と自然を別のものとせず、世界全体を一つの実在としてとらえる考えを『エチカ』で論じました。また、聖書を歴史的な文書として読み、思想や言論の自由を守る政治を主張しました。生前は危険な思想家と見なされましたが、理性・寛容・自由を重んじる近代思想の先駆者として再評価されました。",
        "whatDid": "神と自然を一体として考える哲学を築き、聖書の歴史的研究と言論の自由を主張しました。",
        "whyImportant": "宗教的権威から独立して考える自由と寛容を説き、近代の民主的思想へ道を開いたためです。"
      },
      "image": "assets/people/person-e66d9b1378.webp",
      "imageAlt": "スピノザの画像"
    },
    {
      "name": "ライプニッツ",
      "kana": "らいぷにっつ",
      "era": "大航海と出会いの時代",
      "genre": [
        "philosopher",
        "science"
      ],
      "field": "ドイツ",
      "title": "論理と数学を考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者・数学者で、外交官や図書館員として働きながら幅広い研究を進めました。ニュートンとは別に微積分を考案し、現在も使われる記号を整えました。計算機の改良や二進法の研究にも取り組み、複雑な思考を記号と計算で扱う構想を持ちました。哲学では、世界は「モナド」と呼ぶ精神的な単位から成り、神が調和を与えたと説明しました。数学、論理学、情報科学につながる発想を残しました。",
        "whatDid": "微積分の記号体系を整え、二進法や計算機を研究して、数学と論理学の発展を進めました。",
        "whyImportant": "記号によって計算と思考を扱う発想が、近代数学だけでなくコンピューター科学にもつながったためです。"
      },
      "image": "assets/people/person-161894fd4d.webp",
      "imageAlt": "ライプニッツの画像"
    },
    {
      "name": "カント",
      "kana": "かんと",
      "era": "革命と工場の時代",
      "genre": [
        "political-scholar",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "人間の理性を考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "プロイセン王国の哲学者で、人間は世界をそのまま知るのではなく、時間・空間・因果関係など自らの認識の枠組みを通して理解すると論じました。『純粋理性批判』で経験論と合理論を結びつけ、知識が成り立つ条件を検討しました。倫理では、利益や感情ではなく、誰にでも当てはまる道徳法則に従って行動すべきだと説きました。理性の可能性と限界を示し、近代哲学の流れを大きく変えました。",
        "whatDid": "人間の認識が成り立つ条件を分析し、普遍的な道徳法則にもとづく倫理学を築きました。",
        "whyImportant": "知識・道徳・自由を理性から考え直し、その後の哲学や政治思想の出発点となったためです。"
      },
      "image": "assets/people/person-3d930c4d52.webp",
      "imageAlt": "カントの画像"
    },
    {
      "name": "シラー",
      "kana": "しらー",
      "era": "革命と工場の時代",
      "genre": [
        "culture",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "自由と美を考えた詩人",
      "icon": "🎭",
      "modal": {
        "profile": "ドイツの詩人・劇作家・思想家で、若いころに公国の厳しい支配を批判する戯曲『群盗』を書きました。歴史劇『ヴィルヘルム・テル』などで、自由を求める人間と圧政への抵抗を描きました。ゲーテと交流し、感情だけに流されず理性と美を調和させる古典主義文学を発展させました。詩「歓喜に寄す」は後にベートーヴェンの交響曲第9番に用いられ、自由と人類の連帯を象徴する作品となりました。",
        "whatDid": "『群盗』『ヴィルヘルム・テル』などを書き、自由・尊厳・圧政への抵抗を文学で表しました。",
        "whyImportant": "文学と美を通して人間の自由を問い、ドイツ古典主義と近代の市民意識を形づくったためです。"
      },
      "image": "assets/people/person-deca27f0a0.webp",
      "imageAlt": "シラーの画像"
    },
    {
      "name": "ヘーゲル",
      "kana": "へーげる",
      "era": "革命と工場の時代",
      "genre": [
        "political-scholar",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "歴史の動きを考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者で、歴史を偶然な出来事の集まりではなく、人間が自由を自覚していく発展の過程として考えました。ある考えがその矛盾や反対を生み、それを乗り越えることで新しい段階へ進むという弁証法を用いて、社会・国家・宗教・芸術を体系的に説明しました。フランス革命を自由の前進として評価する一方、自由は法律と国家の中で実現すると考えました。その思想はマルクスを含む多くの思想家に影響しました。",
        "whatDid": "弁証法を用いて、歴史を人間が自由を自覚していく発展過程として体系的に説明しました。",
        "whyImportant": "社会の矛盾と変化を歴史の動きとして捉える考えが、後の哲学・政治思想・歴史学に影響したためです。"
      },
      "image": "assets/people/person-23a2f61d69.webp",
      "imageAlt": "ヘーゲルの画像"
    },
    {
      "name": "ショーペンハウアー",
      "kana": "しょーぺんはうあー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "生きる苦しみを考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者で、世界の根底には理性ではなく、満たされることのない盲目的な「意志」があると考えました。人間は欲望を満たしても新たな欲望に苦しむため、人生には苦しみが絶えないと『意志と表象としての世界』で論じました。一方で、芸術に没頭すること、他者への同情を持つこと、欲望への執着を弱めることに救いを求めました。インド思想や仏教にも関心を持ち、近代ヨーロッパの悲観主義を代表しました。",
        "whatDid": "世界を盲目的な「意志」の現れと考え、欲望と苦しみの関係を哲学として体系化しました。",
        "whyImportant": "理性中心の哲学に異議を唱え、人間の欲望・苦悩・無意識を考える後代の思想へ影響したためです。"
      },
      "image": "assets/people/person-54ea3251de.webp",
      "imageAlt": "ショーペンハウアーの画像"
    },
    {
      "name": "ニーチェ",
      "kana": "にーちぇ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "価値を問い直した哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者で、キリスト教道徳や当時の社会が当然としていた価値を根本から問い直しました。「神は死んだ」という言葉で、宗教が人々の価値観を支える力を失いつつある時代を表しました。既成の善悪に従うだけでなく、自ら価値を生み出して生きる人間を「超人」と呼びました。著作は断片的で多様な解釈を生み、のちに政治的に悪用された面もありますが、実存主義や文学、心理学へ強い影響を与えました。",
        "whatDid": "宗教や道徳の既成価値を批判し、人間が自ら新しい価値を創り出す生き方を問いかけました。",
        "whyImportant": "近代社会で価値の基準が揺らぐ問題を示し、哲学・文学・心理学に大きな論争を起こしたためです。"
      },
      "image": "assets/people/person-3dcf243a52.webp",
      "imageAlt": "ニーチェの画像"
    },
    {
      "name": "シェイクスピア",
      "kana": "しぇいくすぴあ",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture"
      ],
      "field": "イギリス",
      "title": "イギリスの大劇作家",
      "icon": "🎭",
      "modal": {
        "profile": "イングランドの劇作家・詩人で、ロンドンの劇場を中心に喜劇、悲劇、歴史劇を数多く書きました。『ハムレット』『マクベス』『ロミオとジュリエット』では、愛、野心、復讐、迷いなど人間の複雑な心を、身分や時代をこえて伝わる物語として描きました。また、イングランド史を題材に王権や政治の争いも表現しました。豊かな比喩と言葉づかいは英語そのものに影響し、世界各地で上演され続けています。",
        "whatDid": "悲劇・喜劇・歴史劇を通して、人間の愛・野心・迷い・権力争いを多面的に描きました。",
        "whyImportant": "人間心理を深く描いた作品が言語や文化を越えて読まれ、近代演劇と英文学の基準となったためです。"
      },
      "image": "assets/people/person-bc57242847.webp",
      "imageAlt": "シェイクスピアの画像"
    },
    {
      "name": "バッハ",
      "kana": "ばっは",
      "era": "革命と工場の時代",
      "genre": [
        "other"
      ],
      "field": "ドイツ",
      "title": "音楽の父",
      "icon": "🎵",
      "modal": {
        "profile": "ドイツの作曲家・オルガン奏者で、教会や宮廷に仕えながら宗教曲、器楽曲、鍵盤曲を数多く作りました。複数の旋律を同時に組み合わせる対位法を高度に発展させ、『平均律クラヴィーア曲集』『マタイ受難曲』『ブランデンブルク協奏曲』などを残しました。生前は主に演奏家として知られましたが、死後に作品が再評価されました。西洋音楽の技法を集大成し、後の作曲家が学ぶ基礎を築きました。",
        "whatDid": "対位法を高度に発展させ、宗教曲・協奏曲・鍵盤曲でバロック音楽を集大成しました。",
        "whyImportant": "旋律と和声を組み立てる技法が、西洋音楽の作曲教育と後代の作品の基礎になったためです。"
      },
      "image": "assets/people/person-09dac962cb.webp",
      "imageAlt": "バッハの画像"
    },
    {
      "name": "モーツァルト",
      "kana": "もーつぁると",
      "era": "革命と工場の時代",
      "genre": [
        "other"
      ],
      "field": "オーストリア",
      "title": "天才作曲家",
      "icon": "🎵",
      "modal": {
        "profile": "オーストリアの作曲家で、幼いころからヨーロッパ各地で演奏し、宮廷音楽から市民向けの演奏会まで幅広く活動しました。交響曲、協奏曲、室内楽、宗教曲、オペラのすべてで作品を残し、『フィガロの結婚』『ドン・ジョヴァンニ』『魔笛』では音楽によって人物の性格や社会の対立を生き生きと表しました。宮廷に完全には依存しない作曲家を目指しましたが、生活は安定せず、35歳で亡くなりました。",
        "whatDid": "交響曲・協奏曲・オペラなど多くの分野で、古典派音楽を完成度の高い作品へ発展させました。",
        "whyImportant": "人間の感情と劇を音楽で巧みに表し、古典派の形式を後世の作曲家が学ぶ模範にしたためです。"
      },
      "image": "assets/people/person-a84fa933f1.webp",
      "imageAlt": "モーツァルトの画像"
    },
    {
      "name": "ベートーヴェン",
      "kana": "べーとーべん",
      "era": "革命と工場の時代",
      "genre": [
        "other"
      ],
      "field": "ドイツ",
      "title": "楽聖と呼ばれた作曲家",
      "icon": "🎵",
      "modal": {
        "profile": "ドイツ生まれでウィーンを中心に活動した作曲家です。ハイドンらの古典派音楽を学びながら、交響曲やソナタの規模と表現を広げ、個人の苦悩、自由、勝利への意志を強く打ち出しました。聴力を失っていくなかでも作曲を続け、交響曲第3番「英雄」、第5番、第9番などを完成させました。宮廷の注文だけでなく芸術家自身の理念を作品に込め、古典派からロマン派へ向かう道を開きました。",
        "whatDid": "聴力を失いながら交響曲やソナタを革新し、個人の感情と理念を大規模な音楽で表しました。",
        "whyImportant": "作曲家を宮廷の職人から自立した芸術家へ近づけ、ロマン派音楽への道を開いたためです。"
      },
      "image": "assets/people/person-4ab47aade7.webp",
      "imageAlt": "ベートーヴェンの画像"
    },
    {
      "name": "ノーベル",
      "kana": "のーべる",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "スウェーデン",
      "title": "ダイナマイトの発明者",
      "icon": "🏅",
      "modal": {
        "profile": "スウェーデンの化学者・発明家・実業家で、ニトログリセリンを安全に扱いやすくしたダイナマイトを発明しました。爆薬はトンネルや鉱山の工事を進める一方、兵器にも利用され、彼は巨額の財産を築きました。自分の死を伝える誤報で「死の商人」と評されたことなどをきっかけに、財産を人類に貢献した人へ贈る賞に使う遺言を残しました。その遺産から物理学、化学、医学、文学、平和のノーベル賞が始まりました。",
        "whatDid": "ダイナマイトを発明して産業用爆薬を実用化し、遺産を人類貢献者へ贈る賞の創設に残しました。",
        "whyImportant": "発明がもたらす利益と危険の両面を示し、科学・文化・平和への貢献を顕彰する制度を残したためです。"
      },
      "image": "assets/people/person-f14b621d96.webp",
      "imageAlt": "ノーベルの画像"
    },
    {
      "name": "レントゲン",
      "kana": "れんとげん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "ドイツ",
      "title": "エックス線を発見した科学者",
      "icon": "🔬",
      "modal": {
        "profile": "ドイツの物理学者で、真空放電の実験中に、物体を通り抜けて写真乾板を感光させる未知の放射線を発見しました。正体が分からなかったため「X線」と名づけ、妻の手を撮影して骨と指輪が写ることを示しました。この発見により、体を切り開かずに骨折や体内の異常を調べられるようになりました。特許を取らず広く利用できるようにし、1901年に第1回ノーベル物理学賞を受賞しました。",
        "whatDid": "物体を通過して骨などを写し出すX線を発見し、医療で体内を調べる新しい方法を開きました。",
        "whyImportant": "手術をせず体内を画像で確認できるようにし、診断医学と物理学の利用を大きく変えたためです。"
      },
      "image": "assets/people/person-95977db14a.webp",
      "imageAlt": "レントゲンの画像"
    },
    {
      "name": "アムンゼン",
      "kana": "あむんぜん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "ノルウェー",
      "title": "南極点に初到達した探検家",
      "icon": "🧭",
      "modal": {
        "profile": "ノルウェーの探検家で、寒冷地に暮らす人々の技術を学び、犬ぞりや毛皮の衣服を取り入れて極地探検を進めました。北西航路を初めて船で通過した後、南極点到達を目標に変更し、1911年12月にイギリスのスコット隊より先に到達しました。食料配置や移動計画を細かく準備し、全員が生還しました。その後は飛行船で北極圏も探検し、遭難者の捜索中に消息を絶ちました。",
        "whatDid": "犬ぞりと周到な補給計画を用い、1911年に人類で初めて南極点へ到達しました。",
        "whyImportant": "極地の環境に適した技術と計画の重要性を示し、地理的探検の歴史に大きな記録を残したためです。"
      },
      "image": "assets/people/person-f2de242edd.webp",
      "imageAlt": "アムンゼンの画像"
    },
    {
      "name": "ペリー",
      "kana": "ぺりー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "アメリカ",
      "title": "日本に開国を求めた人",
      "icon": "⚓",
      "modal": {
        "profile": "アメリカ海軍の軍人で、東インド艦隊司令長官として日本へ派遣されました。1853年、蒸気船を含む艦隊で浦賀に来航し、アメリカ大統領の国書を幕府へ渡して開国を求めました。翌年に再来航し、日米和親条約を結ばせ、下田と箱館を開港させました。この交渉は軍事力を背景としたもので、日本の鎖国体制を終わらせるきっかけとなり、幕府の権威低下や明治維新へつながる政治変動を促しました。",
        "whatDid": "艦隊を率いて日本に開国を要求し、1854年の日米和親条約締結へ導きました。",
        "whyImportant": "日本の鎖国体制を揺るがし、幕末の政治対立と明治維新へ向かう変化を引き起こしたためです。"
      },
      "image": "assets/people/person-f3d03b470d.webp",
      "imageAlt": "ペリーの画像"
    },
    {
      "name": "ジョン・F・ケネディ",
      "kana": "じょん・えふ・けねでぃ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "アメリカ",
      "title": "冷戦期のアメリカ大統領",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ合衆国第35代大統領で、冷戦が緊張する1961年に就任しました。キューバ危機ではソ連の核ミサイル配備に海上封鎖で対抗しつつ交渉を続け、核戦争を回避しました。宇宙開発では人間を月へ送る目標を掲げ、公民権問題では人種差別を禁じる法案を提案しました。一方、ピッグス湾侵攻の失敗やベトナムへの関与拡大もありました。1963年に暗殺され、政策の多くは後継政権へ引き継がれました。",
        "whatDid": "キューバ危機で核戦争を回避し、月面着陸計画と公民権法案を進めました。",
        "whyImportant": "冷戦の最危機を交渉で収める一方、宇宙開発と人種差別撤廃を国家課題として示したためです。"
      },
      "image": "assets/people/person-a4b37a4032.webp",
      "imageAlt": "ジョン・F・ケネディの画像"
    },
    {
      "name": "ゴルバチョフ",
      "kana": "ごるばちょふ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ソ連・ロシア",
      "title": "冷戦を終わらせた指導者",
      "icon": "🕊️",
      "modal": {
        "profile": "ソビエト連邦最後の最高指導者で、経済停滞と社会の閉塞を立て直すため、ペレストロイカと呼ばれる改革と、情報公開を進めるグラスノスチを始めました。アメリカとの核軍縮交渉を進め、東ヨーロッパ諸国の民主化を武力で抑え込まなかったため、冷戦終結へ大きく貢献しました。しかし改革は経済混乱や民族対立も広げ、1991年にソ連は解体しました。平和的変化を進めた一方、国内では評価が分かれる人物です。",
        "whatDid": "ペレストロイカとグラスノスチを進め、核軍縮と東欧の民主化を通じて冷戦終結を促しました。",
        "whyImportant": "大国間対立を緩和して冷戦を終わらせた一方、改革がソ連解体へつながる転機にもなったためです。"
      },
      "image": "assets/people/person-5bdbdabb4c.webp",
      "imageAlt": "ゴルバチョフの画像"
    },
    {
      "name": "鄧小平",
      "kana": "とうしょうへい",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "中国",
      "title": "改革開放を進めた指導者",
      "icon": "👤",
      "modal": {
        "profile": "四川省の地主の家に生まれ、フランス留学と中国共産党での活動を経て指導部に入りました。文化大革命で失脚しましたが、毛沢東の死後に復権し、1978年以降は人民公社の解体、経済特区の設置、外国資本の導入を進めました。これらの「改革開放」は中国経済を急成長させる一方、1989年の天安門事件では民主化運動を武力で抑え、政治改革を制限しました。",
        "whatDid": "人民公社の解体、経済特区の設置、外国資本の導入を進め、中国を改革開放へ転換しました。",
        "whyImportant": "社会主義体制を保ちながら市場経済を取り入れ、現在の中国の経済発展の方向を決めたためです。"
      },
      "image": "assets/people/person-92d4a96fa6.webp",
      "imageAlt": "鄧小平の画像"
    },
    {
      "name": "ナルメル",
      "kana": "ナルメル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前3100年ごろのエジプト王で、上エジプトと下エジプトの統一に関わった人物と考えられています。王冠の異なる二つの地域を支配する姿を刻んだ「ナルメルのパレット」は、王が軍事・宗教・儀礼を通して統一を示した重要な資料です。ただし、統一が一度の征服で完成したのか、後世のメネスと同一人物なのかは確定しておらず、考古資料から初期王権を考える中心人物です。",
        "whatDid": "上エジプトと下エジプトを統合する王権を築き、その支配を儀礼的な図像で示しました。",
        "whyImportant": "エジプト統一王国とファラオの支配が成立する過程を示す最古級の資料と結び付くためです。"
      },
      "image": "assets/people/person-f462836a0c.webp",
      "imageAlt": "ナルメルの画像"
    },
    {
      "name": "サルゴン",
      "kana": "サルゴン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前24世紀ごろ、アッカドを拠点にメソポタミアのシュメール都市国家を征服した王です。各都市に自らの支配を受け入れる役人を置き、軍事力と行政によって広い地域をまとめました。娘エンヘドゥアンナをウルの最高祭司に任じ、宗教も王権の統合に利用しました。後世には低い身分から王になった英雄として伝説化され、複数の都市を越える初期帝国を築いた人物として記憶されています。",
        "whatDid": "シュメールの都市国家を征服し、軍隊・役人・宗教を用いてアッカド帝国を築きました。",
        "whyImportant": "一つの都市を越えてメソポタミアを広域支配する帝国の先例をつくったためです。"
      },
      "image": "assets/people/person-1679093f0c.webp",
      "imageAlt": "サルゴンの画像"
    },
    {
      "name": "ソロン",
      "kana": "ソロン",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前6世紀初めのアテネで、貴族と平民の対立を調整するために選ばれた政治家・詩人です。借金を理由に市民が奴隷となる慣行を廃止し、負債を軽減する改革を行いました。また、財産の額に応じて市民を区分し、貴族以外にも政治参加の道を広げました。改革は対立を完全には解決しませんでしたが、血筋だけで政治を独占する仕組みを崩し、後のアテネ民主政につながる基礎を築きました。",
        "whatDid": "債務奴隷を廃止し、財産に応じた政治参加の制度を整えて貴族の独占を弱めました。",
        "whyImportant": "アテネで身分より財産を基準とする政治参加を広げ、民主政への道を開いたためです。"
      },
      "image": "assets/people/person-0c00fa8a54.webp"
    },
    {
      "name": "スキピオ・アフリカヌス",
      "kana": "スキピオ・アフリカヌス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマの将軍で、第二次ポエニ戦争でハンニバルと戦いました。カルタゴ軍の補給地だったイベリア半島を攻略した後、戦場を北アフリカへ移し、紀元前202年のザマの戦いでハンニバルを破りました。この勝利によってカルタゴは軍事力と領土を大きく失い、ローマは西地中海の主導権を握りました。敵の戦術を学びながら戦場を移す判断で、戦争の流れを逆転させた将軍として知られます。",
        "whatDid": "イベリア半島を攻略してカルタゴの基盤を崩し、ザマの戦いでハンニバルを破りました。",
        "whyImportant": "ローマがカルタゴを退け、西地中海の覇権を握る決定的な勝利をもたらしたためです。"
      },
      "image": "assets/people/person-cfb119e804.webp"
    },
    {
      "name": "キュロス2世",
      "kana": "キュロス2世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前6世紀にアケメネス朝ペルシャを開いた王で、メディア、リディア、新バビロニアを征服して西アジアの広い地域を支配しました。紀元前539年にバビロンへ入城した後、追放されていた人々の帰還や神殿の再建を認め、地域の宗教や慣習を利用して統治しました。キュロスの円筒碑文は当時の王の宣伝文書でもあり、近代的な人権宣言と同一視はできませんが、征服地を柔軟に治めた王として後世に理想化されました。",
        "whatDid": "メディア・リディア・新バビロニアを征服し、地域の宗教や慣習を認めながら帝国を築きました。",
        "whyImportant": "アケメネス朝の基礎を築き、多民族・多宗教の広大な帝国を統治する先例を示したためです。"
      },
      "image": "assets/people/person-4ceade843b.webp",
      "imageAlt": "キュロス2世の画像"
    },
    {
      "name": "パウロ",
      "kana": "パウロ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "小アジアのタルソスに生まれたユダヤ人で、当初はイエスを信じる人々を迫害しましたが、回心後は地中海各地を旅して宣教しました。ユダヤ人以外も信仰によって救われると説き、アンティオキア、コリント、エフェソスなどに教会共同体を広げました。各地の信者へ送った手紙は新約聖書に収められ、イエスの死と復活の意味、信仰、共同体のあり方を説明する初期キリスト教の重要な土台になりました。",
        "whatDid": "地中海各地で非ユダヤ人にも宣教し、手紙によって初期キリスト教の教えを整理しました。",
        "whyImportant": "キリスト教をユダヤ社会の外へ広げ、世界宗教へ発展する方向を強く形づくったためです。"
      },
      "image": "assets/people/person-7407a9e24f.webp",
      "imageAlt": "パウロの画像"
    },
    {
      "name": "太宗",
      "kana": "太宗",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の二代皇帝で、父の高祖を助けて王朝の建国に功績を立て、626年の玄武門の変を経て即位しました。律令や科挙を整え、魏徴ら臣下の意見を聞く政治を行った時代は「貞観の治」と呼ばれます。東突厥を破って中央アジアへ勢力を広げ、都の長安には各地の商人・使節・宗教が集まりました。一方、高句麗遠征は十分な成果を上げられず、強い帝国を築く過程には長い戦争も伴いました。",
        "whatDid": "律令・科挙を整え、臣下の意見を取り入れながら唐の支配を安定させ、領域を広げました。",
        "whyImportant": "唐を東アジアと中央アジアに影響を及ぼす国際帝国へ成長させたためです。"
      },
      "image": "assets/people/person-64ef03b0e4.webp"
    },
    {
      "name": "ユスティニアヌス",
      "kana": "ユスティニアヌス",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "6世紀の東ローマ帝国皇帝で、かつてのローマ帝国領を取り戻すため、将軍ベリサリウスらを北アフリカやイタリアへ送りました。法学者に命じて複雑なローマ法を整理し、『ローマ法大全』として編さんしました。ニカの反乱後には首都コンスタンティノープルを再建し、ハギア・ソフィア聖堂を完成させました。征服戦争と建築は帝国の威信を高めましたが、重い税と長い戦争は国力を消耗させました。",
        "whatDid": "ローマ法大全を編さんし、ハギア・ソフィアを建設して、西方領土の再征服を進めました。",
        "whyImportant": "ローマ法を体系化して後世の法学に残し、東ローマ帝国の文化と皇帝権を象徴したためです。"
      },
      "image": "assets/people/person-eb7674eb9e.webp",
      "imageAlt": "ユスティニアヌスの画像"
    },
    {
      "name": "エンリケ航海王子",
      "kana": "エンリケ航海王子",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "15世紀のポルトガル王子で、1415年の北アフリカの港セウタ攻略に参加した後、西アフリカ沿岸への航海を支援しました。自ら長距離航海を率いた航海者ではありませんが、船と資金を用意し、航海者たちはボジャドール岬を越えてアフリカ西岸を南下しました。その活動は金・香辛料・キリスト教国を求める海外進出を進める一方、アフリカ人の捕獲と売買を伴い、大西洋奴隷貿易の始まりにも結び付きました。",
        "whatDid": "西アフリカ沿岸への航海を資金面で支援し、ポルトガルの大西洋進出を進めました。",
        "whyImportant": "大航海時代の海路開拓を促すと同時に、大西洋奴隷貿易の拡大にも道を開いたためです。"
      },
      "image": "assets/people/person-17ac22d8f8.webp"
    },
    {
      "name": "ジョン・ロック",
      "kana": "ジョン・ロック",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "17世紀イングランドの思想家で、王権と議会が争う時代に政治と人間の認識を考えました。『統治二論』で、人は生まれながら生命・自由・財産を守る権利を持ち、政府は人々の同意によって成立すると説きました。政府が権利を侵害すれば、人々には抵抗して政府を変える権利があるとも論じました。こうした自然権と社会契約の考えは、名誉革命の説明やアメリカ独立宣言、近代の立憲政治に影響しました。",
        "whatDid": "自然権と社会契約を説き、政府は人民の同意に基づき、権利を守るべきだと論じました。",
        "whyImportant": "近代の自由主義、立憲政治、アメリカ独立の思想的な土台を与えたためです。"
      },
      "image": "assets/people/person-3f43fd47bb.webp"
    },
    {
      "name": "リチャード・アークライト",
      "kana": "リチャード・アークライト",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18世紀イギリスの発明家・事業家で、水力紡績機を改良して特許を取得し、綿糸を大量生産する工場を建てました。クロムフォードの工場では、水車を動力に多数の機械と労働者を一か所へ集め、原料調達から労働時間までを管理しました。発明のすべてが彼一人によるものではなく、特許の一部は後に無効となりましたが、機械・資本・労働を集中させる工場制度を広め、綿工業の急成長を支えました。",
        "whatDid": "水力紡績機を用いた大規模工場を経営し、綿糸を大量生産する工場制度を広めました。",
        "whyImportant": "家庭内手工業から機械制工場へ生産を移し、産業革命の働き方と生産方式を変えたためです。"
      },
      "image": "assets/people/person-620e13ee9b.webp"
    },
    {
      "name": "セシル・ローズ",
      "kana": "セシル・ローズ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半、南部アフリカでダイヤモンド鉱山を集めてデビアス社を築いたイギリス人実業家・政治家です。ケープ植民地首相となり、イギリス南アフリカ会社を通して現在のジンバブエやザンビアへ支配を広げました。黒人の土地と参政権を制限する政策を進め、ジェームソン侵入事件への関与で首相を辞任しました。奨学金に名を残す一方、その富と権力が植民地支配と人種差別に支えられていたため、現在も評価が争われています。",
        "whatDid": "鉱山会社と植民地会社を通じて南部アフリカの資源と土地を支配し、英国領を拡大しました。",
        "whyImportant": "企業・政治・軍事が結び付いた帝国主義と、植民地支配が残した不平等を示すためです。"
      },
      "image": "assets/people/person-d7452f90ef.webp"
    },
    {
      "name": "ウィンストン・チャーチル",
      "kana": "ウィンストン・チャーチル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリスの政治家で、第二次世界大戦中の1940年に首相となりました。フランス敗北後もナチス・ドイツとの戦争継続を訴え、演説で国民を励まし、アメリカ・ソ連と連携して連合国の勝利を目指しました。戦後は「鉄のカーテン」と表現してソ連への警戒を呼びかけました。一方、イギリス帝国と植民地支配を強く支持し、インド人への差別的発言やベンガル飢饉への対応をめぐって厳しい批判も受けています。",
        "whatDid": "第二次世界大戦でイギリスを率い、アメリカ・ソ連との連携によってナチス・ドイツと戦いました。",
        "whyImportant": "対独戦の継続と連合国の協力を支えた一方、帝国主義の限界も示す政治家だからです。"
      },
      "image": "assets/people/person-ad26ae571c.webp"
    },
    {
      "name": "ジャワハルラール・ネルー",
      "kana": "ジャワハルラール・ネルー",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリス支配下のインドで弁護士となり、ガンディーとともに国民会議派の独立運動を進めた政治家です。独立後の1947年に初代首相となり、議会制民主主義、世俗主義、科学技術教育、重工業を重視した国家建設を進めました。冷戦では米ソどちらの陣営にも属さない非同盟外交を掲げました。一方、カシミール問題や中国との国境戦争、貧困の解消など、独立後の国家統合には多くの課題を残しました。",
        "whatDid": "独立運動を率い、初代首相として議会制民主主義・計画経済・非同盟外交を進めました。",
        "whyImportant": "植民地から独立したインドの国家像を形づくり、アジア・アフリカの非同盟運動を導いたためです。"
      },
      "image": "assets/people/person-d1557caa09.webp"
    },
    {
      "name": "メネス",
      "kana": "メネス",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプトを最初に統一した王として、紀元前3世紀の神官マネトなど後世の記録に名を残す伝説的な人物です。上エジプトと下エジプトをまとめ、都をメンフィスに置いたと伝えられますが、同時代の確実な碑文は見つかっていません。初期王朝時代のナルメル、またはその後継者ホル・アハと同一人物とする説があります。実在と伝承の区別が難しく、王名表と考古資料を照合してエジプト統一を考える人物です。",
        "whatDid": "後世の記録で上下エジプトを統一し、最初の王朝を開いた王として伝えられました。",
        "whyImportant": "伝承上の建国者であり、ナルメルなどの考古資料と王名表の関係を考える鍵となるためです。"
      },
      "image": "assets/people/person-019b1d8d30.webp"
    },
    {
      "name": "エンヘドゥアンナ",
      "kana": "エンヘドゥアンナ",
      "era": "最初の文明の時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前23世紀ごろ、アッカド王サルゴンの娘として生まれ、シュメール都市ウルの月神ナンナに仕える最高祭司となりました。王家の人物を重要都市の宗教職に置くことで、アッカド王権とシュメールの伝統を結ぶ役割を担いました。女神イナンナをたたえる賛歌などに自らの名を記し、政治的追放と復帰の経験も作品に表しました。作者名が分かる世界最古級の文学者として、女性・宗教・王権・文字文化の歴史に残っています。",
        "whatDid": "ウルの最高祭司として王権と宗教を結び、自らの名を記した女神への賛歌を残しました。",
        "whyImportant": "作者名が知られる最古級の文学者で、古代の女性・宗教・政治を伝えるためです。"
      },
      "image": "assets/people/person-bae8508d5f.webp"
    },
    {
      "name": "クレイステネス",
      "kana": "クレイステネス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前6世紀末のアテネで、僭主追放後の権力争いに勝ち、市民の支持を得て政治改革を行いました。血縁と地域の有力者に結び付いた旧来の部族を改め、海岸・都市・内陸の地区を組み合わせた十部族を編成しました。五百人評議会を設け、市民が抽選や民会を通じて政治に参加する仕組みを広げました。女性・奴隷・在留外国人は市民から除かれていましたが、後のアテネ民主政の制度的な基礎を築いた人物です。",
        "whatDid": "血縁中心の部族制を地域別の十部族へ改め、五百人評議会など市民参加の制度を整えました。",
        "whyImportant": "アテネ民主政の制度的な土台をつくり、市民が政治に参加する範囲を広げたためです。"
      },
      "image": "assets/people/person-d5e93608d0.webp"
    },
    {
      "name": "カトー",
      "kana": "カトー",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマの政治家・軍人で、「大カトー」または「監察官カトー」と呼ばれます。執政官や監察官を務め、贅沢を戒め、祖先以来の質素な生活と農業をローマ人の理想として説きました。著書『農業論』は、ラテン語で全文が残る最古の散文作品です。晩年にはカルタゴの復興を危険視し、元老院で滅亡を繰り返し主張しました。その姿は、ローマの伝統主義と地中海帝国への拡大が併存した時代を示しています。",
        "whatDid": "監察官として風紀を引き締め、著書『農業論』を残し、カルタゴへの強硬策を訴えました。",
        "whyImportant": "共和政ローマの伝統的価値観と、カルタゴを滅ぼして地中海へ拡大する動きを象徴するためです。"
      },
      "image": "assets/people/person-e6cb8aba55.webp"
    },
    {
      "name": "ダレイオス1世",
      "kana": "ダレイオス1世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前6世紀末にアケメネス朝の王となり、反乱を鎮圧して帝国の支配を立て直しました。領土を州に分けて総督を置き、監察官を派遣して地方を監督しました。また、「王の道」と駅伝制、金貨ダリクを整え、税の基準を定めて、エジプトからインダス川流域までを結びました。ペルセポリスの建設を始めて多民族帝国の威信を示す一方、ギリシャ遠征では紀元前490年のマラトンの戦いに敗れました。",
        "whatDid": "帝国を州に分け、王の道・駅伝制・貨幣・税制を整えて広大な領土を統治しました。",
        "whyImportant": "多民族から成る大帝国を行政と交通で結び、後世の帝国統治の模範を示したためです。"
      },
      "image": "assets/people/person-91f786f396.webp"
    },
    {
      "name": "ペテロ",
      "kana": "ペテロ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ガリラヤの漁師で、イエスに従った十二使徒の一人です。福音書ではイエスを救い主と告白する一方、逮捕時には三度関係を否定し、その後に弟子たちの中心となった人物として描かれます。イエスの死後、エルサレムの信者共同体を指導し、異邦人を受け入れる議論にも関わりました。ローマで殉教したという伝承から、カトリック教会は彼を初代教皇と位置付け、教皇の権威を説明する重要人物としています。",
        "whatDid": "イエスの弟子集団を率い、死後の信者共同体を指導して初期教会の形成に関わりました。",
        "whyImportant": "初期キリスト教共同体の中心となり、後のローマ教皇の権威の起源とされたためです。"
      },
      "image": "assets/people/person-597700fe67.webp"
    },
    {
      "name": "則天武后",
      "kana": "則天武后",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の太宗の後宮に入り、のちに高宗の皇后となって政治への影響力を強めました。高宗の死後は皇太后として実権を握り、690年に国号を周と改め、中国史上ただ一人「皇帝」を名乗った女性です。科挙を通じて新しい官僚を登用し、仏教を保護して自らの支配を正当化しました。一方、政敵への厳しい弾圧も行いました。女性皇帝への偏見を含む後世の記録を読み直す必要がある人物でもあります。",
        "whatDid": "皇后・皇太后から実権を握り、周を建てて皇帝となり、科挙官僚と仏教を支配に用いました。",
        "whyImportant": "中国史上唯一の女性皇帝として、唐代の官僚制・宗教・女性の政治権力を示すためです。"
      },
      "image": "assets/people/person-634820150e.webp"
    },
    {
      "name": "テオドラ",
      "kana": "テオドラ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "6世紀の東ローマ皇帝ユスティニアヌスの皇后で、低い身分の出身から宮廷の最高位へ上ったと伝えられます。532年のニカの反乱で皇帝が都から逃れようとした際、抵抗を続けるよう主張したという記録で有名です。皇帝とともに政治判断や宗教政策に関わり、女性を強制売春から守る措置や、離婚・財産に関する権利の改善にも影響したとされます。ただし、生涯の記録には敵対者プロコピオスの偏見が強く、史料批判が必要です。",
        "whatDid": "ニカの反乱で政権を支え、皇帝とともに政治・宗教政策や女性保護の法整備に関わりました。",
        "whyImportant": "東ローマ宮廷で大きな政治権力を持った女性であり、史料の偏見を考える例でもあるためです。"
      },
      "image": "assets/people/person-7294fab46e.webp"
    },
    {
      "name": "バルトロメウ・ディアス",
      "kana": "バルトロメウ・ディアス",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "15世紀のポルトガル航海者で、アフリカ西岸を南下する船団を率いました。1488年、嵐で陸地を見失った後に進路を北へ変え、アフリカ南端を回ってインド洋側へ到達しました。帰路に確認した岬は当初「嵐の岬」と呼ばれ、国王がアジア航路への期待を込めて「喜望峰」と改名しました。本人はインドへ到達しませんでしたが、この航路の発見が、後のヴァスコ・ダ・ガマによるインド航海を可能にしました。",
        "whatDid": "1488年にアフリカ南端を回ってインド洋へ入り、ヨーロッパからアジアへ向かう海路を示しました。",
        "whyImportant": "喜望峰経由のインド航路を開く決定的な一歩となり、ポルトガルの海上進出を進めたためです。"
      },
      "image": "assets/people/person-4800aabf28.webp"
    },
    {
      "name": "モンテスキュー",
      "kana": "モンテスキュー",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀フランスの貴族・法思想家で、各国の政治制度や法律を比較して、権力がどのように働くかを研究しました。主著『法の精神』で、政治権力を立法・行政・司法に分け、互いに抑制させることで専制を防ぐ考えを示しました。これは三権分立と呼ばれます。イギリス政治の分析には単純化もありましたが、その理論はアメリカ合衆国憲法やフランス革命後の憲法をはじめ、近代国家の権力分立に大きな影響を与えました。",
        "whatDid": "『法の精神』で立法・行政・司法を分け、互いに抑制させる三権分立を説きました。",
        "whyImportant": "権力の集中を防ぐ制度原理を示し、近代の憲法と民主政治の設計に影響したためです。"
      },
      "image": "assets/people/person-2562164bcc.webp"
    },
    {
      "name": "ジェームズ・ハーグリーブス",
      "kana": "ジェームズ・ハーグリーブス",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18世紀イギリスの織工・発明家で、綿糸の需要が急増するなか、一人で複数の糸を同時に紡げる「ジェニー紡績機」を考案しました。手回しで動かす比較的簡単な機械でしたが、従来より多くの糸を短時間で生産でき、紡績工程の機械化を進めました。一方、機械によって仕事を失うことを恐れた職人から反発を受け、工場を移すことにもなりました。発明は後の大型紡績機へつながり、産業革命初期の綿工業を変えました。",
        "whatDid": "一人で複数の糸を同時に紡げるジェニー紡績機を考案し、綿糸生産を効率化しました。",
        "whyImportant": "紡績の機械化を進め、綿工業を産業革命の中心産業へ成長させる一歩となったためです。"
      },
      "image": "assets/people/person-94098b32d7.webp"
    },
    {
      "name": "デイヴィッド・リヴィングストン",
      "kana": "デイヴィッド・リヴィングストン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀のスコットランド出身の宣教師・探検家で、医師として南部・中部アフリカを各地へ進みました。ザンベジ川やビクトリア滝をヨーロッパに紹介し、地理情報を記録するとともに、東アフリカで続く奴隷貿易の実態を訴えました。現地社会を十分に理解できず、探検が後の植民地進出に利用された面もあります。消息を絶った後にスタンリーが発見した出来事でも有名ですが、現在は探検・宣教・帝国主義の関係を考える人物として扱われます。",
        "whatDid": "南部・中部アフリカを探検して河川や地形を記録し、奴隷貿易の廃止を訴えました。",
        "whyImportant": "アフリカ内陸の地理を欧州へ伝える一方、その記録が植民地進出にも利用されたためです。"
      },
      "image": "assets/people/person-90abd8cf8f.webp"
    },
    {
      "name": "ネヴィル・チェンバレン",
      "kana": "ネヴィル・チェンバレン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリス保守党の政治家で、1937年に首相となりました。戦争回避を優先し、1938年のミュンヘン会談でヒトラーによるチェコスロバキアのズデーテン地方併合を認める宥和政策を進めました。しかしドイツは翌年に残る地域も占領し、ポーランド侵攻によって第二次世界大戦が始まりました。開戦後はドイツへ宣戦しましたが、ノルウェー作戦の失敗で退陣しました。宥和政策は批判される一方、再軍備の時間を稼いだとの評価もあります。",
        "whatDid": "ミュンヘン会談でドイツへの譲歩を選び、戦争回避を目指す宥和政策を進めました。",
        "whyImportant": "独裁国への譲歩が侵略を止められなかった事例として、第二次世界大戦前史の中心となるためです。"
      },
      "image": "assets/people/person-fd6656d191.webp"
    },
    {
      "name": "インディラ・ガンディー",
      "kana": "インディラ・ガンディー",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "インド初代首相ネルーの娘として政治に入り、1966年に同国初の女性首相となりました。銀行国有化や農業増産を進め、1971年の第三次印パ戦争では東パキスタンの独立を支援し、バングラデシュ成立につなげました。一方、1975年から非常事態を宣言して反対派を拘束し、報道の自由を制限しました。1984年にはシク教徒武装勢力を排除するため黄金寺院へ軍を投入し、その報復として護衛に暗殺されました。",
        "whatDid": "銀行国有化や農業増産を進め、バングラデシュ独立を支援する一方、非常事態で反対派を抑えました。",
        "whyImportant": "インドの国家統合と地域秩序を動かす一方、民主政治と強権統治の緊張を示したためです。"
      },
      "image": "assets/people/person-83952b1af6.webp"
    },
    {
      "name": "ジェセル",
      "kana": "ジェセル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第3王朝の王で、紀元前27世紀ごろに統治したと考えられています。宰相イムホテプのもと、サッカラに石造の階段ピラミッドを建設しました。これは従来の長方形の墓マスタバを重ねた形から発展した、エジプト最古級の大規模石造建築です。周囲には神殿や祭礼施設も整えられ、王の死後の再生と永続する支配を表しました。後のギザのピラミッドへつながる建築技術と王権表現の出発点となった王です。",
        "whatDid": "イムホテプに命じてサッカラの階段ピラミッドと大規模な葬祭施設を建設しました。",
        "whyImportant": "大規模石造建築とピラミッド建設の出発点をつくり、古代エジプト王権を形で示したためです。"
      },
      "image": "assets/people/person-af40024a79.webp"
    },
    {
      "name": "ウルナンム",
      "kana": "ウルナンム",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前21世紀末、メソポタミアでウル第3王朝を開いた王です。アッカド帝国崩壊後の混乱を収め、シュメール地方を再統一し、各地の都市と灌漑施設を整えました。ウルでは月神ナンナを祭る巨大なジッグラトの建設を始め、王朝の宗教的中心を築きました。また、現存する最古級の法典「ウル・ナンム法典」と結び付けられ、罪に対して銀による賠償を定めた条文も残ります。国家再建と法の歴史を示す王です。",
        "whatDid": "ウル第3王朝を開いてシュメールを再統一し、ジッグラト建設と法の整備を進めました。",
        "whyImportant": "都市国家を再統合し、現存最古級の法典と大規模神殿建築を残したためです。"
      },
      "image": "assets/people/person-6ef4c2baf1.webp"
    },
    {
      "name": "ペリクレス",
      "kana": "ペリクレス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネ政治家で、民会での演説と支持を背景に長く指導的地位に立ちました。公職に手当を支給して、裕福でない市民も政治に参加しやすくし、民主政を発展させました。ペルシャ戦争後にはデロス同盟の資金を用いてパルテノン神殿などを建設し、文化を栄えさせました。一方、同盟諸国への支配を強め、スパルタとの対立からペロポネソス戦争に入りました。民主政と帝国的支配の両面を持つ人物です。",
        "whatDid": "公職手当で市民参加を広げ、パルテノン神殿を建設する一方、デロス同盟への支配を強めました。",
        "whyImportant": "アテネ民主政と古典文化の最盛期を築く一方、同盟を帝国化した矛盾も示すためです。"
      },
      "image": "assets/people/person-b9aecbca6c.webp"
    },
    {
      "name": "ティベリウス・グラックス",
      "kana": "ティベリウス・グラックス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前2世紀の共和政ローマの政治家で、護民官として大土地所有の拡大と自作農の減少に取り組みました。国有地の占有上限を定め、余った土地を貧しい市民へ分配する農地法を成立させました。しかし元老院の強い反対を受け、再選を目指した際に支持者とともに殺害されました。改革は一部実行されましたが、政治的暴力が公然と用いられる転機となりました。弟ガイウスの改革とともに共和政末期の社会対立を象徴します。",
        "whatDid": "国有地の占有を制限し、土地を失った市民へ農地を再分配する改革を進めました。",
        "whyImportant": "貧富の差と自作農の衰退を政治課題にし、共和政ローマの激しい党派対立の始まりを示したためです。"
      },
      "image": "assets/people/person-4e2e5a3bee.webp"
    },
    {
      "name": "クセルクセス1世",
      "kana": "クセルクセス1世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "アケメネス朝ペルシャの王で、父ダレイオス1世の死後に即位しました。エジプトとバビロニアの反乱を鎮圧した後、大軍と艦隊を率いてギリシャへ遠征しました。テルモピュライでスパルタ軍を破り、アテネを占領しましたが、サラミスの海戦で艦隊が敗れ、翌年にはプラタイアでも敗北しました。ペルセポリスの建設も進め、帝国の威信を示しました。ギリシャ側史料の誇張に注意しながら、東西の大戦争を考える王です。",
        "whatDid": "大軍でギリシャへ遠征し、テルモピュライを突破してアテネを占領しましたが、海戦で敗れました。",
        "whyImportant": "ペルシャ戦争の最大規模の遠征を行い、ギリシャ諸都市の結束とその後の歴史を左右したためです。"
      },
      "image": "assets/people/person-653eee17f8.webp"
    },
    {
      "name": "アウグスティヌス",
      "kana": "アウグスティヌス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "北アフリカのタガステに生まれ、修辞学を学んだ後、さまざまな思想を遍歴してキリスト教へ回心した神学者です。司教として教会を指導し、『告白』では自らの罪と回心を内面から描きました。西ローマが揺らぐなかで書いた『神の国』では、地上の国家と神を愛する共同体を区別し、ローマ帝国の運命とキリスト教の価値を同一視しませんでした。原罪・恩恵・自由意志をめぐる思想は、中世西欧の神学と哲学に長く影響しました。",
        "whatDid": "『告白』『神の国』を著し、原罪・恩恵・国家と信仰の関係を体系的に論じました。",
        "whyImportant": "古代哲学とキリスト教を結び、中世西ヨーロッパの神学と人間観の基礎を築いたためです。"
      },
      "image": "assets/people/person-0dbfd89beb.webp",
      "imageAlt": "アウグスティヌスの画像"
    },
    {
      "name": "玄宗",
      "kana": "玄宗",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の皇帝で、即位初期には有能な官僚を登用し、財政や行政を立て直しました。この時期は「開元の治」と呼ばれ、都長安には各地の商人や文化が集まり、唐は繁栄しました。しかし晩年は楊貴妃とその一族を重用し、節度使に大きな軍事力を持たせた結果、755年に安禄山らが反乱を起こしました。安史の乱の途中で退位し、唐は鎮圧後も地方軍人への依存を強めました。繁栄から衰退への転機を示す皇帝です。",
        "whatDid": "開元の治で唐を繁栄させましたが、節度使の勢力拡大を招き、安史の乱の途中で退位しました。",
        "whyImportant": "唐の最盛期と、地方軍人が強まって王朝が衰退へ向かう転換点の双方を体現するためです。"
      },
      "image": "assets/people/person-0cc7ff9461.webp"
    },
    {
      "name": "ヘラクレイオス",
      "kana": "ヘラクレイオス",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "7世紀初めの東ローマ皇帝で、ササン朝ペルシャ軍にシリア・エジプト・小アジアを奪われる危機に直面しました。軍制と財政を立て直し、自ら反攻を指揮してペルシャ領深くへ進み、奪われた聖十字架をエルサレムへ戻しました。しかし両国が消耗した直後、イスラム勢力が急速に進出し、シリアとエジプトを失いました。公用語や皇帝称号のギリシャ化も進み、古代ローマ帝国から中世ビザンツ帝国への転換期を代表します。",
        "whatDid": "ササン朝に反攻して失地を回復しましたが、その後イスラム勢力にシリアとエジプトを奪われました。",
        "whyImportant": "東ローマ帝国を一度は再建しつつ、イスラム進出とビザンツ化という大転換に直面したためです。"
      },
      "image": "assets/people/person-9a6aa60aa5.webp"
    },
    {
      "name": "ヴァスコ・ダ・ガマ",
      "kana": "ヴァスコ・ダ・ガマ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ポルトガルの航海者で、1497年にリスボンを出発し、喜望峰を回って東アフリカ沿岸を進みました。現地の航海知識を借りてインド洋を横断し、1498年にインドのカリカットへ到達しました。これによりヨーロッパとインドを直接結ぶ海路が開かれました。後の航海では武力で港を攻撃し、イスラム商人や住民に大きな被害を与えました。香辛料貿易の新航路を築く一方、ポルトガルの海上帝国と暴力的支配を始めた人物です。",
        "whatDid": "喜望峰を回ってインドへ到達し、ヨーロッパとアジアを結ぶ直接の海上航路を開きました。",
        "whyImportant": "香辛料貿易の経路を変え、ポルトガルのインド洋支配と欧州の海外進出を加速させたためです。"
      },
      "image": "assets/people/person-3b613a629f.webp"
    },
    {
      "name": "ヴォルテール",
      "kana": "ヴォルテール",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀フランスの思想家・作家で、王権や教会の権威が強い社会を風刺と論争で批判しました。宗教戦争や裁判の不正を問題にし、思想・言論・信仰の自由と宗教的寛容を訴えました。『哲学書簡』ではイギリスの政治や科学を紹介し、『カンディード』では戦争や楽観主義を風刺しました。一方、啓蒙専制君主との交流や当時の偏見も持っていました。その著作は、権威を理性で批判する啓蒙思想をヨーロッパへ広めました。",
        "whatDid": "風刺作品と論説で教会・専制政治・不正な裁判を批判し、信仰と言論の自由を訴えました。",
        "whyImportant": "理性と寛容を重んじる啓蒙思想を広め、フランス革命前の社会批判を強めたためです。"
      },
      "image": "assets/people/person-90ebf84bd9.webp"
    },
    {
      "name": "サミュエル・クロムプトン",
      "kana": "サミュエル・クロムプトン",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18世紀イギリスの紡績工で、ハーグリーブスのジェニー紡績機とアークライトの水力紡績機の長所を組み合わせ、ミュール紡績機を発明しました。この機械は細くて丈夫な糸を大量に紡げたため、上質な綿布の生産を可能にしました。しかし特許を取らずに設計を公開したため、多くの工場が利益を得る一方、本人は十分な報酬を得られませんでした。発明と資本の関係を示しながら、綿工業の機械化を大きく進めた人物です。",
        "whatDid": "ジェニー紡績機と水力紡績機を組み合わせ、細く丈夫な糸を紡ぐミュール紡績機を発明しました。",
        "whyImportant": "上質な綿糸の大量生産を可能にし、イギリス綿工業の国際的な成長を支えたためです。"
      },
      "image": "assets/people/person-51af8aee07.webp"
    },
    {
      "name": "ヘンリー・モートン・スタンリー",
      "kana": "ヘンリー・モートン・スタンリー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ウェールズ生まれでアメリカへ渡り、新聞記者・探検家としてアフリカで活動しました。消息不明だったリヴィングストンを1871年に発見したことで有名になり、その後コンゴ川流域を横断して地理を記録しました。ベルギー王レオポルド2世の依頼で各地の首長と条約を結び、道路や拠点を築きましたが、これらはコンゴ自由国の成立と過酷な植民地支配に利用されました。探検の成果と帝国主義への加担を切り離せない人物です。",
        "whatDid": "リヴィングストンを発見し、コンゴ川流域を調査して、ベルギー王の植民地建設に協力しました。",
        "whyImportant": "アフリカ探検が欧州の領土獲得と暴力的な植民地支配へ直結した過程を示すためです。"
      },
      "image": "assets/people/person-b08f4fcadd.webp"
    },
    {
      "name": "アドルフ・ヒトラー",
      "kana": "アドルフ・ヒトラー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "オーストリア生まれで第一次世界大戦後にドイツ労働者党へ入り、ナチ党の指導者となりました。世界恐慌と政治不安を利用して1933年に首相となり、議会制民主主義を破壊して独裁体制を築きました。人種差別と反ユダヤ主義を国家政策にし、侵略戦争を始めて第二次世界大戦を引き起こしました。ユダヤ人を中心に約600万人を組織的に殺害したホロコーストを進め、ヨーロッパ各地に甚大な被害を与え、敗戦直前に自殺しました。",
        "whatDid": "ナチ党独裁を築き、侵略戦争とホロコーストを進め、第二次世界大戦と大量虐殺を引き起こしました。",
        "whyImportant": "独裁・人種差別・侵略が社会と国家を破壊し、世界規模の戦争と大量虐殺へ至る危険を示すためです。"
      },
      "image": "assets/people/person-860e88f804.webp"
    },
    {
      "name": "ベナジル・ブット",
      "kana": "ベナジル・ブット",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "パキスタン元首相ズルフィカール・アリー・ブットの娘で、父が軍事政権に処刑された後、民主化運動の中心となりました。1988年に首相へ就任し、イスラム教徒が多数を占める国で初めて政府を率いた女性となりました。二度首相を務めましたが、軍や大統領との対立、汚職疑惑によりいずれも任期途中で解任されました。亡命後に帰国し、軍政から民政への移行を訴える選挙運動中の2007年、襲撃されて死亡しました。",
        "whatDid": "軍事政権に対する民主化運動を率い、イスラム教徒多数国で初の女性首相となりました。",
        "whyImportant": "女性の政治参加を切り開く一方、軍・政党・汚職が絡むパキスタン政治の困難を示したためです。"
      },
      "image": "assets/people/person-1f3e28397d.webp"
    },
    {
      "name": "スネフェル",
      "kana": "スネフェル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第4王朝を開いた王で、紀元前26世紀ごろに統治しました。メイドゥム、ダハシュールに複数のピラミッドを建設し、途中で傾斜を変えた屈折ピラミッドを経て、滑らかな斜面を持つ赤いピラミッドを完成させました。この試行錯誤によって真正ピラミッドの技術が確立され、子のクフ王によるギザの大ピラミッドへつながりました。シナイ半島への遠征や木材輸入も行い、王朝の経済力と組織力を高めた王です。",
        "whatDid": "複数のピラミッド建設を通して真正ピラミッドの技術を確立し、第4王朝の基盤を築きました。",
        "whyImportant": "ギザの大ピラミッドへつながる建築技術と、古王国の強い王権を完成へ導いたためです。"
      },
      "image": "assets/people/person-62e4a669a6.webp"
    },
    {
      "name": "グデア",
      "kana": "グデア",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician",
        "culture"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前22世紀ごろ、アッカド帝国崩壊後の南メソポタミアで都市国家ラガシュを治めた王です。戦争の記録よりも、神殿や運河の建設、神々への奉納を強調した多数の碑文で知られます。遠方から閃緑岩、木材、金属などを取り寄せ、交易の広がりを示しました。祈る姿を表した多くの石像には王名と事業が刻まれ、王が神々と都市を結ぶ存在であることを示しました。像と碑文から統治を詳しく知ることができる人物です。",
        "whatDid": "ラガシュで神殿と運河を建設し、遠隔地交易で資材を集め、多数の奉納像と碑文を残しました。",
        "whyImportant": "王の宗教的役割、都市建設、交易を像と碑文から具体的に知ることができるためです。"
      },
      "image": "assets/people/person-d69e810d52.webp"
    },
    {
      "name": "レオニダス",
      "kana": "レオニダス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "古代ギリシャの都市国家スパルタの王で、紀元前480年のペルシャ戦争でテルモピュライの防衛を指揮しました。狭い道を利用し、スパルタ兵を含むギリシャ連合軍でクセルクセス1世の大軍を数日間食い止めました。迂回路を知られると、一部の兵を退却させ、自身は残った兵とともに戦死しました。後世には「300人」の英雄物語として語られましたが、実際には他の都市の兵も参加しており、史実と伝説を区別する必要があります。",
        "whatDid": "テルモピュライの狭路でギリシャ連合軍を指揮し、ペルシャ大軍を食い止めて戦死しました。",
        "whyImportant": "ギリシャ側の抵抗を象徴する一方、後世の英雄伝説と実際の連合軍の違いを考えられるためです。"
      },
      "image": "assets/people/person-ceda1bc256.webp"
    },
    {
      "name": "マリウス",
      "kana": "マリウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前2世紀末から1世紀初めの共和政ローマの将軍・政治家です。ユグルタ戦争や北方から侵入したキンブリ人・テウトニ人との戦いで勝利し、異例の七度にわたり執政官となりました。財産の少ない市民も軍へ参加させ、装備を国家が支える方向を進めたため、兵士が国家より将軍個人へ忠誠を寄せる傾向が強まりました。晩年にはスッラとの権力争いでローマを武力制圧し、内戦と軍事独裁の時代への道を開きました。",
        "whatDid": "無産市民を軍へ採用してローマ軍を強化し、外敵を破りましたが、スッラとの内戦を招きました。",
        "whyImportant": "軍隊の性格を変え、将軍同士の私兵化と内戦が共和政を崩す流れを強めたためです。"
      },
      "image": "assets/people/person-3b825e131f.webp"
    },
    {
      "name": "アルタクセルクセス1世",
      "kana": "アルタクセルクセス1世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "アケメネス朝ペルシャの王で、父クセルクセス1世が宮廷内で暗殺された後、混乱を経て即位しました。エジプトで起きた反乱にはアテネ艦隊も加わりましたが、将軍メガビュゾスを送り鎮圧し、帝国の支配を維持しました。ギリシャ諸国とは長い戦争の後、カリアスの和約と呼ばれる和平が結ばれたと伝えられますが、その実在や内容には議論があります。聖書のエズラ記・ネヘミヤ記とも関係づけられ、多民族帝国の統治を考える王です。",
        "whatDid": "宮廷混乱後に帝国を立て直し、エジプト反乱を鎮圧してアケメネス朝の広域支配を保ちました。",
        "whyImportant": "反乱とギリシャ勢力の介入に対応し、多民族帝国の安定を維持した統治を示すためです。"
      },
      "image": "assets/people/person-3b4f71264c.webp",
      "imageAlt": "アルタクセルクセス1世の画像"
    },
    {
      "name": "ヒエロニムス",
      "kana": "ヒエロニムス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4～5世紀のキリスト教神学者で、ローマで学んだ後、修道生活を送りながらギリシャ語とヘブライ語を研究しました。教皇ダマスス1世の依頼を受け、聖書をラテン語へ翻訳・校訂しました。旧約聖書では当時一般的だったギリシャ語訳だけでなく、ヘブライ語本文を重視しました。この翻訳は後に「ウルガタ」と呼ばれ、西ヨーロッパの教会、学問、文学で千年以上使われました。翻訳が宗教理解と文化形成を左右することを示す人物です。",
        "whatDid": "ヘブライ語とギリシャ語の聖書をラテン語へ翻訳・校訂し、ウルガタ聖書の基礎を築きました。",
        "whyImportant": "西ヨーロッパで長く標準となる聖書本文を整え、宗教・教育・文学へ影響したためです。"
      },
      "image": "assets/people/person-e2200bc7bb.webp"
    },
    {
      "name": "楊貴妃",
      "kana": "楊貴妃",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "war",
        "society"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の玄宗皇帝に寵愛された女性で、本名を楊玉環といいます。もとは玄宗の子の妃でしたが、のちに道教の女道士を経て皇帝の貴妃となりました。親族の楊国忠らが高い地位を得たため、宮廷政治の腐敗を象徴する存在として後世に語られました。755年に安史の乱が起こり、玄宗が都を逃れる途中、兵士たちの要求によって殺されました。ただし乱の原因を彼女一人へ負わせる見方は、女性への偏見を含む文学的伝承として検討が必要です。",
        "whatDid": "玄宗の貴妃として宮廷で大きな影響力を持ち、安史の乱の逃避行中に兵士の要求で殺されました。",
        "whyImportant": "唐の繁栄と崩壊を語る象徴となり、女性へ政治責任を負わせる歴史叙述も考えられるためです。"
      },
      "image": "assets/people/person-7226500134.webp"
    },
    {
      "name": "バシレイオス2世",
      "kana": "バシレイオス2世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "10～11世紀の東ローマ皇帝で、若い頃は有力貴族の反乱に苦しみましたが、親政を確立すると大土地所有者を抑え、軍事と財政を立て直しました。長年にわたりブルガリアと戦い、1018年に征服してバルカン半島の支配を広げました。キエフ公ウラジーミル1世から援軍を得る代わりに皇妹を結婚させ、ルーシのキリスト教化にも関わりました。帝国を最盛期へ導いた一方、後継体制を十分に整えなかった点が限界でした。",
        "whatDid": "有力貴族の反乱を抑えて皇帝権を強め、ブルガリアを征服して東ローマ帝国を最盛期へ導きました。",
        "whyImportant": "東ローマ帝国の軍事・財政・領土を立て直し、中世ビザンツの最盛期を築いたためです。"
      },
      "image": "assets/people/person-8ef1cb23d8.webp"
    },
    {
      "name": "フェルディナンド・マゼラン",
      "kana": "フェルディナンド・マゼラン",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ポルトガル出身の航海者で、スペイン王の支援を受け、西回りで香辛料諸島へ到達する航路を探しました。1519年に五隻の船で出航し、南アメリカ南端の海峡を通過して太平洋を横断しました。航海の途中、フィリピンのマクタン島で現地勢力との戦いに介入して死亡したため、本人は世界一周を完成していません。残った船団をエルカーノが率いて帰国し、地球を海路で一周できることと、世界の海の広大さを実証しました。",
        "whatDid": "スペイン船団を率いて南アメリカ南端の海峡を通過し、ヨーロッパ人として初めて太平洋を横断しました。",
        "whyImportant": "船団の航海が初の世界一周を実現し、地球の大きさと海洋のつながりを具体的に示したためです。"
      },
      "image": "assets/people/person-c04ca7626f.webp",
      "imageAlt": "フェルディナンド・マゼランの画像"
    },
    {
      "name": "ルソー",
      "kana": "ルソー",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀ジュネーヴ生まれの思想家で、身分制社会と文明のあり方を批判しました。『社会契約論』で、政治の正当な権力は人民全体の意思である「一般意志」に基づくと説き、主権は人民にあると主張しました。『エミール』では、子どもの成長段階と自主性を重んじる教育を論じました。一方、一般意志の解釈には、少数者の自由を抑える危険も指摘されています。その思想はフランス革命と近代民主主義、教育論に大きな影響を与えました。",
        "whatDid": "『社会契約論』で人民主権と一般意志を説き、『エミール』で子どもの自主性を重んじる教育を論じました。",
        "whyImportant": "人民が政治の主権者であるという考えを広め、革命・民主政治・近代教育へ影響したためです。"
      },
      "image": "assets/people/person-59efcfe648.webp"
    },
    {
      "name": "ジョージ・スティーブンソン",
      "kana": "ジョージ・スティーブンソン",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "イギリス北部の炭鉱で働きながら機械技術を学び、蒸気機関車を実用的な交通手段へ発展させた技術者です。1825年にストックトン・ダーリントン鉄道で「ロコモーション号」を走らせ、1830年開業のリバプール・マンチェスター鉄道では、息子ロバートらと製作した「ロケット号」が採用されました。鉄道の線路幅や勾配の設計にも関わり、石炭・原料・製品・人を大量に速く運ぶ鉄道網の普及を進めました。",
        "whatDid": "蒸気機関車を改良し、営業鉄道の建設と運行を進めて、人や物を大量に運ぶ鉄道輸送を普及させました。",
        "whyImportant": "鉄道で炭鉱・工場・港・都市を結び、産業革命期の生産・流通・人の移動を大きく変えたためです。"
      },
      "image": "assets/people/person-19fc20ba52.webp"
    },
    {
      "name": "レオポルド2世",
      "kana": "レオポルド2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半のベルギー国王で、植民地獲得を目指し、探検家スタンリーを利用してコンゴ川流域の支配権を集めました。1885年に国ではなく自らの私領としてコンゴ自由国を成立させ、象牙と天然ゴムを強制的に集めさせました。現地住民は人質、暴行、身体切断、殺害などの過酷な支配を受け、人口も大きく減少しました。国際的な告発を受けて1908年にベルギー政府へ統治権を渡しましたが、植民地暴力の象徴として記憶されています。",
        "whatDid": "コンゴ自由国を私領として支配し、象牙とゴムを強制徴収する過程で住民へ大規模な暴力を加えました。",
        "whyImportant": "欧州帝国主義が企業利益と王権のもとで大量の強制労働と人命被害を生んだことを示すためです。"
      },
      "image": "assets/people/person-ff61fbc0c9.webp"
    },
    {
      "name": "ベニート・ムッソリーニ",
      "kana": "ベニート・ムッソリーニ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "第一次世界大戦後のイタリアでファシスト党を組織し、社会不安と反共感情を利用して勢力を広げました。1922年のローマ進軍後に首相となり、政党・議会・報道の自由を奪って一党独裁を築きました。企業と国家を統制する体制を掲げ、エチオピア侵略やアルバニア占領を進め、ナチス・ドイツと同盟して第二次世界大戦へ参戦しました。敗戦が近づくと失脚し、1945年にパルチザンに捕らえられて処刑されました。",
        "whatDid": "ファシスト党独裁を築き、エチオピア侵略とナチス・ドイツとの同盟を進めて第二次世界大戦へ参戦しました。",
        "whyImportant": "ファシズムが民主政治を破壊し、民族主義と軍事侵略を正当化した過程を示すためです。"
      },
      "image": "assets/people/person-af4451019a.webp"
    },
    {
      "name": "アウンサンスーチー",
      "kana": "アウンサンスーチー",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ミャンマー独立運動を率いたアウンサンの娘で、1988年の民主化運動を機に国民民主連盟を率いました。軍政下で長期間の自宅軟禁を受け、非暴力の民主化運動を象徴してノーベル平和賞を受賞しました。2015年の選挙後は国家顧問として事実上の政権指導者となりましたが、ロヒンギャへの迫害を止めず、国際司法裁判所で軍の行動を擁護して強い批判を受けました。2021年の軍事クーデター後に拘束され、民主化の象徴と政権担当時の責任の両面で評価されています。",
        "whatDid": "軍政下で民主化運動を率いて長く拘束され、政権参加後は国家顧問としてミャンマー政府を指導しました。",
        "whyImportant": "民主化運動の象徴となる一方、ロヒンギャ迫害への対応で人権と政治責任の問題を残したためです。"
      },
      "image": "assets/people/person-2a1b09d78c.webp",
      "imageAlt": "アウンサンスーチーの画像"
    },
    {
      "name": "カフラー",
      "kana": "カフラー",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第4王朝の王で、クフ王の後継世代として紀元前26世紀ごろに統治しました。ギザにクフ王のものに次ぐ大きさのピラミッドを建設し、参道、河岸神殿、葬祭殿を組み合わせた施設を整えました。近くの大スフィンクスも一般にカフラーの時代と結び付けられていますが、造営者には議論があります。王の頭にハヤブサ神ホルスが翼を広げる座像は、王が神々に守られる存在であることを示し、王権と葬祭信仰を伝えます。",
        "whatDid": "ギザにピラミッドと葬祭施設を建設し、神に守られた王権を壮大な石造建築と像で示しました。",
        "whyImportant": "ピラミッド複合体と王の神聖性を通して、古王国の強大な王権と死後信仰を伝えるためです。"
      },
      "image": "assets/people/person-404d9369ab.webp"
    },
    {
      "name": "シュルギ",
      "kana": "シュルギ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "ウル第3王朝の二代王で、父ウルナンムの死後、紀元前21世紀に長期間統治しました。道路と宿駅を整え、税や労働、家畜、物資を粘土板で記録する官僚制度を発達させ、度量衡や暦の統一も進めました。自らを神格化し、運動能力や学識をたたえる賛歌を作らせて王権を強調しました。周辺地域への遠征で帝国を広げましたが、巨大な行政組織と強制的な労働・徴税は人々へ重い負担を課しました。",
        "whatDid": "道路・宿駅・税制・記録制度を整え、王を神格化してウル第3王朝の中央集権的支配を完成させました。",
        "whyImportant": "粘土板記録を用いた官僚国家と、神格化された王権の具体的な姿を示すためです。"
      },
      "image": "assets/people/person-939d90be9c.webp"
    },
    {
      "name": "テミストクレス",
      "kana": "テミストクレス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀初めのアテネ政治家・将軍で、ペルシャの再侵攻に備えて海軍力の強化を進めました。ラウリオン銀山の収入を市民へ分配せず、多数の三段櫂船建造へ使うよう説得しました。クセルクセス1世の大軍が侵攻すると、アテネ市民を避難させ、狭いサラミス海峡へペルシャ艦隊を誘い込んで勝利しました。この海戦はギリシャ側の反攻を可能にしましたが、後に陶片追放され、最終的にはペルシャ王のもとへ亡命しました。",
        "whatDid": "アテネ海軍を強化し、サラミスの海戦で地形を利用してペルシャ艦隊を破る作戦を指揮しました。",
        "whyImportant": "ペルシャのギリシャ征服を防ぎ、アテネが海上勢力として発展する基礎を築いたためです。"
      },
      "image": "assets/people/person-9d2f5f6c9d.webp"
    },
    {
      "name": "スラ",
      "kana": "スラ",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の将軍・政治家で、ユグルタ戦争や同盟市戦争で軍功を立てました。東方遠征の指揮権をめぐってマリウス派と争い、ローマ軍を率いて首都へ進軍した最初の将軍となりました。内戦に勝利すると独裁官となり、政敵を「追放者名簿」に載せて殺害と財産没収を進めました。元老院の権限を強め、護民官を弱体化する改革を行った後、自ら辞任しましたが、軍事力で政治を支配する先例を残しました。",
        "whatDid": "軍を率いてローマを占領し、独裁官として政敵を粛清し、元老院中心の政治制度を再編しました。",
        "whyImportant": "将軍が私兵化した軍で政権を奪う先例をつくり、共和政崩壊への流れを決定的に強めたためです。"
      },
      "image": "assets/people/person-0894103d82.webp"
    },
    {
      "name": "ゾロアスター",
      "kana": "ゾロアスター",
      "era": "古代の大国の時代",
      "genre": [
        "religion"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "古代イランの宗教家で、活動した年代は紀元前2千年紀から前6世紀まで諸説があり、確定していません。聖典『アヴェスター』のうち最古層の賛歌「ガーサー」は本人の教えに近いと考えられます。最高神アフラ・マズダーを信じ、人は善い思い・善い言葉・善い行いを選び、真理と虚偽の戦いに責任を持つと説きました。後のゾロアスター教では最後の審判や死者の復活が体系化され、古代イラン諸王朝の宗教文化に長く影響しました。",
        "whatDid": "アフラ・マズダーへの信仰と、善悪の間で人が自ら正しい行いを選ぶ責任を説きました。",
        "whyImportant": "古代イランの宗教を形づくり、善悪・審判・復活をめぐる後世の宗教思想にも影響したためです。"
      },
      "image": "assets/people/person-0d919524fb.webp"
    },
    {
      "name": "アタナシウス",
      "kana": "アタナシウス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4世紀のアレクサンドリア司教で、キリストの神性をめぐる論争でニカイア信条を擁護しました。アリウス派が、子であるキリストは父なる神に造られた存在だと考えたのに対し、アタナシウスは父と子は同じ神性を持つと主張しました。皇帝の宗教政策と対立して何度も追放されましたが、著作と教会活動を通して立場を守りました。彼の主張は381年のコンスタンティノープル公会議へ受け継がれ、三位一体教義の確立に重要な役割を果たしました。",
        "whatDid": "アリウス派に反対してキリストは父なる神と同じ神性を持つと主張し、ニカイア信条を守りました。",
        "whyImportant": "キリスト教の三位一体教義と、正統・異端を定める教会の枠組みの形成に影響したためです。"
      },
      "image": "assets/people/person-8a96ecd37a.webp",
      "imageAlt": "アタナシウスの画像"
    },
    {
      "name": "安禄山",
      "kana": "安禄山",
      "era": "交易と王国の時代",
      "genre": [
        "war"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "ソグド系と突厥系の背景を持ち、唐の北東辺境で軍功を重ねた節度使です。玄宗と楊貴妃の信任を得て複数の軍管区を支配しましたが、宰相楊国忠との対立などから755年に反乱を起こし、洛陽を占領して燕の皇帝を名乗りました。反乱軍は長安も陥落させ、玄宗は都を逃れました。安禄山自身は757年に部下に殺されましたが、安史の乱は763年まで続き、多数の死者と社会混乱を生み、唐の中央支配を大きく弱めました。",
        "whatDid": "辺境軍を率いて唐へ反乱を起こし、洛陽と長安を占領して燕の皇帝を名乗りました。",
        "whyImportant": "安史の乱によって唐の人口・財政・中央集権を打撃し、王朝衰退の転換点をつくったためです。"
      },
      "image": "assets/people/person-1da973da9b.webp"
    },
    {
      "name": "ウラジーミル1世",
      "kana": "ウラジーミル1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "10世紀末のキエフ大公で、内戦を勝ち抜いてルーシの広い地域を支配しました。東ローマ皇帝バシレイオス2世へ軍事援助を送り、その皇妹アンナとの結婚を実現した後、988年ごろに東方正教会のキリスト教へ改宗しました。キエフの住民にも洗礼を受けさせ、教会建設と聖職者の受け入れを進めました。改宗は政治的統合と国際的地位の向上に役立ち、ルーシの文字文化・美術・宗教をビザンツ世界へ結び付けました。",
        "whatDid": "東方正教会へ改宗し、キエフ・ルーシに洗礼と教会制度を広め、ビザンツ世界との結び付きを強めました。",
        "whyImportant": "ロシア・ウクライナ・ベラルーシへ続く東スラヴ世界の宗教と文化の方向を定めたためです。"
      },
      "image": "assets/people/person-0d144a0281.webp"
    },
    {
      "name": "フアン・セバスティアン・エルカーノ",
      "kana": "フアン・セバスティアン・エルカーノ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "スペイン北部バスク地方出身の航海者で、マゼラン船団の一員として1519年に出航しました。マゼランがフィリピンで死亡し、船団が大きく損耗した後、「ビクトリア号」の船長として香辛料諸島からインド洋を西へ進みました。ポルトガル勢力を避けながら喜望峰を回り、1522年にスペインへ帰還しました。出航した約270人のうち帰国できたのはわずかでしたが、エルカーノと生存者は史上初の世界一周を完成させ、海洋世界の一体性を実証しました。",
        "whatDid": "マゼラン死後にビクトリア号を率い、香辛料諸島から喜望峰を回って史上初の世界一周を完成させました。",
        "whyImportant": "地球を船で一周できることを実証し、大西洋・太平洋・インド洋がつながる世界像を示したためです。"
      },
      "image": "assets/people/person-3e59131edb.webp"
    },
    {
      "name": "ディドロ",
      "kana": "ディドロ",
      "era": "革命と工場の時代",
      "genre": [
        "culture",
        "political-scholar"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀フランスの思想家・作家で、ダランベールらとともに『百科全書』の編集を主導しました。この書物は科学、技術、産業、哲学など当時の知識を体系的に集め、職人の作業工程も図版で詳しく紹介しました。教会や王権への批判を含んだため発禁処分や検閲を受けましたが、協力者をまとめて刊行を続けました。知識を一部の権威から解放し、理性と経験によって社会を改善できるという啓蒙思想を広い読者へ伝えました。",
        "whatDid": "『百科全書』を編集し、科学・技術・産業・思想の知識を集めて、検閲下でも刊行を続けました。",
        "whyImportant": "知識を体系化して広く共有し、権威を理性で批判する啓蒙思想を社会へ広めたためです。"
      },
      "image": "assets/people/person-55a6d1017e.webp"
    },
    {
      "name": "イザムバード・キングダム・ブルネル",
      "kana": "イザムバード・キングダム・ブルネル",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの土木技術者で、鉄道・橋・トンネル・蒸気船を大規模に設計しました。グレート・ウェスタン鉄道では高速で安定した運行を目指して広軌を採用し、橋梁や駅も一体的に整備しました。テムズ川の下を通るトンネル工事に参加し、蒸気船「グレート・ウェスタン」「グレート・ブリテン」「グレート・イースタン」では長距離航海に新技術を導入しました。失敗や巨額費用も伴いましたが、産業革命期の交通網を世界規模へ広げました。",
        "whatDid": "鉄道・橋・トンネル・大型蒸気船を設計し、陸上と海上の長距離交通を一体的に発展させました。",
        "whyImportant": "産業革命の土木・造船技術を大型交通網へ結び付け、人と物の移動範囲を世界規模に広げたためです。"
      },
      "image": "assets/people/person-e6f9de6616.webp",
      "imageAlt": "イザムバード・キングダム・ブルネルの画像"
    },
    {
      "name": "メネリク2世",
      "kana": "メネリク2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀末から20世紀初めのエチオピア皇帝で、地方勢力を統合しながら領土を南方へ拡大し、現在の国土の基礎を築きました。武器を輸入して軍を近代化し、1896年のアドワの戦いでイタリア軍を破り、エチオピアの独立を守りました。首都アディスアベバを発展させ、鉄道・電信・学校なども導入しました。一方、征服された南部の人々には土地喪失や重い負担が生じました。反植民地勝利の象徴であると同時に、国内征服の問題も持つ皇帝です。",
        "whatDid": "軍を近代化してアドワの戦いでイタリアを破り、エチオピアの独立を守りながら領土を拡大しました。",
        "whyImportant": "アフリカ分割期に欧州軍を破って独立を守り、反植民地主義の象徴となったためです。"
      },
      "image": "assets/people/person-4353807e5f.webp"
    },
    {
      "name": "フランシスコ・フランコ",
      "kana": "フランシスコ・フランコ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "スペインの軍人で、1936年に左派連合政府へ反乱を起こし、内戦でドイツ・イタリアの支援を受けました。1939年に勝利すると国家元首となり、政党活動、労働運動、地方言語を抑圧し、反対派を処刑・投獄する独裁体制を築きました。第二次世界大戦では正式参戦を避けましたが、枢軸国へ協力しました。冷戦期には反共国家として西側諸国との関係を改善し、1975年に死去するまで統治しました。その後スペインは立憲君主制と民主政治へ移行しました。",
        "whatDid": "内戦で共和国政府を倒し、反対派を弾圧する権威主義的独裁を死去まで続けました。",
        "whyImportant": "20世紀ヨーロッパで長期独裁を維持し、その死後の民主化と歴史記憶の問題を残したためです。"
      },
      "image": "assets/people/person-c77ab10d24.webp"
    },
    {
      "name": "リー・クアンユー",
      "kana": "リー・クアンユー",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリス植民地時代のシンガポールで弁護士となり、人民行動党を結成して自治と独立を進めました。1959年から首相を務め、マレーシアから分離した1965年以後、外国企業の誘致、住宅建設、教育、港湾整備、汚職対策を進め、都市国家を国際的な貿易・金融拠点へ成長させました。一方、野党や報道への訴訟、集会・言論の制限などで長期的な一党優位を維持しました。経済発展と政治的自由の制約を併せ持つ統治で評価が分かれます。",
        "whatDid": "独立後のシンガポールで工業化・住宅・教育・汚職対策を進め、国際的な都市国家を築きました。",
        "whyImportant": "資源の乏しい小国を急成長させた一方、経済発展と政治的自由の両立をめぐる論点を残したためです。"
      },
      "image": "assets/people/person-185b55fcd2.webp",
      "imageAlt": "リー・クアンユーの画像"
    },
    {
      "name": "メンカウラー",
      "kana": "メンカウラー",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第4王朝の王で、紀元前26世紀ごろ、クフ王・カフラー王に続く時期を治めました。ギザに三大ピラミッドのうち最も小さいピラミッドを築き、下部には高価で硬い花崗岩を用いました。王を女神ハトホルや地方神とともに表した三体像が複数残り、王権が各地の神々と結ばれていたことを示します。後世の伝承では温和な王として描かれましたが、同時代資料は限られています。建築と彫刻から古王国の王権を知る人物です。",
        "whatDid": "ギザにピラミッドと葬祭施設を築き、女神や地方神と並ぶ王の三体像を多数残しました。",
        "whyImportant": "建築と彫刻によって、古王国の王が神々や地方社会と結び付く仕組みを伝えるためです。"
      },
      "image": "assets/people/person-b366e7633a.webp",
      "imageAlt": "メンカウラーの画像"
    },
    {
      "name": "リムシン",
      "kana": "リムシン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前19～18世紀、南メソポタミアの都市国家ラルサを長く治めた王です。周辺都市との戦争を重ね、紀元前1794年ごろに宿敵イシンを征服して南部の広い地域を支配しました。運河の開削、神殿建設、農地管理を進め、年名や行政文書に事業を記録しました。しかしバビロン王ハンムラビとの戦いに敗れ、紀元前1763年ごろにラルサは併合されました。その治世は、都市国家間の競争からバビロン統一へ移る直前の政治と経済を示します。",
        "whatDid": "ラルサを拠点にイシンを征服し、運河・神殿・農地を整備して南メソポタミアを広く支配しました。",
        "whyImportant": "ハンムラビによる統一直前の都市国家間競争と、灌漑農業を支える王権の姿を示すためです。"
      },
      "image": "assets/people/person-724f1ef951.webp"
    },
    {
      "name": "ミルティアデス",
      "kana": "ミルティアデス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀初めのアテネ将軍で、かつてトラキアのケルソネソスを支配し、ペルシャ帝国にも従属した経験を持ちました。ペルシャ軍がアッティカ地方へ上陸すると、紀元前490年のマラトンの戦いでアテネ軍を指揮し、重装歩兵を素早く前進させて敵軍を破りました。この勝利はアテネ市民に大きな自信を与えました。しかし翌年のパロス島遠征に失敗し、裁判で罰金を科され、負傷が原因で死亡しました。英雄化だけでなく失敗も残る人物です。",
        "whatDid": "マラトンの戦いでアテネ重装歩兵を指揮してペルシャ軍を破りましたが、後の遠征には失敗しました。",
        "whyImportant": "ペルシャの第一次侵攻を退け、アテネ市民と重装歩兵の政治的自信を高めたためです。"
      },
      "image": "assets/people/person-ede741edb8.webp",
      "imageAlt": "ミルティアデスの画像"
    },
    {
      "name": "ポンペイウス",
      "kana": "ポンペイウス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の将軍で、若くしてスラ側で軍功を立て、「大ポンペイウス」と呼ばれました。地中海の海賊を短期間で制圧し、ミトリダテス戦争を終わらせ、シリアを属州化して東方支配を再編しました。カエサル、クラッススと第一回三頭政治を結びましたが、クラッススの死後にカエサルと対立し、内戦へ入りました。ファルサロスの戦いで敗れてエジプトへ逃れ、上陸直後に殺害されました。軍事的成功と共和政崩壊を結ぶ人物です。",
        "whatDid": "海賊を制圧してローマの東方領土を再編し、三頭政治後はカエサルと内戦を戦いました。",
        "whyImportant": "ローマの地中海支配を広げる一方、将軍同士の権力争いで共和政崩壊を加速させたためです。"
      },
      "image": "assets/people/person-ecc998d997.webp",
      "imageAlt": "ポンペイウスの画像"
    },
    {
      "name": "チャンドラグプタ",
      "kana": "チャンドラグプタ",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前4世紀末、ナンダ朝を倒してマウリヤ朝を開いたインドの王です。アレクサンドロス大王の死後に北西インドへ進出し、セレウコス1世との戦いではインダス川周辺の支配を認めさせました。都パータリプトラを中心に官僚・軍隊・徴税制度を整え、北インドの広い地域を統一しました。後世のジャイナ教伝承では晩年に出家したとされます。大帝国の基礎を築き、孫アショーカ王の時代へつなげた人物です。",
        "whatDid": "ナンダ朝を倒してマウリヤ朝を開き、北インドを統一して官僚・軍事・徴税制度を整えました。",
        "whyImportant": "インド最初の大規模な統一帝国を築き、アショーカ王による広域支配の土台をつくったためです。"
      },
      "image": "assets/people/person-60e45e3ac2.webp",
      "imageAlt": "チャンドラグプタの画像"
    },
    {
      "name": "アリウス",
      "kana": "アリウス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4世紀初めのアレクサンドリアの司祭で、父なる神と子であるキリストの関係をめぐって論争を起こしました。アリウスは、キリストは神に造られた特別な存在であり、父なる神と同じ永遠の神性を持たないと説きました。これに対しアタナシウスらは父と子は同じ神性を持つと主張しました。325年のニカイア公会議でアリウスの教えは退けられましたが、各地の王や司教に支持され、論争は長く続きました。",
        "whatDid": "キリストは父なる神に造られた存在だと説き、三位一体をめぐる大論争を引き起こしました。",
        "whyImportant": "キリスト教が正統教義と異端を定め、三位一体の考えを形成するきっかけとなったためです。"
      },
      "image": "assets/people/person-e75a7651c8.webp",
      "imageAlt": "アリウスの画像"
    },
    {
      "name": "杜甫",
      "kana": "杜甫",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "8世紀、唐の繁栄から安史の乱へ向かう時代を生きた詩人です。官僚として国を支えることを望みましたが、十分な地位を得られず、反乱と飢えのなかで家族と各地を流浪しました。戦争で苦しむ民衆、徴兵される若者、自身の貧困や家族への思いを具体的に詠み、その詩は当時の社会を伝える記録にもなっています。形式の整った律詩を高い水準へ発展させ、後世には「詩聖」と呼ばれ、中国詩の模範となりました。",
        "whatDid": "安史の乱や貧困のなかで、戦争に苦しむ民衆と自身の生活を律詩に詠みました。",
        "whyImportant": "唐代社会の現実を詩で伝え、中国文学における社会詩と律詩の最高峰を築いたためです。"
      },
      "image": "assets/people/person-c468737f5e.webp",
      "imageAlt": "杜甫の画像"
    },
    {
      "name": "ヤロスラフ賢公",
      "kana": "ヤロスラフ賢公",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "11世紀のキエフ大公で、父ウラジーミル1世の死後に兄弟との争いを勝ち抜き、キエフ・ルーシを統一しました。法慣習をまとめた『ルーシ法典』の基礎を整え、キエフに聖ソフィア大聖堂を建て、学校や聖書翻訳を支援しました。娘たちをフランス、ノルウェー、ハンガリーなどの王家へ嫁がせ、ヨーロッパ外交を広げました。一方、死後は領土を息子たちへ分けたため、後の分裂を防げませんでした。",
        "whatDid": "ルーシ法典の基礎を整え、聖ソフィア大聖堂を建て、王家の婚姻で欧州諸国との関係を広げました。",
        "whyImportant": "キエフ・ルーシの法・宗教・文化・外交を発展させ、国家の最盛期を築いたためです。"
      },
      "image": "assets/people/person-728b094372.webp",
      "imageAlt": "ヤロスラフ賢公の画像"
    },
    {
      "name": "アメリゴ・ヴェスプッチ",
      "kana": "アメリゴ・ヴェスプッチ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "フィレンツェ出身の商人・航海者で、15世紀末から16世紀初めにスペインやポルトガルの船で南アメリカ沿岸を航海しました。航海記録では、コロンブスが到達した土地はアジアの一部ではなく、ヨーロッパ人にとって未知の大陸だと考えました。1507年、地理学者ヴァルトゼーミュラーがその記述をもとに地図へ「アメリカ」と記しました。ただし本人の航海回数や書簡の真偽には議論があり、地名成立の経緯を慎重に見る必要があります。",
        "whatDid": "南アメリカ沿岸を航海し、到達地をアジアではなく未知の大陸だと説明しました。",
        "whyImportant": "新大陸を独立した大陸として認識する動きを促し、「アメリカ」という地名の由来となったためです。"
      },
      "image": "assets/people/person-8d1e003d26.webp",
      "imageAlt": "アメリゴ・ヴェスプッチの画像"
    },
    {
      "name": "メアリ・ウルストンクラフト",
      "kana": "メアリ・ウルストンクラフト",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀イギリスの著述家で、女性が十分な教育を受けられず、結婚と家庭に従属させられていた社会を批判しました。『女性の権利の擁護』で、女性も男性と同じ理性を持つため、教育・職業・市民としての権利を得るべきだと主張しました。女性を飾りや妻としてではなく、自立した人格として育てる必要を説きました。私生活への攻撃で長く評価を下げられましたが、近代女性解放思想とフェミニズムの先駆者として再評価されています。",
        "whatDid": "『女性の権利の擁護』を著し、女性にも教育・職業・市民としての権利が必要だと訴えました。",
        "whyImportant": "男女の理性的平等を主張し、近代の女性解放運動とフェミニズムの思想的基礎を築いたためです。"
      },
      "image": "assets/people/person-b6d9da0629.webp",
      "imageAlt": "メアリ・ウルストンクラフトの画像"
    },
    {
      "name": "ロバート・フルトン",
      "kana": "ロバート・フルトン",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "アメリカの発明家・技術者で、絵画を学んだ後、運河や潜水艇、蒸気船の開発に取り組みました。蒸気船そのものの発明者ではありませんが、1807年に「ノース・リバー・スチームボート」をハドソン川で定期運航し、蒸気船が商業輸送に使えることを示しました。風向きに左右されず、川を上流へ進める船は人や農産物の輸送を速めました。その成功により、アメリカやヨーロッパで河川・沿岸交通の蒸気化が急速に広がりました。",
        "whatDid": "蒸気船を改良してハドソン川で定期商業運航を成功させ、河川交通へ普及させました。",
        "whyImportant": "風に頼らない定期水運を実用化し、産業革命期の物流と地域間交流を速めたためです。"
      },
      "image": "assets/people/person-7c916ac45b.webp",
      "imageAlt": "ロバート・フルトンの画像"
    },
    {
      "name": "テオドロス2世",
      "kana": "テオドロス2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀のエチオピア皇帝で、地方諸侯が分立していた「諸侯の時代」を終わらせ、国家を再統一しようとしました。常備軍の整備、武器製造、行政の中央集権化を進め、教会や有力者の力を抑えました。しかし改革は重い徴税と軍事動員を伴い、反乱が相次ぎました。イギリス政府への書簡が無視されたことから外交官らを拘束し、1868年にイギリス軍の遠征を招きました。山城メクダラが陥落すると自害し、近代化の先駆者と強権的支配者の両面を残しました。",
        "whatDid": "地方諸侯を抑えてエチオピア再統一と軍事・行政改革を進めましたが、英国遠征軍に敗れて自害しました。",
        "whyImportant": "分裂したエチオピアの中央集権化と近代化を試み、後の国家再建の出発点となったためです。"
      },
      "image": "assets/people/person-db50e4e512.webp",
      "imageAlt": "テオドロス2世の画像"
    },
    {
      "name": "シャルル・ド・ゴール",
      "kana": "シャルル・ド・ゴール",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの軍人・政治家で、第二次世界大戦中に本国政府がドイツへ降伏するとロンドンから抗戦を呼びかけ、自由フランスを率いました。解放後の臨時政府を指導し、1958年のアルジェリア危機では政権へ復帰して第五共和政を成立させ、大統領権限を強めました。最終的にアルジェリア独立を認め、核戦力を整えてアメリカと距離を置く自主外交を進めました。戦時抵抗と強い大統領制を通して現代フランスの国家像を形づくりました。",
        "whatDid": "自由フランスを率いて対独抗戦を続け、第五共和政を創設して強い大統領制と自主外交を進めました。",
        "whyImportant": "第二次世界大戦後のフランス再建と、現在まで続く第五共和政の政治制度を築いたためです。"
      },
      "image": "assets/people/person-f4a709fcd6.webp",
      "imageAlt": "シャルル・ド・ゴールの画像"
    },
    {
      "name": "ダライ・ラマ14世",
      "kana": "ダライ・ラマ14世",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "religion",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "チベット仏教ゲルク派の最高指導者とされ、幼少期に第13世ダライ・ラマの転生者として認定されました。中国人民解放軍の進駐後、1959年のチベット蜂起を機にインドへ亡命し、ダラムサラで亡命政府を指導しました。チベット文化と宗教の保護、自治の実現を非暴力で訴え、1989年にノーベル平和賞を受賞しました。2011年には政治的権限を選挙で選ばれた指導者へ移しましたが、転生制度やチベットの地位をめぐり中国政府との対立が続いています。",
        "whatDid": "亡命後もチベット文化と宗教を守り、中国支配下での自治を非暴力で求め続けました。",
        "whyImportant": "民族・宗教・自治をめぐる問題を国際社会へ伝え、非暴力運動の象徴となったためです。"
      },
      "image": "assets/people/person-144a0ad2f0.webp",
      "imageAlt": "ダライ・ラマ14世の画像"
    },
    {
      "name": "ペピ2世",
      "kana": "ペピ2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第6王朝の王で、幼くして即位し、非常に長く統治したと伝えられますが、在位90年以上という記録には議論があります。治世中にはヌビア方面への遠征や交易が続き、神殿へ税や労役の免除を与える勅令も残りました。長い治世の後半には地方長官が世襲化して力を強め、中央政府の統制が弱まりました。ただし古王国崩壊は王の長寿だけでなく、地方分権、気候変動、財政など複数の原因から考える必要があります。",
        "whatDid": "長期統治のもとで遠征と神殿保護を続けましたが、地方長官の自立が進み中央支配が弱まりました。",
        "whyImportant": "古王国末期の地方分権と中央政府の衰退を考える中心人物であり、崩壊原因の複雑さを示すためです。"
      },
      "image": "assets/people/person-71387cd266.webp",
      "imageAlt": "ペピ2世の画像"
    },
    {
      "name": "シャムシアダド1世",
      "kana": "シャムシアダド1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前19～18世紀に北メソポタミアで勢力を広げたアムル系の王です。アッシュル、マリ、シェフナなどを支配し、都をシュバト・エンリルに置いて広域王国を築きました。二人の息子を重要都市の王に任じ、手紙で軍事、外交、農業、役人の行動まで細かく指示しました。マリ文書からは、同盟・婚姻・情報収集を組み合わせた統治が分かります。死後、王国は急速に崩れましたが、アッシリアが広域国家へ成長する前段階を示します。",
        "whatDid": "北メソポタミアの都市を征服し、息子と役人へ詳細な指示を送りながら広域王国を統治しました。",
        "whyImportant": "マリ文書を通して、古代国家の軍事・外交・行政と初期アッシリア支配の実態を知ることができるためです。"
      },
      "image": "assets/people/person-dd2e481457.webp",
      "imageAlt": "シャムシアダド1世の画像"
    },
    {
      "name": "アルキビアデス",
      "kana": "アルキビアデス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネ政治家・将軍で、名門出身の才能ある演説家として民衆の支持を集めました。ペロポネソス戦争中、シチリア遠征を強く提案しましたが、宗教像破壊事件への関与を疑われ、裁判を避けて敵国スパルタへ亡命しました。その後はペルシャや再びアテネ側へ移り、立場を変えながら軍事行動に関わりました。大胆な戦略と個人的野心はアテネの政策を大きく動かしましたが、シチリア遠征の壊滅など国を深刻な危機へ導きました。",
        "whatDid": "シチリア遠征を提案し、告発後はスパルタやペルシャへ移りながら各勢力の軍事・外交に関わりました。",
        "whyImportant": "民主政下の人気政治家の野心と変節が、アテネの戦争政策と敗北を左右したためです。"
      },
      "image": "assets/people/person-f6ad9aafd6.webp",
      "imageAlt": "アルキビアデスの画像"
    },
    {
      "name": "クラッスス",
      "kana": "クラッスス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家・将軍で、土地や奴隷への投資によって巨万の富を築きました。スパルタクスの反乱を鎮圧して軍事的名声を得た後、カエサル、ポンペイウスと第一回三頭政治を結び、政治的影響力を強めました。さらに栄誉を求めてシリア属州からパルティアへ遠征しましたが、紀元前53年のカルラエの戦いで騎馬軍に大敗し、自身も殺されました。彼の死で三頭政治の均衡が崩れ、カエサルとポンペイウスの内戦が近づきました。",
        "whatDid": "スパルタクスの反乱を鎮圧し、三頭政治を結びましたが、パルティア遠征で大敗して死亡しました。",
        "whyImportant": "莫大な富と軍事力が共和政政治を動かし、その死がカエサルとポンペイウスの内戦を促したためです。"
      },
      "image": "assets/people/person-5f4fa51d15.webp",
      "imageAlt": "クラッススの画像"
    },
    {
      "name": "カウティリヤ",
      "kana": "カウティリヤ",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "古代インドの政治思想家で、チャンドラグプタの宰相チャーナキヤと同一人物とされますが、詳しい生涯や著作年代には議論があります。彼に帰される『実利論』は、王の教育、官僚制度、税、農業、外交、諜報、戦争まで国家運営を体系的に説明しました。理想だけでなく、同盟、策略、情報活動を現実的に使う政治を説いています。現在の本文は後世に編集された可能性がありますが、古代インドの国家観とマウリヤ朝の統治を考える重要資料です。",
        "whatDid": "『実利論』で官僚制・税・外交・諜報・戦争を含む現実的な国家統治の方法を体系化しました。",
        "whyImportant": "古代インドの政治思想と行政制度を詳しく伝え、国家運営を現実的に論じた代表的著作を残したためです。"
      },
      "image": "assets/people/person-bfba8d3268.webp",
      "imageAlt": "カウティリヤの画像"
    },
    {
      "name": "ベネディクトゥス",
      "kana": "ベネディクトゥス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "5～6世紀のイタリアの修道士で、ローマでの学びを離れて隠修生活を送り、のちにモンテ・カッシーノ修道院を築きました。共同生活の規則をまとめた『ベネディクトゥス戒律』では、修道院長への服従、祈り、読書、労働、節度ある生活を重んじました。極端な苦行ではなく、共同体の秩序と日々の生活を両立させた点に特徴があります。この戒律は中世西ヨーロッパの多くの修道院で採用され、農業、教育、写本保存、救貧活動の基盤となりました。",
        "whatDid": "モンテ・カッシーノ修道院を築き、祈り・読書・労働を組み合わせた修道生活の戒律を定めました。",
        "whyImportant": "西ヨーロッパの修道院制度を形づくり、学問・農業・写本・救貧を支える共同体を広めたためです。"
      },
      "image": "assets/people/person-fd893ccc0c.webp",
      "imageAlt": "ベネディクトゥスの画像"
    },
    {
      "name": "李白",
      "kana": "李白",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "8世紀の唐を代表する詩人で、各地を旅しながら自然、友情、酒、別れ、自由への思いを大胆な言葉で詠みました。玄宗の宮廷に招かれましたが、官僚として長く仕えることはできず、再び各地を放浪しました。安史の乱では永王の軍に加わったため流罪を命じられましたが、途中で赦免されました。豊かな想像力と伸びやかな表現から「詩仙」と呼ばれ、杜甫と並んで中国詩の最高峰とされ、日本を含む東アジアの文学へ長く影響しました。",
        "whatDid": "旅の生活を通して自然・友情・酒・自由を豊かな想像力で詠み、唐詩を代表する作品を残しました。",
        "whyImportant": "自由で壮大な詩風を完成させ、中国と東アジアの文学表現に長く影響したためです。"
      },
      "image": "assets/people/person-10331154cc.webp",
      "imageAlt": "李白の画像"
    },
    {
      "name": "ウィリアム1世",
      "kana": "ウィリアム1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "フランス北部ノルマンディー公で、イングランド王エドワード懺悔王の死後、自らに王位継承権があると主張しました。1066年に海を渡り、ヘイスティングズの戦いでハロルド2世を破ってイングランド王となりました。各地の反乱を鎮圧し、ノルマン人へ土地を与えて封建的支配を築きました。全国の土地・人口・家畜・財産を調査した『ドゥームズデイ・ブック』を作らせ、課税と統治に利用しました。征服は言語・貴族・政治制度を大きく変えました。",
        "whatDid": "1066年にイングランドを征服し、ノルマン人による土地支配と全国的な土地調査を進めました。",
        "whyImportant": "ノルマン征服によってイングランドの王権・貴族・言語・土地制度を大きく変えたためです。"
      },
      "image": "assets/people/person-2f3074da59.webp",
      "imageAlt": "ウィリアム1世の画像"
    },
    {
      "name": "ジョン・カボット",
      "kana": "ジョン・カボット",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "イタリア出身の航海者で、イングランド王ヘンリー7世の許可を受け、西回りでアジアへ向かう航路を探しました。1497年にブリストルを出航し、北大西洋を渡って現在のカナダ東岸付近へ到達したと考えられます。到達地点はニューファンドランド島かラブラドル地方か確定していません。航海はアジアへの道を発見できませんでしたが、後にイングランドが北アメリカへの領有権を主張する根拠として利用されました。翌年の航海の最期は不明です。",
        "whatDid": "イングランド王の支援で北大西洋を渡り、現在のカナダ東岸付近へ到達しました。",
        "whyImportant": "イングランドによる北アメリカ進出と、後の植民地領有主張の出発点となったためです。"
      },
      "image": "assets/people/person-13c548e36d.webp",
      "imageAlt": "ジョン・カボットの画像"
    },
    {
      "name": "アダム・スミス",
      "kana": "アダム・スミス",
      "era": "革命と工場の時代",
      "genre": [
        "political-scholar"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀スコットランドの道徳哲学者で、人間の共感と市場経済の仕組みを研究しました。『道徳感情論』では、人は他者の立場を想像する共感によって行動を判断すると論じました。『国富論』では分業が生産力を高め、個人の利益追求が競争と交換を通して社会全体の富へ結び付く仕組みを説明しました。ただし政府不要を唱えたのではなく、国防、司法、公共事業、教育の役割も認めました。近代経済学の基礎を築いた人物です。",
        "whatDid": "『国富論』で分業・交換・競争が富を生む仕組みを説明し、政府の公共的役割も論じました。",
        "whyImportant": "市場経済を体系的に分析し、近代経済学と自由主義的経済政策の基礎を築いたためです。"
      },
      "image": "assets/people/person-632d4656c3.webp",
      "imageAlt": "アダム・スミスの画像"
    },
    {
      "name": "アレッサンドロ・ボルタ",
      "kana": "アレッサンドロ・ボルタ",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18～19世紀のイタリアの物理学者で、電気現象を実験的に研究しました。ガルヴァーニが動物の筋肉に固有の電気があると考えたのに対し、ボルタは異なる金属と液体の接触で電気が生じると考えました。1800年、亜鉛と銅の円板を塩水を含む布と交互に重ねた「ボルタ電池」を発表し、連続して電流を得ることに成功しました。この電池によって電気分解や電磁気の研究が進み、電圧の単位「ボルト」に名が残りました。",
        "whatDid": "亜鉛・銅・電解液を重ねたボルタ電池を発明し、連続した電流を取り出せるようにしました。",
        "whyImportant": "安定した電流源をつくり、電気化学・電磁気学・電池技術の発展を可能にしたためです。"
      },
      "image": "assets/people/person-fadfe26fcd.webp",
      "imageAlt": "アレッサンドロ・ボルタの画像"
    },
    {
      "name": "サモリ・トゥーレ",
      "kana": "サモリ・トゥーレ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半、西アフリカで商人・軍事指導者から台頭し、現在のギニア、マリ、コートジボワール周辺にワスル帝国を築きました。イスラムを統治の柱にし、近代的な銃器を備えた軍と行政組織を整えました。フランスの植民地拡大に対して長年抵抗し、退却時には物資を敵へ渡さない焦土戦術も用いました。そのため住民へ大きな負担と移住を強いました。1898年に捕らえられ、反植民地抵抗の英雄と強権的征服者の両面で評価されています。",
        "whatDid": "ワスル帝国を築き、銃器を備えた軍でフランスの西アフリカ植民地化へ長く抵抗しました。",
        "whyImportant": "アフリカ分割に対する大規模な武力抵抗を指導し、植民地征服の過程を示すためです。"
      },
      "image": "assets/people/person-06bdb26b39.webp",
      "imageAlt": "サモリ・トゥーレの画像"
    },
    {
      "name": "フィリップ・ペタン",
      "kana": "フィリップ・ペタン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの軍人で、第一次世界大戦のヴェルダンの戦いで防衛を指揮し、国民的英雄となりました。しかし第二次世界大戦でドイツ軍が侵攻すると、1940年に首相となって休戦を選び、ヴィシー政権の国家元首となりました。議会制を廃止し、ドイツへの協力、反対派の弾圧、ユダヤ人の登録・逮捕・移送に加担しました。解放後に反逆罪で死刑判決を受け、終身刑へ減刑されました。戦争英雄が占領協力者へ転じた人物として記憶されています。",
        "whatDid": "第一次世界大戦でヴェルダンを守りましたが、第二次世界大戦ではヴィシー政権を率いてナチスへ協力しました。",
        "whyImportant": "占領下の協力政治と、国家指導者が人権侵害へ加担する責任を考える中心人物だからです。"
      },
      "image": "assets/people/person-110c05dd70.webp",
      "imageAlt": "フィリップ・ペタンの画像"
    },
    {
      "name": "デズモンド・ツツ",
      "kana": "デズモンド・ツツ",
      "era": "今につながる時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "南アフリカの聖公会聖職者で、教師を経て司祭となり、黒人として初めて南アフリカ聖公会の大主教に就きました。アパルトヘイトに対し、説教や国際的な経済制裁の呼びかけによって非暴力で反対し、1984年にノーベル平和賞を受賞しました。民主化後は真実和解委員会の委員長として、過去の殺害・拷問・誘拐を公に証言させ、条件付き恩赦と被害者の尊厳回復を進めました。報復ではなく正義と和解を両立させようとした人物です。",
        "whatDid": "非暴力でアパルトヘイトへ反対し、民主化後は真実和解委員会を率いて人権侵害を調査しました。",
        "whyImportant": "差別撤廃と、独裁後の社会で真相究明・赦し・責任を結ぶ和解の方法を示したためです。"
      },
      "image": "assets/people/person-3a9c424ead.webp",
      "imageAlt": "デズモンド・ツツの画像"
    },
    {
      "name": "メントゥホテプ2世",
      "kana": "メントゥホテプ2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前21世紀ごろ、上エジプトのテーベを拠点にした第11王朝の王です。当時のエジプトは北部と南部に分裂していましたが、北部のヘラクレオポリス勢力を破り、全土を再統一しました。これにより第一中間期が終わり、中王国時代が始まりました。テーベ西岸のデイル・アル＝バハリに、山の地形を利用した葬祭殿を建設し、地方長官を統制して中央王権を立て直しました。統一後の行政・交易・建築の再出発を導いた王です。",
        "whatDid": "テーベから北部勢力を破ってエジプトを再統一し、中央王権と行政を立て直しました。",
        "whyImportant": "第一中間期の分裂を終わらせ、中王国という新しい安定と繁栄の時代を開いたためです。"
      },
      "image": "assets/people/person-2bba3388e0.webp",
      "imageAlt": "メントゥホテプ2世の画像"
    },
    {
      "name": "ジムリリム",
      "kana": "ジムリリム",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前18世紀、ユーフラテス川中流の都市国家マリを治めた王です。父の王国を奪ったシャムシアダド1世の死後、バビロン王ハンムラビの支援を受けて王位を回復しました。周辺諸国と同盟・婚姻・贈答を結び、宮殿から役人や将軍へ多数の手紙を送り、軍事・交易・灌漑を細かく管理しました。しかし後にハンムラビと対立し、マリは征服されました。宮殿文書は古代オリエント外交と日常行政を詳しく伝えます。",
        "whatDid": "王位を回復してマリを統治し、同盟・婚姻・書簡を用いて軍事・外交・灌漑を管理しました。",
        "whyImportant": "マリ文書を通して、古代オリエントの外交・行政・交易・宮廷生活を具体的に知ることができるためです。"
      },
      "image": "assets/people/person-9392e4eca4.webp",
      "imageAlt": "ジムリリムの画像"
    },
    {
      "name": "ソクラテス",
      "kana": "ソクラテス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネで活動した哲学者で、自ら著作を残さず、対話を通して相手の思い込みを問い直しました。「正義とは何か」「よく生きるとは何か」を質問し、無知を自覚して考え続けることを重んじました。ペロポネソス戦争後の政治不安のなかで、若者を堕落させ、国家の神々を認めないとして裁判にかけられ、死刑判決を受けました。逃亡を勧められても法に従い、毒杯を飲んだ姿は哲学と市民社会の関係を考える象徴となりました。",
        "whatDid": "対話と問いかけで人々の思い込みを検討し、正義・徳・よく生きることを考えさせました。",
        "whyImportant": "答えを教えるより問いを深める哲学的方法を示し、西洋哲学と倫理思想の出発点となったためです。"
      },
      "image": "assets/people/person-4e6d38825c.webp",
      "imageAlt": "ソクラテスの画像"
    },
    {
      "name": "キケロ",
      "kana": "キケロ",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家・弁論家で、騎士身分の家から執政官まで上りつめました。カティリナの陰謀を弾劾して共和政を守ろうとし、内戦ではポンペイウス側に立ちました。カエサル暗殺後はアントニウスを激しく批判しましたが、第二回三頭政治の追放者名簿に載せられ殺害されました。『国家論』『義務について』などでギリシャ哲学をラテン語で紹介し、自然法、共和政、市民の義務を論じました。その文章は後世の政治思想とラテン文学の模範となりました。",
        "whatDid": "演説で共和政を擁護し、著作を通してギリシャ哲学・自然法・市民の義務をローマ世界へ伝えました。",
        "whyImportant": "共和政理念と自然法思想を後世へ伝え、ヨーロッパの政治思想・法・ラテン文学へ影響したためです。"
      },
      "image": "assets/people/person-0ee4733742.webp",
      "imageAlt": "キケロの画像"
    },
    {
      "name": "カニシカ王",
      "kana": "カニシカ王",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "2世紀ごろのクシャーナ朝の王で、中央アジアから北インドにかけて広い領域を支配しました。シルクロード交易の要地を押さえ、ローマ・イラン・インド・中国を結ぶ人と物の交流を活発にしました。仏教を保護した王として知られ、ガンダーラやマトゥラーでは仏像制作が発展しました。第四回仏典結集を開いたという伝承もありますが、史実性には議論があります。多文化が交わる帝国と大乗仏教の広がりを象徴する人物です。",
        "whatDid": "中央アジアと北インドを支配し、シルクロード交易と仏教文化を保護しました。",
        "whyImportant": "東西交易を結ぶ多民族帝国を築き、大乗仏教と仏像文化の広がりを支えたためです。"
      },
      "image": "assets/people/person-9bc6eb495f.webp",
      "imageAlt": "カニシカ王の画像"
    },
    {
      "name": "グレゴリウス1世",
      "kana": "グレゴリウス1世",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "6世紀末のローマ教皇で、元ローマ市長から修道士となり、590年に教皇へ選ばれました。西ローマ帝国崩壊後の混乱のなか、飢饉救済や教会財産の管理を進め、ランゴバルド人との交渉も行いました。イングランドへ宣教師アウグスティヌスを派遣し、キリスト教布教を後押ししました。聖職者の務めを説いた『司牧規則』は中世教会の指針となりました。後世にグレゴリオ聖歌と結び付けられますが、本人が体系を直接作ったとは言い切れません。",
        "whatDid": "救貧・教会行政・外交を進め、宣教師をイングランドへ派遣して西欧教会の基盤を整えました。",
        "whyImportant": "ローマ教皇の宗教的・社会的・政治的役割を強め、中世西ヨーロッパ教会の形を整えたためです。"
      },
      "image": "assets/people/person-0f47b6e160.webp",
      "imageAlt": "グレゴリウス1世の画像"
    },
    {
      "name": "白居易",
      "kana": "白居易",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "9世紀の唐で官僚として働きながら、多くの詩を残した詩人です。難しい表現を避け、政治の失敗、重い税、戦争、貧しい人々の暮らしを分かりやすい言葉で詠みました。代表作『長恨歌』では玄宗と楊貴妃の悲劇を描き、『新楽府』では社会問題を批判しました。官僚として左遷も経験しましたが、地方行政にも携わりました。その詩は中国だけでなく平安時代の日本でも広く読まれ、『源氏物語』などの文学へ強い影響を与えました。",
        "whatDid": "社会問題を平易な詩で批判し、『長恨歌』『新楽府』などを残して唐詩を広い読者へ開きました。",
        "whyImportant": "文学で政治と民衆生活を結び、中国と日本の詩文・物語文学へ長く影響したためです。"
      },
      "image": "assets/people/person-97745cc5b2.webp",
      "imageAlt": "白居易の画像"
    },
    {
      "name": "エレノア・オブ・アキテーヌ",
      "kana": "エレノア・オブ・アキテーヌ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "12世紀西ヨーロッパの大領主で、広大なアキテーヌ公領の相続人としてフランス王ルイ7世と結婚し、第2回十字軍にも同行しました。離婚後はイングランド王ヘンリー2世と再婚し、リチャード1世やジョン王の母となりました。夫に反乱した息子たちを支援して幽閉されましたが、リチャード即位後は摂政として王国を支え、身代金交渉や政治調整を担いました。女性が土地相続・婚姻・宮廷外交を通じて大きな政治力を持った例です。",
        "whatDid": "二つの王家の王妃となり、領地・婚姻・摂政を通じてフランスとイングランドの政治へ影響しました。",
        "whyImportant": "中世ヨーロッパで女性領主が広大な領地と王家の婚姻を通じて国際政治を動かしたためです。"
      },
      "image": "assets/people/person-467fb1f1dd.webp",
      "imageAlt": "エレノア・オブ・アキテーヌの画像"
    },
    {
      "name": "ジャック・カルティエ",
      "kana": "ジャック・カルティエ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "16世紀フランスの航海者で、フランス王フランソワ1世の命を受け、北アメリカ北東部を三度航海しました。セントローレンス湾と川を調査し、現在のケベックやモントリオール周辺へ到達しました。現地のイロコイ系住民から聞いた「カナタ」という言葉を地域名として記録し、後のカナダという国名につながりました。一方、先住民の首長らをフランスへ連れ去り、植民地建設を試みました。探検記録はフランスの北米進出の基礎となりました。",
        "whatDid": "セントローレンス湾と川を探検し、フランスによる北アメリカ進出の地理的基礎をつくりました。",
        "whyImportant": "後のカナダ植民地形成へつながる航路と地域情報をフランスへもたらしたためです。"
      },
      "image": "assets/people/person-0a7750b30b.webp",
      "imageAlt": "ジャック・カルティエの画像"
    },
    {
      "name": "トマス・ペイン",
      "kana": "トマス・ペイン",
      "era": "革命と工場の時代",
      "genre": [
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "イギリス生まれで北アメリカへ渡り、革命期の政治著述家となりました。1776年の『コモン・センス』で、世襲君主制を批判し、植民地がイギリスから独立する必要を平易な言葉で訴えました。フランス革命期には『人間の権利』で共和政と社会保障を支持し、王政批判のためイギリスで反逆罪に問われました。『理性の時代』では教会制度を批判し、理性に基づく信仰を主張しました。政治思想を一般市民へ広げた人物です。",
        "whatDid": "『コモン・センス』『人間の権利』で独立・共和政・人民の権利を平易な文章で訴えました。",
        "whyImportant": "革命思想を知識人だけでなく一般市民へ広げ、アメリカ独立と近代民主主義を後押ししたためです。"
      },
      "image": "assets/people/person-cf92a1d76a.webp",
      "imageAlt": "トマス・ペインの画像"
    },
    {
      "name": "マイケル・ファラデー",
      "kana": "マイケル・ファラデー",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの科学者で、製本職人の見習いから独学で科学を学び、王立研究所で実験を重ねました。電流が磁場を生む現象を調べ、磁石を動かすことで電流が生じる電磁誘導を1831年に発見しました。この原理は発電機や変圧器の基礎です。また、電気分解の法則を示し、電場・磁場を「力線」として捉えました。高度な数学より実験を重視し、子ども向け講演も行いました。電気を産業と生活へ結び付ける基礎を築いた人物です。",
        "whatDid": "電磁誘導と電気分解の法則を発見し、発電機・変圧器・電気化学の基礎を築きました。",
        "whyImportant": "電気を連続的に生み出して利用する原理を明らかにし、電力社会の成立を可能にしたためです。"
      },
      "image": "assets/people/person-2b988d0d7b.webp",
      "imageAlt": "マイケル・ファラデーの画像"
    },
    {
      "name": "ムハンマド・アフマド",
      "kana": "ムハンマド・アフマド",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀スーダンの宗教指導者で、エジプトとイギリスの支配、重税、奴隷貿易規制への不満が高まるなか、自らをイスラム世界を正す救世主「マフディー」と宣言しました。各地の部族と信者をまとめ、1885年にハルツームを攻略して英軍のゴードン将軍を破りました。その直後に病死しましたが、後継者はマフディー国家を維持しました。運動は宗教改革、反植民地抵抗、地域権力の形成が結び付いた出来事として重要です。",
        "whatDid": "自らをマフディーと宣言して反乱を率い、ハルツームを攻略してスーダンに独立政権を築きました。",
        "whyImportant": "宗教運動と反植民地抵抗を結び付け、アフリカで欧州支配へ大きな打撃を与えたためです。"
      },
      "image": "assets/people/person-643753188b.webp",
      "imageAlt": "ムハンマド・アフマドの画像"
    },
    {
      "name": "ヨシップ・ブロズ・チトー",
      "kana": "ヨシップ・ブロズ・チトー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "クロアチア出身の共産主義者で、第二次世界大戦中にパルチザンを率い、ナチス・ドイツとその協力政権へ抵抗しました。戦後はユーゴスラビア社会主義連邦を築き、複数の民族と共和国を連邦制でまとめました。1948年にスターリンと決裂し、ソ連から独立した社会主義路線を取り、インドやエジプトと非同盟運動を進めました。一方、一党支配と秘密警察による弾圧も行いました。死後、経済危機と民族対立が表面化し、連邦は解体へ向かいました。",
        "whatDid": "反ナチ抵抗を率い、戦後は多民族連邦を統治し、ソ連から自立した社会主義と非同盟外交を進めました。",
        "whyImportant": "冷戦下で米ソどちらにも従わない路線を示し、多民族国家統合の成功と限界を残したためです。"
      },
      "image": "assets/people/person-534caeb458.webp",
      "imageAlt": "ヨシップ・ブロズ・チトーの画像"
    },
    {
      "name": "パトリス・ルムンバ",
      "kana": "パトリス・ルムンバ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ベルギー領コンゴの独立運動を率いた政治家で、民族を越えた統一国家を目指しました。1960年の独立時に初代首相となり、式典では植民地支配の暴力と差別を公然と批判しました。独立直後、軍の反乱とカタンガ州の分離が起こり、国連やアメリカ、ベルギー、ソ連が関与する危機へ発展しました。大統領と対立して解任され、モブツのクーデター後に拘束され、1961年に殺害されました。冷戦と植民地権益がアフリカ独立を左右した象徴です。",
        "whatDid": "コンゴ独立運動を率いて初代首相となり、植民地支配を批判して統一国家を目指しました。",
        "whyImportant": "アフリカ独立が旧宗主国の利害と冷戦介入によって妨げられた過程を象徴するためです。"
      },
      "image": "assets/people/person-e7e80e4d08.webp",
      "imageAlt": "パトリス・ルムンバの画像"
    },
    {
      "name": "アメンエムハト1世",
      "kana": "アメンエムハト1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前20世紀初め、中王国時代のエジプト第12王朝を開いた王です。前王朝の宰相から王位へ上った可能性があり、都をテーベから北のイチ・タウイへ移して上下エジプトを統治しやすくしました。地方長官の力を抑え、東部国境に防衛施設を築き、ヌビアへの進出を進めました。息子センウセレト1世を共同統治者にしたと考えられ、王位継承の安定を図りました。暗殺されたという文学作品も残り、中央集権再建の姿を伝えます。",
        "whatDid": "第12王朝を開き、都を移して地方勢力を抑え、国境防衛と共同統治で中央集権を強めました。",
        "whyImportant": "中王国エジプトの安定した官僚国家と王位継承制度の基礎を築いたためです。"
      },
      "image": "assets/people/person-7ebbc36049.webp",
      "imageAlt": "アメンエムハト1世の画像"
    },
    {
      "name": "ティグラトピレセル1世",
      "kana": "ティグラトピレセル1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前12～11世紀の中アッシリア王で、周辺諸国が混乱するなか、北メソポタミアからシリア方面へ遠征を重ねました。ヒッタイト系諸勢力やアラム人と戦い、地中海沿岸へ到達したことを碑文に記しました。征服地から貢納を集め、狩猟や建築事業も王の力として宣伝しました。しかし死後はアラム人の進出などで領土が縮小しました。後の新アッシリア帝国に見られる遠征・貢納・王碑文による支配の先例をつくった王です。",
        "whatDid": "シリア・アナトリア方面へ遠征し、貢納と王碑文によって中アッシリアの勢力を広げました。",
        "whyImportant": "後の新アッシリア帝国へつながる軍事遠征・貢納・王権宣伝の型を示したためです。"
      },
      "image": "assets/people/person-1ea4d9c270.webp",
      "imageAlt": "ティグラトピレセル1世の画像"
    },
    {
      "name": "プラトン",
      "kana": "プラトン",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前4世紀のアテネ哲学者で、ソクラテスの弟子としてその対話と処刑を記録しました。目に見える個々の物の背後に、変わらない本質である「イデア」があると考え、知識・正義・魂・国家を対話篇で論じました。『国家』では哲学を学んだ者が統治する理想国家を描きましたが、民主政への不信も示しました。アテネ郊外に学園アカデメイアを開き、数学と哲学を教育しました。その思想は西洋哲学・宗教・政治論の基礎となりました。",
        "whatDid": "対話篇でイデア・正義・魂・理想国家を論じ、学園アカデメイアを開いて哲学教育を行いました。",
        "whyImportant": "哲学を体系的な学問へ発展させ、西洋の形而上学・倫理・政治思想の基礎を築いたためです。"
      },
      "image": "assets/people/person-9f131c1d44.webp",
      "imageAlt": "プラトンの画像"
    },
    {
      "name": "ブルートゥス",
      "kana": "ブルートゥス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家で、名門ユニウス氏の出身です。内戦ではポンペイウス側に立ちましたが、敗北後にカエサルから許され、要職へ取り立てられました。しかしカエサルが終身独裁官となると共和政が失われると考え、紀元前44年の暗殺計画に参加しました。暗殺後は民衆の支持を得られず、アントニウスとオクタウィアヌスの軍にフィリッピの戦いで敗れ、自害しました。暴政への抵抗と政治的暗殺の是非を考える人物です。",
        "whatDid": "共和政を守るためカエサル暗殺に加わりましたが、内戦で敗れて自害しました。",
        "whyImportant": "独裁への抵抗が共和政回復ではなく新たな内戦と帝政成立へつながった矛盾を示すためです。"
      },
      "image": "assets/people/person-82b0cb9417.webp",
      "imageAlt": "ブルートゥスの画像"
    },
    {
      "name": "チャンドラグプタ2世",
      "kana": "チャンドラグプタ2世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "4～5世紀のグプタ朝の王で、「ヴィクラマーディティヤ」の称号を用いました。西インドのサカ勢力を征服してアラビア海への交易路を確保し、婚姻関係も利用して王国を安定させました。都周辺ではサンスクリット文学、天文学、数学、美術が栄え、中国僧法顕もこの時代のインドを訪れました。後世には文化の黄金時代を築いた理想の王として伝説化されましたが、繁栄は地域や身分によって一様ではありません。グプタ朝最盛期を代表する王です。",
        "whatDid": "西インドのサカ勢力を征服して交易路を確保し、グプタ朝の政治・文化を最盛期へ導きました。",
        "whyImportant": "古典インド文化とサンスクリット文学・科学・美術が発展した時代を代表するためです。"
      },
      "image": "assets/people/person-c22cd8b635.webp",
      "imageAlt": "チャンドラグプタ2世の画像"
    },
    {
      "name": "キュリロス",
      "kana": "キュリロス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "9世紀のビザンツ帝国出身の宣教師・学者で、兄メトディオスとともにスラヴ人への布教を担いました。モラヴィアの要請を受け、スラヴ語で礼拝と聖書翻訳を行うため、グラゴル文字を考案したとされます。弟子たちはその伝統を受け継ぎ、後にキリル文字が発達しました。ラテン語・ギリシャ語だけでなく現地語による典礼を認めるようローマ教皇へ働きかけました。スラヴ世界のキリスト教化と文字文化の形成に重要な役割を果たしました。",
        "whatDid": "兄メトディオスとスラヴ語で布教し、聖書翻訳のための文字を整えて現地語典礼を広めました。",
        "whyImportant": "スラヴ諸民族のキリスト教化と文字・文学の発達を促し、東欧文化の基礎を築いたためです。"
      },
      "image": "assets/people/person-eb75bd0e4d.webp",
      "imageAlt": "キュリロスの画像"
    },
    {
      "name": "韓愈",
      "kana": "韓愈",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "8～9世紀の唐の官僚・文学者で、華美で形式的な文章に反対し、先秦・漢代の簡潔で力強い文体へ戻る「古文運動」を進めました。儒家の道徳と政治を重んじ、仏教や道教が国家と社会へ強く入り込むことを批判しました。皇帝が仏舎利を迎える行事を諫めた『論仏骨表』によって左遷されましたが、その文章は後世の模範となりました。宋代の儒学者にも影響し、儒教復興と散文改革を結び付けた人物です。",
        "whatDid": "古文運動を進め、儒教の立場から仏教を批判し、簡潔で論理的な散文を復興しました。",
        "whyImportant": "唐宋の散文と儒教復興を結び付け、後の宋学・科挙文体・東アジア漢文へ影響したためです。"
      },
      "image": "assets/people/person-e00f0d5a5a.webp",
      "imageAlt": "韓愈の画像"
    },
    {
      "name": "リチャード1世",
      "kana": "リチャード1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "12世紀末のイングランド王で、「獅子心王」と呼ばれました。即位後すぐ第3回十字軍へ参加し、アッコン攻略やアルスフの戦いでサラディン軍と戦いました。エルサレムは奪回できませんでしたが、キリスト教巡礼者の訪問を認める和平を結びました。帰国途中に捕らえられ、多額の身代金で解放されました。治世の多くを国外で過ごし、戦争費用のため重い課税を行いました。騎士英雄として有名ですが、国内統治の負担も大きい王でした。",
        "whatDid": "第3回十字軍でサラディンと戦い、沿岸都市を確保して巡礼を認める和平を結びました。",
        "whyImportant": "十字軍時代の騎士王を象徴する一方、遠征費と長期不在が王国へ与えた負担も示すためです。"
      },
      "image": "assets/people/person-2ef0566623.webp",
      "imageAlt": "リチャード1世の画像"
    },
    {
      "name": "フランシス・ドレーク",
      "kana": "フランシス・ドレーク",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "16世紀イングランドの航海者・私掠船長で、スペイン船や植民地を襲い、得た財宝を王室と分け合いました。1577年から1580年にかけて世界一周を達成し、太平洋の航路とスペイン領の情報を持ち帰りました。1588年のスペイン無敵艦隊との戦いでは副司令官としてイングランド海軍に参加しました。イングランドでは英雄視されましたが、スペイン側には海賊であり、初期の奴隷貿易にも関与しました。海洋進出と暴力的略奪の両面を持つ人物です。",
        "whatDid": "私掠船でスペイン船を襲い、世界一周を達成し、無敵艦隊との戦いでイングランド海軍を指揮しました。",
        "whyImportant": "イングランドの海洋進出を進める一方、私掠・植民地略奪・奴隷貿易の暴力を示すためです。"
      },
      "image": "assets/people/person-2ad64ec7f9.webp",
      "imageAlt": "フランシス・ドレークの画像"
    },
    {
      "name": "ベンジャミン・フランクリン",
      "kana": "ベンジャミン・フランクリン",
      "era": "革命と工場の時代",
      "genre": [
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "北アメリカ植民地の印刷業者・科学者・政治家で、新聞や暦を発行し、市民図書館や消防組織をつくりました。雷が電気現象であることを実験で示し、避雷針を考案しました。独立運動では大陸会議に参加し、独立宣言の起草に関わりました。フランスへ派遣され、軍事・資金援助を引き出して独立戦争を支え、講和条約にも署名しました。一方、若い頃は奴隷を所有していましたが、晩年には奴隷制廃止を訴えました。",
        "whatDid": "電気研究と避雷針の発明を進め、外交官としてフランスの支援を得てアメリカ独立を支えました。",
        "whyImportant": "科学・市民活動・外交を結び、アメリカ独立と近代的な公共社会の形成に貢献したためです。"
      },
      "image": "assets/people/person-7f04eb839b.webp",
      "imageAlt": "ベンジャミン・フランクリンの画像"
    },
    {
      "name": "チャールズ・バベッジ",
      "kana": "チャールズ・バベッジ",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの数学者・発明家で、数表の計算ミスを減らすため機械式計算機を構想しました。「階差機関」は多項式計算を自動化する装置で、さらに「解析機関」では演算装置、記憶装置、入力用パンチカード、条件分岐を備えた汎用計算機を設計しました。資金・技術・設計変更の問題で生前に完成しませんでしたが、エイダ・ラブレスはその可能性を詳しく説明しました。現代コンピューターの基本構成を19世紀に先取りした人物です。",
        "whatDid": "階差機関と解析機関を設計し、演算・記憶・入力・条件分岐を備えた汎用計算機を構想しました。",
        "whyImportant": "現代コンピューターに通じる基本構成とプログラム制御の考えを早くから示したためです。"
      },
      "image": "assets/people/person-4358cf3b38.webp",
      "imageAlt": "チャールズ・バベッジの画像"
    },
    {
      "name": "ジャマールッディーン・アフガーニー",
      "kana": "ジャマールッディーン・アフガーニー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半、イスラム世界各地で活動した思想家・政治運動家です。出身地には議論がありますが、アフガニスタン人を名乗り、エジプト、イラン、オスマン帝国、ヨーロッパを移動しました。欧州帝国主義へ対抗するため、イスラム諸国の連帯と政治改革、科学教育の必要を説きました。専制君主も批判し、弟子ムハンマド・アブドゥフらに影響しました。宗教を守るだけでなく、近代科学と政治参加を取り入れて社会を立て直そうとした人物です。",
        "whatDid": "イスラム諸国の連帯、反帝国主義、政治改革、科学教育を説き、各地の改革運動へ影響しました。",
        "whyImportant": "西欧支配へ対抗しながらイスラム社会を近代化する思想を広め、民族運動と改革思想の源流となったためです。"
      },
      "image": "assets/people/person-b309fce24a.webp",
      "imageAlt": "ジャマールッディーン・アフガーニーの画像"
    },
    {
      "name": "ハリー・トルーマン",
      "kana": "ハリー・トルーマン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの政治家で、副大統領就任から間もない1945年、ルーズベルト大統領の死去により大統領となりました。第二次世界大戦末期に広島・長崎への原子爆弾投下を承認し、日本の降伏後は国際連合の発足を支えました。ソ連の勢力拡大に対してトルーマン・ドクトリンとマーシャル・プランを進め、冷戦の封じ込め政策を形づくりました。朝鮮戦争では国連軍を派遣し、国内では軍の人種隔離廃止を命じました。",
        "whatDid": "原爆投下を承認し、戦後はソ連を封じ込める外交政策と欧州復興支援を進めました。",
        "whyImportant": "第二次世界大戦の終結と冷戦秩序の形成を主導し、核兵器時代の政治的責任を残したためです。"
      },
      "image": "assets/people/person-b5ea16b9f6.webp",
      "imageAlt": "ハリー・トルーマンの画像"
    },
    {
      "name": "ジュリウス・ニエレレ",
      "kana": "ジュリウス・ニエレレ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリス領タンガニーカの教師から独立運動の指導者となり、1961年の独立後に初代首相、のち大統領となりました。ザンジバルとの統合でタンザニアを成立させ、アフリカ的社会主義「ウジャマー」を掲げて村落共同化、教育普及、民族間の統合を進めました。共通語スワヒリ語の普及は国家統合に役立ちましたが、強制移住や国有化は農業生産の低下と経済停滞を招きました。反植民地運動を支援し、長期政権後に自ら退任しました。",
        "whatDid": "タンザニア独立と国家統合を進め、ウジャマー政策で村落共同化・教育・社会主義建設を試みました。",
        "whyImportant": "アフリカ独立後の国家統合と自立的発展を模索し、その理想と経済的限界を示したためです。"
      },
      "image": "assets/people/person-e54e9c2266.webp",
      "imageAlt": "ジュリウス・ニエレレの画像"
    },
    {
      "name": "センウセレト3世",
      "kana": "センウセレト3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前19世紀ごろのエジプト第12王朝の王で、中王国の中央集権を強化しました。ヌビアへ繰り返し遠征し、ナイル川第二急流付近に要塞群を築いて、金・交易路・国境を管理しました。国内では地方長官の権力を抑え、行政を王宮へ集中させたと考えられます。従来の理想化された王像と異なり、深いしわと厳しい表情を表した彫像が多く残ります。軍事・行政・王の表現を通して中王国最盛期を示す人物です。",
        "whatDid": "ヌビアへ遠征して国境要塞を築き、地方長官を抑えて中央集権的な統治を強めました。",
        "whyImportant": "中王国エジプトの領土・交易・行政を安定させ、強い王権を完成へ導いたためです。"
      },
      "image": "assets/people/person-6f7c0ec1f3.webp",
      "imageAlt": "センウセレト3世の画像"
    },
    {
      "name": "アッシュルナツィルパル2世",
      "kana": "アッシュルナツィルパル2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前9世紀の新アッシリア王で、反乱を鎮圧しながら北メソポタミアからシリアへ遠征し、貢納を集めました。敵への皮剝ぎや串刺しなどの処罰を碑文に詳しく記し、恐怖を支配の手段として宣伝しました。都をアッシュルからカルフへ移し、巨大な北西宮殿を建設しました。宮殿の浮彫には戦争・狩猟・宗教儀礼が描かれ、王の力を視覚的に示しました。後のアッシリア帝国の軍事・行政・王権宣伝の型を確立した王です。",
        "whatDid": "遠征と威圧的な処罰で支配を広げ、カルフに新都と宮殿を築いて王権を宣伝しました。",
        "whyImportant": "新アッシリア帝国の軍事支配・恐怖政治・宮殿美術による権力表現を確立したためです。"
      },
      "image": "assets/people/person-c3eae4eafa.webp",
      "imageAlt": "アッシュルナツィルパル2世の画像"
    },
    {
      "name": "アリストテレス",
      "kana": "アリストテレス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher",
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前4世紀のギリシャ哲学者で、プラトンの学園で学んだ後、マケドニア王子アレクサンドロスの教育を担当しました。アテネに学園リュケイオンを開き、論理学、政治学、倫理学、生物学、詩学など幅広い分野を研究しました。物事を観察・分類し、原因を考える方法を重んじ、『政治学』では複数の政体を比較しました。自然科学には誤りもありましたが、知識を分野別に体系化した著作は、イスラム世界と中世ヨーロッパの学問へ大きな影響を与えました。",
        "whatDid": "論理学・倫理学・政治学・生物学などを観察と分類によって体系化し、リュケイオンで研究しました。",
        "whyImportant": "多くの学問分野の基礎概念と研究方法を整え、古代から中世の知識体系を形づくったためです。"
      },
      "image": "assets/people/person-69ee112116.webp",
      "imageAlt": "アリストテレスの画像"
    },
    {
      "name": "アウグストゥス",
      "kana": "アウグストゥス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "カエサルの養子オクタウィアヌスとして内戦を勝ち抜き、紀元前27年に元老院から「アウグストゥス」の称号を受けました。共和政の制度を残す形を取りながら、軍隊・属州・財政を実質的に一人で支配し、ローマ帝政を始めました。常備軍と親衛隊を整え、属州統治や税制を安定させ、長い内戦を終わらせました。この時代から「ローマの平和」と呼ばれる安定が始まりましたが、権力の世襲化と皇帝独裁への道も開きました。",
        "whatDid": "内戦を終結させ、共和政の外形を保ちながら軍・属州・財政を掌握してローマ帝政を始めました。",
        "whyImportant": "ローマの政治を共和政から帝政へ転換し、約二百年続く地中海世界の安定の基礎を築いたためです。"
      },
      "image": "assets/people/person-fa1becdf28.webp",
      "imageAlt": "アウグストゥスの画像"
    },
    {
      "name": "コンスタンティヌス帝",
      "kana": "コンスタンティヌスてい",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "religion"
      ],
      "field": "ローマと地中海",
      "title": "キリスト教を公認したローマ皇帝",
      "icon": "🏛️",
      "aliases": [
        "コンスタンティヌス1世"
      ],
      "modal": {
        "profile": "ローマ帝国の皇帝で、313年のミラノ勅令によってキリスト教を公認しました。内戦を勝ち抜いて帝国の統一を進め、325年にはニカイア公会議を開いて、キリスト教の教義をめぐる対立を整理しようとしました。また、ビザンティウムを整備してコンスタンティノープルとし、後の東ローマ帝国の中心となる都市の基礎を築きました。",
        "whatDid": "ミラノ勅令でキリスト教を公認し、ニカイア公会議を開いて教会のまとまりを支えました。コンスタンティノープルも整備しました。",
        "whyImportant": "ローマ帝国とキリスト教の関係を大きく変え、ヨーロッパの宗教・政治・都市の歴史に長く影響したためです。"
      },
      "image": "assets/people/person-8efd9d5df5.webp",
      "imageAlt": "コンスタンティヌス帝の画像"
    },
    {
      "name": "カーリダーサ",
      "kana": "カーリダーサ",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "4～5世紀ごろのグプタ朝期に活動したと考えられるサンスクリット文学の詩人・劇作家です。詳しい生涯は分かっていませんが、戯曲『シャクンタラー』では愛、別離、記憶、王の責任を描きました。叙事詩『ラグ王統史』や抒情詩『雲の使者』では、自然描写と人間感情を豊かな比喩で結びました。古代インドの神話・宮廷文化・恋愛表現を洗練された文学へ高め、その作品はインド諸語だけでなく、近代ヨーロッパの文学者にも高く評価されました。",
        "whatDid": "『シャクンタラー』『雲の使者』などを著し、自然と人間感情を結ぶサンスクリット文学を完成させました。",
        "whyImportant": "古典インド文学の表現を最高水準へ高め、インドと世界の演劇・詩へ長く影響したためです。"
      },
      "image": "assets/people/person-7823662b3a.webp",
      "imageAlt": "カーリダーサの画像"
    },
    {
      "name": "メトディオス",
      "kana": "メトディオス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "9世紀のビザンツ帝国出身の宣教師で、弟キュリロスとともにモラヴィアでスラヴ人への布教を行いました。兄弟はスラヴ語による聖書と典礼を整え、キュリロスの死後もメトディオスは大司教として現地語礼拝を守りました。ラテン語だけを認めようとする聖職者と対立し、一時拘束されましたが、ローマ教皇からスラヴ語典礼の承認を得ました。弟子たちはブルガリアへ移り、翻訳と文字文化を発展させ、東欧のキリスト教文化の基礎を築きました。",
        "whatDid": "キュリロスとスラヴ語で布教し、弟の死後も現地語の典礼・聖書翻訳・聖職者教育を守りました。",
        "whyImportant": "スラヴ語によるキリスト教文化を定着させ、東欧の文字・文学・教会制度の発展を支えたためです。"
      },
      "image": "assets/people/person-b40b93d232.webp",
      "imageAlt": "メトディオスの画像"
    },
    {
      "name": "柳宗元",
      "kana": "柳宗元",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の官僚・文学者で、王叔文らと政治改革を進めましたが、政変で失脚し、永州や柳州へ長く左遷されました。中央から遠ざけられた生活のなかで、山水を描いた紀行文、寓話、政治論を執筆しました。韓愈とともに古文運動を進め、形式にとらわれない簡潔で論理的な散文を重んじました。地方官としては井戸や学校の整備、奴婢の解放にも取り組んだと伝えられます。失意と自然観察を文学へ変え、唐宋八大家の一人に数えられます。",
        "whatDid": "政治改革に参加し、左遷後は山水紀行・寓話・論説を著して古文運動を進めました。",
        "whyImportant": "政治的挫折と地方経験を新しい散文表現へ変え、中国の紀行文・寓話・論説文を発展させたためです。"
      },
      "image": "assets/people/person-1c4d5725fe.webp",
      "imageAlt": "柳宗元の画像"
    },
    {
      "name": "ジョン王",
      "kana": "ジョン王",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "12～13世紀のイングランド王で、兄リチャード1世の死後に即位しました。フランス王フィリップ2世との戦争に敗れ、ノルマンディーなど大陸領の多くを失いました。戦費を集めるため重い課税を行い、カンタベリー大司教の任命をめぐって教皇とも対立しました。貴族の反乱を受け、1215年にマグナ・カルタへ同意し、王も法と慣習に従うことを認めました。直後に無効化を求めて内戦となりましたが、文書は後世の立憲政治の象徴となりました。",
        "whatDid": "大陸領を失い重税で反発を招き、貴族の圧力を受けてマグナ・カルタへ同意しました。",
        "whyImportant": "王権も法の制約を受けるという原則につながる文書を成立させ、立憲政治の象徴となったためです。"
      },
      "image": "assets/people/person-be42dd692c.webp",
      "imageAlt": "ジョン王の画像"
    },
    {
      "name": "ウォルター・ローリー",
      "kana": "ウォルター・ローリー",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "16～17世紀イングランドの廷臣・軍人・航海事業家で、女王エリザベス1世の信任を得ました。北アメリカの植民事業を計画し、現在のノースカロライナ沖ロアノーク島へ入植者を送りましたが、植民地は消失して失敗しました。「ヴァージニア」という名称の普及にも関わりました。女王死後は反逆罪で投獄され、釈放後に南アメリカで黄金郷を探しましたが、スペインとの和平を破った責任を問われ処刑されました。文学作品も残しました。",
        "whatDid": "ロアノーク植民事業を支援し、北アメリカへのイングランド進出を試みましたが、入植は失敗しました。",
        "whyImportant": "後のイギリス北米植民地建設の先駆けとなり、探検・宮廷政治・植民地主義の結び付きを示すためです。"
      },
      "image": "assets/people/person-aff3615f9f.webp",
      "imageAlt": "ウォルター・ローリーの画像"
    },
    {
      "name": "トマス・ジェファーソン",
      "kana": "トマス・ジェファーソン",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "北アメリカ植民地の政治家で、1776年の独立宣言を主に起草し、「すべての人は平等」と自然権を掲げました。第3代大統領としてルイジアナをフランスから購入し、領土を大きく広げました。政教分離と公教育を重んじ、バージニア大学を創設しました。一方、多数の奴隷を所有し、奴隷女性サリー・ヘミングスとの間に子どもをもうけたと認められています。自由と平等の理念と奴隷制を併存させた矛盾を体現する人物です。",
        "whatDid": "独立宣言を起草し、大統領としてルイジアナ購入を行い、政教分離と公教育を進めました。",
        "whyImportant": "近代的な自由・平等の理念を示す一方、自ら奴隷を所有したアメリカ建国の矛盾を体現するためです。"
      },
      "image": "assets/people/person-a38e1e2af3.webp",
      "imageAlt": "トマス・ジェファーソンの画像"
    },
    {
      "name": "エイダ・ラブレス",
      "kana": "エイダ・ラブレス",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの数学愛好家で、詩人バイロンの娘として生まれました。チャールズ・バベッジが構想した解析機関について、イタリア語論文を英訳し、原文を上回る詳しい注釈を加えました。そこではベルヌーイ数を計算する手順を示し、機械が数値計算だけでなく、規則に変換できれば音楽や記号も扱える可能性を論じました。解析機関は完成しませんでしたが、機械へ処理手順を与える発想から、最初期のプログラム記述者として知られます。",
        "whatDid": "解析機関の解説と計算手順を記し、機械が数値以外の記号も扱える可能性を示しました。",
        "whyImportant": "コンピューターを単なる計算器ではなく、手順に従って多様な情報を処理する機械として捉えたためです。"
      },
      "image": "assets/people/person-87cf8f01a8.webp",
      "imageAlt": "エイダ・ラブレスの画像"
    },
    {
      "name": "ムハンマド・アブドゥフ",
      "kana": "ムハンマド・アブドゥフ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19～20世紀初めのエジプトのイスラム法学者・改革思想家で、アズハル学院で学びました。師アフガーニーとともに帝国主義と専制政治を批判し、亡命後に帰国してエジプトの最高法官となりました。イスラムの基本教義は理性や近代科学と両立すると考え、教育改革、法解釈の見直し、女性の地位改善を訴えました。過去の学説をそのまま模倣するのではなく、時代に応じて自ら判断する「イジュティハード」の復活を重視しました。",
        "whatDid": "イスラム法を理性と時代に応じて解釈し直し、教育・司法・社会制度の改革を進めました。",
        "whyImportant": "イスラム信仰と近代科学・教育・法改革を両立させる近代イスラム改革思想を形づくったためです。"
      },
      "image": "assets/people/person-8d5aaf0810.webp",
      "imageAlt": "ムハンマド・アブドゥフの画像"
    },
    {
      "name": "ドワイト・アイゼンハワー",
      "kana": "ドワイト・アイゼンハワー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの軍人・政治家で、第二次世界大戦では連合国遠征軍最高司令官としてノルマンディー上陸作戦を指揮し、西ヨーロッパ解放を進めました。戦後はNATO軍最高司令官を経て大統領となり、朝鮮戦争の休戦を実現しました。高速道路網を建設し、冷戦では核抑止と同盟を重視しました。一方、イランやグアテマラの政権転覆に中央情報局を用いました。退任演説では軍需産業と政府が結び付く「軍産複合体」の影響を警告しました。",
        "whatDid": "欧州連合軍を指揮してノルマンディー上陸を成功させ、大統領として朝鮮休戦と高速道路建設を進めました。",
        "whyImportant": "第二次世界大戦の勝利と冷戦期の米国戦略を担い、軍産複合体の危険も警告したためです。"
      },
      "image": "assets/people/person-0d77053913.webp",
      "imageAlt": "ドワイト・アイゼンハワーの画像"
    },
    {
      "name": "トーマス・サンカラ",
      "kana": "トーマス・サンカラ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "西アフリカのオートボルタで軍人となり、1983年のクーデター後に政権を握りました。国名を「高潔な人々の国」を意味するブルキナファソへ改め、予防接種、識字教育、植樹、土地改革、女性の強制結婚や性器切除への反対を進めました。政府のぜいたくを減らし、外国援助への依存を批判しました。一方、革命委員会による反対派への圧力や労働組合との対立もありました。1987年にクーデターで殺害され、反帝国主義と清廉な政治の象徴となりました。",
        "whatDid": "保健・教育・植樹・女性解放を進め、外国援助への依存と政治家の特権を減らそうとしました。",
        "whyImportant": "アフリカの自立と社会改革を急速に進め、反帝国主義と清廉な統治の象徴となったためです。"
      },
      "image": "assets/people/person-606de7d4d2.webp",
      "imageAlt": "トーマス・サンカラの画像"
    },
    {
      "name": "ハトシェプスト",
      "kana": "ハトシェプスト",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王で、幼いトトメス3世の共同統治者から、自らファラオの称号を用いて約二十年間統治しました。男性王の付けひげや王冠を身に着けた姿で表され、王位の正統性を神アメンの意志として示しました。プント国への交易遠征で香料・木材・珍しい動植物を運ばせ、デイル・アル＝バハリに壮大な葬祭殿を建設しました。死後に名や像が削られましたが、近年は平和的交易と建築を進めた女性王として再評価されています。",
        "whatDid": "女性としてファラオとなり、プント交易を進め、デイル・アル＝バハリに大葬祭殿を建設しました。",
        "whyImportant": "王権が男性に限られた社会で長期統治を実現し、新王国の交易・建築・王権表現を発展させたためです。"
      },
      "image": "assets/people/person-c2b318a3aa.webp",
      "imageAlt": "ハトシェプストの画像"
    },
    {
      "name": "シャルマネセル3世",
      "kana": "シャルマネセル3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前9世紀の新アッシリア王で、三十年以上にわたりシリア、アナトリア、バビロニア方面へ遠征を続けました。紀元前853年のカルカルの戦いでは、ダマスクスやイスラエルを含む連合軍と戦いました。決定的勝利には至らなかったものの、各地から貢納を集め、帝国の影響力を広げました。「黒色オベリスク」には諸国の使節が貢物を差し出す姿が描かれています。晩年には息子の反乱が起こり、広大な帝国支配の不安定さも表れました。",
        "whatDid": "シリア・アナトリアへ繰り返し遠征し、諸国から貢納を集めて新アッシリアの影響圏を広げました。",
        "whyImportant": "遠征記録と黒色オベリスクによって、アッシリア帝国の軍事・外交・貢納支配を具体的に伝えるためです。"
      },
      "image": "assets/people/person-b4c0e8c892.webp",
      "imageAlt": "シャルマネセル3世の画像"
    },
    {
      "name": "ピタゴラス",
      "kana": "ピタゴラス",
      "era": "古代の大国の時代",
      "genre": [
        "philosopher",
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前6世紀ごろ、エーゲ海のサモス島出身で、南イタリアのクロトンに宗教的・哲学的共同体を築いた人物です。魂は死後に別の生物へ移ると考え、節制と共同生活を重んじました。数の比が音の高さや宇宙の秩序を説明すると考え、数学と音楽を結び付けました。直角三角形の定理は彼以前にも知られており、本人が証明したかは不明です。著作を残さず、弟子の伝承が混ざるため、史実と伝説を区別する必要があります。",
        "whatDid": "数と音楽・宇宙の秩序を結び付け、魂の転生と共同生活を重んじる学派を南イタリアに築きました。",
        "whyImportant": "数学を実用計算だけでなく、自然と宇宙を理解する原理として考える伝統を広めたためです。"
      },
      "image": "assets/people/person-71f920164b.webp",
      "imageAlt": "ピタゴラスの画像"
    },
    {
      "name": "リウィア",
      "kana": "リウィア",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ初代皇帝アウグストゥスの妻で、前夫との子ティベリウスを連れて再婚し、帝政初期の皇室を支えました。公的には貞淑な妻と母の模範として表され、財産管理、縁故、請願の仲介を通して大きな影響力を持ちました。アウグストゥス死後には「ユリア・アウグスタ」の名を与えられ、息子ティベリウスの治世にも皇太后として存在感を示しました。後世の歴史書では毒殺を企てた女性と描かれますが、その多くは確証のない宮廷伝説です。",
        "whatDid": "アウグストゥスの妻として皇室を支え、財産・人脈・請願仲介を通じて帝政初期の政治へ影響しました。",
        "whyImportant": "公式の役職を持たない皇帝家の女性が、婚姻と家族関係を通じて政治力を持った例だからです。"
      },
      "image": "assets/people/person-ed179a7e57.webp",
      "imageAlt": "リウィアの画像"
    },
    {
      "name": "パーニニ",
      "kana": "パーニニ",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前5～4世紀ごろに活動したと考えられる古代インドの文法学者です。著書『アシュターディヤーイー』で、サンスクリット語の音、語形、語の作り方、文法規則を約四千の短い規則にまとめました。例外処理や規則の適用順序まで厳密に示し、限られた記号で複雑な言語を生成する体系を築きました。この文法はヴェーダ聖典の正確な伝承を支え、サンスクリット語の標準化に役立ちました。近代言語学や形式体系との類似でも注目されています。",
        "whatDid": "『アシュターディヤーイー』でサンスクリット語の音・語形・生成規則を厳密な体系にまとめました。",
        "whyImportant": "世界最古級の精密な文法体系を築き、インドの聖典伝承・言語学・論理的分析へ影響したためです。"
      },
      "image": "assets/people/person-e7bd6b387b.webp",
      "imageAlt": "パーニニの画像"
    },
    {
      "name": "ナーガールジュナ",
      "kana": "ナーガールジュナ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "2～3世紀ごろのインド仏教思想家で、大乗仏教の中観派の基礎を築きました。主著『中論』で、すべての存在は他との関係や原因によって成り立ち、変わらない本質を持たないという「空」を論じました。空は何も存在しないという意味ではなく、物事を固定的に捉える執着から離れる考えです。また、日常の言葉で成り立つ世俗の真理と、究極の理解を区別する二諦説を示しました。その思想はチベット・中国・日本の仏教へ広く影響しました。",
        "whatDid": "『中論』で縁起と空を論じ、物事に固定した本質があるとする考えを論理的に批判しました。",
        "whyImportant": "大乗仏教の中観思想を体系化し、東アジアとチベットの仏教哲学の基礎を築いたためです。"
      },
      "image": "assets/people/person-df73badf15.webp",
      "imageAlt": "ナーガールジュナの画像"
    },
    {
      "name": "王安石",
      "kana": "王安石",
      "era": "交易と王国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "11世紀の北宋の政治家・文学者で、神宗皇帝のもとで「新法」と呼ばれる改革を進めました。農民へ低利で資金を貸す青苗法、物資価格を調整する市易法、兵役を地域で担う保甲法などにより、財政を立て直し、農民と国家を守ろうとしました。しかし役人の強制や運用上の負担が生じ、司馬光ら旧法派と激しく対立しました。改革は本人の退任後に多くが廃止されましたが、国家が経済と社会へ介入する政策論争を引き起こしました。",
        "whatDid": "青苗法・市易法・保甲法などの新法を実施し、農民支援・財政再建・軍事改革を進めました。",
        "whyImportant": "国家が金融・流通・社会保障へ介入する大規模改革を行い、中国政治史の代表的論争を生んだためです。"
      },
      "image": "assets/people/person-5a0f7044b8.webp",
      "imageAlt": "王安石の画像"
    },
    {
      "name": "サラディン",
      "kana": "サラディン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "クルド系の軍人としてエジプトで台頭し、ファーティマ朝を終わらせてアイユーブ朝を開きました。エジプトとシリアを統合し、十字軍国家に対抗しました。1187年のヒッティーンの戦いで十字軍主力を破り、エルサレムを奪回しました。住民の多くを身代金で解放し、1099年の十字軍占領時の大量殺害とは異なる対応を取りました。第3回十字軍ではリチャード1世と戦い、沿岸部を十字軍側に残しつつ巡礼を認める和平を結びました。",
        "whatDid": "エジプトとシリアを統合し、ヒッティーンで十字軍を破ってエルサレムを奪回しました。",
        "whyImportant": "イスラム勢力を再統合して十字軍国家へ反撃し、宗教戦争下の統治と外交の象徴となったためです。"
      },
      "image": "assets/people/person-fdca958aaf.webp",
      "imageAlt": "サラディンの画像"
    },
    {
      "name": "アベル・タスマン",
      "kana": "アベル・タスマン",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "17世紀オランダ東インド会社の航海者で、東南アジアから南方海域の航路と交易地を探しました。1642年の航海で、現在のタスマニア島へヨーロッパ人として初めて到達し、続いてニュージーランドを記録しました。現地マオリとの衝突で乗組員が死亡し、上陸せずに離れました。その後トンガやフィジー周辺を航海し、オーストラリア大陸北岸も調査しました。航海は南方大陸の全体像を解明しませんでしたが、南太平洋地図の空白を大きく埋めました。",
        "whatDid": "オランダ東インド会社の船で南太平洋を探検し、タスマニアとニュージーランドを欧州の地図へ記録しました。",
        "whyImportant": "南太平洋の地理認識を広げ、後のオーストラリア・ニュージーランド探検の基礎をつくったためです。"
      },
      "image": "assets/people/person-1cc2e1acb2.webp",
      "imageAlt": "アベル・タスマンの画像"
    },
    {
      "name": "ジョン・アダムズ",
      "kana": "ジョン・アダムズ",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ独立運動を担った法律家・政治家で、ボストン虐殺事件では被告となったイギリス兵の弁護を引き受け、法の手続きを守りました。大陸会議では独立を主張し、ヨーロッパ外交でフランスの支援と講和条約締結に関わりました。初代副大統領を経て第2代大統領となり、フランスとの戦争を避けました。一方、政府批判を取り締まる外国人・治安諸法を成立させ、言論の自由をめぐる強い反発を招きました。",
        "whatDid": "独立を支持し、欧州外交と講和に関わり、第2代大統領として対仏全面戦争を回避しました。",
        "whyImportant": "独立国家の外交と大統領制を形づくる一方、国家安全と表現の自由の対立を残したためです。"
      },
      "image": "assets/people/person-a15bda296f.webp",
      "imageAlt": "ジョン・アダムズの画像"
    },
    {
      "name": "ルイ・パスツール",
      "kana": "ルイ・パスツール",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀フランスの化学者・微生物学者で、発酵や腐敗が自然に起こるのではなく、微生物の働きによることを実験で示しました。空気中の微生物が入りにくい白鳥の首型フラスコを使い、自然発生説を退けました。ワインや牛乳を低温で加熱して腐敗を防ぐパスツール法を考案し、蚕病の原因も調査しました。さらに炭疽や狂犬病のワクチン開発を進め、感染症予防と衛生管理に大きな道を開きました。",
        "whatDid": "微生物が発酵・腐敗・病気を起こすことを示し、低温殺菌法と炭疽・狂犬病ワクチンを開発しました。",
        "whyImportant": "微生物学と予防医学の基礎を築き、食品衛生・感染症対策・ワクチン研究を大きく進めたためです。"
      },
      "image": "assets/people/person-5a8e297376.webp",
      "imageAlt": "ルイ・パスツールの画像"
    },
    {
      "name": "サイイド・アフマド・ハーン",
      "kana": "サイイド・アフマド・ハーン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀インドのイスラム教徒知識人・官僚で、1857年のインド大反乱後、ムスリム社会の教育と政治的立場の立て直しに取り組みました。イスラムの信仰と近代科学は両立できると考え、英語・科学・西洋の学問を学ぶ学校をアリーガルに設立しました。この学校は後のアリーガル・ムスリム大学へ発展しました。ヒンドゥー教徒とムスリムの政治的利害は異なると主張したため、後の分離主義へ影響したとの評価もあります。",
        "whatDid": "アリーガルに近代学校を設立し、イスラム信仰と英語・科学教育を両立させる改革を進めました。",
        "whyImportant": "インドのムスリム教育改革を進め、近代化と宗教共同体の政治意識の形成へ影響したためです。"
      },
      "image": "assets/people/person-7e57baa118.webp",
      "imageAlt": "サイイド・アフマド・ハーンの画像"
    },
    {
      "name": "ダグラス・マッカーサー",
      "kana": "ダグラス・マッカーサー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカ陸軍の軍人で、第二次世界大戦ではフィリピン防衛に失敗して脱出した後、南西太平洋方面の連合軍を指揮し、島伝いに反攻してフィリピンへ戻りました。日本降伏後は連合国軍最高司令官として占領政策を進め、日本国憲法の制定、軍の解体、農地改革、女性参政権などに関わりました。朝鮮戦争では国連軍を指揮しましたが、中国への戦争拡大を公然と主張し、文民統制に反したとしてトルーマン大統領に解任されました。",
        "whatDid": "太平洋戦争の反攻と日本占領改革を指揮し、朝鮮戦争では戦争拡大を主張して解任されました。",
        "whyImportant": "戦後日本の制度改革に大きく関わり、軍人も文民政府に従う原則を示す事件の中心となったためです。"
      },
      "image": "assets/people/person-7215bb0731.webp",
      "imageAlt": "ダグラス・マッカーサーの画像"
    },
    {
      "name": "エレン・ジョンソン・サーリーフ",
      "kana": "エレン・ジョンソン・サーリーフ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "リベリアの経済官僚・政治家で、軍事政権への批判や亡命を経験した後、内戦終結後の2005年選挙で大統領に選ばれました。アフリカで選挙によって国家元首となった最初の女性です。国家財政の再建、対外債務の削減、教育・保健、女性の政治参加を進めました。女性の平和運動を支えた功績でノーベル平和賞を受賞しました。一方、汚職対策の限界や親族登用への批判も受け、復興と統治責任の両面で評価されています。",
        "whatDid": "内戦後のリベリアで大統領となり、債務削減・国家再建・女性の政治参加を進めました。",
        "whyImportant": "アフリカ初の選挙による女性国家元首として、内戦後復興と女性の政治参加を象徴したためです。"
      },
      "image": "assets/people/person-d76f4801e9.webp",
      "imageAlt": "エレン・ジョンソン・サーリーフの画像"
    },
    {
      "name": "トトメス3世",
      "kana": "トトメス3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王で、幼少期には継母ハトシェプストと共同統治し、その死後に単独支配を始めました。メギドの戦いをはじめシリア・パレスチナへ多数の遠征を行い、諸都市から貢納を集めて新王国の支配を広げました。ヌビア方面も統制し、カルナック神殿を増築して戦勝記録を刻ませました。後世に軍事王として称賛されましたが、支配は遠征と人質、貢納に支えられていました。",
        "whatDid": "メギドの戦いなど多数の遠征でシリア・パレスチナを支配し、新王国エジプトの勢力を最大級に広げました。",
        "whyImportant": "エジプトを広域帝国へ成長させ、遠征記録から新王国の軍事・外交・貢納支配を知ることができるためです。"
      },
      "image": "assets/people/person-6279b7538a.webp",
      "imageAlt": "トトメス3世の画像"
    },
    {
      "name": "サルゴン2世",
      "kana": "サルゴン2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前8世紀の新アッシリア王で、王位継承の事情には不明な点がありますが、各地の反乱を鎮圧して帝国支配を立て直しました。イスラエル王国の首都サマリア征服を完成させ、住民の一部を移住させました。ウラルトゥ遠征やバビロニア支配も進め、新都ドゥル・シャルキンを建設しました。宮殿には巨大な人面有翼雄牛像を置き、王の権力を示しました。戦死後、新都は放棄されましたが、帝国拡張の転機をつくりました。",
        "whatDid": "反乱を鎮圧し、サマリア・ウラルトゥ・バビロニアへ勢力を広げ、新都ドゥル・シャルキンを築きました。",
        "whyImportant": "新アッシリア帝国の領土拡大と住民移住政策、巨大宮殿による王権表現を代表するためです。"
      },
      "image": "assets/people/person-fe80e980ec.webp",
      "imageAlt": "サルゴン2世の画像"
    },
    {
      "name": "ヘロドトス",
      "kana": "ヘロドトス",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のギリシャ人著述家で、ペルシャ戦争がなぜ起こったのかを調べ、『歴史』にまとめました。ギリシャだけでなく、エジプト、リディア、ペルシャ、スキタイなど各地の地理・風習・伝承を聞き取り、複数の説を並べて読者に示しました。神話や誇張、確認できない話も含まれますが、原因を探り、異なる社会を比較しようとしました。後世に「歴史の父」と呼ばれ、戦争だけでなく文化の違いを記録した点でも重要です。",
        "whatDid": "ペルシャ戦争の原因を調べ、旅と聞き取りを通じて諸民族の歴史・地理・風習を『歴史』にまとめました。",
        "whyImportant": "出来事の原因を調査し、異文化を比較して記録する歴史叙述の基礎をつくったためです。"
      },
      "image": "assets/people/person-53e377756e.webp",
      "imageAlt": "ヘロドトスの画像"
    },
    {
      "name": "ティベリウス",
      "kana": "ティベリウス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ第2代皇帝で、アウグストゥスの養子として帝位を継ぎました。若い頃はゲルマニアやバルカンで軍功を立て、即位後は財政を慎重に管理し、属州統治の安定を重視しました。しかし元老院との関係は悪化し、晩年はカプリ島へ移って近衛長官セイヤヌスへ政治を任せました。反逆罪裁判と密告が増え、恐怖政治の印象を残しました。古代史家の敵意も強いため、私生活の悪評をそのまま史実とすることには注意が必要です。",
        "whatDid": "軍事経験を生かして帝国の財政と属州を安定させましたが、晩年は反逆罪裁判と密告政治を招きました。",
        "whyImportant": "アウグストゥス後の帝政継承を定着させる一方、皇帝と元老院の緊張を深めたためです。"
      },
      "image": "assets/people/person-6a7d9d902d.webp",
      "imageAlt": "ティベリウスの画像"
    },
    {
      "name": "荘子",
      "kana": "荘子",
      "era": "古代の大国の時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代の中国で活動したと考えられる思想家で、道家の代表的古典『荘子』と結び付けられます。書物には後世の文章も含まれますが、夢で蝶になった話、巨大な魚が鳥へ変わる話などを通して、人間が決めた善悪・有用無用・生死の区別を絶対視しないよう説きました。自然の変化に逆らわず、物事を一つの立場だけで判断しない自由な生き方を重んじました。その寓話と逆説は中国思想・文学・芸術へ大きな影響を与えました。",
        "whatDid": "寓話と逆説を用いて固定した価値判断を問い直し、自然の変化に従う自由な生き方を説きました。",
        "whyImportant": "道家思想を豊かな文学表現へ発展させ、中国の哲学・詩文・絵画・人生観へ影響したためです。"
      },
      "image": "assets/people/person-906371073c.webp",
      "imageAlt": "荘子の画像"
    },
    {
      "name": "アサンガ",
      "kana": "アサンガ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4世紀ごろのインド仏教思想家で、弟ヴァスバンドゥとともに瑜伽行派の形成に関わりました。瞑想修行の体験を重視し、人が認識する世界は心の働きと深く結び付いていると論じました。経験や行為の影響を蓄える基盤として「阿頼耶識」という深層の意識を説明し、迷いから悟りへ至る心の変化を体系化しました。著作の帰属には議論がありますが、その思想は中国の法相宗やチベット仏教へ伝わり、大乗仏教の心の理論を発展させました。",
        "whatDid": "瑜伽行派の思想を整え、阿頼耶識を用いて認識・行為・迷い・悟りの関係を説明しました。",
        "whyImportant": "大乗仏教の心と認識の理論を体系化し、中国・日本・チベットの仏教思想へ影響したためです。"
      },
      "image": "assets/people/person-0090ac93db.webp",
      "imageAlt": "アサンガの画像"
    },
    {
      "name": "司馬光",
      "kana": "司馬光",
      "era": "交易と王国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "11世紀の北宋の政治家・歴史家で、王安石の新法に反対し、急激な国家介入より既存制度の安定を重んじました。長年をかけて編年体の歴史書『資治通鑑』を編集し、戦国時代から五代まで約千三百年の政治・戦争・人物を年代順にまとめました。書名には、過去を統治の参考とする意味があります。哲宗の即位後に宰相となり新法を廃止しましたが、党派対立をさらに深めました。その著作は東アジアの政治と歴史教育に長く用いられました。",
        "whatDid": "『資治通鑑』を編さんし、宰相として王安石の新法を廃止して旧法へ戻しました。",
        "whyImportant": "歴史を政治の教訓として体系化し、東アジアの歴史叙述と統治思想へ長く影響したためです。"
      },
      "image": "assets/people/person-846b68113b.webp",
      "imageAlt": "司馬光の画像"
    },
    {
      "name": "リチャード・ド・ベリー",
      "kana": "リチャード・ド・ベリー",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "14世紀イングランドの聖職者・政治家で、王エドワード3世に仕え、財務長官や大法官、ダラム司教を務めました。外交や宮廷勤務のかたわら多数の写本を集め、書物への愛と学問の価値をラテン語著作『フィロビブロン』に記しました。書物を大切に扱う方法、学者を支援する必要、図書を教育へ生かす考えを説きました。計画した図書館がそのまま実現したかには不明点がありますが、中世の書物収集と知識保存を語る代表人物です。",
        "whatDid": "写本を収集し、『フィロビブロン』で書物の保存・利用・学者支援の重要性を論じました。",
        "whyImportant": "印刷以前の中世社会で、書物と図書館が知識を保存し教育へ伝える価値を明確に説いたためです。"
      },
      "image": "assets/people/person-22dedfd93d.webp",
      "imageAlt": "リチャード・ド・ベリーの画像"
    },
    {
      "name": "ジェームズ・クック",
      "kana": "ジェームズ・クック",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "18世紀イギリス海軍の航海者で、三度の太平洋航海を指揮しました。金星の太陽面通過観測を目的にタヒチへ向かい、その後ニュージーランド周航とオーストラリア東岸の測量を行いました。二度目の航海では南極圏へ入り、未知の巨大南方大陸が温暖な海域に存在するという説を弱めました。航海では壊血病予防にも成果を上げました。一方、記録と領有宣言はイギリス植民地化に利用され、ハワイで現地住民との衝突により死亡しました。",
        "whatDid": "太平洋を三度航海し、ニュージーランド・オーストラリア東岸・南太平洋を精密に測量しました。",
        "whyImportant": "太平洋地図と長距離航海を発展させる一方、その成果が先住民の土地の植民地化へ利用されたためです。"
      },
      "image": "assets/people/person-80df1afa33.webp",
      "imageAlt": "ジェームズ・クックの画像"
    },
    {
      "name": "アビゲイル・アダムズ",
      "kana": "アビゲイル・アダムズ",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ独立期の女性で、第2代大統領ジョン・アダムズの妻、第6代大統領ジョン・クインシー・アダムズの母です。夫が大陸会議や海外外交で不在の間、農場と家計を管理し、政治・戦争・教育について多数の手紙を交わしました。独立時には新しい法律を作る際に「女性たちを忘れないで」と訴え、夫の権力を制限する必要を指摘しました。奴隷制にも批判的でした。公職には就きませんでしたが、書簡は女性の政治意識と日常生活を伝えます。",
        "whatDid": "独立期の政治を手紙で論じ、女性の権利と教育を求め、家庭と農場を自ら管理しました。",
        "whyImportant": "公的政治から女性が排除された時代に、その権利要求と生活経験を具体的な書簡として残したためです。"
      },
      "image": "assets/people/person-a9d2a04956.webp",
      "imageAlt": "アビゲイル・アダムズの画像"
    },
    {
      "name": "エドワード・ジェンナー",
      "kana": "エドワード・ジェンナー",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18～19世紀イギリスの医師で、牛痘にかかった人は天然痘にかかりにくいという農村の経験に注目しました。1796年、牛痘の膿を少年へ接種し、その後に天然痘への抵抗を確かめました。当時の実験方法は現代の倫理基準を満たしませんが、結果を記録して予防接種を広めました。「ワクチン」という言葉は牛を意味するラテン語に由来します。天然痘予防は世界へ広がり、長い取り組みの末、1980年に天然痘根絶が宣言されました。",
        "whatDid": "牛痘を用いた天然痘予防法を実験・記録し、ワクチン接種を広めました。",
        "whyImportant": "人類初の本格的なワクチンを実用化し、感染症を予防し最終的に根絶する道を開いたためです。"
      },
      "image": "assets/people/person-187e9cbe5e.webp",
      "imageAlt": "エドワード・ジェンナーの画像"
    },
    {
      "name": "ラーマクリシュナ",
      "kana": "ラーマクリシュナ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀ベンガルのヒンドゥー教宗教家で、コルカタ近郊ダクシネーシュワルのカーリー寺院で祭司を務めました。女神カーリーへの信仰を中心に、ヴィシュヌ信仰、タントラ、イスラム教、キリスト教の修行も試み、異なる宗教は同じ究極の真理へ至る道だと説きました。体系的な著作は残さず、弟子たちが言行を記録しました。弟子ヴィヴェーカーナンダはその教えを世界へ紹介し、宗教間対話と近代ヒンドゥー教改革へ影響を与えました。",
        "whatDid": "複数の宗教的修行を実践し、異なる信仰は同じ究極の真理へ至る道だと説きました。",
        "whyImportant": "宗教間の共通性を強調し、近代ヒンドゥー教の改革と世界的な宗教対話へ影響したためです。"
      },
      "image": "assets/people/person-b9fe782cb4.webp",
      "imageAlt": "ラーマクリシュナの画像"
    },
    {
      "name": "ジョージ・マーシャル",
      "kana": "ジョージ・マーシャル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカ陸軍の軍人・政治家で、第二次世界大戦中に陸軍参謀総長として兵力拡大、将校登用、連合国との作戦調整を進めました。戦後は国務長官となり、1947年にヨーロッパ復興計画を提案しました。後にマーシャル・プランと呼ばれ、食料・機械・資金を供給して西ヨーロッパの経済再建と政治安定を支えました。中国内戦の仲介は失敗しましたが、国防長官も務め、軍事と外交の両面から戦後秩序の形成に関わりました。",
        "whatDid": "米陸軍の戦時動員を統括し、戦後はマーシャル・プランで西ヨーロッパの復興を支援しました。",
        "whyImportant": "連合国の戦争遂行と戦後欧州復興を支え、冷戦初期の西側秩序を安定させたためです。"
      },
      "image": "assets/people/person-19bc08aafd.webp",
      "imageAlt": "ジョージ・マーシャルの画像"
    },
    {
      "name": "ルース・ベイダー・ギンズバーグ",
      "kana": "ルース・ベイダー・ギンズバーグ",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの法律家・連邦最高裁判所判事で、女性が法曹界で強い差別を受けた時代に弁護士となりました。女性の権利訴訟では、性別による固定観念が女性だけでなく男性にも不利益を与えることを示し、連邦最高裁で段階的に違憲判決を積み重ねました。1993年に女性として二人目の最高裁判事となり、男女平等、投票権、労働者の権利などで多数意見にも反対意見にも重要な文章を残しました。司法を通じた平等実現の象徴です。",
        "whatDid": "性差別訴訟を通じて男女平等を進め、最高裁判事として権利を守る多数意見・反対意見を残しました。",
        "whyImportant": "性別に基づく法的差別を段階的に崩し、司法による平等と少数意見の役割を示したためです。"
      },
      "image": "assets/people/person-99609838d7.webp",
      "imageAlt": "ルース・ベイダー・ギンズバーグの画像"
    },
    {
      "name": "アメンホテプ3世",
      "kana": "アメンホテプ3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前14世紀のエジプト第18王朝の王で、大規模な征服戦争より外交と建築によって新王国の繁栄を示しました。ミタンニやバビロニアなどの王家と婚姻・贈答を行い、アマルナ文書には各国王との外交交渉が残ります。ルクソール神殿や巨大な葬祭殿を建て、入口の巨像は「メムノンの巨像」として残っています。王妃ティイも強い地位を持ちました。豊かな金と国際外交に支えられた王権は、次代アクエンアテンの宗教改革へつながりました。",
        "whatDid": "婚姻と贈答外交で周辺大国との関係を保ち、ルクソール神殿など巨大建築を進めました。",
        "whyImportant": "新王国エジプトの国際外交・富・建築・王妃の地位が最盛期に達した時代を代表するためです。"
      },
      "image": "assets/people/person-3402a471f3.webp",
      "imageAlt": "アメンホテプ3世の画像"
    },
    {
      "name": "トゥキディデス",
      "kana": "トゥキディデス",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネ人で、将軍としてペロポネソス戦争に参加しました。アンフィポリス救援に失敗して追放された後、戦争の経過を『戦史』にまとめました。神々の意志より、人間の恐怖・利益・名誉、国家間の力関係を重視して原因を分析しました。演説は実際の言葉をそのまま記録したものではなく、状況にふさわしい内容を再構成したと説明しています。証言を比較し、誇張を避けようとする姿勢から、批判的歴史研究の先駆者とされます。",
        "whatDid": "ペロポネソス戦争を証言比較と因果分析によって記録し、国家の力・恐怖・利益を検討しました。",
        "whyImportant": "神話ではなく人間の行動と政治構造から戦争原因を分析する批判的歴史学の基礎を示したためです。"
      },
      "image": "assets/people/person-66b848de50.webp",
      "imageAlt": "トゥキディデスの画像"
    },
    {
      "name": "老子",
      "kana": "老子",
      "era": "古代の大国の時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "古代中国の思想家として伝えられ、『老子』または『道徳経』の著者とされますが、実在した一人の人物か、書物が複数時代の思想を集めたものかは確定していません。『老子』は、万物の根源で言葉では捉えきれない「道」に従い、無理に支配せず自然な働きを生かす「無為」を説きました。強さより柔らかさ、多く持つことより足ることを知る姿勢を重んじます。その思想は道家・道教だけでなく、中国の政治論・文学・芸術へ深く影響しました。",
        "whatDid": "『老子』に結び付けられ、万物の根源である道と、無理に支配しない無為の生き方を説きました。",
        "whyImportant": "道家思想の基礎をつくり、中国の政治観・自然観・宗教・文学・芸術へ長く影響したためです。"
      },
      "image": "assets/people/person-99a3a0b6f0.webp",
      "imageAlt": "老子の画像"
    },
    {
      "name": "エルナン・コルテス",
      "kana": "エルナン・コルテス",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "スペイン出身の征服者で、1519年にキューバ総督の命令を越えてメキシコへ進みました。アステカに反発するトラスカラなどの先住民勢力と同盟し、通訳マリンチェの協力も得て都テノチティトランへ入りました。一度は追放されましたが、天然痘の流行と包囲戦を利用して1521年に都を陥落させました。その結果、アステカ帝国は滅び、スペイン植民地ヌエバ・エスパーニャが成立しました。征服は大量死・強制労働・文化破壊を伴いました。",
        "whatDid": "先住民勢力との同盟と軍事力を用いてテノチティトランを陥落させ、アステカ帝国を征服しました。",
        "whyImportant": "スペインのアメリカ大陸支配を決定的に進める一方、先住民社会へ大規模な破壊をもたらしたためです。"
      },
      "image": "assets/people/person-0abcdd8016.webp",
      "imageAlt": "エルナン・コルテスの画像"
    },
    {
      "name": "ローザ・パークス",
      "kana": "ローザ・パークス",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカ南部アラバマ州の裁縫師・公民権活動家で、長く全米黒人地位向上協会に参加していました。1955年、モンゴメリーの市バスで白人へ席を譲るよう命じられましたが拒否し、逮捕されました。この行動は偶然の反発ではなく、差別撤廃運動の蓄積の上にありました。黒人住民は一年以上にわたりバス利用を拒否し、連邦裁判所は人種分離を違憲と判断しました。その後も住宅・雇用・政治参加の平等を求め続けました。",
        "whatDid": "人種分離されたバスで席を譲る命令を拒否し、モンゴメリー・バス・ボイコットのきっかけをつくりました。",
        "whyImportant": "日常生活の差別へ抵抗し、公民権運動を全国的な非暴力運動へ発展させる象徴となったためです。"
      },
      "image": "assets/people/person-6fc044a210.webp",
      "imageAlt": "ローザ・パークスの画像"
    },
    {
      "name": "アクエンアテン",
      "kana": "アクエンアテン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王で、即位当初はアメンホテプ4世と名乗りました。太陽円盤神アテンを唯一に近い特別な神として崇拝し、名をアクエンアテンへ改め、新都アケトアテンを建設しました。アメン神官団の力を弱め、王家がアテンと人々を結ぶ宗教体制を進めました。美術では王族の身体や家庭場面を従来より自然に表しました。しかし死後、改革は急速に否定され、都と王名も破壊されました。宗教改革の目的や範囲には議論があります。",
        "whatDid": "アテン神崇拝を中心とする宗教改革を進め、新都アケトアテンを建設して神官勢力を再編しました。",
        "whyImportant": "古代エジプトで王権主導の大規模宗教改革を行い、宗教・美術・政治の関係を大きく変えたためです。"
      },
      "image": "assets/people/person-5282fde232.webp",
      "imageAlt": "アクエンアテンの画像"
    },
    {
      "name": "ヒポクラテス",
      "kana": "ヒポクラテス",
      "era": "古代の大国の時代",
      "genre": [
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5～4世紀ごろのギリシャの医師で、コス島で活動したと伝えられます。病気を神罰だけで説明せず、患者の食事、生活、気候、症状の変化を観察して治療する姿勢と結び付けられています。『ヒポクラテス全集』は一人の著作ではなく、複数の医師による文書集です。体液説など現代医学では誤りとなった考えも含みますが、診察記録、予後判断、医師の倫理を重視しました。後世の「ヒポクラテスの誓い」は医療倫理の象徴となりました。",
        "whatDid": "病気を自然現象として観察し、症状・生活・環境の記録と医師の倫理を重んじる医学を発展させました。",
        "whyImportant": "病気を超自然的説明から切り離し、観察と倫理に基づく西洋医学の伝統を築いたためです。"
      },
      "image": "assets/people/person-fcd6694e3d.webp",
      "imageAlt": "ヒポクラテスの画像"
    },
    {
      "name": "ネロ",
      "kana": "ネロ",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ第5代皇帝で、母アグリッピナの支援により若くして即位しました。治世初期は哲学者セネカらの補佐で比較的安定しましたが、のちに母や妻を殺害し、反対派への弾圧を強めました。64年のローマ大火後には復興と都市整備を進めましたが、キリスト教徒を処刑し、豪華な黄金宮殿を建てたことで反感を招きました。火災を自ら起こしたという話は確証がありません。各地の反乱で元老院に敵とされ、自殺しました。",
        "whatDid": "ローマ大火後の復興を進める一方、皇族・反対派・キリスト教徒を弾圧し、反乱の末に自殺しました。",
        "whyImportant": "皇帝独裁の不安定さと、後世の暴君像が史実・宣伝・伝承から形成される過程を示すためです。"
      },
      "image": "assets/people/person-b5cbace420.webp",
      "imageAlt": "ネロの画像"
    },
    {
      "name": "孟子",
      "kana": "孟子",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代の中国で諸国を巡り、君主へ儒家の政治を説いた思想家です。人には他人の苦しみを見過ごせない心があり、それを育てれば仁・義・礼・智という徳になると考え、性善説を唱えました。民衆の生活を安定させる「仁政」を行うことが王の務めであり、暴政を行う君主は正当性を失うと主張しました。言行は弟子たちとまとめた『孟子』に伝わります。孔子の教えを政治思想として発展させ、後の儒教に大きな影響を与えました。",
        "whatDid": "性善説と仁政を説き、民衆を苦しめる君主は統治の正当性を失うと主張しました。",
        "whyImportant": "儒教を人間観と政治論の両面で発展させ、東アジアの道徳・教育・統治思想へ影響したためです。"
      },
      "image": "assets/people/person-e106018db8.webp",
      "imageAlt": "孟子の画像"
    },
    {
      "name": "フランシスコ・ピサロ",
      "kana": "フランシスコ・ピサロ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "スペイン出身の征服者で、パナマから南アメリカ西岸を探検し、インカ帝国の富を知りました。1532年、内戦直後のインカへ少数の兵で入り、カハマルカで皇帝アタワルパを捕らえました。莫大な身代金を受け取った後も皇帝を処刑し、1533年に都クスコを占領しました。先住民の協力者、鉄製武器、馬、天然痘による混乱を利用して支配を広げました。スペイン植民地を築く一方、略奪・強制労働・内戦を招き、最後は対立するスペイン人に殺されました。",
        "whatDid": "内戦中のインカ帝国へ侵入し、皇帝アタワルパを捕らえて処刑し、クスコを占領しました。",
        "whyImportant": "スペインの南米支配を確立する一方、インカ社会へ略奪・強制労働・人口減少をもたらしたためです。"
      },
      "image": "assets/people/person-1f806de4af.webp",
      "imageAlt": "フランシスコ・ピサロの画像"
    },
    {
      "name": "ロベスピエール",
      "kana": "ロベスピエール",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "フランス革命期の弁護士・政治家で、身分制廃止、普通選挙、奴隷制反対などを主張し、「清廉の人」と呼ばれました。王政廃止後はジャコバン派の指導者となり、対外戦争と国内反乱の危機に対して公安委員会を中心とする非常体制を進めました。革命裁判所と反革命容疑者法のもとで多数が処刑され、この時期は恐怖政治と呼ばれます。徳に基づく共和国を目指しましたが、1794年に政敵から独裁を疑われ、逮捕・処刑されました。",
        "whatDid": "共和政と社会改革を進めましたが、公安委員会を中心に反対派を処刑する恐怖政治を主導しました。",
        "whyImportant": "革命を守る非常措置が権利侵害と政治的暴力へ転じる危険を示す中心人物だからです。"
      },
      "image": "assets/people/person-5fd87cbb63.webp",
      "imageAlt": "ロベスピエールの画像"
    },
    {
      "name": "カール・マルクス",
      "kana": "カール・マルクス",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "society"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀ドイツ生まれの思想家・経済学者で、政治活動のため各地を追われ、ロンドンで研究を続けました。エンゲルスと『共産党宣言』を著し、歴史は生産手段を持つ階級と働く階級の対立によって動くと論じました。『資本論』では、労働者が生み出した価値の一部を資本家が利潤として得る仕組みや、資本主義が恐慌と格差を生む過程を分析しました。その思想は労働運動と社会主義革命へ影響しましたが、後の国家による実践は本人の構想と同一ではありません。",
        "whatDid": "資本主義の階級対立と利潤の仕組みを分析し、『共産党宣言』『資本論』で社会変革を論じました。",
        "whyImportant": "資本主義批判と社会主義思想を体系化し、世界の労働運動・革命・政治体制へ大きく影響したためです。"
      },
      "image": "assets/people/person-b83304a316.webp",
      "imageAlt": "カール・マルクスの画像"
    },
    {
      "name": "マルコムX",
      "kana": "マルコムX",
      "era": "今につながる時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの黒人公民権活動家で、犯罪と服役を経験した後、ネーション・オブ・イスラムへ入りました。白人社会の人種差別を激しく批判し、黒人の自尊心、経済的自立、自衛を訴え、非暴力だけを求める運動とは異なる立場を示しました。組織と決別後、メッカ巡礼で多民族のイスラム教徒に接し、人種を越えた連帯へ考えを広げました。アフリカ系アメリカ人の問題を国際的人権問題として訴えましたが、1965年に演説会場で暗殺されました。",
        "whatDid": "黒人の自尊・自立・自衛を訴え、後には人種差別を国際的な人権問題として提起しました。",
        "whyImportant": "公民権運動に急進的な視点を加え、黒人解放・自己決定・国際的人権の議論を広げたためです。"
      },
      "image": "assets/people/person-99c89b30cf.webp",
      "imageAlt": "マルコムXの画像"
    },
    {
      "name": "ネフェルティティ",
      "kana": "ネフェルティティ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王妃で、アクエンアテンの妻として宗教改革を支えました。アテン神へ祈る場面では夫とほぼ同じ大きさで描かれ、娘たちとともに王家の親密な家庭像を示しました。新都アケトアテンの神殿儀礼にも重要な役割を持ち、単なる王妃以上の政治・宗教的地位を持ったと考えられます。有名な彩色胸像はアマルナ美術を代表します。晩年の記録は少なく、共同統治者や王になったという説もありますが、確定していません。",
        "whatDid": "アクエンアテンの宗教改革を支え、アテン神の儀礼と王権表現で重要な役割を果たしました。",
        "whyImportant": "アマルナ時代の女性王権・宗教改革・美術を示す中心人物であり、王妃の政治的地位を考えられるためです。"
      },
      "image": "assets/people/person-84e02673e2.webp",
      "imageAlt": "ネフェルティティの画像"
    },
    {
      "name": "アッシュルバニパル",
      "kana": "アッシュルバニパル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前7世紀の新アッシリア王で、父エサルハドンの後継者として帝国を統治しました。エジプトへ遠征し、兄が治めるバビロンの反乱を鎮圧し、エラムを破壊して領土を維持しました。宮殿浮彫には獅子狩りや戦争が精密に描かれています。一方、ニネヴェの宮殿に各地の粘土板を集め、神話・医学・占星術・辞書などを収めた大図書館を築きました。『ギルガメシュ叙事詩』が現代へ伝わったのも、この収集のおかげです。",
        "whatDid": "軍事遠征で帝国を維持し、ニネヴェに膨大な粘土板を集めた王立図書館を築きました。",
        "whyImportant": "新アッシリアの最盛期を支え、古代メソポタミアの文学・科学・宗教文書を後世へ残したためです。"
      },
      "image": "assets/people/person-ed3d36e598.webp",
      "imageAlt": "アッシュルバニパルの画像"
    },
    {
      "name": "ユークリッド",
      "kana": "ユークリッド",
      "era": "古代の大国の時代",
      "genre": [
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前3世紀ごろ、エジプトのアレクサンドリアで活動した数学者です。著書『原論』で、点・線・角などの定義と、証明せずに出発点とする公理・公準を置き、そこから命題を論理的に証明しました。平面幾何だけでなく、比例、数論、無理数、立体も扱いました。内容のすべてを本人が発見したわけではなく、先行するギリシャ数学を整理したものです。それでも、少数の前提から体系を築く方法は二千年以上にわたり数学教育と論理的思考の模範となりました。",
        "whatDid": "『原論』で定義・公理・公準から命題を証明する形に、幾何学と数論を体系化しました。",
        "whyImportant": "数学を論理的証明の体系として整え、二千年以上にわたり科学と数学教育の基礎となったためです。"
      },
      "image": "assets/people/person-838a2f0c28.webp",
      "imageAlt": "ユークリッドの画像"
    },
    {
      "name": "荀子",
      "kana": "荀子",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代末の中国で活動した儒家の思想家です。人は生まれつき欲望を持つため、そのままでは争いが起こるという「性悪説」を唱えました。ただし人が悪のままでよいとしたのではなく、学問・礼・正しい習慣によって欲望を整え、善へ変わることができると説きました。天を人間社会へ直接罰や恵みを与える存在とは考えず、自然の法則として捉えました。弟子に韓非や李斯がいたとされ、儒家と法家の双方へ影響しました。",
        "whatDid": "性悪説を唱え、学問と礼によって欲望を整え、人は善へ変わることができると説きました。",
        "whyImportant": "儒教に教育と制度を重視する現実的な人間観を加え、法家思想の形成にも影響したためです。"
      },
      "image": "assets/people/person-4a209266db.webp",
      "imageAlt": "荀子の画像"
    },
    {
      "name": "ダントン",
      "kana": "ダントン",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "フランス革命期の弁護士・政治家で、王政打倒を進めたコルドリエ派の指導者でした。1792年に司法大臣となり、外国軍の侵攻に対して国民へ抵抗を呼びかけました。王政廃止後は国民公会で活動し、革命裁判所や公安委員会の成立にも関わりました。しかし戦局が安定すると処刑の縮小と和解を求め、ロベスピエール派と対立しました。汚職の疑いも受け、1794年に仲間とともに処刑されました。革命的暴力を推進した側が、その停止を求めて犠牲となった人物です。",
        "whatDid": "王政打倒と共和国防衛を進め、革命政府の制度を整えましたが、恐怖政治の緩和を求めて処刑されました。",
        "whyImportant": "革命を守る暴力に関わった指導者が、その暴力の拡大によって排除される革命政治の矛盾を示すためです。"
      },
      "image": "assets/people/person-c4428b21b4.webp",
      "imageAlt": "ダントンの画像"
    },
    {
      "name": "山本五十六",
      "kana": "山本五十六",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本海軍の軍人で、アメリカ駐在経験から同国の工業力を理解し、日独伊三国同盟や対米長期戦に慎重でした。しかし連合艦隊司令長官として開戦が決まると、真珠湾攻撃を立案して太平洋戦争の初期作戦を指揮しました。空母航空戦力を重視しましたが、ミッドウェー海戦では作戦の複雑さと情報判断の失敗により主力空母四隻を失いました。1943年、前線視察中に暗号を解読した米軍機に撃墜され死亡しました。慎戦論と開戦実行の責任を併せ持つ人物です。",
        "whatDid": "真珠湾攻撃と太平洋初期作戦を指揮し、空母航空戦を進めましたが、ミッドウェー海戦で大敗しました。",
        "whyImportant": "対米戦の危険を理解しながら開戦作戦を実行し、日本海軍の初期勝利と戦局転換の双方を担ったためです。"
      },
      "image": "assets/people/person-42eae66dde.webp",
      "imageAlt": "山本五十六の画像"
    },
    {
      "name": "ツタンカーメン",
      "kana": "ツタンカーメン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "少年期に即位した古代エジプト第18王朝の王で、アクエンアテンの宗教改革後に統治しました。名をツタンカーテンからツタンカーメンへ改め、都をアケトアテンから移し、アメン神をはじめ従来の神々と神殿を復興しました。十代後半で死去し、政治的実績は大きくありません。しかし1922年にハワード・カーターが王墓を発見し、黄金の棺・仮面・日用品がほぼまとまって残っていたため、新王国の葬祭・工芸・王宮生活を知る重要資料となりました。",
        "whatDid": "アテン中心の宗教改革を終わらせ、アメン神など従来の信仰と神殿を復興しました。",
        "whyImportant": "ほぼ未盗掘の王墓と膨大な副葬品が発見され、新王国エジプトの葬祭と生活を詳しく伝えるためです。"
      },
      "image": "assets/people/person-0d6bdcc9e3.webp",
      "imageAlt": "ツタンカーメンの画像"
    },
    {
      "name": "アルキメデス",
      "kana": "アルキメデス",
      "era": "古代の大国の時代",
      "genre": [
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前3世紀、シチリア島の都市シラクサで活動した数学者・技術者です。円周率の範囲、球と円柱の体積、放物線で囲まれた面積などを、図形を細かく分けて考える方法で求めました。物体が液体から受ける浮力を説明する「アルキメデスの原理」や、てこの原理も体系化しました。ローマ軍によるシラクサ包囲では防御機械を考案したと伝えられ、陥落時に殺されました。「エウレカ」の逸話には伝説的要素があります。",
        "whatDid": "浮力とてこの原理を明らかにし、円・球・放物線の面積や体積を高度な数学で求めました。",
        "whyImportant": "数学的証明を力学・流体・工学へ結び付け、近代科学へ続く数理的な自然研究の模範となったためです。"
      },
      "image": "assets/people/person-490f054f0e.webp",
      "imageAlt": "アルキメデスの画像"
    },
    {
      "name": "韓非",
      "kana": "韓非",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代末の韓の王族出身の思想家で、荀子に学んだと伝えられます。人は利益を求めて行動すると考え、君主は道徳的な善意に頼らず、明確な法律である「法」、役職に伴う権力である「勢」、臣下を操る統治技術である「術」を組み合わせるべきだと説きました。著作『韓非子』は秦王政に高く評価されましたが、秦へ使者として赴いた後、李斯らの策で投獄され、自害しました。その思想は秦の中央集権統治へ強く影響しました。",
        "whatDid": "法・勢・術を組み合わせ、君主が法律と制度によって臣下と国家を統制する方法を体系化しました。",
        "whyImportant": "法家思想を完成させ、秦の中央集権国家と後世の中国統治論へ大きな影響を与えたためです。"
      },
      "image": "assets/people/person-a053e7d49e.webp",
      "imageAlt": "韓非の画像"
    },
    {
      "name": "忽必烈",
      "kana": "忽必烈",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "チンギス・ハンの孫で、モンゴル帝国の大ハンとなり、1271年に中国王朝として元を建てました。南宋を滅ぼして中国全土を統一し、都を大都、現在の北京に置きました。中国式の官僚制度を取り入れながら、モンゴル人を頂点とする身分秩序を設け、駅伝制と紙幣で広域支配と交易を支えました。朝鮮や東南アジアへ影響を広げましたが、日本・ベトナム・ジャワへの遠征は失敗し、戦費と重税が国民へ大きな負担を与えました。",
        "whatDid": "元を建てて南宋を滅ぼし、中国を統一して大都・駅伝制・紙幣を用いた広域統治を進めました。",
        "whyImportant": "モンゴル帝国と中国王朝を結び、ユーラシア交流を活発化させる一方、遠征と支配の負担も残したためです。"
      },
      "image": "assets/people/person-148ecd3c87.webp",
      "imageAlt": "忽必烈の画像"
    },
    {
      "name": "アベラール",
      "kana": "アベラール",
      "era": "交易と王国の時代",
      "genre": [
        "culture",
        "philosopher"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "11～12世紀フランスの哲学者・神学者で、各地の学者と公開討論を行い、パリで多くの学生を集めました。普遍的な概念が実在するかをめぐる問題で、言葉と個物の関係を論理的に考えました。著書『然りと否』では、教父たちの矛盾する文章を並べ、権威ある言葉も理性によって検討する方法を示しました。弟子エロイーズとの恋愛と結婚、処罰、修道生活は往復書簡で知られます。信仰と理性を結ぶスコラ学の発展に重要な役割を果たしました。",
        "whatDid": "討論と『然りと否』を通して、権威ある教説の矛盾を論理的に検討する学問方法を広めました。",
        "whyImportant": "信仰内容を理性と論理で研究するスコラ学を発展させ、中世大学の討論教育へ影響したためです。"
      },
      "image": "assets/people/person-309ec64c9d.webp",
      "imageAlt": "アベラールの画像"
    },
    {
      "name": "ホセ・リサール",
      "kana": "ホセ・リサール",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "スペイン植民地時代のフィリピンの医師・作家・改革運動家です。ヨーロッパ留学で医学を学び、小説『ノリ・メ・タンヘレ』『エル・フィリブステリスモ』により、修道会の権力、植民地官僚の腐敗、差別を告発しました。武装独立ではなく、法の平等、教育、言論の自由、フィリピン人の政治参加を求めました。秘密結社カティプナンの反乱への直接関与はありませんでしたが、反乱の責任を問われ、1896年に処刑されました。その死は独立運動を強く刺激しました。",
        "whatDid": "小説でスペイン植民地支配と修道会の腐敗を批判し、教育・法の平等・政治参加を求めました。",
        "whyImportant": "言論による改革運動と処刑がフィリピン人の民族意識を高め、独立革命の象徴となったためです。"
      },
      "image": "assets/people/person-4f62e94133.webp",
      "imageAlt": "ホセ・リサールの画像"
    },
    {
      "name": "東条英機",
      "kana": "東条英機",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本陸軍の軍人・政治家で、満州事変後に関東軍参謀長、陸軍大臣などを務め、軍部主導の政治を進めました。1941年に首相となり、対米交渉を続けながらも開戦を決定し、真珠湾攻撃によって太平洋戦争へ入りました。首相・陸相を兼ねて権力を集中しましたが、戦局悪化とサイパン島陥落後に辞任しました。敗戦後、極東国際軍事裁判で侵略戦争を指導した責任を問われ、死刑判決を受けて処刑されました。",
        "whatDid": "首相として対米開戦を決定し、太平洋戦争中に軍事・政治権力を集中して戦争を指導しました。",
        "whyImportant": "日本の対米英開戦と侵略戦争を最高指導部で進め、戦争指導責任を裁かれた中心人物だからです。"
      },
      "image": "assets/people/person-6a7901b7d1.webp",
      "imageAlt": "東条英機の画像"
    }
  ],
  "peopleByName": {
    "ハンムラビ": {
      "name": "ハンムラビ",
      "kana": "はんむらび",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミア",
      "title": "法律を整えた王",
      "icon": "📜",
      "modal": {
        "profile": "古バビロニア王国の第6代王で、周辺都市を征服してメソポタミア南部を統一しました。各地の慣習法や裁判例を整理し、王が正義を守るという形で法を石碑に刻ませたものが「ハンムラビ法典」です。刑罰には身分差がありましたが、契約、相続、結婚、賃金など当時の社会生活を詳しく伝える史料として残っています。 王権と法の関係も読み取れます。",
        "whatDid": "メソポタミア南部を統一し、裁判の基準となる法律を「ハンムラビ法典」として石碑に刻ませました。",
        "whyImportant": "古代社会の法律、身分、家族、商業の仕組みを具体的に知ることができる代表的な法典を残したためです。"
      },
      "image": "assets/people/person-bd338fe558.webp",
      "imageAlt": "ハンムラビの画像"
    },
    "クフ王": {
      "name": "クフ王",
      "kana": "くふおう",
      "era": "最初の文明の時代",
      "genre": [
        "monarch"
      ],
      "field": "古代エジプト",
      "title": "大ピラミッドの時代の王",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト古王国時代の第4王朝の王で、ギザにある三大ピラミッドのうち最大のものを築かせた王と考えられています。巨大な石材を切り出して運び、職人や農民へ食料を配る大規模な国家事業には、王権の強さと行政組織の発達が表れています。ピラミッドは王の墓であると同時に、古代エジプトの技術と信仰を今に伝えます。",
        "whatDid": "ギザの大ピラミッドを築かせ、大量の石材・労働者・食料を動かす国家事業を指揮しました。",
        "whyImportant": "大ピラミッドが、古代エジプトの王権、宗教観、測量・建築技術を示す世界的遺産となったためです。"
      },
      "image": "assets/people/person-5119f42d3b.webp",
      "imageAlt": "クフ王の画像"
    },
    "孔子": {
      "name": "孔子",
      "kana": "こうし",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "中国",
      "title": "人の道を説いた思想家",
      "icon": "📘",
      "modal": {
        "profile": "中国の春秋時代、諸国の争いで政治や社会の秩序が揺らぐなか、人を思いやる「仁」と、礼儀や社会の決まりである「礼」を重んじた思想家です。身分にかかわらず学ぶ意欲のある弟子を教え、政治を行う者は徳によって人々を導くべきだと説きました。死後、弟子たちが言葉や対話をまとめた書物が『論語』です。孔子の教えをもとに発達した儒教は、東アジアの政治、教育、家族観に長く影響しました。",
        "whatDid": "「仁」と「礼」を重んじる政治と生き方を説き、多くの弟子を育てました。その言葉は『論語』にまとめられました。",
        "whyImportant": "孔子の教えから儒教が発達し、中国・朝鮮・日本の政治、教育、道徳の基本となったためです。"
      },
      "image": "assets/people/person-3c08d403ad.webp",
      "imageAlt": "孔子の画像"
    },
    "始皇帝": {
      "name": "始皇帝",
      "kana": "しこうてい",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "中国",
      "title": "中国を統一した皇帝",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代の秦王として六国を滅ぼし、紀元前221年に中国を初めて統一して「皇帝」を名乗りました。地域ごとに異なっていた文字、貨幣、度量衡、車輪の幅をそろえ、郡県制で全国を中央から支配しました。一方、万里の長城の建設や宮殿工事に多くの人々を動員し、厳しい法と思想統制への反発を招きました。短命に終わった秦の制度は、後の中国王朝の統治の土台になりました。",
        "whatDid": "六国を征服して中国を統一し、文字・貨幣・度量衡を統一して郡県制による中央集権を進めました。",
        "whyImportant": "中国を一つの皇帝国家として治める制度と標準を整え、後の歴代王朝の統治モデルを築いたためです。"
      },
      "image": "assets/people/person-6ce46faf8f.webp",
      "imageAlt": "始皇帝の画像"
    },
    "アレクサンドロス大王": {
      "name": "アレクサンドロス大王",
      "kana": "あれくさんどろすだいおう",
      "era": "古代の大国の時代",
      "genre": [
        "monarch"
      ],
      "field": "ギリシャ・西アジア",
      "title": "東西を結んだ征服者",
      "icon": "🛡️",
      "modal": {
        "profile": "マケドニア王フィリッポス2世の子として即位し、ギリシャ諸都市を従えてペルシャ帝国への遠征を始めました。小アジア、エジプト、メソポタミア、イランを征服し、軍はインド北西部に達しました。各地にギリシャ風都市を建設し、現地の支配層も取り入れたため、ギリシャ文化と西アジア・エジプトの文化が混ざるヘレニズム文化が広がりました。死後、帝国は後継者に分割されました。",
        "whatDid": "ペルシャ帝国を征服してインド北西部まで進軍し、各地にギリシャ風都市を建設しました。",
        "whyImportant": "征服地で東西の文化が交流し、地中海から西アジアに広がるヘレニズム世界が成立したためです。"
      },
      "image": "assets/people/person-6f8c27de1c.webp",
      "imageAlt": "アレクサンドロス大王の画像"
    },
    "アショーカ王": {
      "name": "アショーカ王",
      "kana": "あしょーかおう",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "インド",
      "title": "仏教を保護した王",
      "icon": "🕊️",
      "modal": {
        "profile": "インドのマウリヤ朝第3代の王で、紀元前3世紀にほぼ全インドを治めました。カリンガ征服で多数の死者や捕虜が出たことを悔い、武力による拡大よりも生命の尊重や寛容を重んじる「ダルマ」に基づく政治を掲げました。命令や教えを石柱・岩壁の碑文に刻み、道路、井戸、医療施設も整えました。仏教を保護し、使節を各地へ送り、その広がりを後押ししました。",
        "whatDid": "カリンガ戦争後に武力征服を反省し、ダルマによる政治を行って仏教を保護・普及しました。",
        "whyImportant": "王の政治に生命尊重と宗教的寛容を取り入れ、仏教がインド内外へ広がる契機をつくったためです。"
      },
      "image": "assets/people/person-3b5126d5ca.webp",
      "imageAlt": "アショーカ王の画像"
    },
    "イエス": {
      "name": "イエス",
      "kana": "いえす",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "西アジア",
      "title": "キリスト教の中心となった人",
      "icon": "🕊️",
      "modal": {
        "profile": "ローマ帝国支配下のユダヤ地方で活動し、神への愛と隣人への愛、弱い立場の人への救いを説いた人物です。律法の形式だけでなく、人の心と行いを重視し、多くの弟子を集めました。エルサレムで捕らえられ、ローマ総督ポンティウス・ピラトゥスのもとで十字架刑に処されました。弟子たちはイエスが復活したと信じ、その教えと救いを広めたため、のちにキリスト教が成立しました。",
        "whatDid": "神と隣人への愛、罪のゆるし、弱い人への救いを説き、その教えのもとに弟子たちを集めました。",
        "whyImportant": "死後、弟子たちが教えと復活への信仰を広め、世界宗教の一つであるキリスト教が成立したためです。"
      },
      "image": "assets/people/person-83bd092dca.webp",
      "imageAlt": "イエスの画像"
    },
    "ムハンマド": {
      "name": "ムハンマド",
      "kana": "むはんまど",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "アラビア半島",
      "title": "イスラム教を始めた預言者",
      "icon": "☪️",
      "modal": {
        "profile": "アラビア半島の商業都市メッカに生まれ、唯一神アッラーから啓示を受けた預言者として人々に信仰を説きました。迫害を受けて622年にメディナへ移住し、信者の共同体を築きました。この移住はヒジュラと呼ばれ、イスラム暦の元年とされます。のちにメッカを支配下に入れ、アラビア半島の多くをまとめました。啓示は死後に『コーラン』として編まれ、イスラム教の教えと生活規範の基礎になりました。",
        "whatDid": "唯一神アッラーへの信仰を説き、メディナにイスラム共同体を築いてアラビア半島をまとめました。",
        "whyImportant": "『コーラン』に基づくイスラム教と共同体の基礎を築き、その後の西アジア世界を大きく形づくったためです。"
      },
      "image": "assets/people/person-920c89b7aa.webp",
      "imageAlt": "ムハンマドの画像"
    },
    "玄奘": {
      "name": "玄奘",
      "kana": "げんじょう",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "中国・インド",
      "title": "仏教を学んだ旅の僧",
      "icon": "📚",
      "modal": {
        "profile": "唐の僧で、中国に伝わった仏典の内容に疑問を持ち、原典を求めて629年ごろ長安を出発しました。砂漠や高山を越え、中央アジアを通ってインドへ入り、ナーランダー僧院などで仏教、とくに唯識思想を学びました。約17年後に多数の経典や仏像を持ち帰り、長安で正確な漢訳に取り組みました。旅の記録『大唐西域記』は、当時のインドや中央アジアを知る重要史料でもあります。",
        "whatDid": "仏教の原典を求めてインドへ旅し、帰国後に多くのサンスクリット経典を漢訳しました。",
        "whyImportant": "中国仏教の理解を深め、『大唐西域記』によって7世紀の中央アジアとインドの姿も伝えたためです。"
      },
      "image": "assets/people/person-077a351fc3.webp",
      "imageAlt": "玄奘の画像"
    },
    "クローヴィス": {
      "name": "クローヴィス",
      "kana": "くろーびす",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランク王国",
      "title": "フランク王国を建てた王",
      "icon": "👑",
      "modal": {
        "profile": "フランク人の一部族サリ族の王から勢力を広げ、ガリア北部のローマ系勢力や他のゲルマン諸部族を破ってフランク王国の基礎を築きました。496年ごろ、家臣とともにカトリックへ改宗したと伝えられます。当時の多くのゲルマン王がアリウス派だったなか、ローマ教会と同じ信仰を選んだことで、ガリアのローマ系住民や司教の支持を得ました。王国とカトリック教会の結びつきは西ヨーロッパの政治に長く影響しました。",
        "whatDid": "フランク諸部族をまとめてガリアを支配し、カトリックへ改宗してローマ教会と結びつきました。",
        "whyImportant": "フランク王国とカトリック教会の協力関係を築き、中世西ヨーロッパの政治秩序の出発点となったためです。"
      },
      "image": "assets/people/person-547e05b8b9.webp",
      "imageAlt": "クローヴィスの画像"
    },
    "カール大帝": {
      "name": "カール大帝",
      "kana": "かーるたいてい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランク王国",
      "title": "西ヨーロッパをまとめた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "フランク王国のカロリング朝の王で、ザクセン、ランゴバルド王国などを征服し、西ヨーロッパの広い地域を支配しました。800年、ローマ教皇から皇帝の冠を授けられ、西ローマ帝国滅亡後の西欧に皇帝権を復活させました。各地に伯を置き、巡察使を送って統治を監督し、修道院や宮廷学校で古典学問とラテン語教育を保護しました。この文化復興はカロリング・ルネサンスと呼ばれます。",
        "whatDid": "西ヨーロッパの広域を統一し、皇帝に即位して統治制度と学問・教育の整備を進めました。",
        "whyImportant": "王権・ローマ教会・古典文化を結びつけ、中世西ヨーロッパの政治と文化の基礎を築いたためです。"
      },
      "image": "assets/people/person-5ba86ccd42.webp",
      "imageAlt": "カール大帝の画像"
    },
    "オットー1世": {
      "name": "オットー1世",
      "kana": "おっとーいっせい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "神聖ローマ帝国",
      "title": "神聖ローマ帝国を始めた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "東フランク王国のザクセン朝第2代の王で、反乱する諸侯を抑え、司教や修道院長を統治に利用して王権を強めました。955年のレヒフェルトの戦いでマジャール人を破り、東方からの侵入を止めました。イタリアへ遠征して教皇を保護し、962年にローマ皇帝の冠を受けました。この戴冠は、後に神聖ローマ帝国と呼ばれるドイツ・イタリアを中心とした政治秩序の出発点とされています。",
        "whatDid": "諸侯を抑えて王権を強化し、マジャール人を破った後、962年にローマ皇帝として戴冠しました。",
        "whyImportant": "ドイツ王とローマ皇帝を結びつけ、神聖ローマ帝国へ続く中世ヨーロッパの政治秩序を築いたためです。"
      },
      "image": "assets/people/person-516f25fdf3.webp",
      "imageAlt": "オットー1世の画像"
    },
    "フリードリヒ・バルバロッサ": {
      "name": "フリードリヒ・バルバロッサ",
      "kana": "ふりーどりひ・ばるばろっさ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "神聖ローマ帝国",
      "title": "イタリアへ遠征した皇帝",
      "icon": "👑",
      "modal": {
        "profile": "ホーエンシュタウフェン朝の神聖ローマ皇帝で、「赤ひげ」を意味するバルバロッサの名で知られます。ドイツ諸侯をまとめ、北イタリア都市に皇帝権を及ぼそうと何度も遠征しましたが、ロンバルディア同盟と教皇の抵抗を受け、1176年のレニャーノの戦いで敗れました。その後は和約を結び、第三回十字軍に参加しましたが、小アジアの川で溺死しました。皇帝と都市・教皇の対立を象徴する人物です。",
        "whatDid": "北イタリアへ遠征して皇帝権の拡大を図り、晩年には第三回十字軍を率いました。",
        "whyImportant": "皇帝・教皇・自治都市が競い合った中世ヨーロッパの複雑な権力関係を示す代表的な皇帝だからです。"
      },
      "image": "assets/people/person-dfb6ed9cfb.webp",
      "imageAlt": "フリードリヒ・バルバロッサの画像"
    },
    "エドワード1世": {
      "name": "エドワード1世",
      "kana": "えどわーどいっせい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "イングランド",
      "title": "議会と法を整えた王",
      "icon": "👑",
      "modal": {
        "profile": "13世紀後半のイングランド王で、法令と裁判制度を整え、王権による国内統治を強めました。ウェールズを征服して城を築き、王子に「プリンス・オブ・ウェールズ」の称号を与えました。スコットランドにも介入しましたが、ウィリアム・ウォレスらの抵抗を受けました。1295年には聖職者、貴族、騎士、都市代表を集めた模範議会を開き、後のイングランド議会の構成に大きな影響を与えました。",
        "whatDid": "法と裁判制度を整え、ウェールズを征服し、1295年に各身分の代表を集めた模範議会を開きました。",
        "whyImportant": "王の課税に各身分の代表が関わる議会の形を整え、後のイギリス議会政治の発達につながったためです。"
      },
      "image": "assets/people/person-2108b78bda.webp",
      "imageAlt": "エドワード1世の画像"
    },
    "ヘンリー5世": {
      "name": "ヘンリー5世",
      "kana": "へんりーごせい",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "イングランド",
      "title": "百年戦争を戦った王",
      "icon": "👑",
      "modal": {
        "profile": "ランカスター朝のイングランド王で、国内の反乱を抑えた後、フランス王位への権利を主張して百年戦争を再開しました。1415年のアジャンクールの戦いでは、長弓兵を生かして数で勝るフランス軍に大勝しました。1420年のトロワ条約でフランス王シャルル6世の娘と結婚し、将来のフランス王位継承を認めさせましたが、若くして病死し、両国統一は実現しませんでした。",
        "whatDid": "百年戦争を再開し、アジャンクールで大勝してトロワ条約によりフランス王位継承権を得ました。",
        "whyImportant": "イングランドが百年戦争で最も優位に立った時期を築き、その後のフランス側の反撃を招いたためです。"
      },
      "image": "assets/people/person-9e2923c493.webp",
      "imageAlt": "ヘンリー5世の画像"
    },
    "ジャンヌ・ダルク": {
      "name": "ジャンヌ・ダルク",
      "kana": "じゃんぬ・だるく",
      "era": "交易と王国の時代",
      "genre": [
        "war"
      ],
      "field": "フランス",
      "title": "百年戦争で人々を励ました少女",
      "icon": "⚔️",
      "modal": {
        "profile": "フランス北東部の農村に生まれ、神の声を聞いたと信じて百年戦争中の王太子シャルルを訪ねました。1429年、包囲されたオルレアンへ入り、フランス軍の士気を高めて解放に導き、シャルル7世のランスでの戴冠を実現させました。翌年ブルゴーニュ軍に捕らえられ、イングランド側の宗教裁判で異端とされて火刑に処されました。後に裁判は無効とされ、フランス抵抗の象徴となりました。",
        "whatDid": "オルレアン解放を助け、王太子シャルルをランスへ導いてフランス王シャルル7世として戴冠させました。",
        "whyImportant": "百年戦争で劣勢だったフランスの士気と王権を立て直し、国民的結束の象徴となったためです。"
      },
      "image": "assets/people/person-9f857af814.webp",
      "imageAlt": "ジャンヌ・ダルクの画像"
    },
    "ジル・ド・レエ": {
      "name": "ジル・ド・レエ",
      "kana": "じる・ど・れえ",
      "era": "交易と王国の時代",
      "genre": [
        "war"
      ],
      "field": "フランス",
      "title": "ジャンヌとともに戦った貴族",
      "icon": "⚔️",
      "modal": {
        "profile": "フランス西部の有力貴族で、百年戦争ではジャンヌ・ダルクとともにオルレアン解放やシャルル7世の戴冠行軍に参加し、若くしてフランス元帥となりました。戦後は領地で豪華な生活や演劇に巨額を費やし、財産を失っていきました。1440年、聖職者への暴行や多数の子どもの殺害などの罪で教会と世俗の裁判にかけられ、処刑されました。軍事的栄誉と重大犯罪の記録が併存する人物です。",
        "whatDid": "百年戦争でジャンヌ・ダルクと戦いフランス元帥となりましたが、後に重大犯罪で裁かれ処刑されました。",
        "whyImportant": "英雄的な軍歴と犯罪者としての裁判記録が併存し、中世貴族社会と司法の暗部を示す人物だからです。"
      },
      "imageAlt": "ジル・ド・レエの画像",
      "image": "assets/people/person-f6d272d91c.webp"
    },
    "マクシミリアン1世": {
      "name": "マクシミリアン1世",
      "kana": "まくしみりあんいっせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch"
      ],
      "field": "神聖ローマ帝国",
      "title": "結婚で領土を広げた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "ハプスブルク家の神聖ローマ皇帝で、ブルゴーニュ公女マリーとの結婚によってネーデルラントなどの領地を獲得しました。子どもたちをスペイン王家と結婚させ、さらに孫の婚姻でボヘミア・ハンガリーへの継承権も確保しました。「戦争は他国にさせよ、幸いなるオーストリアよ、結婚せよ」と表現される婚姻政策により、孫カール5世が広大な領土を受け継ぐ土台を築きました。",
        "whatDid": "自らと子孫の婚姻を利用して、ネーデルラント・スペイン・中欧へハプスブルク家の継承権を広げました。",
        "whyImportant": "戦争だけでなく婚姻によって領土を拡大するハプスブルク家の国際戦略を確立したためです。"
      },
      "image": "assets/people/person-6686946bab.webp",
      "imageAlt": "マクシミリアン1世の画像"
    },
    "カール5世": {
      "name": "カール5世",
      "kana": "かーるごせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch"
      ],
      "field": "スペイン・神聖ローマ帝国",
      "title": "広大な領土を治めた皇帝",
      "icon": "👑",
      "modal": {
        "profile": "ハプスブルク家に生まれ、スペイン王カルロス1世と神聖ローマ皇帝を兼ね、ネーデルラント、イタリアの一部、アメリカ大陸の植民地を含む広大な領土を治めました。宗教改革ではルター派諸侯と対立し、フランス王フランソワ1世やオスマン帝国とも戦いました。しかし領土が広すぎて統一支配は難しく、晩年に退位してスペイン系とオーストリア系に領地を分けました。",
        "whatDid": "スペイン王と神聖ローマ皇帝を兼ね、宗教改革・フランス・オスマン帝国との戦いに対応しました。",
        "whyImportant": "ヨーロッパとアメリカにまたがるハプスブルク帝国を治め、16世紀国際政治の中心となったためです。"
      },
      "image": "assets/people/person-32c3a12328.webp",
      "imageAlt": "カール5世の画像"
    },
    "ヘンリー8世": {
      "name": "ヘンリー8世",
      "kana": "へんりーはっせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "イングランド",
      "title": "イギリス国教会をつくった王",
      "icon": "👑",
      "modal": {
        "profile": "テューダー朝のイングランド王で、王妃キャサリンとの結婚無効を教皇が認めなかったことからローマ教会と対立しました。1534年の国王至上法で国王をイングランド教会の最高首長とし、修道院を解散して土地と財産を没収しました。教義にはカトリック的要素を残しましたが、教皇から独立した国教会を成立させたことで、イングランドの宗教改革と王権強化を大きく進めました。",
        "whatDid": "国王至上法を制定してローマ教皇から離れ、国王を首長とするイングランド国教会を成立させました。",
        "whyImportant": "宗教と政治の中心を教皇から国王へ移し、イングランド宗教改革と近代国家形成を進めたためです。"
      },
      "image": "assets/people/person-f58f3d2792.webp",
      "imageAlt": "ヘンリー8世の画像"
    },
    "アンリ4世": {
      "name": "アンリ4世",
      "kana": "あんりよんせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランス",
      "title": "宗教戦争をしずめた王",
      "icon": "👑",
      "modal": {
        "profile": "フランスのブルボン朝最初の王で、もとはプロテスタントでした。ユグノー戦争のさなかに王位を継ぎ、国内多数派のカトリックへ改宗してパリに入りました。1598年のナントの勅令で、カトリックを国の宗教としながらプロテスタントにも一定の信仰と権利を認め、長い宗教戦争を終結させました。財政と農業の再建も進めましたが、1610年にカトリック過激派に暗殺されました。",
        "whatDid": "カトリックへ改宗して王位を固め、ナントの勅令でプロテスタントにも一定の信仰の自由を認めました。",
        "whyImportant": "宗教内戦を収めてフランスの統一と復興を進め、ブルボン朝による王権強化の基礎を築いたためです。"
      },
      "image": "assets/people/person-1ac2f10bef.webp",
      "imageAlt": "アンリ4世の画像"
    },
    "ルイ14世": {
      "name": "ルイ14世",
      "kana": "るいじゅうよんせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch"
      ],
      "field": "フランス",
      "title": "絶対王政の太陽王",
      "icon": "👑",
      "modal": {
        "profile": "ブルボン朝のフランス王で、幼少期の反乱フロンドの乱を経験した後、宰相に頼らず自ら政治を行いました。貴族をベルサイユ宮殿に集めて宮廷生活へ組み込み、官僚と常備軍を用いて絶対王政を強化しました。財務総監コルベールは重商主義政策を進めましたが、度重なる戦争と宮殿建設は財政を圧迫しました。ナントの勅令を廃止してユグノーを迫害したことも社会と経済に損失を与えました。",
        "whatDid": "貴族をベルサイユ宮殿に集め、官僚・常備軍・重商主義政策によって絶対王政を完成に近づけました。",
        "whyImportant": "フランス絶対王政の最盛期を築く一方、戦争と宗教迫害で後の財政危機も深めたためです。"
      },
      "image": "assets/people/person-4305b9c03d.webp",
      "imageAlt": "ルイ14世の画像"
    },
    "チャールズ1世": {
      "name": "チャールズ1世",
      "kana": "ちゃーるずいっせい",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "イングランド",
      "title": "革命で処刑された王",
      "icon": "👑",
      "modal": {
        "profile": "ステュアート朝のイングランド王で、王権神授説を信じ、議会の同意を得ずに課税しようとして対立しました。権利の請願を受け入れた後も議会を長く開かず、宗教政策への反発も重なって清教徒革命が始まりました。王党派を率いて議会軍と戦いましたが敗北し、国家への反逆罪で公開裁判にかけられ、1649年に処刑されました。王を法の下で裁いた出来事は、王権の限界を示しました。",
        "whatDid": "議会を無視して統治しようとして内戦を招き、敗北後に国王として公開裁判を受け処刑されました。",
        "whyImportant": "国王も法と議会に制約されることを示し、イギリス立憲政治の発展につながったためです。"
      },
      "image": "assets/people/person-d3cc59b102.webp",
      "imageAlt": "チャールズ1世の画像"
    },
    "ルイ16世": {
      "name": "ルイ16世",
      "kana": "るいじゅうろくせい",
      "era": "革命と工場の時代",
      "genre": [
        "monarch"
      ],
      "field": "フランス",
      "title": "フランス革命で処刑された王",
      "icon": "👑",
      "modal": {
        "profile": "ブルボン朝末期のフランス王で、即位時から戦争による国の借金と不公平な税制度に直面しました。財政改革を試みましたが、特権身分の抵抗で進まず、1789年に三部会を招集したことがフランス革命の始まりにつながりました。王一家は国外逃亡を企てたヴァレンヌ逃亡事件で信用を失い、王政は廃止されました。外国と通じた反逆の罪で裁かれ、1793年に処刑されました。",
        "whatDid": "財政危機を解決できず三部会を招集し、革命後は国外逃亡に失敗して王政廃止と処刑に至りました。",
        "whyImportant": "絶対王政の崩壊と国民主権への転換を象徴し、フランス革命の進展に大きく関わったためです。"
      },
      "image": "assets/people/person-4fd5b57d17.webp",
      "imageAlt": "ルイ16世の画像"
    },
    "マリー・アントワネット": {
      "name": "マリー・アントワネット",
      "kana": "まりー・あんとわねっと",
      "era": "革命と工場の時代",
      "genre": [
        "monarch"
      ],
      "field": "フランス",
      "title": "フランス革命で処刑された王妃",
      "icon": "👑",
      "modal": {
        "profile": "オーストリアのハプスブルク家からフランス王太子ルイへ嫁ぎ、のちに王妃となりました。フランスとオーストリアの同盟を支える政略結婚でしたが、外国出身であることや宮廷の浪費の象徴として批判され、実際以上の悪評も広まりました。革命後、王一家のヴァレンヌ逃亡に加わり、外国勢力との連絡を疑われました。王政廃止後に裁判へかけられ、1793年に処刑されました。",
        "whatDid": "フランス王妃として宮廷に入り、革命後は王一家の逃亡計画と外国勢力への働きかけに関わりました。",
        "whyImportant": "旧体制の特権と浪費の象徴として革命宣伝に利用され、王政崩壊の過程を示す人物となったためです。"
      },
      "image": "assets/people/person-11e8ea1d8a.webp",
      "imageAlt": "マリー・アントワネットの画像"
    },
    "マンサ・ムーサ": {
      "name": "マンサ・ムーサ",
      "kana": "まんさ・むーさ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "西アフリカ",
      "title": "マリ帝国の王",
      "icon": "💰",
      "modal": {
        "profile": "14世紀の西アフリカに栄えたマリ帝国の王で、金と塩の交易路を支配し、帝国の最盛期を築きました。1324年からのメッカ巡礼では多数の従者と大量の金を伴い、カイロなどで金を広く使ったため、その富が地中海世界に知られました。帰国後、トンブクトゥやガオにモスクを建て、イスラム学者や建築家を招きました。マリ帝国を交易とイスラム学問の中心として世界地図に刻んだ王です。",
        "whatDid": "金と塩の交易でマリ帝国を繁栄させ、メッカ巡礼と学者・建築家の招致でイスラム文化を発展させました。",
        "whyImportant": "サハラ交易で栄えた西アフリカの富と学問を地中海・イスラム世界へ広く知らせたためです。"
      },
      "image": "assets/people/person-827fa9b9b3.webp",
      "imageAlt": "マンサ・ムーサの画像"
    },
    "チンギス・ハン": {
      "name": "チンギス・ハン",
      "kana": "ちんぎす・はん",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "モンゴル高原",
      "title": "モンゴル帝国を築いた人",
      "icon": "🐎",
      "modal": {
        "profile": "モンゴル高原の有力者テムジンとして生まれ、部族間の争いを勝ち抜き、1206年に諸部族からチンギス・ハンに推戴されました。軍を十進法で編成し、能力を重んじる人事と厳しい軍律で統率して、金、西夏、中央アジアのホラズム朝へ征服を広げました。遠征では都市破壊と大量殺害も起こりました。彼が築いた帝国は子孫の時代にユーラシアの広域を結び、人・物・技術の交流を活発にしました。",
        "whatDid": "モンゴル諸部族を統一し、機動力の高い騎馬軍を組織して中央アジアまで大帝国を築きました。",
        "whyImportant": "征服の大被害を伴いながらも、ユーラシアを一つの広域支配圏へ結ぶモンゴル帝国の基礎を築いたためです。"
      },
      "image": "assets/people/person-94822eef6a.webp",
      "imageAlt": "チンギス・ハンの画像"
    },
    "イブン・バットゥータ": {
      "name": "イブン・バットゥータ",
      "kana": "いぶん・ばっとぅーた",
      "era": "交易と王国の時代",
      "genre": [
        "religion",
        "culture",
        "political-scholar"
      ],
      "field": "イスラム世界",
      "title": "広い世界を旅した学者",
      "icon": "🧭",
      "modal": {
        "profile": "モロッコのタンジールに生まれたイスラム法学者で、1325年にメッカ巡礼へ出発した後、約30年にわたり北アフリカ、西アジア、東アフリカ、インド、東南アジア、中国などを旅しました。各地で法官や使節として働き、イスラム圏に共通する信仰と、地域ごとに異なる政治・習慣を見聞しました。帰国後、口述した旅行記『大旅行記』は、14世紀の交易路、港町、宮廷、庶民生活を伝える重要史料です。",
        "whatDid": "約30年かけてアフリカ・西アジア・インド・中国を旅し、その見聞を『大旅行記』に残しました。",
        "whyImportant": "14世紀のイスラム世界とインド洋交易圏の広さ、多様な社会と人々の交流を具体的に伝えたためです。"
      },
      "image": "assets/people/person-ad4f3ff7e7.webp",
      "imageAlt": "イブン・バットゥータの画像"
    },
    "鄭和": {
      "name": "鄭和",
      "kana": "ていわ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "中国・インド洋",
      "title": "大船団を率いた航海者",
      "icon": "⛵",
      "modal": {
        "profile": "雲南出身のイスラム教徒の家に生まれ、明の永楽帝に仕えた宦官・武将です。1405年から1433年まで七回にわたり大船団を率い、東南アジア、インド、アラビア半島、東アフリカ沿岸まで航海しました。各地の支配者へ明皇帝の威信を示し、朝貢と交易を求め、使節や珍しい産物を中国へ連れ帰りました。領土征服を目的とした航海ではありませんが、明とインド洋世界の外交・交易を大きく広げました。",
        "whatDid": "七回の大航海で大船団を率い、東南アジアから東アフリカまで明の外交と朝貢貿易を広げました。",
        "whyImportant": "ヨーロッパの大航海に先立ち、中国とインド洋沿岸諸国を結ぶ大規模な海上交流を実現したためです。"
      },
      "image": "assets/people/person-be824ba0ed.webp",
      "imageAlt": "鄭和の画像"
    },
    "クリストファー・コロンブス": {
      "name": "クリストファー・コロンブス",
      "kana": "くりすとふぁー・ころんぶす",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "大西洋",
      "title": "大西洋を渡った航海者",
      "icon": "⛵",
      "modal": {
        "profile": "ジェノヴァ出身の航海者で、地球が球形であることを前提に大西洋を西へ進めばアジアへ着けると考え、スペイン王室の支援を得ました。1492年にカリブ海の島へ到達しましたが、そこをアジア周辺と誤認したまま四回航海しました。この航海を契機にヨーロッパ人のアメリカ進出が本格化し、作物・家畜・病原体の交換が進みました。一方、先住民への暴力、強制労働、植民地支配も始まりました。",
        "whatDid": "スペイン王室の支援で大西洋を横断し、1492年にカリブ海へ到達してヨーロッパの進出を促しました。",
        "whyImportant": "旧大陸とアメリカ大陸の恒常的接触を始め、世界規模の交流と植民地支配の時代を開いたためです。"
      },
      "image": "assets/people/person-ad25b28545.webp",
      "imageAlt": "クリストファー・コロンブスの画像"
    },
    "バルトロメ・デ・ラス・カサス": {
      "name": "バルトロメ・デ・ラス・カサス",
      "kana": "ばるとろめ・で・らす・かさす",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "スペイン領アメリカ",
      "title": "先住民の苦しみを訴えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "スペイン人植民者としてカリブ海へ渡り、当初は先住民を労働させるエンコミエンダ制の利益を受けました。しかし征服と強制労働による虐待を目にして土地と先住民を手放し、ドミニコ会修道士として保護運動に転じました。著書『インディアスの破壊についての簡潔な報告』で残虐行為を告発し、スペイン王室へ制度改革を求めました。ただし初期にはアフリカ人奴隷導入を提案したという限界もあり、後に悔いています。",
        "whatDid": "植民地支配に加担した立場を改め、先住民への虐待と強制労働を記録してスペイン王室へ改革を訴えました。",
        "whyImportant": "ヨーロッパの征服を内部から批判し、植民地支配と先住民の権利をめぐる議論を早くから起こしたためです。"
      },
      "image": "assets/people/person-96f97c1af2.webp",
      "imageAlt": "バルトロメ・デ・ラス・カサスの画像"
    },
    "ジョージ・ワシントン": {
      "name": "ジョージ・ワシントン",
      "kana": "じょーじ・わしんとん",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "北アメリカ",
      "title": "アメリカ独立を導いた人",
      "icon": "🗽",
      "modal": {
        "profile": "バージニアの農園主・軍人で、イギリス本国と植民地の対立が深まると、大陸軍総司令官としてアメリカ独立戦争を指揮しました。フランスの支援を得てヨークタウンで英軍を降伏させ、独立を実現しました。1789年に初代大統領となり、連邦政府の制度と権威を整えました。二期で退任して平和的に政権を譲る慣例をつくりましたが、自身は奴隷を所有しており、自由の理念との矛盾も残しました。",
        "whatDid": "大陸軍を率いて独立戦争に勝利し、初代大統領として連邦政府の基礎と政権交代の慣例を築きました。",
        "whyImportant": "アメリカ独立と共和政国家の出発を導き、大統領制の実際の運用に先例を残したためです。"
      },
      "image": "assets/people/person-b128c0aae5.webp",
      "imageAlt": "ジョージ・ワシントンの画像"
    },
    "ナポレオン": {
      "name": "ナポレオン",
      "kana": "なぽれおん",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "フランス・ヨーロッパ",
      "title": "ヨーロッパを動かした指導者",
      "icon": "⚔️",
      "modal": {
        "profile": "コルシカ島出身の砲兵将校で、フランス革命戦争の勝利によって頭角を現し、1799年のクーデタで第一統領、1804年に皇帝となりました。行政・教育・税制を中央集権化し、法の下の平等や私有財産を定めたナポレオン法典を整えました。ヨーロッパ各地を征服して革命の制度を広げる一方、支配への抵抗と民族意識も高めました。ロシア遠征の失敗後に敗れ、ワーテルローで最終的に退位しました。",
        "whatDid": "革命後のフランスで皇帝となり、ナポレオン法典と中央集権制度を整え、ヨーロッパ各地へ進出しました。",
        "whyImportant": "革命の法と制度を欧州へ広げる一方、征服戦争が民族主義と国際秩序の再編を促したためです。"
      },
      "image": "assets/people/person-aecb5fefb9.webp",
      "imageAlt": "ナポレオンの画像"
    },
    "シモン・ボリバル": {
      "name": "シモン・ボリバル",
      "kana": "しもん・ぼりばる",
      "era": "革命と工場の時代",
      "genre": [
        "politician"
      ],
      "field": "南アメリカ",
      "title": "独立運動を進めた人",
      "icon": "🗽",
      "modal": {
        "profile": "南米北部の裕福なクリオーリョに生まれ、啓蒙思想とアメリカ・フランス革命の影響を受けてスペインからの独立運動に参加しました。アンデス山脈を越える遠征などを指揮し、ベネズエラ、コロンビア、エクアドル、ペルーの解放に貢献しました。大コロンビアを中心に南米諸国の統合を目指しましたが、地域対立と権力争いで崩壊しました。独立を実現した一方、奴隷制や社会格差の解消は不十分でした。",
        "whatDid": "軍を率いて南米北部のスペイン支配を打破し、大コロンビアを建てて地域統合を目指しました。",
        "whyImportant": "ラテンアメリカ諸国の独立を実現した中心人物で、現在も「解放者」として国家形成の象徴だからです。"
      },
      "image": "assets/people/person-04de7d8241.webp",
      "imageAlt": "シモン・ボリバルの画像"
    },
    "ジェームズ・ワット": {
      "name": "ジェームズ・ワット",
      "kana": "じぇーむず・わっと",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "イギリス",
      "title": "蒸気機関を改良した発明家",
      "icon": "⚙️",
      "modal": {
        "profile": "スコットランド出身の器具製作者・技術者で、ニューコメン型蒸気機関の模型修理をきっかけに、熱が失われる原因を研究しました。蒸気をシリンダーとは別の復水器で冷やす方式を考案し、燃料消費を大きく減らしました。実業家ボールトンと協力し、回転運動を取り出せる機関へ改良したことで、鉱山の排水だけでなく紡績・製粉など工場機械にも利用が広がりました。蒸気動力の普及を支えた技術者です。",
        "whatDid": "分離復水器を考案して蒸気機関の燃料効率を高め、回転運動を利用できる工場動力へ改良しました。",
        "whyImportant": "蒸気機関を多くの工場で使える実用的な動力にし、産業革命の機械化を大きく進めたためです。"
      },
      "image": "assets/people/person-8e2dc5e8cf.webp",
      "imageAlt": "ジェームズ・ワットの画像"
    },
    "フローレンス・ナイチンゲール": {
      "name": "フローレンス・ナイチンゲール",
      "kana": "ふろーれんす・ないちんげーる",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war",
        "culture",
        "medicine"
      ],
      "field": "イギリス",
      "title": "看護を変えた人",
      "icon": "🩺",
      "modal": {
        "profile": "イギリスの裕福な家庭に生まれ、家族の反対を押して看護を学びました。クリミア戦争中、スクタリの陸軍病院で看護団を率い、清掃、換気、洗濯、食事、記録を改善しました。死亡率低下には戦争状況など複数の要因がありますが、衛生環境の重要性を統計図で政府へ示したことが改革を進めました。帰国後は看護学校を設立し、看護を訓練と知識を必要とする専門職へ発展させました。",
        "whatDid": "クリミア戦争の病院で衛生と記録を改善し、統計を用いて軍医療改革を求め、看護学校を設立しました。",
        "whyImportant": "近代看護の教育制度と専門性を確立し、統計を政策改善に用いる公衆衛生の発展にも貢献したためです。"
      },
      "image": "assets/people/person-2dab609327.webp",
      "imageAlt": "フローレンス・ナイチンゲールの画像"
    },
    "チャールズ・ダーウィン": {
      "name": "チャールズ・ダーウィン",
      "kana": "ちゃーるず・だーうぃん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "イギリス",
      "title": "進化論を示した科学者",
      "icon": "🔬",
      "modal": {
        "profile": "イギリスの博物学者で、測量船ビーグル号に乗って南米やガラパゴス諸島などの生物・化石・地層を観察しました。生物の個体差のうち環境に適した特徴をもつものが多く子孫を残し、その特徴が世代を重ねて広がる「自然選択」を進化の仕組みとして考えました。1859年の『種の起源』で発表し、生物は固定された存在ではなく共通祖先から変化したという見方を科学的研究の中心にしました。",
        "whatDid": "世界各地の生物と化石を調べ、自然選択によって生物が進化する仕組みを『種の起源』で示しました。",
        "whyImportant": "生物の多様性を自然の仕組みで説明し、現代生物学の基本となる進化論を確立したためです。"
      },
      "image": "assets/people/person-50808aa356.webp",
      "imageAlt": "チャールズ・ダーウィンの画像"
    },
    "マハトマ・ガンディー": {
      "name": "マハトマ・ガンディー",
      "kana": "まはとま・がんでぃー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "インド",
      "title": "非暴力で独立を求めた人",
      "icon": "🕊️",
      "modal": {
        "profile": "インド出身の弁護士で、南アフリカでインド系住民への差別に直面し、真理に基づく非暴力抵抗「サティヤーグラハ」を実践しました。帰国後、イギリス植民地支配に対し、不買運動、非協力運動、塩の行進を指導し、農民や都市住民を独立運動へ参加させました。ヒンドゥー教徒とイスラム教徒の融和を求めましたが、独立時にインドとパキスタンは分離し、1948年にヒンドゥー至上主義者に暗殺されました。",
        "whatDid": "非暴力・不服従を掲げ、非協力運動や塩の行進を通じてインド独立運動を大衆運動へ発展させました。",
        "whyImportant": "武力に頼らない抵抗で植民地支配へ対抗し、世界各地の公民権運動や人権運動に影響したためです。"
      },
      "image": "assets/people/person-ee0e1c6ce3.webp",
      "imageAlt": "マハトマ・ガンディーの画像"
    },
    "孫文": {
      "name": "孫文",
      "kana": "そんぶん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician"
      ],
      "field": "中国",
      "title": "近代中国の革命家",
      "icon": "📜",
      "modal": {
        "profile": "清末の中国に生まれ、医師となった後、外国支配と清朝の弱体化を批判して革命運動へ進みました。民族の独立、民権による政治、民生の安定を柱とする「三民主義」を掲げ、海外華僑から資金を集めて蜂起を支援しました。1911年の辛亥革命後、南京で中華民国臨時大総統となりましたが、袁世凱へ地位を譲りました。その後は国民党を再編し、中国統一を目指しましたが、実現前に病死しました。",
        "whatDid": "三民主義を掲げて清朝打倒の革命を進め、中華民国の臨時大総統となり、国民党を再編しました。",
        "whyImportant": "皇帝政治から共和政へ移る中国革命の理念と組織を築き、近代中国の国家構想に影響したためです。"
      },
      "image": "assets/people/person-5afa942243.webp",
      "imageAlt": "孫文の画像"
    },
    "アルベルト・アインシュタイン": {
      "name": "アルベルト・アインシュタイン",
      "kana": "あるべると・あいんしゅたいん",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science"
      ],
      "field": "ヨーロッパ・アメリカ",
      "title": "物理学を変えた科学者",
      "icon": "🔬",
      "modal": {
        "profile": "ドイツ生まれの理論物理学者で、1905年に特殊相対性理論を発表し、時間と空間が観測者の運動によって関係づけられることを示しました。質量とエネルギーの等価関係も導き、1915年には重力を時空のゆがみとして説明する一般相対性理論を完成させました。光電効果の研究でノーベル物理学賞を受賞しました。ナチス政権を逃れてアメリカへ移住し、戦後は核兵器管理と平和を訴えました。",
        "whatDid": "特殊・一般相対性理論を築き、光電効果を説明して、時間・空間・重力・光の物理学を変えました。",
        "whyImportant": "現代物理学の理論的基礎を築き、宇宙研究、原子力、精密測定など広い分野に影響したためです。"
      },
      "image": "assets/people/person-48f6387587.webp",
      "imageAlt": "アルベルト・アインシュタインの画像"
    },
    "ネルソン・マンデラ": {
      "name": "ネルソン・マンデラ",
      "kana": "ねるそん・まんでら",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "南アフリカ",
      "title": "差別とたたかった指導者",
      "icon": "🕊️",
      "modal": {
        "profile": "南アフリカの法律家で、アフリカ民族会議に加わり、人種隔離政策アパルトヘイトへ抵抗しました。平和的抗議が弾圧されると武装組織の創設にも関わり、1964年の裁判で終身刑となって27年間投獄されました。1990年の釈放後は白人政府との交渉を進め、報復ではなく多数派支配と少数派保護を両立する民主化を目指しました。1994年、全人種選挙で南アフリカ初の黒人大統領となりました。",
        "whatDid": "アパルトヘイトに抵抗して長期投獄され、釈放後は政府と交渉して全人種参加の民主選挙を実現しました。",
        "whyImportant": "人種隔離体制を終わらせ、内戦を避けながら民主的な南アフリカへ移行する中心的役割を果たしたためです。"
      },
      "image": "assets/people/person-1e015e3361.webp",
      "imageAlt": "ネルソン・マンデラの画像"
    },
    "マーティン・ルーサー・キング・ジュニア": {
      "name": "マーティン・ルーサー・キング・ジュニア",
      "kana": "まーてぃん・るーさー・きんぐ・じゅにあ",
      "era": "今につながる時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "アメリカ",
      "title": "公民権運動の指導者",
      "icon": "🕊️",
      "modal": {
        "profile": "アメリカ南部のバプテスト派牧師で、黒人が公共施設や選挙で差別される制度をなくす公民権運動を指導しました。モンゴメリー・バス・ボイコット、バーミングハム運動、ワシントン大行進などで、非暴力の抗議と演説を用いました。運動は1964年公民権法と1965年投票権法の成立を後押ししました。晩年は貧困やベトナム戦争も批判し、1968年に暗殺されました。",
        "whatDid": "非暴力の抗議運動と演説で人種隔離を批判し、公民権法・投票権法の成立を後押ししました。",
        "whyImportant": "法制度として残っていた人種差別を撤廃する運動を全国化し、世界の人権運動へ影響したためです。"
      },
      "image": "assets/people/person-191db0da82.webp",
      "imageAlt": "マーティン・ルーサー・キング・ジュニアの画像"
    },
    "ワンガリ・マータイ": {
      "name": "ワンガリ・マータイ",
      "kana": "わんがり・まーたい",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ケニア",
      "title": "木を植える環境運動家",
      "icon": "🌱",
      "modal": {
        "profile": "ケニアの生物学者で、東・中央アフリカの女性として初めて博士号を取得しました。農村女性が薪や水を得にくくなった原因に森林破壊があると考え、1977年にグリーンベルト運動を始めました。女性たちが地域で苗木を育て植樹する活動を、土壌保全、生活改善、女性の自立へ結びつけました。さらに森林の私物化や独裁政治にも反対し、環境保護・民主主義・平和を一体の問題として訴えました。",
        "whatDid": "グリーンベルト運動を創設し、女性による植樹を環境保全、生活改善、民主化運動へ発展させました。",
        "whyImportant": "環境問題を女性の権利・貧困・民主主義・平和と結びつけ、持続可能な開発の実践を示したためです。"
      },
      "image": "assets/people/person-57040e1942.webp",
      "imageAlt": "ワンガリ・マータイの画像"
    },
    "マララ・ユスフザイ": {
      "name": "マララ・ユスフザイ",
      "kana": "まらら・ゆすふざい",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "パキスタン",
      "title": "教育の権利を訴える人",
      "icon": "📚",
      "modal": {
        "profile": "パキスタン北西部スワート渓谷に生まれ、武装勢力タリバンが女子教育を禁じるなか、11歳からBBCのブログで学校へ通えない生活を伝えました。教育を受ける権利を公に訴えたため、2012年に通学バスで銃撃されましたが、治療後も活動を続けました。マララ基金を通じて各国の女子教育を支援し、2014年に17歳でノーベル平和賞を受賞しました。教育を基本的人権として訴える世界的象徴です。",
        "whatDid": "タリバンによる女子教育禁止を告発し、銃撃後もマララ基金を通じてすべての子どもの教育権を訴えました。",
        "whyImportant": "女子が学ぶ権利への弾圧を世界へ知らせ、教育を基本的人権として守る国際運動を強めたためです。"
      },
      "image": "assets/people/person-7ca7f530e8.webp",
      "imageAlt": "マララ・ユスフザイの画像"
    },
    "釈迦": {
      "name": "釈迦",
      "kana": "しゃか",
      "era": "古代の大国の時代",
      "genre": [
        "religion"
      ],
      "field": "インド",
      "title": "仏教を開いた人",
      "icon": "📿",
      "modal": {
        "profile": "紀元前5世紀ごろ、現在のネパール南部に近い地域の釈迦族の王子として生まれたと伝えられ、本名をゴータマ・シッダールタといいます。老い、病、死の苦しみに向き合うため出家し、極端な苦行を退けて瞑想を続け、悟りを開きました。欲望や執着が苦しみを生むこと、正しい見方と行いによって苦しみを離れられることを説きました。その教えをもとに仏教教団が成立し、アジア各地へ広がりました。",
        "whatDid": "出家して悟りを開き、苦しみの原因と、それを離れるための生き方を説いて仏教教団を始めました。",
        "whyImportant": "仏教の出発点となる教えを示し、アジアの宗教、思想、芸術、社会に長く影響したためです。"
      },
      "image": "assets/people/person-5efae04b1c.webp",
      "imageAlt": "釈迦の画像"
    },
    "ハンニバル": {
      "name": "ハンニバル",
      "kana": "はんにばる",
      "era": "古代の大国の時代",
      "genre": [
        "war"
      ],
      "field": "カルタゴ（北アフリカ）",
      "title": "ローマと戦った将軍",
      "icon": "⚔️",
      "modal": {
        "profile": "北アフリカの都市国家カルタゴの名門に生まれた将軍で、第二次ポエニ戦争ではスペインから軍を率い、象を伴ってピレネー山脈とアルプス山脈を越えてイタリアへ侵入しました。カンナエの戦いで包囲戦術を用いてローマ軍を壊滅させましたが、ローマ市を攻略できず、補給と援軍も不足しました。ローマ軍が北アフリカへ攻め込むと帰国し、ザマの戦いでスキピオに敗れました。",
        "whatDid": "軍象を伴ってアルプスを越え、カンナエの戦いで包囲戦術によりローマ軍を大破しました。",
        "whyImportant": "強大なローマを滅亡寸前まで追い込み、包囲戦術の代表例を残した古代屈指の将軍だからです。"
      },
      "image": "assets/people/person-f420e3f10a.webp",
      "imageAlt": "ハンニバルの画像"
    },
    "カエサル": {
      "name": "カエサル",
      "kana": "かえさる",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマ",
      "title": "ローマを率いた英雄",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家・軍人で、民衆派として勢力を伸ばし、ガリア遠征で現在のフランス周辺を征服しました。元老院から軍の解散を命じられるとルビコン川を渡って内戦を始め、政敵ポンペイウスを破りました。終身独裁官となり、暦の改革、植民市建設、属州民の登用などを進めましたが、王を目指すと疑われ、紀元前44年に元老院議員らに暗殺されました。死後、共和政はさらに崩壊へ向かいました。",
        "whatDid": "ガリアを征服し、内戦で政敵を破って終身独裁官となり、暦や属州統治を改革しました。",
        "whyImportant": "共和政の政治均衡を崩し、ローマが皇帝による支配へ移る決定的な転換を引き起こしたためです。"
      },
      "image": "assets/people/person-de50729232.webp",
      "imageAlt": "カエサルの画像"
    },
    "クレオパトラ": {
      "name": "クレオパトラ",
      "kana": "くれおぱとら",
      "era": "古代の大国の時代",
      "genre": [
        "monarch"
      ],
      "field": "エジプト",
      "title": "エジプト最後の女王",
      "icon": "👑",
      "modal": {
        "profile": "プトレマイオス朝エジプト最後の女王で、ギリシャ系王家に生まれながらエジプト語も用いたとされます。弟との王位争いで追放されましたが、ローマのカエサルの支援で復位しました。カエサル死後はアントニウスと同盟し、ローマの影響下でエジプトの独立を守ろうとしました。しかしオクタウィアヌスとのアクティウムの海戦に敗れ、自死しました。その後エジプトはローマの属州となりました。",
        "whatDid": "カエサルとアントニウスとの同盟を利用して王位とエジプトの独立を守ろうとしました。",
        "whyImportant": "ヘレニズム王国エジプトの終焉と、ローマが地中海世界を統一する過程を象徴する女王だからです。"
      },
      "image": "assets/people/person-c35825710f.webp",
      "imageAlt": "クレオパトラの画像"
    },
    "グーテンベルク": {
      "name": "グーテンベルク",
      "kana": "ぐーてんべるく",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture"
      ],
      "field": "ドイツ",
      "title": "活版印刷を広めた人",
      "icon": "📖",
      "modal": {
        "profile": "15世紀ドイツのマインツで活動した金属加工職人・印刷業者で、金属活字、油性インク、圧搾機を組み合わせ、活字を繰り返し使える印刷方式を実用化しました。1450年代には美しいラテン語聖書を大量に印刷し、手書き写本より速く同じ文章を複製できることを示しました。技術はヨーロッパ各地へ急速に広まり、本の価格を下げ、宗教改革、学問、科学、読み書きの普及を支えました。",
        "whatDid": "金属活字・油性インク・圧搾機を組み合わせた活版印刷を実用化し、聖書を大量印刷しました。",
        "whyImportant": "同じ情報を速く正確に大量配布できるようにし、宗教改革、科学、教育、出版文化を発展させたためです。"
      },
      "image": "assets/people/person-521067b1b1.webp",
      "imageAlt": "グーテンベルクの画像"
    },
    "レオナルド・ダ・ヴィンチ": {
      "name": "レオナルド・ダ・ヴィンチ",
      "kana": "れおなるど・だ・びんち",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture",
        "science"
      ],
      "field": "イタリア",
      "title": "ルネサンスの天才",
      "icon": "🎨",
      "modal": {
        "profile": "ルネサンス期イタリアの芸術家・技術者で、フィレンツェで絵画を学び、ミラノやローマ、フランスの宮廷で活動しました。『最後の晩餐』『モナ・リザ』では人体、光、空気遠近法を観察して写実的表現を深めました。同時に人体解剖、水流、機械、飛行などを多数の手稿に図解しました。多くの構想は当時実用化されませんでしたが、芸術と自然研究を観察によって結びつけたルネサンス的人間の代表です。",
        "whatDid": "『最後の晩餐』『モナ・リザ』を描き、人体・機械・水流・飛行を観察して多数の手稿に記録しました。",
        "whyImportant": "芸術と科学を分けず、観察と実験的発想によって人間と自然を探究したルネサンスの象徴だからです。"
      },
      "image": "assets/people/person-ad6e558560.webp",
      "imageAlt": "レオナルド・ダ・ヴィンチの画像"
    },
    "ミケランジェロ": {
      "name": "ミケランジェロ",
      "kana": "みけらんじぇろ",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture"
      ],
      "field": "イタリア",
      "title": "ダビデ像を作った芸術家",
      "icon": "🎨",
      "modal": {
        "profile": "イタリア・ルネサンスを代表する彫刻家・画家・建築家です。若いころから人体の骨格や筋肉を観察し、大理石から「ダビデ像」や「ピエタ」を生み出しました。教皇の命でシスティナ礼拝堂の天井に旧約聖書の場面を描き、晩年にはサン・ピエトロ大聖堂の設計にも携わりました。人間の肉体と感情を力強く表し、古代文化を学び直したルネサンスの理想を後世に伝えました。",
        "whatDid": "「ダビデ像」やシスティナ礼拝堂天井画を制作し、彫刻・絵画・建築でルネサンスを代表しました。",
        "whyImportant": "人体と感情を壮大に表した作品が、西洋美術の表現と芸術家の地位を大きく高めたためです。"
      },
      "image": "assets/people/person-1141366ba2.webp",
      "imageAlt": "ミケランジェロの画像"
    },
    "ラファエロ": {
      "name": "ラファエロ",
      "kana": "らふぁえろ",
      "era": "大航海と出会いの時代",
      "genre": [
        "other"
      ],
      "field": "イタリア",
      "title": "ルネサンスの三大巨匠の一人",
      "icon": "🎨",
      "modal": {
        "profile": "イタリア・ルネサンス盛期の画家で、調和の取れた人物配置と穏やかな表情で知られます。フィレンツェでレオナルドやミケランジェロの表現を学び、のちに教皇ユリウス2世に招かれてバチカン宮殿の壁画を制作しました。「アテネの学堂」では古代ギリシャの哲学者たちを一つの空間に描き、古典文化とキリスト教世界を結びつけました。短い生涯ながら、その均整の取れた作風は長く模範とされました。",
        "whatDid": "バチカン宮殿に「アテネの学堂」などを描き、調和と均整を重んじる絵画様式を完成させました。",
        "whyImportant": "古典文化とキリスト教文化を結ぶ構成が、ルネサンス美術の理想的な形として受け継がれたためです。"
      },
      "image": "assets/people/person-0011a7b938.webp",
      "imageAlt": "ラファエロの画像"
    },
    "マルティン・ルター": {
      "name": "マルティン・ルター",
      "kana": "まるてぃん・るたー",
      "era": "大航海と出会いの時代",
      "genre": [
        "religion"
      ],
      "field": "ドイツ",
      "title": "宗教改革を始めた人",
      "icon": "📖",
      "modal": {
        "profile": "神聖ローマ帝国の修道士・神学者で、教会が販売した贖宥状に反対して宗教改革を始めました。1517年に「九十五か条の論題」を示し、人は善行や金銭ではなく信仰によって救われると主張しました。教皇から破門された後も、聖書をドイツ語に訳して一般の人々が読めるようにしました。その運動からルター派が成立し、西ヨーロッパの宗教的統一が崩れる大きな転機となりました。",
        "whatDid": "贖宥状を批判して宗教改革を起こし、聖書をドイツ語に訳して信仰の考え方を広めました。",
        "whyImportant": "カトリック教会の権威を揺るがし、プロテスタント諸派の成立と近代ヨーロッパの形成を促したためです。"
      },
      "image": "assets/people/person-b1751a213c.webp",
      "imageAlt": "マルティン・ルターの画像"
    },
    "カルヴァン": {
      "name": "カルヴァン",
      "kana": "かるばん",
      "era": "大航海と出会いの時代",
      "genre": [
        "religion"
      ],
      "field": "スイス・フランス",
      "title": "宗教改革を進めた人",
      "icon": "📖",
      "modal": {
        "profile": "フランス生まれの宗教改革者で、迫害を逃れてスイスのジュネーヴで活動しました。著書『キリスト教綱要』で、救われる者は神によってあらかじめ定められているという予定説を体系化しました。市政と教会を結びつけ、質素な生活と厳しい道徳を信者に求めました。その教えはフランスのユグノー、イングランドのピューリタン、オランダの改革派へ広がり、各地の政治や社会にも影響しました。",
        "whatDid": "予定説を中心に宗教改革の教えを体系化し、ジュネーヴを拠点に改革派教会を広げました。",
        "whyImportant": "カルヴァン派が各国へ広がり、宗教対立だけでなく市民社会や政治の形成にも影響したためです。"
      },
      "image": "assets/people/person-575516c8d8.webp",
      "imageAlt": "カルヴァンの画像"
    },
    "コペルニクス": {
      "name": "コペルニクス",
      "kana": "こぺるにくす",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture",
        "science"
      ],
      "field": "ポーランド",
      "title": "地動説を唱えた学者",
      "icon": "🔭",
      "modal": {
        "profile": "ポーランド王国出身の聖職者・天文学者で、天体観測と古代の学説を検討し、地球が太陽の周りを回る地動説を唱えました。当時広く信じられていた天動説では惑星の複雑な動きを説明しにくいと考え、数学的により整った宇宙像を示しました。主著『天球の回転について』は死の直前に刊行されました。この考えはすぐには受け入れられませんでしたが、近代天文学と科学革命の出発点になりました。",
        "whatDid": "地球を含む惑星が太陽の周りを回る地動説を、数学的な宇宙モデルとして示しました。",
        "whyImportant": "宇宙の中心を地球とする従来の常識を問い直し、観測と計算にもとづく近代天文学を開いたためです。"
      },
      "image": "assets/people/person-e08a16a58c.webp",
      "imageAlt": "コペルニクスの画像"
    },
    "ガリレオ・ガリレイ": {
      "name": "ガリレオ・ガリレイ",
      "kana": "がりれお・がりれい",
      "era": "大航海と出会いの時代",
      "genre": [
        "science"
      ],
      "field": "イタリア",
      "title": "近代科学の父",
      "icon": "🔭",
      "modal": {
        "profile": "イタリアの物理学者・天文学者で、望遠鏡を改良して月の表面、木星の衛星、金星の満ち欠けなどを観測しました。これらは、すべての天体が地球の周りを回るという天動説では説明しにくく、地動説を支える証拠になりました。また、落下運動を実験と数学で研究しました。教会の裁判で地動説を捨てるよう命じられましたが、観測・実験・計算を組み合わせる研究方法は近代科学の基礎となりました。",
        "whatDid": "望遠鏡で木星の衛星や金星の満ち欠けを観測し、実験と数学によって地動説を支えました。",
        "whyImportant": "権威ある学説より観測と実験を重視する姿勢を示し、近代科学の研究方法を形づくったためです。"
      },
      "image": "assets/people/person-ee0d81f4a1.webp",
      "imageAlt": "ガリレオ・ガリレイの画像"
    },
    "ニュートン": {
      "name": "ニュートン",
      "kana": "にゅーとん",
      "era": "大航海と出会いの時代",
      "genre": [
        "science"
      ],
      "field": "イギリス",
      "title": "万有引力を発見した科学者",
      "icon": "🔬",
      "modal": {
        "profile": "イングランドの数学者・物理学者で、地上の物体の落下と月や惑星の運動を同じ法則で説明しました。主著『プリンキピア』で運動の三法則と万有引力の法則を示し、自然界の動きを数学で予測できる体系を築きました。光をプリズムに通して白色光が多くの色から成ることも確かめ、微積分の発展にも貢献しました。宇宙と地上を統一的に理解する考えは、その後の科学と技術の基準になりました。",
        "whatDid": "運動の三法則と万有引力の法則を示し、天体と地上の運動を数学で統一的に説明しました。",
        "whyImportant": "自然現象を数式で予測する近代物理学の体系を築き、科学と技術の発展の土台を作ったためです。"
      },
      "image": "assets/people/person-4ae32d11c2.webp",
      "imageAlt": "ニュートンの画像"
    },
    "デカルト": {
      "name": "デカルト",
      "kana": "でかると",
      "era": "大航海と出会いの時代",
      "genre": [
        "philosopher",
        "science"
      ],
      "field": "フランス",
      "title": "近代哲学を切り開いた人",
      "icon": "🧠",
      "modal": {
        "profile": "フランスの哲学者・数学者で、確実な知識を得るため、疑うことのできるものをすべて疑う方法的懐疑を用いました。その結果、疑っている自分の存在は確実だとして「我思う、ゆえに我あり」と表しました。著書『方法序説』では理性を使い、問題を順序立てて考える方法を説きました。また、座標を用いて図形と方程式を結びつけ、解析幾何学を発展させました。近代哲学と数学の双方に新しい出発点を与えました。",
        "whatDid": "方法的懐疑から「我思う、ゆえに我あり」を示し、理性にもとづく近代哲学を始めました。",
        "whyImportant": "伝統や権威ではなく、自分の理性から確実な知識を築く近代的な思考法を示したためです。"
      },
      "image": "assets/people/person-638347cdb2.webp",
      "imageAlt": "デカルトの画像"
    },
    "スピノザ": {
      "name": "スピノザ",
      "kana": "すぴのざ",
      "era": "大航海と出会いの時代",
      "genre": [
        "philosopher"
      ],
      "field": "オランダ",
      "title": "自然と神を考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "オランダのユダヤ人共同体に生まれた哲学者で、伝統的な宗教解釈に疑問を示したため共同体から追放されました。神と自然を別のものとせず、世界全体を一つの実在としてとらえる考えを『エチカ』で論じました。また、聖書を歴史的な文書として読み、思想や言論の自由を守る政治を主張しました。生前は危険な思想家と見なされましたが、理性・寛容・自由を重んじる近代思想の先駆者として再評価されました。",
        "whatDid": "神と自然を一体として考える哲学を築き、聖書の歴史的研究と言論の自由を主張しました。",
        "whyImportant": "宗教的権威から独立して考える自由と寛容を説き、近代の民主的思想へ道を開いたためです。"
      },
      "image": "assets/people/person-e66d9b1378.webp",
      "imageAlt": "スピノザの画像"
    },
    "ライプニッツ": {
      "name": "ライプニッツ",
      "kana": "らいぷにっつ",
      "era": "大航海と出会いの時代",
      "genre": [
        "philosopher",
        "science"
      ],
      "field": "ドイツ",
      "title": "論理と数学を考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者・数学者で、外交官や図書館員として働きながら幅広い研究を進めました。ニュートンとは別に微積分を考案し、現在も使われる記号を整えました。計算機の改良や二進法の研究にも取り組み、複雑な思考を記号と計算で扱う構想を持ちました。哲学では、世界は「モナド」と呼ぶ精神的な単位から成り、神が調和を与えたと説明しました。数学、論理学、情報科学につながる発想を残しました。",
        "whatDid": "微積分の記号体系を整え、二進法や計算機を研究して、数学と論理学の発展を進めました。",
        "whyImportant": "記号によって計算と思考を扱う発想が、近代数学だけでなくコンピューター科学にもつながったためです。"
      },
      "image": "assets/people/person-161894fd4d.webp",
      "imageAlt": "ライプニッツの画像"
    },
    "カント": {
      "name": "カント",
      "kana": "かんと",
      "era": "革命と工場の時代",
      "genre": [
        "political-scholar",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "人間の理性を考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "プロイセン王国の哲学者で、人間は世界をそのまま知るのではなく、時間・空間・因果関係など自らの認識の枠組みを通して理解すると論じました。『純粋理性批判』で経験論と合理論を結びつけ、知識が成り立つ条件を検討しました。倫理では、利益や感情ではなく、誰にでも当てはまる道徳法則に従って行動すべきだと説きました。理性の可能性と限界を示し、近代哲学の流れを大きく変えました。",
        "whatDid": "人間の認識が成り立つ条件を分析し、普遍的な道徳法則にもとづく倫理学を築きました。",
        "whyImportant": "知識・道徳・自由を理性から考え直し、その後の哲学や政治思想の出発点となったためです。"
      },
      "image": "assets/people/person-3d930c4d52.webp",
      "imageAlt": "カントの画像"
    },
    "シラー": {
      "name": "シラー",
      "kana": "しらー",
      "era": "革命と工場の時代",
      "genre": [
        "culture",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "自由と美を考えた詩人",
      "icon": "🎭",
      "modal": {
        "profile": "ドイツの詩人・劇作家・思想家で、若いころに公国の厳しい支配を批判する戯曲『群盗』を書きました。歴史劇『ヴィルヘルム・テル』などで、自由を求める人間と圧政への抵抗を描きました。ゲーテと交流し、感情だけに流されず理性と美を調和させる古典主義文学を発展させました。詩「歓喜に寄す」は後にベートーヴェンの交響曲第9番に用いられ、自由と人類の連帯を象徴する作品となりました。",
        "whatDid": "『群盗』『ヴィルヘルム・テル』などを書き、自由・尊厳・圧政への抵抗を文学で表しました。",
        "whyImportant": "文学と美を通して人間の自由を問い、ドイツ古典主義と近代の市民意識を形づくったためです。"
      },
      "image": "assets/people/person-deca27f0a0.webp",
      "imageAlt": "シラーの画像"
    },
    "ヘーゲル": {
      "name": "ヘーゲル",
      "kana": "へーげる",
      "era": "革命と工場の時代",
      "genre": [
        "political-scholar",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "歴史の動きを考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者で、歴史を偶然な出来事の集まりではなく、人間が自由を自覚していく発展の過程として考えました。ある考えがその矛盾や反対を生み、それを乗り越えることで新しい段階へ進むという弁証法を用いて、社会・国家・宗教・芸術を体系的に説明しました。フランス革命を自由の前進として評価する一方、自由は法律と国家の中で実現すると考えました。その思想はマルクスを含む多くの思想家に影響しました。",
        "whatDid": "弁証法を用いて、歴史を人間が自由を自覚していく発展過程として体系的に説明しました。",
        "whyImportant": "社会の矛盾と変化を歴史の動きとして捉える考えが、後の哲学・政治思想・歴史学に影響したためです。"
      },
      "image": "assets/people/person-23a2f61d69.webp",
      "imageAlt": "ヘーゲルの画像"
    },
    "ショーペンハウアー": {
      "name": "ショーペンハウアー",
      "kana": "しょーぺんはうあー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "生きる苦しみを考えた哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者で、世界の根底には理性ではなく、満たされることのない盲目的な「意志」があると考えました。人間は欲望を満たしても新たな欲望に苦しむため、人生には苦しみが絶えないと『意志と表象としての世界』で論じました。一方で、芸術に没頭すること、他者への同情を持つこと、欲望への執着を弱めることに救いを求めました。インド思想や仏教にも関心を持ち、近代ヨーロッパの悲観主義を代表しました。",
        "whatDid": "世界を盲目的な「意志」の現れと考え、欲望と苦しみの関係を哲学として体系化しました。",
        "whyImportant": "理性中心の哲学に異議を唱え、人間の欲望・苦悩・無意識を考える後代の思想へ影響したためです。"
      },
      "image": "assets/people/person-54ea3251de.webp",
      "imageAlt": "ショーペンハウアーの画像"
    },
    "ニーチェ": {
      "name": "ニーチェ",
      "kana": "にーちぇ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture",
        "philosopher"
      ],
      "field": "ドイツ",
      "title": "価値を問い直した哲学者",
      "icon": "🧠",
      "modal": {
        "profile": "ドイツの哲学者で、キリスト教道徳や当時の社会が当然としていた価値を根本から問い直しました。「神は死んだ」という言葉で、宗教が人々の価値観を支える力を失いつつある時代を表しました。既成の善悪に従うだけでなく、自ら価値を生み出して生きる人間を「超人」と呼びました。著作は断片的で多様な解釈を生み、のちに政治的に悪用された面もありますが、実存主義や文学、心理学へ強い影響を与えました。",
        "whatDid": "宗教や道徳の既成価値を批判し、人間が自ら新しい価値を創り出す生き方を問いかけました。",
        "whyImportant": "近代社会で価値の基準が揺らぐ問題を示し、哲学・文学・心理学に大きな論争を起こしたためです。"
      },
      "image": "assets/people/person-3dcf243a52.webp",
      "imageAlt": "ニーチェの画像"
    },
    "シェイクスピア": {
      "name": "シェイクスピア",
      "kana": "しぇいくすぴあ",
      "era": "大航海と出会いの時代",
      "genre": [
        "culture"
      ],
      "field": "イギリス",
      "title": "イギリスの大劇作家",
      "icon": "🎭",
      "modal": {
        "profile": "イングランドの劇作家・詩人で、ロンドンの劇場を中心に喜劇、悲劇、歴史劇を数多く書きました。『ハムレット』『マクベス』『ロミオとジュリエット』では、愛、野心、復讐、迷いなど人間の複雑な心を、身分や時代をこえて伝わる物語として描きました。また、イングランド史を題材に王権や政治の争いも表現しました。豊かな比喩と言葉づかいは英語そのものに影響し、世界各地で上演され続けています。",
        "whatDid": "悲劇・喜劇・歴史劇を通して、人間の愛・野心・迷い・権力争いを多面的に描きました。",
        "whyImportant": "人間心理を深く描いた作品が言語や文化を越えて読まれ、近代演劇と英文学の基準となったためです。"
      },
      "image": "assets/people/person-bc57242847.webp",
      "imageAlt": "シェイクスピアの画像"
    },
    "バッハ": {
      "name": "バッハ",
      "kana": "ばっは",
      "era": "革命と工場の時代",
      "genre": [
        "other"
      ],
      "field": "ドイツ",
      "title": "音楽の父",
      "icon": "🎵",
      "modal": {
        "profile": "ドイツの作曲家・オルガン奏者で、教会や宮廷に仕えながら宗教曲、器楽曲、鍵盤曲を数多く作りました。複数の旋律を同時に組み合わせる対位法を高度に発展させ、『平均律クラヴィーア曲集』『マタイ受難曲』『ブランデンブルク協奏曲』などを残しました。生前は主に演奏家として知られましたが、死後に作品が再評価されました。西洋音楽の技法を集大成し、後の作曲家が学ぶ基礎を築きました。",
        "whatDid": "対位法を高度に発展させ、宗教曲・協奏曲・鍵盤曲でバロック音楽を集大成しました。",
        "whyImportant": "旋律と和声を組み立てる技法が、西洋音楽の作曲教育と後代の作品の基礎になったためです。"
      },
      "image": "assets/people/person-09dac962cb.webp",
      "imageAlt": "バッハの画像"
    },
    "モーツァルト": {
      "name": "モーツァルト",
      "kana": "もーつぁると",
      "era": "革命と工場の時代",
      "genre": [
        "other"
      ],
      "field": "オーストリア",
      "title": "天才作曲家",
      "icon": "🎵",
      "modal": {
        "profile": "オーストリアの作曲家で、幼いころからヨーロッパ各地で演奏し、宮廷音楽から市民向けの演奏会まで幅広く活動しました。交響曲、協奏曲、室内楽、宗教曲、オペラのすべてで作品を残し、『フィガロの結婚』『ドン・ジョヴァンニ』『魔笛』では音楽によって人物の性格や社会の対立を生き生きと表しました。宮廷に完全には依存しない作曲家を目指しましたが、生活は安定せず、35歳で亡くなりました。",
        "whatDid": "交響曲・協奏曲・オペラなど多くの分野で、古典派音楽を完成度の高い作品へ発展させました。",
        "whyImportant": "人間の感情と劇を音楽で巧みに表し、古典派の形式を後世の作曲家が学ぶ模範にしたためです。"
      },
      "image": "assets/people/person-a84fa933f1.webp",
      "imageAlt": "モーツァルトの画像"
    },
    "ベートーヴェン": {
      "name": "ベートーヴェン",
      "kana": "べーとーべん",
      "era": "革命と工場の時代",
      "genre": [
        "other"
      ],
      "field": "ドイツ",
      "title": "楽聖と呼ばれた作曲家",
      "icon": "🎵",
      "modal": {
        "profile": "ドイツ生まれでウィーンを中心に活動した作曲家です。ハイドンらの古典派音楽を学びながら、交響曲やソナタの規模と表現を広げ、個人の苦悩、自由、勝利への意志を強く打ち出しました。聴力を失っていくなかでも作曲を続け、交響曲第3番「英雄」、第5番、第9番などを完成させました。宮廷の注文だけでなく芸術家自身の理念を作品に込め、古典派からロマン派へ向かう道を開きました。",
        "whatDid": "聴力を失いながら交響曲やソナタを革新し、個人の感情と理念を大規模な音楽で表しました。",
        "whyImportant": "作曲家を宮廷の職人から自立した芸術家へ近づけ、ロマン派音楽への道を開いたためです。"
      },
      "image": "assets/people/person-4ab47aade7.webp",
      "imageAlt": "ベートーヴェンの画像"
    },
    "ノーベル": {
      "name": "ノーベル",
      "kana": "のーべる",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "スウェーデン",
      "title": "ダイナマイトの発明者",
      "icon": "🏅",
      "modal": {
        "profile": "スウェーデンの化学者・発明家・実業家で、ニトログリセリンを安全に扱いやすくしたダイナマイトを発明しました。爆薬はトンネルや鉱山の工事を進める一方、兵器にも利用され、彼は巨額の財産を築きました。自分の死を伝える誤報で「死の商人」と評されたことなどをきっかけに、財産を人類に貢献した人へ贈る賞に使う遺言を残しました。その遺産から物理学、化学、医学、文学、平和のノーベル賞が始まりました。",
        "whatDid": "ダイナマイトを発明して産業用爆薬を実用化し、遺産を人類貢献者へ贈る賞の創設に残しました。",
        "whyImportant": "発明がもたらす利益と危険の両面を示し、科学・文化・平和への貢献を顕彰する制度を残したためです。"
      },
      "image": "assets/people/person-f14b621d96.webp",
      "imageAlt": "ノーベルの画像"
    },
    "レントゲン": {
      "name": "レントゲン",
      "kana": "れんとげん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "ドイツ",
      "title": "エックス線を発見した科学者",
      "icon": "🔬",
      "modal": {
        "profile": "ドイツの物理学者で、真空放電の実験中に、物体を通り抜けて写真乾板を感光させる未知の放射線を発見しました。正体が分からなかったため「X線」と名づけ、妻の手を撮影して骨と指輪が写ることを示しました。この発見により、体を切り開かずに骨折や体内の異常を調べられるようになりました。特許を取らず広く利用できるようにし、1901年に第1回ノーベル物理学賞を受賞しました。",
        "whatDid": "物体を通過して骨などを写し出すX線を発見し、医療で体内を調べる新しい方法を開きました。",
        "whyImportant": "手術をせず体内を画像で確認できるようにし、診断医学と物理学の利用を大きく変えたためです。"
      },
      "image": "assets/people/person-95977db14a.webp",
      "imageAlt": "レントゲンの画像"
    },
    "アムンゼン": {
      "name": "アムンゼン",
      "kana": "あむんぜん",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "ノルウェー",
      "title": "南極点に初到達した探検家",
      "icon": "🧭",
      "modal": {
        "profile": "ノルウェーの探検家で、寒冷地に暮らす人々の技術を学び、犬ぞりや毛皮の衣服を取り入れて極地探検を進めました。北西航路を初めて船で通過した後、南極点到達を目標に変更し、1911年12月にイギリスのスコット隊より先に到達しました。食料配置や移動計画を細かく準備し、全員が生還しました。その後は飛行船で北極圏も探検し、遭難者の捜索中に消息を絶ちました。",
        "whatDid": "犬ぞりと周到な補給計画を用い、1911年に人類で初めて南極点へ到達しました。",
        "whyImportant": "極地の環境に適した技術と計画の重要性を示し、地理的探検の歴史に大きな記録を残したためです。"
      },
      "image": "assets/people/person-f2de242edd.webp",
      "imageAlt": "アムンゼンの画像"
    },
    "ペリー": {
      "name": "ペリー",
      "kana": "ぺりー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "アメリカ",
      "title": "日本に開国を求めた人",
      "icon": "⚓",
      "modal": {
        "profile": "アメリカ海軍の軍人で、東インド艦隊司令長官として日本へ派遣されました。1853年、蒸気船を含む艦隊で浦賀に来航し、アメリカ大統領の国書を幕府へ渡して開国を求めました。翌年に再来航し、日米和親条約を結ばせ、下田と箱館を開港させました。この交渉は軍事力を背景としたもので、日本の鎖国体制を終わらせるきっかけとなり、幕府の権威低下や明治維新へつながる政治変動を促しました。",
        "whatDid": "艦隊を率いて日本に開国を要求し、1854年の日米和親条約締結へ導きました。",
        "whyImportant": "日本の鎖国体制を揺るがし、幕末の政治対立と明治維新へ向かう変化を引き起こしたためです。"
      },
      "image": "assets/people/person-f3d03b470d.webp",
      "imageAlt": "ペリーの画像"
    },
    "ジョン・F・ケネディ": {
      "name": "ジョン・F・ケネディ",
      "kana": "じょん・えふ・けねでぃ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "アメリカ",
      "title": "冷戦期のアメリカ大統領",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ合衆国第35代大統領で、冷戦が緊張する1961年に就任しました。キューバ危機ではソ連の核ミサイル配備に海上封鎖で対抗しつつ交渉を続け、核戦争を回避しました。宇宙開発では人間を月へ送る目標を掲げ、公民権問題では人種差別を禁じる法案を提案しました。一方、ピッグス湾侵攻の失敗やベトナムへの関与拡大もありました。1963年に暗殺され、政策の多くは後継政権へ引き継がれました。",
        "whatDid": "キューバ危機で核戦争を回避し、月面着陸計画と公民権法案を進めました。",
        "whyImportant": "冷戦の最危機を交渉で収める一方、宇宙開発と人種差別撤廃を国家課題として示したためです。"
      },
      "image": "assets/people/person-a4b37a4032.webp",
      "imageAlt": "ジョン・F・ケネディの画像"
    },
    "ゴルバチョフ": {
      "name": "ゴルバチョフ",
      "kana": "ごるばちょふ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ソ連・ロシア",
      "title": "冷戦を終わらせた指導者",
      "icon": "🕊️",
      "modal": {
        "profile": "ソビエト連邦最後の最高指導者で、経済停滞と社会の閉塞を立て直すため、ペレストロイカと呼ばれる改革と、情報公開を進めるグラスノスチを始めました。アメリカとの核軍縮交渉を進め、東ヨーロッパ諸国の民主化を武力で抑え込まなかったため、冷戦終結へ大きく貢献しました。しかし改革は経済混乱や民族対立も広げ、1991年にソ連は解体しました。平和的変化を進めた一方、国内では評価が分かれる人物です。",
        "whatDid": "ペレストロイカとグラスノスチを進め、核軍縮と東欧の民主化を通じて冷戦終結を促しました。",
        "whyImportant": "大国間対立を緩和して冷戦を終わらせた一方、改革がソ連解体へつながる転機にもなったためです。"
      },
      "image": "assets/people/person-5bdbdabb4c.webp",
      "imageAlt": "ゴルバチョフの画像"
    },
    "鄧小平": {
      "name": "鄧小平",
      "kana": "とうしょうへい",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "中国",
      "title": "改革開放を進めた指導者",
      "icon": "👤",
      "modal": {
        "profile": "四川省の地主の家に生まれ、フランス留学と中国共産党での活動を経て指導部に入りました。文化大革命で失脚しましたが、毛沢東の死後に復権し、1978年以降は人民公社の解体、経済特区の設置、外国資本の導入を進めました。これらの「改革開放」は中国経済を急成長させる一方、1989年の天安門事件では民主化運動を武力で抑え、政治改革を制限しました。",
        "whatDid": "人民公社の解体、経済特区の設置、外国資本の導入を進め、中国を改革開放へ転換しました。",
        "whyImportant": "社会主義体制を保ちながら市場経済を取り入れ、現在の中国の経済発展の方向を決めたためです。"
      },
      "image": "assets/people/person-92d4a96fa6.webp",
      "imageAlt": "鄧小平の画像"
    },
    "ナルメル": {
      "name": "ナルメル",
      "kana": "ナルメル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前3100年ごろのエジプト王で、上エジプトと下エジプトの統一に関わった人物と考えられています。王冠の異なる二つの地域を支配する姿を刻んだ「ナルメルのパレット」は、王が軍事・宗教・儀礼を通して統一を示した重要な資料です。ただし、統一が一度の征服で完成したのか、後世のメネスと同一人物なのかは確定しておらず、考古資料から初期王権を考える中心人物です。",
        "whatDid": "上エジプトと下エジプトを統合する王権を築き、その支配を儀礼的な図像で示しました。",
        "whyImportant": "エジプト統一王国とファラオの支配が成立する過程を示す最古級の資料と結び付くためです。"
      },
      "image": "assets/people/person-f462836a0c.webp",
      "imageAlt": "ナルメルの画像"
    },
    "サルゴン": {
      "name": "サルゴン",
      "kana": "サルゴン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前24世紀ごろ、アッカドを拠点にメソポタミアのシュメール都市国家を征服した王です。各都市に自らの支配を受け入れる役人を置き、軍事力と行政によって広い地域をまとめました。娘エンヘドゥアンナをウルの最高祭司に任じ、宗教も王権の統合に利用しました。後世には低い身分から王になった英雄として伝説化され、複数の都市を越える初期帝国を築いた人物として記憶されています。",
        "whatDid": "シュメールの都市国家を征服し、軍隊・役人・宗教を用いてアッカド帝国を築きました。",
        "whyImportant": "一つの都市を越えてメソポタミアを広域支配する帝国の先例をつくったためです。"
      },
      "image": "assets/people/person-1679093f0c.webp",
      "imageAlt": "サルゴンの画像"
    },
    "ソロン": {
      "name": "ソロン",
      "kana": "ソロン",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前6世紀初めのアテネで、貴族と平民の対立を調整するために選ばれた政治家・詩人です。借金を理由に市民が奴隷となる慣行を廃止し、負債を軽減する改革を行いました。また、財産の額に応じて市民を区分し、貴族以外にも政治参加の道を広げました。改革は対立を完全には解決しませんでしたが、血筋だけで政治を独占する仕組みを崩し、後のアテネ民主政につながる基礎を築きました。",
        "whatDid": "債務奴隷を廃止し、財産に応じた政治参加の制度を整えて貴族の独占を弱めました。",
        "whyImportant": "アテネで身分より財産を基準とする政治参加を広げ、民主政への道を開いたためです。"
      },
      "image": "assets/people/person-0c00fa8a54.webp"
    },
    "スキピオ・アフリカヌス": {
      "name": "スキピオ・アフリカヌス",
      "kana": "スキピオ・アフリカヌス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマの将軍で、第二次ポエニ戦争でハンニバルと戦いました。カルタゴ軍の補給地だったイベリア半島を攻略した後、戦場を北アフリカへ移し、紀元前202年のザマの戦いでハンニバルを破りました。この勝利によってカルタゴは軍事力と領土を大きく失い、ローマは西地中海の主導権を握りました。敵の戦術を学びながら戦場を移す判断で、戦争の流れを逆転させた将軍として知られます。",
        "whatDid": "イベリア半島を攻略してカルタゴの基盤を崩し、ザマの戦いでハンニバルを破りました。",
        "whyImportant": "ローマがカルタゴを退け、西地中海の覇権を握る決定的な勝利をもたらしたためです。"
      },
      "image": "assets/people/person-cfb119e804.webp"
    },
    "キュロス2世": {
      "name": "キュロス2世",
      "kana": "キュロス2世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前6世紀にアケメネス朝ペルシャを開いた王で、メディア、リディア、新バビロニアを征服して西アジアの広い地域を支配しました。紀元前539年にバビロンへ入城した後、追放されていた人々の帰還や神殿の再建を認め、地域の宗教や慣習を利用して統治しました。キュロスの円筒碑文は当時の王の宣伝文書でもあり、近代的な人権宣言と同一視はできませんが、征服地を柔軟に治めた王として後世に理想化されました。",
        "whatDid": "メディア・リディア・新バビロニアを征服し、地域の宗教や慣習を認めながら帝国を築きました。",
        "whyImportant": "アケメネス朝の基礎を築き、多民族・多宗教の広大な帝国を統治する先例を示したためです。"
      },
      "image": "assets/people/person-4ceade843b.webp",
      "imageAlt": "キュロス2世の画像"
    },
    "パウロ": {
      "name": "パウロ",
      "kana": "パウロ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "小アジアのタルソスに生まれたユダヤ人で、当初はイエスを信じる人々を迫害しましたが、回心後は地中海各地を旅して宣教しました。ユダヤ人以外も信仰によって救われると説き、アンティオキア、コリント、エフェソスなどに教会共同体を広げました。各地の信者へ送った手紙は新約聖書に収められ、イエスの死と復活の意味、信仰、共同体のあり方を説明する初期キリスト教の重要な土台になりました。",
        "whatDid": "地中海各地で非ユダヤ人にも宣教し、手紙によって初期キリスト教の教えを整理しました。",
        "whyImportant": "キリスト教をユダヤ社会の外へ広げ、世界宗教へ発展する方向を強く形づくったためです。"
      },
      "image": "assets/people/person-7407a9e24f.webp",
      "imageAlt": "パウロの画像"
    },
    "太宗": {
      "name": "太宗",
      "kana": "太宗",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の二代皇帝で、父の高祖を助けて王朝の建国に功績を立て、626年の玄武門の変を経て即位しました。律令や科挙を整え、魏徴ら臣下の意見を聞く政治を行った時代は「貞観の治」と呼ばれます。東突厥を破って中央アジアへ勢力を広げ、都の長安には各地の商人・使節・宗教が集まりました。一方、高句麗遠征は十分な成果を上げられず、強い帝国を築く過程には長い戦争も伴いました。",
        "whatDid": "律令・科挙を整え、臣下の意見を取り入れながら唐の支配を安定させ、領域を広げました。",
        "whyImportant": "唐を東アジアと中央アジアに影響を及ぼす国際帝国へ成長させたためです。"
      },
      "image": "assets/people/person-64ef03b0e4.webp"
    },
    "ユスティニアヌス": {
      "name": "ユスティニアヌス",
      "kana": "ユスティニアヌス",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "6世紀の東ローマ帝国皇帝で、かつてのローマ帝国領を取り戻すため、将軍ベリサリウスらを北アフリカやイタリアへ送りました。法学者に命じて複雑なローマ法を整理し、『ローマ法大全』として編さんしました。ニカの反乱後には首都コンスタンティノープルを再建し、ハギア・ソフィア聖堂を完成させました。征服戦争と建築は帝国の威信を高めましたが、重い税と長い戦争は国力を消耗させました。",
        "whatDid": "ローマ法大全を編さんし、ハギア・ソフィアを建設して、西方領土の再征服を進めました。",
        "whyImportant": "ローマ法を体系化して後世の法学に残し、東ローマ帝国の文化と皇帝権を象徴したためです。"
      },
      "image": "assets/people/person-eb7674eb9e.webp",
      "imageAlt": "ユスティニアヌスの画像"
    },
    "エンリケ航海王子": {
      "name": "エンリケ航海王子",
      "kana": "エンリケ航海王子",
      "era": "大航海と出会いの時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "15世紀のポルトガル王子で、1415年の北アフリカの港セウタ攻略に参加した後、西アフリカ沿岸への航海を支援しました。自ら長距離航海を率いた航海者ではありませんが、船と資金を用意し、航海者たちはボジャドール岬を越えてアフリカ西岸を南下しました。その活動は金・香辛料・キリスト教国を求める海外進出を進める一方、アフリカ人の捕獲と売買を伴い、大西洋奴隷貿易の始まりにも結び付きました。",
        "whatDid": "西アフリカ沿岸への航海を資金面で支援し、ポルトガルの大西洋進出を進めました。",
        "whyImportant": "大航海時代の海路開拓を促すと同時に、大西洋奴隷貿易の拡大にも道を開いたためです。"
      },
      "image": "assets/people/person-17ac22d8f8.webp"
    },
    "ジョン・ロック": {
      "name": "ジョン・ロック",
      "kana": "ジョン・ロック",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "17世紀イングランドの思想家で、王権と議会が争う時代に政治と人間の認識を考えました。『統治二論』で、人は生まれながら生命・自由・財産を守る権利を持ち、政府は人々の同意によって成立すると説きました。政府が権利を侵害すれば、人々には抵抗して政府を変える権利があるとも論じました。こうした自然権と社会契約の考えは、名誉革命の説明やアメリカ独立宣言、近代の立憲政治に影響しました。",
        "whatDid": "自然権と社会契約を説き、政府は人民の同意に基づき、権利を守るべきだと論じました。",
        "whyImportant": "近代の自由主義、立憲政治、アメリカ独立の思想的な土台を与えたためです。"
      },
      "image": "assets/people/person-3f43fd47bb.webp"
    },
    "リチャード・アークライト": {
      "name": "リチャード・アークライト",
      "kana": "リチャード・アークライト",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18世紀イギリスの発明家・事業家で、水力紡績機を改良して特許を取得し、綿糸を大量生産する工場を建てました。クロムフォードの工場では、水車を動力に多数の機械と労働者を一か所へ集め、原料調達から労働時間までを管理しました。発明のすべてが彼一人によるものではなく、特許の一部は後に無効となりましたが、機械・資本・労働を集中させる工場制度を広め、綿工業の急成長を支えました。",
        "whatDid": "水力紡績機を用いた大規模工場を経営し、綿糸を大量生産する工場制度を広めました。",
        "whyImportant": "家庭内手工業から機械制工場へ生産を移し、産業革命の働き方と生産方式を変えたためです。"
      },
      "image": "assets/people/person-620e13ee9b.webp"
    },
    "セシル・ローズ": {
      "name": "セシル・ローズ",
      "kana": "セシル・ローズ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半、南部アフリカでダイヤモンド鉱山を集めてデビアス社を築いたイギリス人実業家・政治家です。ケープ植民地首相となり、イギリス南アフリカ会社を通して現在のジンバブエやザンビアへ支配を広げました。黒人の土地と参政権を制限する政策を進め、ジェームソン侵入事件への関与で首相を辞任しました。奨学金に名を残す一方、その富と権力が植民地支配と人種差別に支えられていたため、現在も評価が争われています。",
        "whatDid": "鉱山会社と植民地会社を通じて南部アフリカの資源と土地を支配し、英国領を拡大しました。",
        "whyImportant": "企業・政治・軍事が結び付いた帝国主義と、植民地支配が残した不平等を示すためです。"
      },
      "image": "assets/people/person-d7452f90ef.webp"
    },
    "ウィンストン・チャーチル": {
      "name": "ウィンストン・チャーチル",
      "kana": "ウィンストン・チャーチル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリスの政治家で、第二次世界大戦中の1940年に首相となりました。フランス敗北後もナチス・ドイツとの戦争継続を訴え、演説で国民を励まし、アメリカ・ソ連と連携して連合国の勝利を目指しました。戦後は「鉄のカーテン」と表現してソ連への警戒を呼びかけました。一方、イギリス帝国と植民地支配を強く支持し、インド人への差別的発言やベンガル飢饉への対応をめぐって厳しい批判も受けています。",
        "whatDid": "第二次世界大戦でイギリスを率い、アメリカ・ソ連との連携によってナチス・ドイツと戦いました。",
        "whyImportant": "対独戦の継続と連合国の協力を支えた一方、帝国主義の限界も示す政治家だからです。"
      },
      "image": "assets/people/person-ad26ae571c.webp"
    },
    "ジャワハルラール・ネルー": {
      "name": "ジャワハルラール・ネルー",
      "kana": "ジャワハルラール・ネルー",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリス支配下のインドで弁護士となり、ガンディーとともに国民会議派の独立運動を進めた政治家です。独立後の1947年に初代首相となり、議会制民主主義、世俗主義、科学技術教育、重工業を重視した国家建設を進めました。冷戦では米ソどちらの陣営にも属さない非同盟外交を掲げました。一方、カシミール問題や中国との国境戦争、貧困の解消など、独立後の国家統合には多くの課題を残しました。",
        "whatDid": "独立運動を率い、初代首相として議会制民主主義・計画経済・非同盟外交を進めました。",
        "whyImportant": "植民地から独立したインドの国家像を形づくり、アジア・アフリカの非同盟運動を導いたためです。"
      },
      "image": "assets/people/person-d1557caa09.webp"
    },
    "メネス": {
      "name": "メネス",
      "kana": "メネス",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプトを最初に統一した王として、紀元前3世紀の神官マネトなど後世の記録に名を残す伝説的な人物です。上エジプトと下エジプトをまとめ、都をメンフィスに置いたと伝えられますが、同時代の確実な碑文は見つかっていません。初期王朝時代のナルメル、またはその後継者ホル・アハと同一人物とする説があります。実在と伝承の区別が難しく、王名表と考古資料を照合してエジプト統一を考える人物です。",
        "whatDid": "後世の記録で上下エジプトを統一し、最初の王朝を開いた王として伝えられました。",
        "whyImportant": "伝承上の建国者であり、ナルメルなどの考古資料と王名表の関係を考える鍵となるためです。"
      },
      "image": "assets/people/person-019b1d8d30.webp"
    },
    "エンヘドゥアンナ": {
      "name": "エンヘドゥアンナ",
      "kana": "エンヘドゥアンナ",
      "era": "最初の文明の時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前23世紀ごろ、アッカド王サルゴンの娘として生まれ、シュメール都市ウルの月神ナンナに仕える最高祭司となりました。王家の人物を重要都市の宗教職に置くことで、アッカド王権とシュメールの伝統を結ぶ役割を担いました。女神イナンナをたたえる賛歌などに自らの名を記し、政治的追放と復帰の経験も作品に表しました。作者名が分かる世界最古級の文学者として、女性・宗教・王権・文字文化の歴史に残っています。",
        "whatDid": "ウルの最高祭司として王権と宗教を結び、自らの名を記した女神への賛歌を残しました。",
        "whyImportant": "作者名が知られる最古級の文学者で、古代の女性・宗教・政治を伝えるためです。"
      },
      "image": "assets/people/person-bae8508d5f.webp"
    },
    "クレイステネス": {
      "name": "クレイステネス",
      "kana": "クレイステネス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前6世紀末のアテネで、僭主追放後の権力争いに勝ち、市民の支持を得て政治改革を行いました。血縁と地域の有力者に結び付いた旧来の部族を改め、海岸・都市・内陸の地区を組み合わせた十部族を編成しました。五百人評議会を設け、市民が抽選や民会を通じて政治に参加する仕組みを広げました。女性・奴隷・在留外国人は市民から除かれていましたが、後のアテネ民主政の制度的な基礎を築いた人物です。",
        "whatDid": "血縁中心の部族制を地域別の十部族へ改め、五百人評議会など市民参加の制度を整えました。",
        "whyImportant": "アテネ民主政の制度的な土台をつくり、市民が政治に参加する範囲を広げたためです。"
      },
      "image": "assets/people/person-d5e93608d0.webp"
    },
    "カトー": {
      "name": "カトー",
      "kana": "カトー",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマの政治家・軍人で、「大カトー」または「監察官カトー」と呼ばれます。執政官や監察官を務め、贅沢を戒め、祖先以来の質素な生活と農業をローマ人の理想として説きました。著書『農業論』は、ラテン語で全文が残る最古の散文作品です。晩年にはカルタゴの復興を危険視し、元老院で滅亡を繰り返し主張しました。その姿は、ローマの伝統主義と地中海帝国への拡大が併存した時代を示しています。",
        "whatDid": "監察官として風紀を引き締め、著書『農業論』を残し、カルタゴへの強硬策を訴えました。",
        "whyImportant": "共和政ローマの伝統的価値観と、カルタゴを滅ぼして地中海へ拡大する動きを象徴するためです。"
      },
      "image": "assets/people/person-e6cb8aba55.webp"
    },
    "ダレイオス1世": {
      "name": "ダレイオス1世",
      "kana": "ダレイオス1世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前6世紀末にアケメネス朝の王となり、反乱を鎮圧して帝国の支配を立て直しました。領土を州に分けて総督を置き、監察官を派遣して地方を監督しました。また、「王の道」と駅伝制、金貨ダリクを整え、税の基準を定めて、エジプトからインダス川流域までを結びました。ペルセポリスの建設を始めて多民族帝国の威信を示す一方、ギリシャ遠征では紀元前490年のマラトンの戦いに敗れました。",
        "whatDid": "帝国を州に分け、王の道・駅伝制・貨幣・税制を整えて広大な領土を統治しました。",
        "whyImportant": "多民族から成る大帝国を行政と交通で結び、後世の帝国統治の模範を示したためです。"
      },
      "image": "assets/people/person-91f786f396.webp"
    },
    "ペテロ": {
      "name": "ペテロ",
      "kana": "ペテロ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ガリラヤの漁師で、イエスに従った十二使徒の一人です。福音書ではイエスを救い主と告白する一方、逮捕時には三度関係を否定し、その後に弟子たちの中心となった人物として描かれます。イエスの死後、エルサレムの信者共同体を指導し、異邦人を受け入れる議論にも関わりました。ローマで殉教したという伝承から、カトリック教会は彼を初代教皇と位置付け、教皇の権威を説明する重要人物としています。",
        "whatDid": "イエスの弟子集団を率い、死後の信者共同体を指導して初期教会の形成に関わりました。",
        "whyImportant": "初期キリスト教共同体の中心となり、後のローマ教皇の権威の起源とされたためです。"
      },
      "image": "assets/people/person-597700fe67.webp"
    },
    "則天武后": {
      "name": "則天武后",
      "kana": "則天武后",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の太宗の後宮に入り、のちに高宗の皇后となって政治への影響力を強めました。高宗の死後は皇太后として実権を握り、690年に国号を周と改め、中国史上ただ一人「皇帝」を名乗った女性です。科挙を通じて新しい官僚を登用し、仏教を保護して自らの支配を正当化しました。一方、政敵への厳しい弾圧も行いました。女性皇帝への偏見を含む後世の記録を読み直す必要がある人物でもあります。",
        "whatDid": "皇后・皇太后から実権を握り、周を建てて皇帝となり、科挙官僚と仏教を支配に用いました。",
        "whyImportant": "中国史上唯一の女性皇帝として、唐代の官僚制・宗教・女性の政治権力を示すためです。"
      },
      "image": "assets/people/person-634820150e.webp"
    },
    "テオドラ": {
      "name": "テオドラ",
      "kana": "テオドラ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "6世紀の東ローマ皇帝ユスティニアヌスの皇后で、低い身分の出身から宮廷の最高位へ上ったと伝えられます。532年のニカの反乱で皇帝が都から逃れようとした際、抵抗を続けるよう主張したという記録で有名です。皇帝とともに政治判断や宗教政策に関わり、女性を強制売春から守る措置や、離婚・財産に関する権利の改善にも影響したとされます。ただし、生涯の記録には敵対者プロコピオスの偏見が強く、史料批判が必要です。",
        "whatDid": "ニカの反乱で政権を支え、皇帝とともに政治・宗教政策や女性保護の法整備に関わりました。",
        "whyImportant": "東ローマ宮廷で大きな政治権力を持った女性であり、史料の偏見を考える例でもあるためです。"
      },
      "image": "assets/people/person-7294fab46e.webp"
    },
    "バルトロメウ・ディアス": {
      "name": "バルトロメウ・ディアス",
      "kana": "バルトロメウ・ディアス",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "15世紀のポルトガル航海者で、アフリカ西岸を南下する船団を率いました。1488年、嵐で陸地を見失った後に進路を北へ変え、アフリカ南端を回ってインド洋側へ到達しました。帰路に確認した岬は当初「嵐の岬」と呼ばれ、国王がアジア航路への期待を込めて「喜望峰」と改名しました。本人はインドへ到達しませんでしたが、この航路の発見が、後のヴァスコ・ダ・ガマによるインド航海を可能にしました。",
        "whatDid": "1488年にアフリカ南端を回ってインド洋へ入り、ヨーロッパからアジアへ向かう海路を示しました。",
        "whyImportant": "喜望峰経由のインド航路を開く決定的な一歩となり、ポルトガルの海上進出を進めたためです。"
      },
      "image": "assets/people/person-4800aabf28.webp"
    },
    "モンテスキュー": {
      "name": "モンテスキュー",
      "kana": "モンテスキュー",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀フランスの貴族・法思想家で、各国の政治制度や法律を比較して、権力がどのように働くかを研究しました。主著『法の精神』で、政治権力を立法・行政・司法に分け、互いに抑制させることで専制を防ぐ考えを示しました。これは三権分立と呼ばれます。イギリス政治の分析には単純化もありましたが、その理論はアメリカ合衆国憲法やフランス革命後の憲法をはじめ、近代国家の権力分立に大きな影響を与えました。",
        "whatDid": "『法の精神』で立法・行政・司法を分け、互いに抑制させる三権分立を説きました。",
        "whyImportant": "権力の集中を防ぐ制度原理を示し、近代の憲法と民主政治の設計に影響したためです。"
      },
      "image": "assets/people/person-2562164bcc.webp"
    },
    "ジェームズ・ハーグリーブス": {
      "name": "ジェームズ・ハーグリーブス",
      "kana": "ジェームズ・ハーグリーブス",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18世紀イギリスの織工・発明家で、綿糸の需要が急増するなか、一人で複数の糸を同時に紡げる「ジェニー紡績機」を考案しました。手回しで動かす比較的簡単な機械でしたが、従来より多くの糸を短時間で生産でき、紡績工程の機械化を進めました。一方、機械によって仕事を失うことを恐れた職人から反発を受け、工場を移すことにもなりました。発明は後の大型紡績機へつながり、産業革命初期の綿工業を変えました。",
        "whatDid": "一人で複数の糸を同時に紡げるジェニー紡績機を考案し、綿糸生産を効率化しました。",
        "whyImportant": "紡績の機械化を進め、綿工業を産業革命の中心産業へ成長させる一歩となったためです。"
      },
      "image": "assets/people/person-94098b32d7.webp"
    },
    "デイヴィッド・リヴィングストン": {
      "name": "デイヴィッド・リヴィングストン",
      "kana": "デイヴィッド・リヴィングストン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀のスコットランド出身の宣教師・探検家で、医師として南部・中部アフリカを各地へ進みました。ザンベジ川やビクトリア滝をヨーロッパに紹介し、地理情報を記録するとともに、東アフリカで続く奴隷貿易の実態を訴えました。現地社会を十分に理解できず、探検が後の植民地進出に利用された面もあります。消息を絶った後にスタンリーが発見した出来事でも有名ですが、現在は探検・宣教・帝国主義の関係を考える人物として扱われます。",
        "whatDid": "南部・中部アフリカを探検して河川や地形を記録し、奴隷貿易の廃止を訴えました。",
        "whyImportant": "アフリカ内陸の地理を欧州へ伝える一方、その記録が植民地進出にも利用されたためです。"
      },
      "image": "assets/people/person-90abd8cf8f.webp"
    },
    "ネヴィル・チェンバレン": {
      "name": "ネヴィル・チェンバレン",
      "kana": "ネヴィル・チェンバレン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリス保守党の政治家で、1937年に首相となりました。戦争回避を優先し、1938年のミュンヘン会談でヒトラーによるチェコスロバキアのズデーテン地方併合を認める宥和政策を進めました。しかしドイツは翌年に残る地域も占領し、ポーランド侵攻によって第二次世界大戦が始まりました。開戦後はドイツへ宣戦しましたが、ノルウェー作戦の失敗で退陣しました。宥和政策は批判される一方、再軍備の時間を稼いだとの評価もあります。",
        "whatDid": "ミュンヘン会談でドイツへの譲歩を選び、戦争回避を目指す宥和政策を進めました。",
        "whyImportant": "独裁国への譲歩が侵略を止められなかった事例として、第二次世界大戦前史の中心となるためです。"
      },
      "image": "assets/people/person-fd6656d191.webp"
    },
    "インディラ・ガンディー": {
      "name": "インディラ・ガンディー",
      "kana": "インディラ・ガンディー",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "インド初代首相ネルーの娘として政治に入り、1966年に同国初の女性首相となりました。銀行国有化や農業増産を進め、1971年の第三次印パ戦争では東パキスタンの独立を支援し、バングラデシュ成立につなげました。一方、1975年から非常事態を宣言して反対派を拘束し、報道の自由を制限しました。1984年にはシク教徒武装勢力を排除するため黄金寺院へ軍を投入し、その報復として護衛に暗殺されました。",
        "whatDid": "銀行国有化や農業増産を進め、バングラデシュ独立を支援する一方、非常事態で反対派を抑えました。",
        "whyImportant": "インドの国家統合と地域秩序を動かす一方、民主政治と強権統治の緊張を示したためです。"
      },
      "image": "assets/people/person-83952b1af6.webp"
    },
    "ジェセル": {
      "name": "ジェセル",
      "kana": "ジェセル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第3王朝の王で、紀元前27世紀ごろに統治したと考えられています。宰相イムホテプのもと、サッカラに石造の階段ピラミッドを建設しました。これは従来の長方形の墓マスタバを重ねた形から発展した、エジプト最古級の大規模石造建築です。周囲には神殿や祭礼施設も整えられ、王の死後の再生と永続する支配を表しました。後のギザのピラミッドへつながる建築技術と王権表現の出発点となった王です。",
        "whatDid": "イムホテプに命じてサッカラの階段ピラミッドと大規模な葬祭施設を建設しました。",
        "whyImportant": "大規模石造建築とピラミッド建設の出発点をつくり、古代エジプト王権を形で示したためです。"
      },
      "image": "assets/people/person-af40024a79.webp"
    },
    "ウルナンム": {
      "name": "ウルナンム",
      "kana": "ウルナンム",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前21世紀末、メソポタミアでウル第3王朝を開いた王です。アッカド帝国崩壊後の混乱を収め、シュメール地方を再統一し、各地の都市と灌漑施設を整えました。ウルでは月神ナンナを祭る巨大なジッグラトの建設を始め、王朝の宗教的中心を築きました。また、現存する最古級の法典「ウル・ナンム法典」と結び付けられ、罪に対して銀による賠償を定めた条文も残ります。国家再建と法の歴史を示す王です。",
        "whatDid": "ウル第3王朝を開いてシュメールを再統一し、ジッグラト建設と法の整備を進めました。",
        "whyImportant": "都市国家を再統合し、現存最古級の法典と大規模神殿建築を残したためです。"
      },
      "image": "assets/people/person-6ef4c2baf1.webp"
    },
    "ペリクレス": {
      "name": "ペリクレス",
      "kana": "ペリクレス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネ政治家で、民会での演説と支持を背景に長く指導的地位に立ちました。公職に手当を支給して、裕福でない市民も政治に参加しやすくし、民主政を発展させました。ペルシャ戦争後にはデロス同盟の資金を用いてパルテノン神殿などを建設し、文化を栄えさせました。一方、同盟諸国への支配を強め、スパルタとの対立からペロポネソス戦争に入りました。民主政と帝国的支配の両面を持つ人物です。",
        "whatDid": "公職手当で市民参加を広げ、パルテノン神殿を建設する一方、デロス同盟への支配を強めました。",
        "whyImportant": "アテネ民主政と古典文化の最盛期を築く一方、同盟を帝国化した矛盾も示すためです。"
      },
      "image": "assets/people/person-b9aecbca6c.webp"
    },
    "ティベリウス・グラックス": {
      "name": "ティベリウス・グラックス",
      "kana": "ティベリウス・グラックス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前2世紀の共和政ローマの政治家で、護民官として大土地所有の拡大と自作農の減少に取り組みました。国有地の占有上限を定め、余った土地を貧しい市民へ分配する農地法を成立させました。しかし元老院の強い反対を受け、再選を目指した際に支持者とともに殺害されました。改革は一部実行されましたが、政治的暴力が公然と用いられる転機となりました。弟ガイウスの改革とともに共和政末期の社会対立を象徴します。",
        "whatDid": "国有地の占有を制限し、土地を失った市民へ農地を再分配する改革を進めました。",
        "whyImportant": "貧富の差と自作農の衰退を政治課題にし、共和政ローマの激しい党派対立の始まりを示したためです。"
      },
      "image": "assets/people/person-4e2e5a3bee.webp"
    },
    "クセルクセス1世": {
      "name": "クセルクセス1世",
      "kana": "クセルクセス1世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "アケメネス朝ペルシャの王で、父ダレイオス1世の死後に即位しました。エジプトとバビロニアの反乱を鎮圧した後、大軍と艦隊を率いてギリシャへ遠征しました。テルモピュライでスパルタ軍を破り、アテネを占領しましたが、サラミスの海戦で艦隊が敗れ、翌年にはプラタイアでも敗北しました。ペルセポリスの建設も進め、帝国の威信を示しました。ギリシャ側史料の誇張に注意しながら、東西の大戦争を考える王です。",
        "whatDid": "大軍でギリシャへ遠征し、テルモピュライを突破してアテネを占領しましたが、海戦で敗れました。",
        "whyImportant": "ペルシャ戦争の最大規模の遠征を行い、ギリシャ諸都市の結束とその後の歴史を左右したためです。"
      },
      "image": "assets/people/person-653eee17f8.webp"
    },
    "アウグスティヌス": {
      "name": "アウグスティヌス",
      "kana": "アウグスティヌス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "北アフリカのタガステに生まれ、修辞学を学んだ後、さまざまな思想を遍歴してキリスト教へ回心した神学者です。司教として教会を指導し、『告白』では自らの罪と回心を内面から描きました。西ローマが揺らぐなかで書いた『神の国』では、地上の国家と神を愛する共同体を区別し、ローマ帝国の運命とキリスト教の価値を同一視しませんでした。原罪・恩恵・自由意志をめぐる思想は、中世西欧の神学と哲学に長く影響しました。",
        "whatDid": "『告白』『神の国』を著し、原罪・恩恵・国家と信仰の関係を体系的に論じました。",
        "whyImportant": "古代哲学とキリスト教を結び、中世西ヨーロッパの神学と人間観の基礎を築いたためです。"
      },
      "image": "assets/people/person-0dbfd89beb.webp",
      "imageAlt": "アウグスティヌスの画像"
    },
    "玄宗": {
      "name": "玄宗",
      "kana": "玄宗",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の皇帝で、即位初期には有能な官僚を登用し、財政や行政を立て直しました。この時期は「開元の治」と呼ばれ、都長安には各地の商人や文化が集まり、唐は繁栄しました。しかし晩年は楊貴妃とその一族を重用し、節度使に大きな軍事力を持たせた結果、755年に安禄山らが反乱を起こしました。安史の乱の途中で退位し、唐は鎮圧後も地方軍人への依存を強めました。繁栄から衰退への転機を示す皇帝です。",
        "whatDid": "開元の治で唐を繁栄させましたが、節度使の勢力拡大を招き、安史の乱の途中で退位しました。",
        "whyImportant": "唐の最盛期と、地方軍人が強まって王朝が衰退へ向かう転換点の双方を体現するためです。"
      },
      "image": "assets/people/person-0cc7ff9461.webp"
    },
    "ヘラクレイオス": {
      "name": "ヘラクレイオス",
      "kana": "ヘラクレイオス",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "7世紀初めの東ローマ皇帝で、ササン朝ペルシャ軍にシリア・エジプト・小アジアを奪われる危機に直面しました。軍制と財政を立て直し、自ら反攻を指揮してペルシャ領深くへ進み、奪われた聖十字架をエルサレムへ戻しました。しかし両国が消耗した直後、イスラム勢力が急速に進出し、シリアとエジプトを失いました。公用語や皇帝称号のギリシャ化も進み、古代ローマ帝国から中世ビザンツ帝国への転換期を代表します。",
        "whatDid": "ササン朝に反攻して失地を回復しましたが、その後イスラム勢力にシリアとエジプトを奪われました。",
        "whyImportant": "東ローマ帝国を一度は再建しつつ、イスラム進出とビザンツ化という大転換に直面したためです。"
      },
      "image": "assets/people/person-9a6aa60aa5.webp"
    },
    "ヴァスコ・ダ・ガマ": {
      "name": "ヴァスコ・ダ・ガマ",
      "kana": "ヴァスコ・ダ・ガマ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ポルトガルの航海者で、1497年にリスボンを出発し、喜望峰を回って東アフリカ沿岸を進みました。現地の航海知識を借りてインド洋を横断し、1498年にインドのカリカットへ到達しました。これによりヨーロッパとインドを直接結ぶ海路が開かれました。後の航海では武力で港を攻撃し、イスラム商人や住民に大きな被害を与えました。香辛料貿易の新航路を築く一方、ポルトガルの海上帝国と暴力的支配を始めた人物です。",
        "whatDid": "喜望峰を回ってインドへ到達し、ヨーロッパとアジアを結ぶ直接の海上航路を開きました。",
        "whyImportant": "香辛料貿易の経路を変え、ポルトガルのインド洋支配と欧州の海外進出を加速させたためです。"
      },
      "image": "assets/people/person-3b613a629f.webp"
    },
    "ヴォルテール": {
      "name": "ヴォルテール",
      "kana": "ヴォルテール",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀フランスの思想家・作家で、王権や教会の権威が強い社会を風刺と論争で批判しました。宗教戦争や裁判の不正を問題にし、思想・言論・信仰の自由と宗教的寛容を訴えました。『哲学書簡』ではイギリスの政治や科学を紹介し、『カンディード』では戦争や楽観主義を風刺しました。一方、啓蒙専制君主との交流や当時の偏見も持っていました。その著作は、権威を理性で批判する啓蒙思想をヨーロッパへ広めました。",
        "whatDid": "風刺作品と論説で教会・専制政治・不正な裁判を批判し、信仰と言論の自由を訴えました。",
        "whyImportant": "理性と寛容を重んじる啓蒙思想を広め、フランス革命前の社会批判を強めたためです。"
      },
      "image": "assets/people/person-90ebf84bd9.webp"
    },
    "サミュエル・クロムプトン": {
      "name": "サミュエル・クロムプトン",
      "kana": "サミュエル・クロムプトン",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18世紀イギリスの紡績工で、ハーグリーブスのジェニー紡績機とアークライトの水力紡績機の長所を組み合わせ、ミュール紡績機を発明しました。この機械は細くて丈夫な糸を大量に紡げたため、上質な綿布の生産を可能にしました。しかし特許を取らずに設計を公開したため、多くの工場が利益を得る一方、本人は十分な報酬を得られませんでした。発明と資本の関係を示しながら、綿工業の機械化を大きく進めた人物です。",
        "whatDid": "ジェニー紡績機と水力紡績機を組み合わせ、細く丈夫な糸を紡ぐミュール紡績機を発明しました。",
        "whyImportant": "上質な綿糸の大量生産を可能にし、イギリス綿工業の国際的な成長を支えたためです。"
      },
      "image": "assets/people/person-51af8aee07.webp"
    },
    "ヘンリー・モートン・スタンリー": {
      "name": "ヘンリー・モートン・スタンリー",
      "kana": "ヘンリー・モートン・スタンリー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ウェールズ生まれでアメリカへ渡り、新聞記者・探検家としてアフリカで活動しました。消息不明だったリヴィングストンを1871年に発見したことで有名になり、その後コンゴ川流域を横断して地理を記録しました。ベルギー王レオポルド2世の依頼で各地の首長と条約を結び、道路や拠点を築きましたが、これらはコンゴ自由国の成立と過酷な植民地支配に利用されました。探検の成果と帝国主義への加担を切り離せない人物です。",
        "whatDid": "リヴィングストンを発見し、コンゴ川流域を調査して、ベルギー王の植民地建設に協力しました。",
        "whyImportant": "アフリカ探検が欧州の領土獲得と暴力的な植民地支配へ直結した過程を示すためです。"
      },
      "image": "assets/people/person-b08f4fcadd.webp"
    },
    "アドルフ・ヒトラー": {
      "name": "アドルフ・ヒトラー",
      "kana": "アドルフ・ヒトラー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "オーストリア生まれで第一次世界大戦後にドイツ労働者党へ入り、ナチ党の指導者となりました。世界恐慌と政治不安を利用して1933年に首相となり、議会制民主主義を破壊して独裁体制を築きました。人種差別と反ユダヤ主義を国家政策にし、侵略戦争を始めて第二次世界大戦を引き起こしました。ユダヤ人を中心に約600万人を組織的に殺害したホロコーストを進め、ヨーロッパ各地に甚大な被害を与え、敗戦直前に自殺しました。",
        "whatDid": "ナチ党独裁を築き、侵略戦争とホロコーストを進め、第二次世界大戦と大量虐殺を引き起こしました。",
        "whyImportant": "独裁・人種差別・侵略が社会と国家を破壊し、世界規模の戦争と大量虐殺へ至る危険を示すためです。"
      },
      "image": "assets/people/person-860e88f804.webp"
    },
    "ベナジル・ブット": {
      "name": "ベナジル・ブット",
      "kana": "ベナジル・ブット",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "パキスタン元首相ズルフィカール・アリー・ブットの娘で、父が軍事政権に処刑された後、民主化運動の中心となりました。1988年に首相へ就任し、イスラム教徒が多数を占める国で初めて政府を率いた女性となりました。二度首相を務めましたが、軍や大統領との対立、汚職疑惑によりいずれも任期途中で解任されました。亡命後に帰国し、軍政から民政への移行を訴える選挙運動中の2007年、襲撃されて死亡しました。",
        "whatDid": "軍事政権に対する民主化運動を率い、イスラム教徒多数国で初の女性首相となりました。",
        "whyImportant": "女性の政治参加を切り開く一方、軍・政党・汚職が絡むパキスタン政治の困難を示したためです。"
      },
      "image": "assets/people/person-1f3e28397d.webp"
    },
    "スネフェル": {
      "name": "スネフェル",
      "kana": "スネフェル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第4王朝を開いた王で、紀元前26世紀ごろに統治しました。メイドゥム、ダハシュールに複数のピラミッドを建設し、途中で傾斜を変えた屈折ピラミッドを経て、滑らかな斜面を持つ赤いピラミッドを完成させました。この試行錯誤によって真正ピラミッドの技術が確立され、子のクフ王によるギザの大ピラミッドへつながりました。シナイ半島への遠征や木材輸入も行い、王朝の経済力と組織力を高めた王です。",
        "whatDid": "複数のピラミッド建設を通して真正ピラミッドの技術を確立し、第4王朝の基盤を築きました。",
        "whyImportant": "ギザの大ピラミッドへつながる建築技術と、古王国の強い王権を完成へ導いたためです。"
      },
      "image": "assets/people/person-62e4a669a6.webp"
    },
    "グデア": {
      "name": "グデア",
      "kana": "グデア",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician",
        "culture"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前22世紀ごろ、アッカド帝国崩壊後の南メソポタミアで都市国家ラガシュを治めた王です。戦争の記録よりも、神殿や運河の建設、神々への奉納を強調した多数の碑文で知られます。遠方から閃緑岩、木材、金属などを取り寄せ、交易の広がりを示しました。祈る姿を表した多くの石像には王名と事業が刻まれ、王が神々と都市を結ぶ存在であることを示しました。像と碑文から統治を詳しく知ることができる人物です。",
        "whatDid": "ラガシュで神殿と運河を建設し、遠隔地交易で資材を集め、多数の奉納像と碑文を残しました。",
        "whyImportant": "王の宗教的役割、都市建設、交易を像と碑文から具体的に知ることができるためです。"
      },
      "image": "assets/people/person-d69e810d52.webp"
    },
    "レオニダス": {
      "name": "レオニダス",
      "kana": "レオニダス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "古代ギリシャの都市国家スパルタの王で、紀元前480年のペルシャ戦争でテルモピュライの防衛を指揮しました。狭い道を利用し、スパルタ兵を含むギリシャ連合軍でクセルクセス1世の大軍を数日間食い止めました。迂回路を知られると、一部の兵を退却させ、自身は残った兵とともに戦死しました。後世には「300人」の英雄物語として語られましたが、実際には他の都市の兵も参加しており、史実と伝説を区別する必要があります。",
        "whatDid": "テルモピュライの狭路でギリシャ連合軍を指揮し、ペルシャ大軍を食い止めて戦死しました。",
        "whyImportant": "ギリシャ側の抵抗を象徴する一方、後世の英雄伝説と実際の連合軍の違いを考えられるためです。"
      },
      "image": "assets/people/person-ceda1bc256.webp"
    },
    "マリウス": {
      "name": "マリウス",
      "kana": "マリウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前2世紀末から1世紀初めの共和政ローマの将軍・政治家です。ユグルタ戦争や北方から侵入したキンブリ人・テウトニ人との戦いで勝利し、異例の七度にわたり執政官となりました。財産の少ない市民も軍へ参加させ、装備を国家が支える方向を進めたため、兵士が国家より将軍個人へ忠誠を寄せる傾向が強まりました。晩年にはスッラとの権力争いでローマを武力制圧し、内戦と軍事独裁の時代への道を開きました。",
        "whatDid": "無産市民を軍へ採用してローマ軍を強化し、外敵を破りましたが、スッラとの内戦を招きました。",
        "whyImportant": "軍隊の性格を変え、将軍同士の私兵化と内戦が共和政を崩す流れを強めたためです。"
      },
      "image": "assets/people/person-3b825e131f.webp"
    },
    "アルタクセルクセス1世": {
      "name": "アルタクセルクセス1世",
      "kana": "アルタクセルクセス1世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "アケメネス朝ペルシャの王で、父クセルクセス1世が宮廷内で暗殺された後、混乱を経て即位しました。エジプトで起きた反乱にはアテネ艦隊も加わりましたが、将軍メガビュゾスを送り鎮圧し、帝国の支配を維持しました。ギリシャ諸国とは長い戦争の後、カリアスの和約と呼ばれる和平が結ばれたと伝えられますが、その実在や内容には議論があります。聖書のエズラ記・ネヘミヤ記とも関係づけられ、多民族帝国の統治を考える王です。",
        "whatDid": "宮廷混乱後に帝国を立て直し、エジプト反乱を鎮圧してアケメネス朝の広域支配を保ちました。",
        "whyImportant": "反乱とギリシャ勢力の介入に対応し、多民族帝国の安定を維持した統治を示すためです。"
      },
      "image": "assets/people/person-3b4f71264c.webp",
      "imageAlt": "アルタクセルクセス1世の画像"
    },
    "ヒエロニムス": {
      "name": "ヒエロニムス",
      "kana": "ヒエロニムス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4～5世紀のキリスト教神学者で、ローマで学んだ後、修道生活を送りながらギリシャ語とヘブライ語を研究しました。教皇ダマスス1世の依頼を受け、聖書をラテン語へ翻訳・校訂しました。旧約聖書では当時一般的だったギリシャ語訳だけでなく、ヘブライ語本文を重視しました。この翻訳は後に「ウルガタ」と呼ばれ、西ヨーロッパの教会、学問、文学で千年以上使われました。翻訳が宗教理解と文化形成を左右することを示す人物です。",
        "whatDid": "ヘブライ語とギリシャ語の聖書をラテン語へ翻訳・校訂し、ウルガタ聖書の基礎を築きました。",
        "whyImportant": "西ヨーロッパで長く標準となる聖書本文を整え、宗教・教育・文学へ影響したためです。"
      },
      "image": "assets/people/person-e2200bc7bb.webp"
    },
    "楊貴妃": {
      "name": "楊貴妃",
      "kana": "楊貴妃",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "war",
        "society"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の玄宗皇帝に寵愛された女性で、本名を楊玉環といいます。もとは玄宗の子の妃でしたが、のちに道教の女道士を経て皇帝の貴妃となりました。親族の楊国忠らが高い地位を得たため、宮廷政治の腐敗を象徴する存在として後世に語られました。755年に安史の乱が起こり、玄宗が都を逃れる途中、兵士たちの要求によって殺されました。ただし乱の原因を彼女一人へ負わせる見方は、女性への偏見を含む文学的伝承として検討が必要です。",
        "whatDid": "玄宗の貴妃として宮廷で大きな影響力を持ち、安史の乱の逃避行中に兵士の要求で殺されました。",
        "whyImportant": "唐の繁栄と崩壊を語る象徴となり、女性へ政治責任を負わせる歴史叙述も考えられるためです。"
      },
      "image": "assets/people/person-7226500134.webp"
    },
    "バシレイオス2世": {
      "name": "バシレイオス2世",
      "kana": "バシレイオス2世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "10～11世紀の東ローマ皇帝で、若い頃は有力貴族の反乱に苦しみましたが、親政を確立すると大土地所有者を抑え、軍事と財政を立て直しました。長年にわたりブルガリアと戦い、1018年に征服してバルカン半島の支配を広げました。キエフ公ウラジーミル1世から援軍を得る代わりに皇妹を結婚させ、ルーシのキリスト教化にも関わりました。帝国を最盛期へ導いた一方、後継体制を十分に整えなかった点が限界でした。",
        "whatDid": "有力貴族の反乱を抑えて皇帝権を強め、ブルガリアを征服して東ローマ帝国を最盛期へ導きました。",
        "whyImportant": "東ローマ帝国の軍事・財政・領土を立て直し、中世ビザンツの最盛期を築いたためです。"
      },
      "image": "assets/people/person-8ef1cb23d8.webp"
    },
    "フェルディナンド・マゼラン": {
      "name": "フェルディナンド・マゼラン",
      "kana": "フェルディナンド・マゼラン",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ポルトガル出身の航海者で、スペイン王の支援を受け、西回りで香辛料諸島へ到達する航路を探しました。1519年に五隻の船で出航し、南アメリカ南端の海峡を通過して太平洋を横断しました。航海の途中、フィリピンのマクタン島で現地勢力との戦いに介入して死亡したため、本人は世界一周を完成していません。残った船団をエルカーノが率いて帰国し、地球を海路で一周できることと、世界の海の広大さを実証しました。",
        "whatDid": "スペイン船団を率いて南アメリカ南端の海峡を通過し、ヨーロッパ人として初めて太平洋を横断しました。",
        "whyImportant": "船団の航海が初の世界一周を実現し、地球の大きさと海洋のつながりを具体的に示したためです。"
      },
      "image": "assets/people/person-c04ca7626f.webp",
      "imageAlt": "フェルディナンド・マゼランの画像"
    },
    "ルソー": {
      "name": "ルソー",
      "kana": "ルソー",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀ジュネーヴ生まれの思想家で、身分制社会と文明のあり方を批判しました。『社会契約論』で、政治の正当な権力は人民全体の意思である「一般意志」に基づくと説き、主権は人民にあると主張しました。『エミール』では、子どもの成長段階と自主性を重んじる教育を論じました。一方、一般意志の解釈には、少数者の自由を抑える危険も指摘されています。その思想はフランス革命と近代民主主義、教育論に大きな影響を与えました。",
        "whatDid": "『社会契約論』で人民主権と一般意志を説き、『エミール』で子どもの自主性を重んじる教育を論じました。",
        "whyImportant": "人民が政治の主権者であるという考えを広め、革命・民主政治・近代教育へ影響したためです。"
      },
      "image": "assets/people/person-59efcfe648.webp"
    },
    "ジョージ・スティーブンソン": {
      "name": "ジョージ・スティーブンソン",
      "kana": "ジョージ・スティーブンソン",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "イギリス北部の炭鉱で働きながら機械技術を学び、蒸気機関車を実用的な交通手段へ発展させた技術者です。1825年にストックトン・ダーリントン鉄道で「ロコモーション号」を走らせ、1830年開業のリバプール・マンチェスター鉄道では、息子ロバートらと製作した「ロケット号」が採用されました。鉄道の線路幅や勾配の設計にも関わり、石炭・原料・製品・人を大量に速く運ぶ鉄道網の普及を進めました。",
        "whatDid": "蒸気機関車を改良し、営業鉄道の建設と運行を進めて、人や物を大量に運ぶ鉄道輸送を普及させました。",
        "whyImportant": "鉄道で炭鉱・工場・港・都市を結び、産業革命期の生産・流通・人の移動を大きく変えたためです。"
      },
      "image": "assets/people/person-19fc20ba52.webp"
    },
    "レオポルド2世": {
      "name": "レオポルド2世",
      "kana": "レオポルド2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半のベルギー国王で、植民地獲得を目指し、探検家スタンリーを利用してコンゴ川流域の支配権を集めました。1885年に国ではなく自らの私領としてコンゴ自由国を成立させ、象牙と天然ゴムを強制的に集めさせました。現地住民は人質、暴行、身体切断、殺害などの過酷な支配を受け、人口も大きく減少しました。国際的な告発を受けて1908年にベルギー政府へ統治権を渡しましたが、植民地暴力の象徴として記憶されています。",
        "whatDid": "コンゴ自由国を私領として支配し、象牙とゴムを強制徴収する過程で住民へ大規模な暴力を加えました。",
        "whyImportant": "欧州帝国主義が企業利益と王権のもとで大量の強制労働と人命被害を生んだことを示すためです。"
      },
      "image": "assets/people/person-ff61fbc0c9.webp"
    },
    "ベニート・ムッソリーニ": {
      "name": "ベニート・ムッソリーニ",
      "kana": "ベニート・ムッソリーニ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "第一次世界大戦後のイタリアでファシスト党を組織し、社会不安と反共感情を利用して勢力を広げました。1922年のローマ進軍後に首相となり、政党・議会・報道の自由を奪って一党独裁を築きました。企業と国家を統制する体制を掲げ、エチオピア侵略やアルバニア占領を進め、ナチス・ドイツと同盟して第二次世界大戦へ参戦しました。敗戦が近づくと失脚し、1945年にパルチザンに捕らえられて処刑されました。",
        "whatDid": "ファシスト党独裁を築き、エチオピア侵略とナチス・ドイツとの同盟を進めて第二次世界大戦へ参戦しました。",
        "whyImportant": "ファシズムが民主政治を破壊し、民族主義と軍事侵略を正当化した過程を示すためです。"
      },
      "image": "assets/people/person-af4451019a.webp"
    },
    "アウンサンスーチー": {
      "name": "アウンサンスーチー",
      "kana": "アウンサンスーチー",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ミャンマー独立運動を率いたアウンサンの娘で、1988年の民主化運動を機に国民民主連盟を率いました。軍政下で長期間の自宅軟禁を受け、非暴力の民主化運動を象徴してノーベル平和賞を受賞しました。2015年の選挙後は国家顧問として事実上の政権指導者となりましたが、ロヒンギャへの迫害を止めず、国際司法裁判所で軍の行動を擁護して強い批判を受けました。2021年の軍事クーデター後に拘束され、民主化の象徴と政権担当時の責任の両面で評価されています。",
        "whatDid": "軍政下で民主化運動を率いて長く拘束され、政権参加後は国家顧問としてミャンマー政府を指導しました。",
        "whyImportant": "民主化運動の象徴となる一方、ロヒンギャ迫害への対応で人権と政治責任の問題を残したためです。"
      },
      "image": "assets/people/person-2a1b09d78c.webp",
      "imageAlt": "アウンサンスーチーの画像"
    },
    "カフラー": {
      "name": "カフラー",
      "kana": "カフラー",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第4王朝の王で、クフ王の後継世代として紀元前26世紀ごろに統治しました。ギザにクフ王のものに次ぐ大きさのピラミッドを建設し、参道、河岸神殿、葬祭殿を組み合わせた施設を整えました。近くの大スフィンクスも一般にカフラーの時代と結び付けられていますが、造営者には議論があります。王の頭にハヤブサ神ホルスが翼を広げる座像は、王が神々に守られる存在であることを示し、王権と葬祭信仰を伝えます。",
        "whatDid": "ギザにピラミッドと葬祭施設を建設し、神に守られた王権を壮大な石造建築と像で示しました。",
        "whyImportant": "ピラミッド複合体と王の神聖性を通して、古王国の強大な王権と死後信仰を伝えるためです。"
      },
      "image": "assets/people/person-404d9369ab.webp"
    },
    "シュルギ": {
      "name": "シュルギ",
      "kana": "シュルギ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "ウル第3王朝の二代王で、父ウルナンムの死後、紀元前21世紀に長期間統治しました。道路と宿駅を整え、税や労働、家畜、物資を粘土板で記録する官僚制度を発達させ、度量衡や暦の統一も進めました。自らを神格化し、運動能力や学識をたたえる賛歌を作らせて王権を強調しました。周辺地域への遠征で帝国を広げましたが、巨大な行政組織と強制的な労働・徴税は人々へ重い負担を課しました。",
        "whatDid": "道路・宿駅・税制・記録制度を整え、王を神格化してウル第3王朝の中央集権的支配を完成させました。",
        "whyImportant": "粘土板記録を用いた官僚国家と、神格化された王権の具体的な姿を示すためです。"
      },
      "image": "assets/people/person-939d90be9c.webp"
    },
    "テミストクレス": {
      "name": "テミストクレス",
      "kana": "テミストクレス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀初めのアテネ政治家・将軍で、ペルシャの再侵攻に備えて海軍力の強化を進めました。ラウリオン銀山の収入を市民へ分配せず、多数の三段櫂船建造へ使うよう説得しました。クセルクセス1世の大軍が侵攻すると、アテネ市民を避難させ、狭いサラミス海峡へペルシャ艦隊を誘い込んで勝利しました。この海戦はギリシャ側の反攻を可能にしましたが、後に陶片追放され、最終的にはペルシャ王のもとへ亡命しました。",
        "whatDid": "アテネ海軍を強化し、サラミスの海戦で地形を利用してペルシャ艦隊を破る作戦を指揮しました。",
        "whyImportant": "ペルシャのギリシャ征服を防ぎ、アテネが海上勢力として発展する基礎を築いたためです。"
      },
      "image": "assets/people/person-9d2f5f6c9d.webp"
    },
    "スラ": {
      "name": "スラ",
      "kana": "スラ",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の将軍・政治家で、ユグルタ戦争や同盟市戦争で軍功を立てました。東方遠征の指揮権をめぐってマリウス派と争い、ローマ軍を率いて首都へ進軍した最初の将軍となりました。内戦に勝利すると独裁官となり、政敵を「追放者名簿」に載せて殺害と財産没収を進めました。元老院の権限を強め、護民官を弱体化する改革を行った後、自ら辞任しましたが、軍事力で政治を支配する先例を残しました。",
        "whatDid": "軍を率いてローマを占領し、独裁官として政敵を粛清し、元老院中心の政治制度を再編しました。",
        "whyImportant": "将軍が私兵化した軍で政権を奪う先例をつくり、共和政崩壊への流れを決定的に強めたためです。"
      },
      "image": "assets/people/person-0894103d82.webp"
    },
    "ゾロアスター": {
      "name": "ゾロアスター",
      "kana": "ゾロアスター",
      "era": "古代の大国の時代",
      "genre": [
        "religion"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "古代イランの宗教家で、活動した年代は紀元前2千年紀から前6世紀まで諸説があり、確定していません。聖典『アヴェスター』のうち最古層の賛歌「ガーサー」は本人の教えに近いと考えられます。最高神アフラ・マズダーを信じ、人は善い思い・善い言葉・善い行いを選び、真理と虚偽の戦いに責任を持つと説きました。後のゾロアスター教では最後の審判や死者の復活が体系化され、古代イラン諸王朝の宗教文化に長く影響しました。",
        "whatDid": "アフラ・マズダーへの信仰と、善悪の間で人が自ら正しい行いを選ぶ責任を説きました。",
        "whyImportant": "古代イランの宗教を形づくり、善悪・審判・復活をめぐる後世の宗教思想にも影響したためです。"
      },
      "image": "assets/people/person-0d919524fb.webp"
    },
    "アタナシウス": {
      "name": "アタナシウス",
      "kana": "アタナシウス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4世紀のアレクサンドリア司教で、キリストの神性をめぐる論争でニカイア信条を擁護しました。アリウス派が、子であるキリストは父なる神に造られた存在だと考えたのに対し、アタナシウスは父と子は同じ神性を持つと主張しました。皇帝の宗教政策と対立して何度も追放されましたが、著作と教会活動を通して立場を守りました。彼の主張は381年のコンスタンティノープル公会議へ受け継がれ、三位一体教義の確立に重要な役割を果たしました。",
        "whatDid": "アリウス派に反対してキリストは父なる神と同じ神性を持つと主張し、ニカイア信条を守りました。",
        "whyImportant": "キリスト教の三位一体教義と、正統・異端を定める教会の枠組みの形成に影響したためです。"
      },
      "image": "assets/people/person-8a96ecd37a.webp",
      "imageAlt": "アタナシウスの画像"
    },
    "安禄山": {
      "name": "安禄山",
      "kana": "安禄山",
      "era": "交易と王国の時代",
      "genre": [
        "war"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "ソグド系と突厥系の背景を持ち、唐の北東辺境で軍功を重ねた節度使です。玄宗と楊貴妃の信任を得て複数の軍管区を支配しましたが、宰相楊国忠との対立などから755年に反乱を起こし、洛陽を占領して燕の皇帝を名乗りました。反乱軍は長安も陥落させ、玄宗は都を逃れました。安禄山自身は757年に部下に殺されましたが、安史の乱は763年まで続き、多数の死者と社会混乱を生み、唐の中央支配を大きく弱めました。",
        "whatDid": "辺境軍を率いて唐へ反乱を起こし、洛陽と長安を占領して燕の皇帝を名乗りました。",
        "whyImportant": "安史の乱によって唐の人口・財政・中央集権を打撃し、王朝衰退の転換点をつくったためです。"
      },
      "image": "assets/people/person-1da973da9b.webp"
    },
    "ウラジーミル1世": {
      "name": "ウラジーミル1世",
      "kana": "ウラジーミル1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "10世紀末のキエフ大公で、内戦を勝ち抜いてルーシの広い地域を支配しました。東ローマ皇帝バシレイオス2世へ軍事援助を送り、その皇妹アンナとの結婚を実現した後、988年ごろに東方正教会のキリスト教へ改宗しました。キエフの住民にも洗礼を受けさせ、教会建設と聖職者の受け入れを進めました。改宗は政治的統合と国際的地位の向上に役立ち、ルーシの文字文化・美術・宗教をビザンツ世界へ結び付けました。",
        "whatDid": "東方正教会へ改宗し、キエフ・ルーシに洗礼と教会制度を広め、ビザンツ世界との結び付きを強めました。",
        "whyImportant": "ロシア・ウクライナ・ベラルーシへ続く東スラヴ世界の宗教と文化の方向を定めたためです。"
      },
      "image": "assets/people/person-0d144a0281.webp"
    },
    "フアン・セバスティアン・エルカーノ": {
      "name": "フアン・セバスティアン・エルカーノ",
      "kana": "フアン・セバスティアン・エルカーノ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "スペイン北部バスク地方出身の航海者で、マゼラン船団の一員として1519年に出航しました。マゼランがフィリピンで死亡し、船団が大きく損耗した後、「ビクトリア号」の船長として香辛料諸島からインド洋を西へ進みました。ポルトガル勢力を避けながら喜望峰を回り、1522年にスペインへ帰還しました。出航した約270人のうち帰国できたのはわずかでしたが、エルカーノと生存者は史上初の世界一周を完成させ、海洋世界の一体性を実証しました。",
        "whatDid": "マゼラン死後にビクトリア号を率い、香辛料諸島から喜望峰を回って史上初の世界一周を完成させました。",
        "whyImportant": "地球を船で一周できることを実証し、大西洋・太平洋・インド洋がつながる世界像を示したためです。"
      },
      "image": "assets/people/person-3e59131edb.webp"
    },
    "ディドロ": {
      "name": "ディドロ",
      "kana": "ディドロ",
      "era": "革命と工場の時代",
      "genre": [
        "culture",
        "political-scholar"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀フランスの思想家・作家で、ダランベールらとともに『百科全書』の編集を主導しました。この書物は科学、技術、産業、哲学など当時の知識を体系的に集め、職人の作業工程も図版で詳しく紹介しました。教会や王権への批判を含んだため発禁処分や検閲を受けましたが、協力者をまとめて刊行を続けました。知識を一部の権威から解放し、理性と経験によって社会を改善できるという啓蒙思想を広い読者へ伝えました。",
        "whatDid": "『百科全書』を編集し、科学・技術・産業・思想の知識を集めて、検閲下でも刊行を続けました。",
        "whyImportant": "知識を体系化して広く共有し、権威を理性で批判する啓蒙思想を社会へ広めたためです。"
      },
      "image": "assets/people/person-55a6d1017e.webp"
    },
    "イザムバード・キングダム・ブルネル": {
      "name": "イザムバード・キングダム・ブルネル",
      "kana": "イザムバード・キングダム・ブルネル",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの土木技術者で、鉄道・橋・トンネル・蒸気船を大規模に設計しました。グレート・ウェスタン鉄道では高速で安定した運行を目指して広軌を採用し、橋梁や駅も一体的に整備しました。テムズ川の下を通るトンネル工事に参加し、蒸気船「グレート・ウェスタン」「グレート・ブリテン」「グレート・イースタン」では長距離航海に新技術を導入しました。失敗や巨額費用も伴いましたが、産業革命期の交通網を世界規模へ広げました。",
        "whatDid": "鉄道・橋・トンネル・大型蒸気船を設計し、陸上と海上の長距離交通を一体的に発展させました。",
        "whyImportant": "産業革命の土木・造船技術を大型交通網へ結び付け、人と物の移動範囲を世界規模に広げたためです。"
      },
      "image": "assets/people/person-e6f9de6616.webp",
      "imageAlt": "イザムバード・キングダム・ブルネルの画像"
    },
    "メネリク2世": {
      "name": "メネリク2世",
      "kana": "メネリク2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀末から20世紀初めのエチオピア皇帝で、地方勢力を統合しながら領土を南方へ拡大し、現在の国土の基礎を築きました。武器を輸入して軍を近代化し、1896年のアドワの戦いでイタリア軍を破り、エチオピアの独立を守りました。首都アディスアベバを発展させ、鉄道・電信・学校なども導入しました。一方、征服された南部の人々には土地喪失や重い負担が生じました。反植民地勝利の象徴であると同時に、国内征服の問題も持つ皇帝です。",
        "whatDid": "軍を近代化してアドワの戦いでイタリアを破り、エチオピアの独立を守りながら領土を拡大しました。",
        "whyImportant": "アフリカ分割期に欧州軍を破って独立を守り、反植民地主義の象徴となったためです。"
      },
      "image": "assets/people/person-4353807e5f.webp"
    },
    "フランシスコ・フランコ": {
      "name": "フランシスコ・フランコ",
      "kana": "フランシスコ・フランコ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "スペインの軍人で、1936年に左派連合政府へ反乱を起こし、内戦でドイツ・イタリアの支援を受けました。1939年に勝利すると国家元首となり、政党活動、労働運動、地方言語を抑圧し、反対派を処刑・投獄する独裁体制を築きました。第二次世界大戦では正式参戦を避けましたが、枢軸国へ協力しました。冷戦期には反共国家として西側諸国との関係を改善し、1975年に死去するまで統治しました。その後スペインは立憲君主制と民主政治へ移行しました。",
        "whatDid": "内戦で共和国政府を倒し、反対派を弾圧する権威主義的独裁を死去まで続けました。",
        "whyImportant": "20世紀ヨーロッパで長期独裁を維持し、その死後の民主化と歴史記憶の問題を残したためです。"
      },
      "image": "assets/people/person-c77ab10d24.webp"
    },
    "リー・クアンユー": {
      "name": "リー・クアンユー",
      "kana": "リー・クアンユー",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリス植民地時代のシンガポールで弁護士となり、人民行動党を結成して自治と独立を進めました。1959年から首相を務め、マレーシアから分離した1965年以後、外国企業の誘致、住宅建設、教育、港湾整備、汚職対策を進め、都市国家を国際的な貿易・金融拠点へ成長させました。一方、野党や報道への訴訟、集会・言論の制限などで長期的な一党優位を維持しました。経済発展と政治的自由の制約を併せ持つ統治で評価が分かれます。",
        "whatDid": "独立後のシンガポールで工業化・住宅・教育・汚職対策を進め、国際的な都市国家を築きました。",
        "whyImportant": "資源の乏しい小国を急成長させた一方、経済発展と政治的自由の両立をめぐる論点を残したためです。"
      },
      "image": "assets/people/person-185b55fcd2.webp",
      "imageAlt": "リー・クアンユーの画像"
    },
    "メンカウラー": {
      "name": "メンカウラー",
      "kana": "メンカウラー",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第4王朝の王で、紀元前26世紀ごろ、クフ王・カフラー王に続く時期を治めました。ギザに三大ピラミッドのうち最も小さいピラミッドを築き、下部には高価で硬い花崗岩を用いました。王を女神ハトホルや地方神とともに表した三体像が複数残り、王権が各地の神々と結ばれていたことを示します。後世の伝承では温和な王として描かれましたが、同時代資料は限られています。建築と彫刻から古王国の王権を知る人物です。",
        "whatDid": "ギザにピラミッドと葬祭施設を築き、女神や地方神と並ぶ王の三体像を多数残しました。",
        "whyImportant": "建築と彫刻によって、古王国の王が神々や地方社会と結び付く仕組みを伝えるためです。"
      },
      "image": "assets/people/person-b366e7633a.webp",
      "imageAlt": "メンカウラーの画像"
    },
    "リムシン": {
      "name": "リムシン",
      "kana": "リムシン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前19～18世紀、南メソポタミアの都市国家ラルサを長く治めた王です。周辺都市との戦争を重ね、紀元前1794年ごろに宿敵イシンを征服して南部の広い地域を支配しました。運河の開削、神殿建設、農地管理を進め、年名や行政文書に事業を記録しました。しかしバビロン王ハンムラビとの戦いに敗れ、紀元前1763年ごろにラルサは併合されました。その治世は、都市国家間の競争からバビロン統一へ移る直前の政治と経済を示します。",
        "whatDid": "ラルサを拠点にイシンを征服し、運河・神殿・農地を整備して南メソポタミアを広く支配しました。",
        "whyImportant": "ハンムラビによる統一直前の都市国家間競争と、灌漑農業を支える王権の姿を示すためです。"
      },
      "image": "assets/people/person-724f1ef951.webp"
    },
    "ミルティアデス": {
      "name": "ミルティアデス",
      "kana": "ミルティアデス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀初めのアテネ将軍で、かつてトラキアのケルソネソスを支配し、ペルシャ帝国にも従属した経験を持ちました。ペルシャ軍がアッティカ地方へ上陸すると、紀元前490年のマラトンの戦いでアテネ軍を指揮し、重装歩兵を素早く前進させて敵軍を破りました。この勝利はアテネ市民に大きな自信を与えました。しかし翌年のパロス島遠征に失敗し、裁判で罰金を科され、負傷が原因で死亡しました。英雄化だけでなく失敗も残る人物です。",
        "whatDid": "マラトンの戦いでアテネ重装歩兵を指揮してペルシャ軍を破りましたが、後の遠征には失敗しました。",
        "whyImportant": "ペルシャの第一次侵攻を退け、アテネ市民と重装歩兵の政治的自信を高めたためです。"
      },
      "image": "assets/people/person-ede741edb8.webp",
      "imageAlt": "ミルティアデスの画像"
    },
    "ポンペイウス": {
      "name": "ポンペイウス",
      "kana": "ポンペイウス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の将軍で、若くしてスラ側で軍功を立て、「大ポンペイウス」と呼ばれました。地中海の海賊を短期間で制圧し、ミトリダテス戦争を終わらせ、シリアを属州化して東方支配を再編しました。カエサル、クラッススと第一回三頭政治を結びましたが、クラッススの死後にカエサルと対立し、内戦へ入りました。ファルサロスの戦いで敗れてエジプトへ逃れ、上陸直後に殺害されました。軍事的成功と共和政崩壊を結ぶ人物です。",
        "whatDid": "海賊を制圧してローマの東方領土を再編し、三頭政治後はカエサルと内戦を戦いました。",
        "whyImportant": "ローマの地中海支配を広げる一方、将軍同士の権力争いで共和政崩壊を加速させたためです。"
      },
      "image": "assets/people/person-ecc998d997.webp",
      "imageAlt": "ポンペイウスの画像"
    },
    "チャンドラグプタ": {
      "name": "チャンドラグプタ",
      "kana": "チャンドラグプタ",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前4世紀末、ナンダ朝を倒してマウリヤ朝を開いたインドの王です。アレクサンドロス大王の死後に北西インドへ進出し、セレウコス1世との戦いではインダス川周辺の支配を認めさせました。都パータリプトラを中心に官僚・軍隊・徴税制度を整え、北インドの広い地域を統一しました。後世のジャイナ教伝承では晩年に出家したとされます。大帝国の基礎を築き、孫アショーカ王の時代へつなげた人物です。",
        "whatDid": "ナンダ朝を倒してマウリヤ朝を開き、北インドを統一して官僚・軍事・徴税制度を整えました。",
        "whyImportant": "インド最初の大規模な統一帝国を築き、アショーカ王による広域支配の土台をつくったためです。"
      },
      "image": "assets/people/person-60e45e3ac2.webp",
      "imageAlt": "チャンドラグプタの画像"
    },
    "アリウス": {
      "name": "アリウス",
      "kana": "アリウス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4世紀初めのアレクサンドリアの司祭で、父なる神と子であるキリストの関係をめぐって論争を起こしました。アリウスは、キリストは神に造られた特別な存在であり、父なる神と同じ永遠の神性を持たないと説きました。これに対しアタナシウスらは父と子は同じ神性を持つと主張しました。325年のニカイア公会議でアリウスの教えは退けられましたが、各地の王や司教に支持され、論争は長く続きました。",
        "whatDid": "キリストは父なる神に造られた存在だと説き、三位一体をめぐる大論争を引き起こしました。",
        "whyImportant": "キリスト教が正統教義と異端を定め、三位一体の考えを形成するきっかけとなったためです。"
      },
      "image": "assets/people/person-e75a7651c8.webp",
      "imageAlt": "アリウスの画像"
    },
    "杜甫": {
      "name": "杜甫",
      "kana": "杜甫",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "8世紀、唐の繁栄から安史の乱へ向かう時代を生きた詩人です。官僚として国を支えることを望みましたが、十分な地位を得られず、反乱と飢えのなかで家族と各地を流浪しました。戦争で苦しむ民衆、徴兵される若者、自身の貧困や家族への思いを具体的に詠み、その詩は当時の社会を伝える記録にもなっています。形式の整った律詩を高い水準へ発展させ、後世には「詩聖」と呼ばれ、中国詩の模範となりました。",
        "whatDid": "安史の乱や貧困のなかで、戦争に苦しむ民衆と自身の生活を律詩に詠みました。",
        "whyImportant": "唐代社会の現実を詩で伝え、中国文学における社会詩と律詩の最高峰を築いたためです。"
      },
      "image": "assets/people/person-c468737f5e.webp",
      "imageAlt": "杜甫の画像"
    },
    "ヤロスラフ賢公": {
      "name": "ヤロスラフ賢公",
      "kana": "ヤロスラフ賢公",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "11世紀のキエフ大公で、父ウラジーミル1世の死後に兄弟との争いを勝ち抜き、キエフ・ルーシを統一しました。法慣習をまとめた『ルーシ法典』の基礎を整え、キエフに聖ソフィア大聖堂を建て、学校や聖書翻訳を支援しました。娘たちをフランス、ノルウェー、ハンガリーなどの王家へ嫁がせ、ヨーロッパ外交を広げました。一方、死後は領土を息子たちへ分けたため、後の分裂を防げませんでした。",
        "whatDid": "ルーシ法典の基礎を整え、聖ソフィア大聖堂を建て、王家の婚姻で欧州諸国との関係を広げました。",
        "whyImportant": "キエフ・ルーシの法・宗教・文化・外交を発展させ、国家の最盛期を築いたためです。"
      },
      "image": "assets/people/person-728b094372.webp",
      "imageAlt": "ヤロスラフ賢公の画像"
    },
    "アメリゴ・ヴェスプッチ": {
      "name": "アメリゴ・ヴェスプッチ",
      "kana": "アメリゴ・ヴェスプッチ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "フィレンツェ出身の商人・航海者で、15世紀末から16世紀初めにスペインやポルトガルの船で南アメリカ沿岸を航海しました。航海記録では、コロンブスが到達した土地はアジアの一部ではなく、ヨーロッパ人にとって未知の大陸だと考えました。1507年、地理学者ヴァルトゼーミュラーがその記述をもとに地図へ「アメリカ」と記しました。ただし本人の航海回数や書簡の真偽には議論があり、地名成立の経緯を慎重に見る必要があります。",
        "whatDid": "南アメリカ沿岸を航海し、到達地をアジアではなく未知の大陸だと説明しました。",
        "whyImportant": "新大陸を独立した大陸として認識する動きを促し、「アメリカ」という地名の由来となったためです。"
      },
      "image": "assets/people/person-8d1e003d26.webp",
      "imageAlt": "アメリゴ・ヴェスプッチの画像"
    },
    "メアリ・ウルストンクラフト": {
      "name": "メアリ・ウルストンクラフト",
      "kana": "メアリ・ウルストンクラフト",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀イギリスの著述家で、女性が十分な教育を受けられず、結婚と家庭に従属させられていた社会を批判しました。『女性の権利の擁護』で、女性も男性と同じ理性を持つため、教育・職業・市民としての権利を得るべきだと主張しました。女性を飾りや妻としてではなく、自立した人格として育てる必要を説きました。私生活への攻撃で長く評価を下げられましたが、近代女性解放思想とフェミニズムの先駆者として再評価されています。",
        "whatDid": "『女性の権利の擁護』を著し、女性にも教育・職業・市民としての権利が必要だと訴えました。",
        "whyImportant": "男女の理性的平等を主張し、近代の女性解放運動とフェミニズムの思想的基礎を築いたためです。"
      },
      "image": "assets/people/person-b6d9da0629.webp",
      "imageAlt": "メアリ・ウルストンクラフトの画像"
    },
    "ロバート・フルトン": {
      "name": "ロバート・フルトン",
      "kana": "ロバート・フルトン",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "アメリカの発明家・技術者で、絵画を学んだ後、運河や潜水艇、蒸気船の開発に取り組みました。蒸気船そのものの発明者ではありませんが、1807年に「ノース・リバー・スチームボート」をハドソン川で定期運航し、蒸気船が商業輸送に使えることを示しました。風向きに左右されず、川を上流へ進める船は人や農産物の輸送を速めました。その成功により、アメリカやヨーロッパで河川・沿岸交通の蒸気化が急速に広がりました。",
        "whatDid": "蒸気船を改良してハドソン川で定期商業運航を成功させ、河川交通へ普及させました。",
        "whyImportant": "風に頼らない定期水運を実用化し、産業革命期の物流と地域間交流を速めたためです。"
      },
      "image": "assets/people/person-7c916ac45b.webp",
      "imageAlt": "ロバート・フルトンの画像"
    },
    "テオドロス2世": {
      "name": "テオドロス2世",
      "kana": "テオドロス2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀のエチオピア皇帝で、地方諸侯が分立していた「諸侯の時代」を終わらせ、国家を再統一しようとしました。常備軍の整備、武器製造、行政の中央集権化を進め、教会や有力者の力を抑えました。しかし改革は重い徴税と軍事動員を伴い、反乱が相次ぎました。イギリス政府への書簡が無視されたことから外交官らを拘束し、1868年にイギリス軍の遠征を招きました。山城メクダラが陥落すると自害し、近代化の先駆者と強権的支配者の両面を残しました。",
        "whatDid": "地方諸侯を抑えてエチオピア再統一と軍事・行政改革を進めましたが、英国遠征軍に敗れて自害しました。",
        "whyImportant": "分裂したエチオピアの中央集権化と近代化を試み、後の国家再建の出発点となったためです。"
      },
      "image": "assets/people/person-db50e4e512.webp",
      "imageAlt": "テオドロス2世の画像"
    },
    "シャルル・ド・ゴール": {
      "name": "シャルル・ド・ゴール",
      "kana": "シャルル・ド・ゴール",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの軍人・政治家で、第二次世界大戦中に本国政府がドイツへ降伏するとロンドンから抗戦を呼びかけ、自由フランスを率いました。解放後の臨時政府を指導し、1958年のアルジェリア危機では政権へ復帰して第五共和政を成立させ、大統領権限を強めました。最終的にアルジェリア独立を認め、核戦力を整えてアメリカと距離を置く自主外交を進めました。戦時抵抗と強い大統領制を通して現代フランスの国家像を形づくりました。",
        "whatDid": "自由フランスを率いて対独抗戦を続け、第五共和政を創設して強い大統領制と自主外交を進めました。",
        "whyImportant": "第二次世界大戦後のフランス再建と、現在まで続く第五共和政の政治制度を築いたためです。"
      },
      "image": "assets/people/person-f4a709fcd6.webp",
      "imageAlt": "シャルル・ド・ゴールの画像"
    },
    "ダライ・ラマ14世": {
      "name": "ダライ・ラマ14世",
      "kana": "ダライ・ラマ14世",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "religion",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "チベット仏教ゲルク派の最高指導者とされ、幼少期に第13世ダライ・ラマの転生者として認定されました。中国人民解放軍の進駐後、1959年のチベット蜂起を機にインドへ亡命し、ダラムサラで亡命政府を指導しました。チベット文化と宗教の保護、自治の実現を非暴力で訴え、1989年にノーベル平和賞を受賞しました。2011年には政治的権限を選挙で選ばれた指導者へ移しましたが、転生制度やチベットの地位をめぐり中国政府との対立が続いています。",
        "whatDid": "亡命後もチベット文化と宗教を守り、中国支配下での自治を非暴力で求め続けました。",
        "whyImportant": "民族・宗教・自治をめぐる問題を国際社会へ伝え、非暴力運動の象徴となったためです。"
      },
      "image": "assets/people/person-144a0ad2f0.webp",
      "imageAlt": "ダライ・ラマ14世の画像"
    },
    "ペピ2世": {
      "name": "ペピ2世",
      "kana": "ペピ2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第6王朝の王で、幼くして即位し、非常に長く統治したと伝えられますが、在位90年以上という記録には議論があります。治世中にはヌビア方面への遠征や交易が続き、神殿へ税や労役の免除を与える勅令も残りました。長い治世の後半には地方長官が世襲化して力を強め、中央政府の統制が弱まりました。ただし古王国崩壊は王の長寿だけでなく、地方分権、気候変動、財政など複数の原因から考える必要があります。",
        "whatDid": "長期統治のもとで遠征と神殿保護を続けましたが、地方長官の自立が進み中央支配が弱まりました。",
        "whyImportant": "古王国末期の地方分権と中央政府の衰退を考える中心人物であり、崩壊原因の複雑さを示すためです。"
      },
      "image": "assets/people/person-71387cd266.webp",
      "imageAlt": "ペピ2世の画像"
    },
    "シャムシアダド1世": {
      "name": "シャムシアダド1世",
      "kana": "シャムシアダド1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前19～18世紀に北メソポタミアで勢力を広げたアムル系の王です。アッシュル、マリ、シェフナなどを支配し、都をシュバト・エンリルに置いて広域王国を築きました。二人の息子を重要都市の王に任じ、手紙で軍事、外交、農業、役人の行動まで細かく指示しました。マリ文書からは、同盟・婚姻・情報収集を組み合わせた統治が分かります。死後、王国は急速に崩れましたが、アッシリアが広域国家へ成長する前段階を示します。",
        "whatDid": "北メソポタミアの都市を征服し、息子と役人へ詳細な指示を送りながら広域王国を統治しました。",
        "whyImportant": "マリ文書を通して、古代国家の軍事・外交・行政と初期アッシリア支配の実態を知ることができるためです。"
      },
      "image": "assets/people/person-dd2e481457.webp",
      "imageAlt": "シャムシアダド1世の画像"
    },
    "アルキビアデス": {
      "name": "アルキビアデス",
      "kana": "アルキビアデス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネ政治家・将軍で、名門出身の才能ある演説家として民衆の支持を集めました。ペロポネソス戦争中、シチリア遠征を強く提案しましたが、宗教像破壊事件への関与を疑われ、裁判を避けて敵国スパルタへ亡命しました。その後はペルシャや再びアテネ側へ移り、立場を変えながら軍事行動に関わりました。大胆な戦略と個人的野心はアテネの政策を大きく動かしましたが、シチリア遠征の壊滅など国を深刻な危機へ導きました。",
        "whatDid": "シチリア遠征を提案し、告発後はスパルタやペルシャへ移りながら各勢力の軍事・外交に関わりました。",
        "whyImportant": "民主政下の人気政治家の野心と変節が、アテネの戦争政策と敗北を左右したためです。"
      },
      "image": "assets/people/person-f6ad9aafd6.webp",
      "imageAlt": "アルキビアデスの画像"
    },
    "クラッスス": {
      "name": "クラッスス",
      "kana": "クラッスス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家・将軍で、土地や奴隷への投資によって巨万の富を築きました。スパルタクスの反乱を鎮圧して軍事的名声を得た後、カエサル、ポンペイウスと第一回三頭政治を結び、政治的影響力を強めました。さらに栄誉を求めてシリア属州からパルティアへ遠征しましたが、紀元前53年のカルラエの戦いで騎馬軍に大敗し、自身も殺されました。彼の死で三頭政治の均衡が崩れ、カエサルとポンペイウスの内戦が近づきました。",
        "whatDid": "スパルタクスの反乱を鎮圧し、三頭政治を結びましたが、パルティア遠征で大敗して死亡しました。",
        "whyImportant": "莫大な富と軍事力が共和政政治を動かし、その死がカエサルとポンペイウスの内戦を促したためです。"
      },
      "image": "assets/people/person-5f4fa51d15.webp",
      "imageAlt": "クラッススの画像"
    },
    "カウティリヤ": {
      "name": "カウティリヤ",
      "kana": "カウティリヤ",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "古代インドの政治思想家で、チャンドラグプタの宰相チャーナキヤと同一人物とされますが、詳しい生涯や著作年代には議論があります。彼に帰される『実利論』は、王の教育、官僚制度、税、農業、外交、諜報、戦争まで国家運営を体系的に説明しました。理想だけでなく、同盟、策略、情報活動を現実的に使う政治を説いています。現在の本文は後世に編集された可能性がありますが、古代インドの国家観とマウリヤ朝の統治を考える重要資料です。",
        "whatDid": "『実利論』で官僚制・税・外交・諜報・戦争を含む現実的な国家統治の方法を体系化しました。",
        "whyImportant": "古代インドの政治思想と行政制度を詳しく伝え、国家運営を現実的に論じた代表的著作を残したためです。"
      },
      "image": "assets/people/person-bfba8d3268.webp",
      "imageAlt": "カウティリヤの画像"
    },
    "ベネディクトゥス": {
      "name": "ベネディクトゥス",
      "kana": "ベネディクトゥス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "5～6世紀のイタリアの修道士で、ローマでの学びを離れて隠修生活を送り、のちにモンテ・カッシーノ修道院を築きました。共同生活の規則をまとめた『ベネディクトゥス戒律』では、修道院長への服従、祈り、読書、労働、節度ある生活を重んじました。極端な苦行ではなく、共同体の秩序と日々の生活を両立させた点に特徴があります。この戒律は中世西ヨーロッパの多くの修道院で採用され、農業、教育、写本保存、救貧活動の基盤となりました。",
        "whatDid": "モンテ・カッシーノ修道院を築き、祈り・読書・労働を組み合わせた修道生活の戒律を定めました。",
        "whyImportant": "西ヨーロッパの修道院制度を形づくり、学問・農業・写本・救貧を支える共同体を広めたためです。"
      },
      "image": "assets/people/person-fd893ccc0c.webp",
      "imageAlt": "ベネディクトゥスの画像"
    },
    "李白": {
      "name": "李白",
      "kana": "李白",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "8世紀の唐を代表する詩人で、各地を旅しながら自然、友情、酒、別れ、自由への思いを大胆な言葉で詠みました。玄宗の宮廷に招かれましたが、官僚として長く仕えることはできず、再び各地を放浪しました。安史の乱では永王の軍に加わったため流罪を命じられましたが、途中で赦免されました。豊かな想像力と伸びやかな表現から「詩仙」と呼ばれ、杜甫と並んで中国詩の最高峰とされ、日本を含む東アジアの文学へ長く影響しました。",
        "whatDid": "旅の生活を通して自然・友情・酒・自由を豊かな想像力で詠み、唐詩を代表する作品を残しました。",
        "whyImportant": "自由で壮大な詩風を完成させ、中国と東アジアの文学表現に長く影響したためです。"
      },
      "image": "assets/people/person-10331154cc.webp",
      "imageAlt": "李白の画像"
    },
    "ウィリアム1世": {
      "name": "ウィリアム1世",
      "kana": "ウィリアム1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "フランス北部ノルマンディー公で、イングランド王エドワード懺悔王の死後、自らに王位継承権があると主張しました。1066年に海を渡り、ヘイスティングズの戦いでハロルド2世を破ってイングランド王となりました。各地の反乱を鎮圧し、ノルマン人へ土地を与えて封建的支配を築きました。全国の土地・人口・家畜・財産を調査した『ドゥームズデイ・ブック』を作らせ、課税と統治に利用しました。征服は言語・貴族・政治制度を大きく変えました。",
        "whatDid": "1066年にイングランドを征服し、ノルマン人による土地支配と全国的な土地調査を進めました。",
        "whyImportant": "ノルマン征服によってイングランドの王権・貴族・言語・土地制度を大きく変えたためです。"
      },
      "image": "assets/people/person-2f3074da59.webp",
      "imageAlt": "ウィリアム1世の画像"
    },
    "ジョン・カボット": {
      "name": "ジョン・カボット",
      "kana": "ジョン・カボット",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "イタリア出身の航海者で、イングランド王ヘンリー7世の許可を受け、西回りでアジアへ向かう航路を探しました。1497年にブリストルを出航し、北大西洋を渡って現在のカナダ東岸付近へ到達したと考えられます。到達地点はニューファンドランド島かラブラドル地方か確定していません。航海はアジアへの道を発見できませんでしたが、後にイングランドが北アメリカへの領有権を主張する根拠として利用されました。翌年の航海の最期は不明です。",
        "whatDid": "イングランド王の支援で北大西洋を渡り、現在のカナダ東岸付近へ到達しました。",
        "whyImportant": "イングランドによる北アメリカ進出と、後の植民地領有主張の出発点となったためです。"
      },
      "image": "assets/people/person-13c548e36d.webp",
      "imageAlt": "ジョン・カボットの画像"
    },
    "アダム・スミス": {
      "name": "アダム・スミス",
      "kana": "アダム・スミス",
      "era": "革命と工場の時代",
      "genre": [
        "political-scholar"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "18世紀スコットランドの道徳哲学者で、人間の共感と市場経済の仕組みを研究しました。『道徳感情論』では、人は他者の立場を想像する共感によって行動を判断すると論じました。『国富論』では分業が生産力を高め、個人の利益追求が競争と交換を通して社会全体の富へ結び付く仕組みを説明しました。ただし政府不要を唱えたのではなく、国防、司法、公共事業、教育の役割も認めました。近代経済学の基礎を築いた人物です。",
        "whatDid": "『国富論』で分業・交換・競争が富を生む仕組みを説明し、政府の公共的役割も論じました。",
        "whyImportant": "市場経済を体系的に分析し、近代経済学と自由主義的経済政策の基礎を築いたためです。"
      },
      "image": "assets/people/person-632d4656c3.webp",
      "imageAlt": "アダム・スミスの画像"
    },
    "アレッサンドロ・ボルタ": {
      "name": "アレッサンドロ・ボルタ",
      "kana": "アレッサンドロ・ボルタ",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18～19世紀のイタリアの物理学者で、電気現象を実験的に研究しました。ガルヴァーニが動物の筋肉に固有の電気があると考えたのに対し、ボルタは異なる金属と液体の接触で電気が生じると考えました。1800年、亜鉛と銅の円板を塩水を含む布と交互に重ねた「ボルタ電池」を発表し、連続して電流を得ることに成功しました。この電池によって電気分解や電磁気の研究が進み、電圧の単位「ボルト」に名が残りました。",
        "whatDid": "亜鉛・銅・電解液を重ねたボルタ電池を発明し、連続した電流を取り出せるようにしました。",
        "whyImportant": "安定した電流源をつくり、電気化学・電磁気学・電池技術の発展を可能にしたためです。"
      },
      "image": "assets/people/person-fadfe26fcd.webp",
      "imageAlt": "アレッサンドロ・ボルタの画像"
    },
    "サモリ・トゥーレ": {
      "name": "サモリ・トゥーレ",
      "kana": "サモリ・トゥーレ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半、西アフリカで商人・軍事指導者から台頭し、現在のギニア、マリ、コートジボワール周辺にワスル帝国を築きました。イスラムを統治の柱にし、近代的な銃器を備えた軍と行政組織を整えました。フランスの植民地拡大に対して長年抵抗し、退却時には物資を敵へ渡さない焦土戦術も用いました。そのため住民へ大きな負担と移住を強いました。1898年に捕らえられ、反植民地抵抗の英雄と強権的征服者の両面で評価されています。",
        "whatDid": "ワスル帝国を築き、銃器を備えた軍でフランスの西アフリカ植民地化へ長く抵抗しました。",
        "whyImportant": "アフリカ分割に対する大規模な武力抵抗を指導し、植民地征服の過程を示すためです。"
      },
      "image": "assets/people/person-06bdb26b39.webp",
      "imageAlt": "サモリ・トゥーレの画像"
    },
    "フィリップ・ペタン": {
      "name": "フィリップ・ペタン",
      "kana": "フィリップ・ペタン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの軍人で、第一次世界大戦のヴェルダンの戦いで防衛を指揮し、国民的英雄となりました。しかし第二次世界大戦でドイツ軍が侵攻すると、1940年に首相となって休戦を選び、ヴィシー政権の国家元首となりました。議会制を廃止し、ドイツへの協力、反対派の弾圧、ユダヤ人の登録・逮捕・移送に加担しました。解放後に反逆罪で死刑判決を受け、終身刑へ減刑されました。戦争英雄が占領協力者へ転じた人物として記憶されています。",
        "whatDid": "第一次世界大戦でヴェルダンを守りましたが、第二次世界大戦ではヴィシー政権を率いてナチスへ協力しました。",
        "whyImportant": "占領下の協力政治と、国家指導者が人権侵害へ加担する責任を考える中心人物だからです。"
      },
      "image": "assets/people/person-110c05dd70.webp",
      "imageAlt": "フィリップ・ペタンの画像"
    },
    "デズモンド・ツツ": {
      "name": "デズモンド・ツツ",
      "kana": "デズモンド・ツツ",
      "era": "今につながる時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "南アフリカの聖公会聖職者で、教師を経て司祭となり、黒人として初めて南アフリカ聖公会の大主教に就きました。アパルトヘイトに対し、説教や国際的な経済制裁の呼びかけによって非暴力で反対し、1984年にノーベル平和賞を受賞しました。民主化後は真実和解委員会の委員長として、過去の殺害・拷問・誘拐を公に証言させ、条件付き恩赦と被害者の尊厳回復を進めました。報復ではなく正義と和解を両立させようとした人物です。",
        "whatDid": "非暴力でアパルトヘイトへ反対し、民主化後は真実和解委員会を率いて人権侵害を調査しました。",
        "whyImportant": "差別撤廃と、独裁後の社会で真相究明・赦し・責任を結ぶ和解の方法を示したためです。"
      },
      "image": "assets/people/person-3a9c424ead.webp",
      "imageAlt": "デズモンド・ツツの画像"
    },
    "メントゥホテプ2世": {
      "name": "メントゥホテプ2世",
      "kana": "メントゥホテプ2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前21世紀ごろ、上エジプトのテーベを拠点にした第11王朝の王です。当時のエジプトは北部と南部に分裂していましたが、北部のヘラクレオポリス勢力を破り、全土を再統一しました。これにより第一中間期が終わり、中王国時代が始まりました。テーベ西岸のデイル・アル＝バハリに、山の地形を利用した葬祭殿を建設し、地方長官を統制して中央王権を立て直しました。統一後の行政・交易・建築の再出発を導いた王です。",
        "whatDid": "テーベから北部勢力を破ってエジプトを再統一し、中央王権と行政を立て直しました。",
        "whyImportant": "第一中間期の分裂を終わらせ、中王国という新しい安定と繁栄の時代を開いたためです。"
      },
      "image": "assets/people/person-2bba3388e0.webp",
      "imageAlt": "メントゥホテプ2世の画像"
    },
    "ジムリリム": {
      "name": "ジムリリム",
      "kana": "ジムリリム",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前18世紀、ユーフラテス川中流の都市国家マリを治めた王です。父の王国を奪ったシャムシアダド1世の死後、バビロン王ハンムラビの支援を受けて王位を回復しました。周辺諸国と同盟・婚姻・贈答を結び、宮殿から役人や将軍へ多数の手紙を送り、軍事・交易・灌漑を細かく管理しました。しかし後にハンムラビと対立し、マリは征服されました。宮殿文書は古代オリエント外交と日常行政を詳しく伝えます。",
        "whatDid": "王位を回復してマリを統治し、同盟・婚姻・書簡を用いて軍事・外交・灌漑を管理しました。",
        "whyImportant": "マリ文書を通して、古代オリエントの外交・行政・交易・宮廷生活を具体的に知ることができるためです。"
      },
      "image": "assets/people/person-9392e4eca4.webp",
      "imageAlt": "ジムリリムの画像"
    },
    "ソクラテス": {
      "name": "ソクラテス",
      "kana": "ソクラテス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネで活動した哲学者で、自ら著作を残さず、対話を通して相手の思い込みを問い直しました。「正義とは何か」「よく生きるとは何か」を質問し、無知を自覚して考え続けることを重んじました。ペロポネソス戦争後の政治不安のなかで、若者を堕落させ、国家の神々を認めないとして裁判にかけられ、死刑判決を受けました。逃亡を勧められても法に従い、毒杯を飲んだ姿は哲学と市民社会の関係を考える象徴となりました。",
        "whatDid": "対話と問いかけで人々の思い込みを検討し、正義・徳・よく生きることを考えさせました。",
        "whyImportant": "答えを教えるより問いを深める哲学的方法を示し、西洋哲学と倫理思想の出発点となったためです。"
      },
      "image": "assets/people/person-4e6d38825c.webp",
      "imageAlt": "ソクラテスの画像"
    },
    "キケロ": {
      "name": "キケロ",
      "kana": "キケロ",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家・弁論家で、騎士身分の家から執政官まで上りつめました。カティリナの陰謀を弾劾して共和政を守ろうとし、内戦ではポンペイウス側に立ちました。カエサル暗殺後はアントニウスを激しく批判しましたが、第二回三頭政治の追放者名簿に載せられ殺害されました。『国家論』『義務について』などでギリシャ哲学をラテン語で紹介し、自然法、共和政、市民の義務を論じました。その文章は後世の政治思想とラテン文学の模範となりました。",
        "whatDid": "演説で共和政を擁護し、著作を通してギリシャ哲学・自然法・市民の義務をローマ世界へ伝えました。",
        "whyImportant": "共和政理念と自然法思想を後世へ伝え、ヨーロッパの政治思想・法・ラテン文学へ影響したためです。"
      },
      "image": "assets/people/person-0ee4733742.webp",
      "imageAlt": "キケロの画像"
    },
    "カニシカ王": {
      "name": "カニシカ王",
      "kana": "カニシカ王",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "2世紀ごろのクシャーナ朝の王で、中央アジアから北インドにかけて広い領域を支配しました。シルクロード交易の要地を押さえ、ローマ・イラン・インド・中国を結ぶ人と物の交流を活発にしました。仏教を保護した王として知られ、ガンダーラやマトゥラーでは仏像制作が発展しました。第四回仏典結集を開いたという伝承もありますが、史実性には議論があります。多文化が交わる帝国と大乗仏教の広がりを象徴する人物です。",
        "whatDid": "中央アジアと北インドを支配し、シルクロード交易と仏教文化を保護しました。",
        "whyImportant": "東西交易を結ぶ多民族帝国を築き、大乗仏教と仏像文化の広がりを支えたためです。"
      },
      "image": "assets/people/person-9bc6eb495f.webp",
      "imageAlt": "カニシカ王の画像"
    },
    "グレゴリウス1世": {
      "name": "グレゴリウス1世",
      "kana": "グレゴリウス1世",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "6世紀末のローマ教皇で、元ローマ市長から修道士となり、590年に教皇へ選ばれました。西ローマ帝国崩壊後の混乱のなか、飢饉救済や教会財産の管理を進め、ランゴバルド人との交渉も行いました。イングランドへ宣教師アウグスティヌスを派遣し、キリスト教布教を後押ししました。聖職者の務めを説いた『司牧規則』は中世教会の指針となりました。後世にグレゴリオ聖歌と結び付けられますが、本人が体系を直接作ったとは言い切れません。",
        "whatDid": "救貧・教会行政・外交を進め、宣教師をイングランドへ派遣して西欧教会の基盤を整えました。",
        "whyImportant": "ローマ教皇の宗教的・社会的・政治的役割を強め、中世西ヨーロッパ教会の形を整えたためです。"
      },
      "image": "assets/people/person-0f47b6e160.webp",
      "imageAlt": "グレゴリウス1世の画像"
    },
    "白居易": {
      "name": "白居易",
      "kana": "白居易",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "9世紀の唐で官僚として働きながら、多くの詩を残した詩人です。難しい表現を避け、政治の失敗、重い税、戦争、貧しい人々の暮らしを分かりやすい言葉で詠みました。代表作『長恨歌』では玄宗と楊貴妃の悲劇を描き、『新楽府』では社会問題を批判しました。官僚として左遷も経験しましたが、地方行政にも携わりました。その詩は中国だけでなく平安時代の日本でも広く読まれ、『源氏物語』などの文学へ強い影響を与えました。",
        "whatDid": "社会問題を平易な詩で批判し、『長恨歌』『新楽府』などを残して唐詩を広い読者へ開きました。",
        "whyImportant": "文学で政治と民衆生活を結び、中国と日本の詩文・物語文学へ長く影響したためです。"
      },
      "image": "assets/people/person-97745cc5b2.webp",
      "imageAlt": "白居易の画像"
    },
    "エレノア・オブ・アキテーヌ": {
      "name": "エレノア・オブ・アキテーヌ",
      "kana": "エレノア・オブ・アキテーヌ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "12世紀西ヨーロッパの大領主で、広大なアキテーヌ公領の相続人としてフランス王ルイ7世と結婚し、第2回十字軍にも同行しました。離婚後はイングランド王ヘンリー2世と再婚し、リチャード1世やジョン王の母となりました。夫に反乱した息子たちを支援して幽閉されましたが、リチャード即位後は摂政として王国を支え、身代金交渉や政治調整を担いました。女性が土地相続・婚姻・宮廷外交を通じて大きな政治力を持った例です。",
        "whatDid": "二つの王家の王妃となり、領地・婚姻・摂政を通じてフランスとイングランドの政治へ影響しました。",
        "whyImportant": "中世ヨーロッパで女性領主が広大な領地と王家の婚姻を通じて国際政治を動かしたためです。"
      },
      "image": "assets/people/person-467fb1f1dd.webp",
      "imageAlt": "エレノア・オブ・アキテーヌの画像"
    },
    "ジャック・カルティエ": {
      "name": "ジャック・カルティエ",
      "kana": "ジャック・カルティエ",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "16世紀フランスの航海者で、フランス王フランソワ1世の命を受け、北アメリカ北東部を三度航海しました。セントローレンス湾と川を調査し、現在のケベックやモントリオール周辺へ到達しました。現地のイロコイ系住民から聞いた「カナタ」という言葉を地域名として記録し、後のカナダという国名につながりました。一方、先住民の首長らをフランスへ連れ去り、植民地建設を試みました。探検記録はフランスの北米進出の基礎となりました。",
        "whatDid": "セントローレンス湾と川を探検し、フランスによる北アメリカ進出の地理的基礎をつくりました。",
        "whyImportant": "後のカナダ植民地形成へつながる航路と地域情報をフランスへもたらしたためです。"
      },
      "image": "assets/people/person-0a7750b30b.webp",
      "imageAlt": "ジャック・カルティエの画像"
    },
    "トマス・ペイン": {
      "name": "トマス・ペイン",
      "kana": "トマス・ペイン",
      "era": "革命と工場の時代",
      "genre": [
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "イギリス生まれで北アメリカへ渡り、革命期の政治著述家となりました。1776年の『コモン・センス』で、世襲君主制を批判し、植民地がイギリスから独立する必要を平易な言葉で訴えました。フランス革命期には『人間の権利』で共和政と社会保障を支持し、王政批判のためイギリスで反逆罪に問われました。『理性の時代』では教会制度を批判し、理性に基づく信仰を主張しました。政治思想を一般市民へ広げた人物です。",
        "whatDid": "『コモン・センス』『人間の権利』で独立・共和政・人民の権利を平易な文章で訴えました。",
        "whyImportant": "革命思想を知識人だけでなく一般市民へ広げ、アメリカ独立と近代民主主義を後押ししたためです。"
      },
      "image": "assets/people/person-cf92a1d76a.webp",
      "imageAlt": "トマス・ペインの画像"
    },
    "マイケル・ファラデー": {
      "name": "マイケル・ファラデー",
      "kana": "マイケル・ファラデー",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの科学者で、製本職人の見習いから独学で科学を学び、王立研究所で実験を重ねました。電流が磁場を生む現象を調べ、磁石を動かすことで電流が生じる電磁誘導を1831年に発見しました。この原理は発電機や変圧器の基礎です。また、電気分解の法則を示し、電場・磁場を「力線」として捉えました。高度な数学より実験を重視し、子ども向け講演も行いました。電気を産業と生活へ結び付ける基礎を築いた人物です。",
        "whatDid": "電磁誘導と電気分解の法則を発見し、発電機・変圧器・電気化学の基礎を築きました。",
        "whyImportant": "電気を連続的に生み出して利用する原理を明らかにし、電力社会の成立を可能にしたためです。"
      },
      "image": "assets/people/person-2b988d0d7b.webp",
      "imageAlt": "マイケル・ファラデーの画像"
    },
    "ムハンマド・アフマド": {
      "name": "ムハンマド・アフマド",
      "kana": "ムハンマド・アフマド",
      "era": "世界が深く結びついた時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀スーダンの宗教指導者で、エジプトとイギリスの支配、重税、奴隷貿易規制への不満が高まるなか、自らをイスラム世界を正す救世主「マフディー」と宣言しました。各地の部族と信者をまとめ、1885年にハルツームを攻略して英軍のゴードン将軍を破りました。その直後に病死しましたが、後継者はマフディー国家を維持しました。運動は宗教改革、反植民地抵抗、地域権力の形成が結び付いた出来事として重要です。",
        "whatDid": "自らをマフディーと宣言して反乱を率い、ハルツームを攻略してスーダンに独立政権を築きました。",
        "whyImportant": "宗教運動と反植民地抵抗を結び付け、アフリカで欧州支配へ大きな打撃を与えたためです。"
      },
      "image": "assets/people/person-643753188b.webp",
      "imageAlt": "ムハンマド・アフマドの画像"
    },
    "ヨシップ・ブロズ・チトー": {
      "name": "ヨシップ・ブロズ・チトー",
      "kana": "ヨシップ・ブロズ・チトー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "クロアチア出身の共産主義者で、第二次世界大戦中にパルチザンを率い、ナチス・ドイツとその協力政権へ抵抗しました。戦後はユーゴスラビア社会主義連邦を築き、複数の民族と共和国を連邦制でまとめました。1948年にスターリンと決裂し、ソ連から独立した社会主義路線を取り、インドやエジプトと非同盟運動を進めました。一方、一党支配と秘密警察による弾圧も行いました。死後、経済危機と民族対立が表面化し、連邦は解体へ向かいました。",
        "whatDid": "反ナチ抵抗を率い、戦後は多民族連邦を統治し、ソ連から自立した社会主義と非同盟外交を進めました。",
        "whyImportant": "冷戦下で米ソどちらにも従わない路線を示し、多民族国家統合の成功と限界を残したためです。"
      },
      "image": "assets/people/person-534caeb458.webp",
      "imageAlt": "ヨシップ・ブロズ・チトーの画像"
    },
    "パトリス・ルムンバ": {
      "name": "パトリス・ルムンバ",
      "kana": "パトリス・ルムンバ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ベルギー領コンゴの独立運動を率いた政治家で、民族を越えた統一国家を目指しました。1960年の独立時に初代首相となり、式典では植民地支配の暴力と差別を公然と批判しました。独立直後、軍の反乱とカタンガ州の分離が起こり、国連やアメリカ、ベルギー、ソ連が関与する危機へ発展しました。大統領と対立して解任され、モブツのクーデター後に拘束され、1961年に殺害されました。冷戦と植民地権益がアフリカ独立を左右した象徴です。",
        "whatDid": "コンゴ独立運動を率いて初代首相となり、植民地支配を批判して統一国家を目指しました。",
        "whyImportant": "アフリカ独立が旧宗主国の利害と冷戦介入によって妨げられた過程を象徴するためです。"
      },
      "image": "assets/people/person-e7e80e4d08.webp",
      "imageAlt": "パトリス・ルムンバの画像"
    },
    "アメンエムハト1世": {
      "name": "アメンエムハト1世",
      "kana": "アメンエムハト1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前20世紀初め、中王国時代のエジプト第12王朝を開いた王です。前王朝の宰相から王位へ上った可能性があり、都をテーベから北のイチ・タウイへ移して上下エジプトを統治しやすくしました。地方長官の力を抑え、東部国境に防衛施設を築き、ヌビアへの進出を進めました。息子センウセレト1世を共同統治者にしたと考えられ、王位継承の安定を図りました。暗殺されたという文学作品も残り、中央集権再建の姿を伝えます。",
        "whatDid": "第12王朝を開き、都を移して地方勢力を抑え、国境防衛と共同統治で中央集権を強めました。",
        "whyImportant": "中王国エジプトの安定した官僚国家と王位継承制度の基礎を築いたためです。"
      },
      "image": "assets/people/person-7ebbc36049.webp",
      "imageAlt": "アメンエムハト1世の画像"
    },
    "ティグラトピレセル1世": {
      "name": "ティグラトピレセル1世",
      "kana": "ティグラトピレセル1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前12～11世紀の中アッシリア王で、周辺諸国が混乱するなか、北メソポタミアからシリア方面へ遠征を重ねました。ヒッタイト系諸勢力やアラム人と戦い、地中海沿岸へ到達したことを碑文に記しました。征服地から貢納を集め、狩猟や建築事業も王の力として宣伝しました。しかし死後はアラム人の進出などで領土が縮小しました。後の新アッシリア帝国に見られる遠征・貢納・王碑文による支配の先例をつくった王です。",
        "whatDid": "シリア・アナトリア方面へ遠征し、貢納と王碑文によって中アッシリアの勢力を広げました。",
        "whyImportant": "後の新アッシリア帝国へつながる軍事遠征・貢納・王権宣伝の型を示したためです。"
      },
      "image": "assets/people/person-1ea4d9c270.webp",
      "imageAlt": "ティグラトピレセル1世の画像"
    },
    "プラトン": {
      "name": "プラトン",
      "kana": "プラトン",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前4世紀のアテネ哲学者で、ソクラテスの弟子としてその対話と処刑を記録しました。目に見える個々の物の背後に、変わらない本質である「イデア」があると考え、知識・正義・魂・国家を対話篇で論じました。『国家』では哲学を学んだ者が統治する理想国家を描きましたが、民主政への不信も示しました。アテネ郊外に学園アカデメイアを開き、数学と哲学を教育しました。その思想は西洋哲学・宗教・政治論の基礎となりました。",
        "whatDid": "対話篇でイデア・正義・魂・理想国家を論じ、学園アカデメイアを開いて哲学教育を行いました。",
        "whyImportant": "哲学を体系的な学問へ発展させ、西洋の形而上学・倫理・政治思想の基礎を築いたためです。"
      },
      "image": "assets/people/person-9f131c1d44.webp",
      "imageAlt": "プラトンの画像"
    },
    "ブルートゥス": {
      "name": "ブルートゥス",
      "kana": "ブルートゥス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "共和政ローマ末期の政治家で、名門ユニウス氏の出身です。内戦ではポンペイウス側に立ちましたが、敗北後にカエサルから許され、要職へ取り立てられました。しかしカエサルが終身独裁官となると共和政が失われると考え、紀元前44年の暗殺計画に参加しました。暗殺後は民衆の支持を得られず、アントニウスとオクタウィアヌスの軍にフィリッピの戦いで敗れ、自害しました。暴政への抵抗と政治的暗殺の是非を考える人物です。",
        "whatDid": "共和政を守るためカエサル暗殺に加わりましたが、内戦で敗れて自害しました。",
        "whyImportant": "独裁への抵抗が共和政回復ではなく新たな内戦と帝政成立へつながった矛盾を示すためです。"
      },
      "image": "assets/people/person-82b0cb9417.webp",
      "imageAlt": "ブルートゥスの画像"
    },
    "チャンドラグプタ2世": {
      "name": "チャンドラグプタ2世",
      "kana": "チャンドラグプタ2世",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "4～5世紀のグプタ朝の王で、「ヴィクラマーディティヤ」の称号を用いました。西インドのサカ勢力を征服してアラビア海への交易路を確保し、婚姻関係も利用して王国を安定させました。都周辺ではサンスクリット文学、天文学、数学、美術が栄え、中国僧法顕もこの時代のインドを訪れました。後世には文化の黄金時代を築いた理想の王として伝説化されましたが、繁栄は地域や身分によって一様ではありません。グプタ朝最盛期を代表する王です。",
        "whatDid": "西インドのサカ勢力を征服して交易路を確保し、グプタ朝の政治・文化を最盛期へ導きました。",
        "whyImportant": "古典インド文化とサンスクリット文学・科学・美術が発展した時代を代表するためです。"
      },
      "image": "assets/people/person-c22cd8b635.webp",
      "imageAlt": "チャンドラグプタ2世の画像"
    },
    "キュリロス": {
      "name": "キュリロス",
      "kana": "キュリロス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "9世紀のビザンツ帝国出身の宣教師・学者で、兄メトディオスとともにスラヴ人への布教を担いました。モラヴィアの要請を受け、スラヴ語で礼拝と聖書翻訳を行うため、グラゴル文字を考案したとされます。弟子たちはその伝統を受け継ぎ、後にキリル文字が発達しました。ラテン語・ギリシャ語だけでなく現地語による典礼を認めるようローマ教皇へ働きかけました。スラヴ世界のキリスト教化と文字文化の形成に重要な役割を果たしました。",
        "whatDid": "兄メトディオスとスラヴ語で布教し、聖書翻訳のための文字を整えて現地語典礼を広めました。",
        "whyImportant": "スラヴ諸民族のキリスト教化と文字・文学の発達を促し、東欧文化の基礎を築いたためです。"
      },
      "image": "assets/people/person-eb75bd0e4d.webp",
      "imageAlt": "キュリロスの画像"
    },
    "韓愈": {
      "name": "韓愈",
      "kana": "韓愈",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "8～9世紀の唐の官僚・文学者で、華美で形式的な文章に反対し、先秦・漢代の簡潔で力強い文体へ戻る「古文運動」を進めました。儒家の道徳と政治を重んじ、仏教や道教が国家と社会へ強く入り込むことを批判しました。皇帝が仏舎利を迎える行事を諫めた『論仏骨表』によって左遷されましたが、その文章は後世の模範となりました。宋代の儒学者にも影響し、儒教復興と散文改革を結び付けた人物です。",
        "whatDid": "古文運動を進め、儒教の立場から仏教を批判し、簡潔で論理的な散文を復興しました。",
        "whyImportant": "唐宋の散文と儒教復興を結び付け、後の宋学・科挙文体・東アジア漢文へ影響したためです。"
      },
      "image": "assets/people/person-e00f0d5a5a.webp",
      "imageAlt": "韓愈の画像"
    },
    "リチャード1世": {
      "name": "リチャード1世",
      "kana": "リチャード1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "12世紀末のイングランド王で、「獅子心王」と呼ばれました。即位後すぐ第3回十字軍へ参加し、アッコン攻略やアルスフの戦いでサラディン軍と戦いました。エルサレムは奪回できませんでしたが、キリスト教巡礼者の訪問を認める和平を結びました。帰国途中に捕らえられ、多額の身代金で解放されました。治世の多くを国外で過ごし、戦争費用のため重い課税を行いました。騎士英雄として有名ですが、国内統治の負担も大きい王でした。",
        "whatDid": "第3回十字軍でサラディンと戦い、沿岸都市を確保して巡礼を認める和平を結びました。",
        "whyImportant": "十字軍時代の騎士王を象徴する一方、遠征費と長期不在が王国へ与えた負担も示すためです。"
      },
      "image": "assets/people/person-2ef0566623.webp",
      "imageAlt": "リチャード1世の画像"
    },
    "フランシス・ドレーク": {
      "name": "フランシス・ドレーク",
      "kana": "フランシス・ドレーク",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "16世紀イングランドの航海者・私掠船長で、スペイン船や植民地を襲い、得た財宝を王室と分け合いました。1577年から1580年にかけて世界一周を達成し、太平洋の航路とスペイン領の情報を持ち帰りました。1588年のスペイン無敵艦隊との戦いでは副司令官としてイングランド海軍に参加しました。イングランドでは英雄視されましたが、スペイン側には海賊であり、初期の奴隷貿易にも関与しました。海洋進出と暴力的略奪の両面を持つ人物です。",
        "whatDid": "私掠船でスペイン船を襲い、世界一周を達成し、無敵艦隊との戦いでイングランド海軍を指揮しました。",
        "whyImportant": "イングランドの海洋進出を進める一方、私掠・植民地略奪・奴隷貿易の暴力を示すためです。"
      },
      "image": "assets/people/person-2ad64ec7f9.webp",
      "imageAlt": "フランシス・ドレークの画像"
    },
    "ベンジャミン・フランクリン": {
      "name": "ベンジャミン・フランクリン",
      "kana": "ベンジャミン・フランクリン",
      "era": "革命と工場の時代",
      "genre": [
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "北アメリカ植民地の印刷業者・科学者・政治家で、新聞や暦を発行し、市民図書館や消防組織をつくりました。雷が電気現象であることを実験で示し、避雷針を考案しました。独立運動では大陸会議に参加し、独立宣言の起草に関わりました。フランスへ派遣され、軍事・資金援助を引き出して独立戦争を支え、講和条約にも署名しました。一方、若い頃は奴隷を所有していましたが、晩年には奴隷制廃止を訴えました。",
        "whatDid": "電気研究と避雷針の発明を進め、外交官としてフランスの支援を得てアメリカ独立を支えました。",
        "whyImportant": "科学・市民活動・外交を結び、アメリカ独立と近代的な公共社会の形成に貢献したためです。"
      },
      "image": "assets/people/person-7f04eb839b.webp",
      "imageAlt": "ベンジャミン・フランクリンの画像"
    },
    "チャールズ・バベッジ": {
      "name": "チャールズ・バベッジ",
      "kana": "チャールズ・バベッジ",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの数学者・発明家で、数表の計算ミスを減らすため機械式計算機を構想しました。「階差機関」は多項式計算を自動化する装置で、さらに「解析機関」では演算装置、記憶装置、入力用パンチカード、条件分岐を備えた汎用計算機を設計しました。資金・技術・設計変更の問題で生前に完成しませんでしたが、エイダ・ラブレスはその可能性を詳しく説明しました。現代コンピューターの基本構成を19世紀に先取りした人物です。",
        "whatDid": "階差機関と解析機関を設計し、演算・記憶・入力・条件分岐を備えた汎用計算機を構想しました。",
        "whyImportant": "現代コンピューターに通じる基本構成とプログラム制御の考えを早くから示したためです。"
      },
      "image": "assets/people/person-4358cf3b38.webp",
      "imageAlt": "チャールズ・バベッジの画像"
    },
    "ジャマールッディーン・アフガーニー": {
      "name": "ジャマールッディーン・アフガーニー",
      "kana": "ジャマールッディーン・アフガーニー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀後半、イスラム世界各地で活動した思想家・政治運動家です。出身地には議論がありますが、アフガニスタン人を名乗り、エジプト、イラン、オスマン帝国、ヨーロッパを移動しました。欧州帝国主義へ対抗するため、イスラム諸国の連帯と政治改革、科学教育の必要を説きました。専制君主も批判し、弟子ムハンマド・アブドゥフらに影響しました。宗教を守るだけでなく、近代科学と政治参加を取り入れて社会を立て直そうとした人物です。",
        "whatDid": "イスラム諸国の連帯、反帝国主義、政治改革、科学教育を説き、各地の改革運動へ影響しました。",
        "whyImportant": "西欧支配へ対抗しながらイスラム社会を近代化する思想を広め、民族運動と改革思想の源流となったためです。"
      },
      "image": "assets/people/person-b309fce24a.webp",
      "imageAlt": "ジャマールッディーン・アフガーニーの画像"
    },
    "ハリー・トルーマン": {
      "name": "ハリー・トルーマン",
      "kana": "ハリー・トルーマン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの政治家で、副大統領就任から間もない1945年、ルーズベルト大統領の死去により大統領となりました。第二次世界大戦末期に広島・長崎への原子爆弾投下を承認し、日本の降伏後は国際連合の発足を支えました。ソ連の勢力拡大に対してトルーマン・ドクトリンとマーシャル・プランを進め、冷戦の封じ込め政策を形づくりました。朝鮮戦争では国連軍を派遣し、国内では軍の人種隔離廃止を命じました。",
        "whatDid": "原爆投下を承認し、戦後はソ連を封じ込める外交政策と欧州復興支援を進めました。",
        "whyImportant": "第二次世界大戦の終結と冷戦秩序の形成を主導し、核兵器時代の政治的責任を残したためです。"
      },
      "image": "assets/people/person-b5ea16b9f6.webp",
      "imageAlt": "ハリー・トルーマンの画像"
    },
    "ジュリウス・ニエレレ": {
      "name": "ジュリウス・ニエレレ",
      "kana": "ジュリウス・ニエレレ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリス領タンガニーカの教師から独立運動の指導者となり、1961年の独立後に初代首相、のち大統領となりました。ザンジバルとの統合でタンザニアを成立させ、アフリカ的社会主義「ウジャマー」を掲げて村落共同化、教育普及、民族間の統合を進めました。共通語スワヒリ語の普及は国家統合に役立ちましたが、強制移住や国有化は農業生産の低下と経済停滞を招きました。反植民地運動を支援し、長期政権後に自ら退任しました。",
        "whatDid": "タンザニア独立と国家統合を進め、ウジャマー政策で村落共同化・教育・社会主義建設を試みました。",
        "whyImportant": "アフリカ独立後の国家統合と自立的発展を模索し、その理想と経済的限界を示したためです。"
      },
      "image": "assets/people/person-e54e9c2266.webp",
      "imageAlt": "ジュリウス・ニエレレの画像"
    },
    "センウセレト3世": {
      "name": "センウセレト3世",
      "kana": "センウセレト3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前19世紀ごろのエジプト第12王朝の王で、中王国の中央集権を強化しました。ヌビアへ繰り返し遠征し、ナイル川第二急流付近に要塞群を築いて、金・交易路・国境を管理しました。国内では地方長官の権力を抑え、行政を王宮へ集中させたと考えられます。従来の理想化された王像と異なり、深いしわと厳しい表情を表した彫像が多く残ります。軍事・行政・王の表現を通して中王国最盛期を示す人物です。",
        "whatDid": "ヌビアへ遠征して国境要塞を築き、地方長官を抑えて中央集権的な統治を強めました。",
        "whyImportant": "中王国エジプトの領土・交易・行政を安定させ、強い王権を完成へ導いたためです。"
      },
      "image": "assets/people/person-6f7c0ec1f3.webp",
      "imageAlt": "センウセレト3世の画像"
    },
    "アッシュルナツィルパル2世": {
      "name": "アッシュルナツィルパル2世",
      "kana": "アッシュルナツィルパル2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前9世紀の新アッシリア王で、反乱を鎮圧しながら北メソポタミアからシリアへ遠征し、貢納を集めました。敵への皮剝ぎや串刺しなどの処罰を碑文に詳しく記し、恐怖を支配の手段として宣伝しました。都をアッシュルからカルフへ移し、巨大な北西宮殿を建設しました。宮殿の浮彫には戦争・狩猟・宗教儀礼が描かれ、王の力を視覚的に示しました。後のアッシリア帝国の軍事・行政・王権宣伝の型を確立した王です。",
        "whatDid": "遠征と威圧的な処罰で支配を広げ、カルフに新都と宮殿を築いて王権を宣伝しました。",
        "whyImportant": "新アッシリア帝国の軍事支配・恐怖政治・宮殿美術による権力表現を確立したためです。"
      },
      "image": "assets/people/person-c3eae4eafa.webp",
      "imageAlt": "アッシュルナツィルパル2世の画像"
    },
    "アリストテレス": {
      "name": "アリストテレス",
      "kana": "アリストテレス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher",
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前4世紀のギリシャ哲学者で、プラトンの学園で学んだ後、マケドニア王子アレクサンドロスの教育を担当しました。アテネに学園リュケイオンを開き、論理学、政治学、倫理学、生物学、詩学など幅広い分野を研究しました。物事を観察・分類し、原因を考える方法を重んじ、『政治学』では複数の政体を比較しました。自然科学には誤りもありましたが、知識を分野別に体系化した著作は、イスラム世界と中世ヨーロッパの学問へ大きな影響を与えました。",
        "whatDid": "論理学・倫理学・政治学・生物学などを観察と分類によって体系化し、リュケイオンで研究しました。",
        "whyImportant": "多くの学問分野の基礎概念と研究方法を整え、古代から中世の知識体系を形づくったためです。"
      },
      "image": "assets/people/person-69ee112116.webp",
      "imageAlt": "アリストテレスの画像"
    },
    "アウグストゥス": {
      "name": "アウグストゥス",
      "kana": "アウグストゥス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "カエサルの養子オクタウィアヌスとして内戦を勝ち抜き、紀元前27年に元老院から「アウグストゥス」の称号を受けました。共和政の制度を残す形を取りながら、軍隊・属州・財政を実質的に一人で支配し、ローマ帝政を始めました。常備軍と親衛隊を整え、属州統治や税制を安定させ、長い内戦を終わらせました。この時代から「ローマの平和」と呼ばれる安定が始まりましたが、権力の世襲化と皇帝独裁への道も開きました。",
        "whatDid": "内戦を終結させ、共和政の外形を保ちながら軍・属州・財政を掌握してローマ帝政を始めました。",
        "whyImportant": "ローマの政治を共和政から帝政へ転換し、約二百年続く地中海世界の安定の基礎を築いたためです。"
      },
      "image": "assets/people/person-fa1becdf28.webp",
      "imageAlt": "アウグストゥスの画像"
    },
    "カーリダーサ": {
      "name": "カーリダーサ",
      "kana": "カーリダーサ",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "4～5世紀ごろのグプタ朝期に活動したと考えられるサンスクリット文学の詩人・劇作家です。詳しい生涯は分かっていませんが、戯曲『シャクンタラー』では愛、別離、記憶、王の責任を描きました。叙事詩『ラグ王統史』や抒情詩『雲の使者』では、自然描写と人間感情を豊かな比喩で結びました。古代インドの神話・宮廷文化・恋愛表現を洗練された文学へ高め、その作品はインド諸語だけでなく、近代ヨーロッパの文学者にも高く評価されました。",
        "whatDid": "『シャクンタラー』『雲の使者』などを著し、自然と人間感情を結ぶサンスクリット文学を完成させました。",
        "whyImportant": "古典インド文学の表現を最高水準へ高め、インドと世界の演劇・詩へ長く影響したためです。"
      },
      "image": "assets/people/person-7823662b3a.webp",
      "imageAlt": "カーリダーサの画像"
    },
    "メトディオス": {
      "name": "メトディオス",
      "kana": "メトディオス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "9世紀のビザンツ帝国出身の宣教師で、弟キュリロスとともにモラヴィアでスラヴ人への布教を行いました。兄弟はスラヴ語による聖書と典礼を整え、キュリロスの死後もメトディオスは大司教として現地語礼拝を守りました。ラテン語だけを認めようとする聖職者と対立し、一時拘束されましたが、ローマ教皇からスラヴ語典礼の承認を得ました。弟子たちはブルガリアへ移り、翻訳と文字文化を発展させ、東欧のキリスト教文化の基礎を築きました。",
        "whatDid": "キュリロスとスラヴ語で布教し、弟の死後も現地語の典礼・聖書翻訳・聖職者教育を守りました。",
        "whyImportant": "スラヴ語によるキリスト教文化を定着させ、東欧の文字・文学・教会制度の発展を支えたためです。"
      },
      "image": "assets/people/person-b40b93d232.webp",
      "imageAlt": "メトディオスの画像"
    },
    "柳宗元": {
      "name": "柳宗元",
      "kana": "柳宗元",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "唐の官僚・文学者で、王叔文らと政治改革を進めましたが、政変で失脚し、永州や柳州へ長く左遷されました。中央から遠ざけられた生活のなかで、山水を描いた紀行文、寓話、政治論を執筆しました。韓愈とともに古文運動を進め、形式にとらわれない簡潔で論理的な散文を重んじました。地方官としては井戸や学校の整備、奴婢の解放にも取り組んだと伝えられます。失意と自然観察を文学へ変え、唐宋八大家の一人に数えられます。",
        "whatDid": "政治改革に参加し、左遷後は山水紀行・寓話・論説を著して古文運動を進めました。",
        "whyImportant": "政治的挫折と地方経験を新しい散文表現へ変え、中国の紀行文・寓話・論説文を発展させたためです。"
      },
      "image": "assets/people/person-1c4d5725fe.webp",
      "imageAlt": "柳宗元の画像"
    },
    "ジョン王": {
      "name": "ジョン王",
      "kana": "ジョン王",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "12～13世紀のイングランド王で、兄リチャード1世の死後に即位しました。フランス王フィリップ2世との戦争に敗れ、ノルマンディーなど大陸領の多くを失いました。戦費を集めるため重い課税を行い、カンタベリー大司教の任命をめぐって教皇とも対立しました。貴族の反乱を受け、1215年にマグナ・カルタへ同意し、王も法と慣習に従うことを認めました。直後に無効化を求めて内戦となりましたが、文書は後世の立憲政治の象徴となりました。",
        "whatDid": "大陸領を失い重税で反発を招き、貴族の圧力を受けてマグナ・カルタへ同意しました。",
        "whyImportant": "王権も法の制約を受けるという原則につながる文書を成立させ、立憲政治の象徴となったためです。"
      },
      "image": "assets/people/person-be42dd692c.webp",
      "imageAlt": "ジョン王の画像"
    },
    "ウォルター・ローリー": {
      "name": "ウォルター・ローリー",
      "kana": "ウォルター・ローリー",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "16～17世紀イングランドの廷臣・軍人・航海事業家で、女王エリザベス1世の信任を得ました。北アメリカの植民事業を計画し、現在のノースカロライナ沖ロアノーク島へ入植者を送りましたが、植民地は消失して失敗しました。「ヴァージニア」という名称の普及にも関わりました。女王死後は反逆罪で投獄され、釈放後に南アメリカで黄金郷を探しましたが、スペインとの和平を破った責任を問われ処刑されました。文学作品も残しました。",
        "whatDid": "ロアノーク植民事業を支援し、北アメリカへのイングランド進出を試みましたが、入植は失敗しました。",
        "whyImportant": "後のイギリス北米植民地建設の先駆けとなり、探検・宮廷政治・植民地主義の結び付きを示すためです。"
      },
      "image": "assets/people/person-aff3615f9f.webp",
      "imageAlt": "ウォルター・ローリーの画像"
    },
    "トマス・ジェファーソン": {
      "name": "トマス・ジェファーソン",
      "kana": "トマス・ジェファーソン",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "北アメリカ植民地の政治家で、1776年の独立宣言を主に起草し、「すべての人は平等」と自然権を掲げました。第3代大統領としてルイジアナをフランスから購入し、領土を大きく広げました。政教分離と公教育を重んじ、バージニア大学を創設しました。一方、多数の奴隷を所有し、奴隷女性サリー・ヘミングスとの間に子どもをもうけたと認められています。自由と平等の理念と奴隷制を併存させた矛盾を体現する人物です。",
        "whatDid": "独立宣言を起草し、大統領としてルイジアナ購入を行い、政教分離と公教育を進めました。",
        "whyImportant": "近代的な自由・平等の理念を示す一方、自ら奴隷を所有したアメリカ建国の矛盾を体現するためです。"
      },
      "image": "assets/people/person-a38e1e2af3.webp",
      "imageAlt": "トマス・ジェファーソンの画像"
    },
    "エイダ・ラブレス": {
      "name": "エイダ・ラブレス",
      "kana": "エイダ・ラブレス",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀イギリスの数学愛好家で、詩人バイロンの娘として生まれました。チャールズ・バベッジが構想した解析機関について、イタリア語論文を英訳し、原文を上回る詳しい注釈を加えました。そこではベルヌーイ数を計算する手順を示し、機械が数値計算だけでなく、規則に変換できれば音楽や記号も扱える可能性を論じました。解析機関は完成しませんでしたが、機械へ処理手順を与える発想から、最初期のプログラム記述者として知られます。",
        "whatDid": "解析機関の解説と計算手順を記し、機械が数値以外の記号も扱える可能性を示しました。",
        "whyImportant": "コンピューターを単なる計算器ではなく、手順に従って多様な情報を処理する機械として捉えたためです。"
      },
      "image": "assets/people/person-87cf8f01a8.webp",
      "imageAlt": "エイダ・ラブレスの画像"
    },
    "ムハンマド・アブドゥフ": {
      "name": "ムハンマド・アブドゥフ",
      "kana": "ムハンマド・アブドゥフ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19～20世紀初めのエジプトのイスラム法学者・改革思想家で、アズハル学院で学びました。師アフガーニーとともに帝国主義と専制政治を批判し、亡命後に帰国してエジプトの最高法官となりました。イスラムの基本教義は理性や近代科学と両立すると考え、教育改革、法解釈の見直し、女性の地位改善を訴えました。過去の学説をそのまま模倣するのではなく、時代に応じて自ら判断する「イジュティハード」の復活を重視しました。",
        "whatDid": "イスラム法を理性と時代に応じて解釈し直し、教育・司法・社会制度の改革を進めました。",
        "whyImportant": "イスラム信仰と近代科学・教育・法改革を両立させる近代イスラム改革思想を形づくったためです。"
      },
      "image": "assets/people/person-8d5aaf0810.webp",
      "imageAlt": "ムハンマド・アブドゥフの画像"
    },
    "ドワイト・アイゼンハワー": {
      "name": "ドワイト・アイゼンハワー",
      "kana": "ドワイト・アイゼンハワー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの軍人・政治家で、第二次世界大戦では連合国遠征軍最高司令官としてノルマンディー上陸作戦を指揮し、西ヨーロッパ解放を進めました。戦後はNATO軍最高司令官を経て大統領となり、朝鮮戦争の休戦を実現しました。高速道路網を建設し、冷戦では核抑止と同盟を重視しました。一方、イランやグアテマラの政権転覆に中央情報局を用いました。退任演説では軍需産業と政府が結び付く「軍産複合体」の影響を警告しました。",
        "whatDid": "欧州連合軍を指揮してノルマンディー上陸を成功させ、大統領として朝鮮休戦と高速道路建設を進めました。",
        "whyImportant": "第二次世界大戦の勝利と冷戦期の米国戦略を担い、軍産複合体の危険も警告したためです。"
      },
      "image": "assets/people/person-0d77053913.webp",
      "imageAlt": "ドワイト・アイゼンハワーの画像"
    },
    "トーマス・サンカラ": {
      "name": "トーマス・サンカラ",
      "kana": "トーマス・サンカラ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "西アフリカのオートボルタで軍人となり、1983年のクーデター後に政権を握りました。国名を「高潔な人々の国」を意味するブルキナファソへ改め、予防接種、識字教育、植樹、土地改革、女性の強制結婚や性器切除への反対を進めました。政府のぜいたくを減らし、外国援助への依存を批判しました。一方、革命委員会による反対派への圧力や労働組合との対立もありました。1987年にクーデターで殺害され、反帝国主義と清廉な政治の象徴となりました。",
        "whatDid": "保健・教育・植樹・女性解放を進め、外国援助への依存と政治家の特権を減らそうとしました。",
        "whyImportant": "アフリカの自立と社会改革を急速に進め、反帝国主義と清廉な統治の象徴となったためです。"
      },
      "image": "assets/people/person-606de7d4d2.webp",
      "imageAlt": "トーマス・サンカラの画像"
    },
    "ハトシェプスト": {
      "name": "ハトシェプスト",
      "kana": "ハトシェプスト",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王で、幼いトトメス3世の共同統治者から、自らファラオの称号を用いて約二十年間統治しました。男性王の付けひげや王冠を身に着けた姿で表され、王位の正統性を神アメンの意志として示しました。プント国への交易遠征で香料・木材・珍しい動植物を運ばせ、デイル・アル＝バハリに壮大な葬祭殿を建設しました。死後に名や像が削られましたが、近年は平和的交易と建築を進めた女性王として再評価されています。",
        "whatDid": "女性としてファラオとなり、プント交易を進め、デイル・アル＝バハリに大葬祭殿を建設しました。",
        "whyImportant": "王権が男性に限られた社会で長期統治を実現し、新王国の交易・建築・王権表現を発展させたためです。"
      },
      "image": "assets/people/person-c2b318a3aa.webp",
      "imageAlt": "ハトシェプストの画像"
    },
    "シャルマネセル3世": {
      "name": "シャルマネセル3世",
      "kana": "シャルマネセル3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前9世紀の新アッシリア王で、三十年以上にわたりシリア、アナトリア、バビロニア方面へ遠征を続けました。紀元前853年のカルカルの戦いでは、ダマスクスやイスラエルを含む連合軍と戦いました。決定的勝利には至らなかったものの、各地から貢納を集め、帝国の影響力を広げました。「黒色オベリスク」には諸国の使節が貢物を差し出す姿が描かれています。晩年には息子の反乱が起こり、広大な帝国支配の不安定さも表れました。",
        "whatDid": "シリア・アナトリアへ繰り返し遠征し、諸国から貢納を集めて新アッシリアの影響圏を広げました。",
        "whyImportant": "遠征記録と黒色オベリスクによって、アッシリア帝国の軍事・外交・貢納支配を具体的に伝えるためです。"
      },
      "image": "assets/people/person-b4c0e8c892.webp",
      "imageAlt": "シャルマネセル3世の画像"
    },
    "ピタゴラス": {
      "name": "ピタゴラス",
      "kana": "ピタゴラス",
      "era": "古代の大国の時代",
      "genre": [
        "philosopher",
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前6世紀ごろ、エーゲ海のサモス島出身で、南イタリアのクロトンに宗教的・哲学的共同体を築いた人物です。魂は死後に別の生物へ移ると考え、節制と共同生活を重んじました。数の比が音の高さや宇宙の秩序を説明すると考え、数学と音楽を結び付けました。直角三角形の定理は彼以前にも知られており、本人が証明したかは不明です。著作を残さず、弟子の伝承が混ざるため、史実と伝説を区別する必要があります。",
        "whatDid": "数と音楽・宇宙の秩序を結び付け、魂の転生と共同生活を重んじる学派を南イタリアに築きました。",
        "whyImportant": "数学を実用計算だけでなく、自然と宇宙を理解する原理として考える伝統を広めたためです。"
      },
      "image": "assets/people/person-71f920164b.webp",
      "imageAlt": "ピタゴラスの画像"
    },
    "リウィア": {
      "name": "リウィア",
      "kana": "リウィア",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ初代皇帝アウグストゥスの妻で、前夫との子ティベリウスを連れて再婚し、帝政初期の皇室を支えました。公的には貞淑な妻と母の模範として表され、財産管理、縁故、請願の仲介を通して大きな影響力を持ちました。アウグストゥス死後には「ユリア・アウグスタ」の名を与えられ、息子ティベリウスの治世にも皇太后として存在感を示しました。後世の歴史書では毒殺を企てた女性と描かれますが、その多くは確証のない宮廷伝説です。",
        "whatDid": "アウグストゥスの妻として皇室を支え、財産・人脈・請願仲介を通じて帝政初期の政治へ影響しました。",
        "whyImportant": "公式の役職を持たない皇帝家の女性が、婚姻と家族関係を通じて政治力を持った例だからです。"
      },
      "image": "assets/people/person-ed179a7e57.webp",
      "imageAlt": "リウィアの画像"
    },
    "パーニニ": {
      "name": "パーニニ",
      "kana": "パーニニ",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前5～4世紀ごろに活動したと考えられる古代インドの文法学者です。著書『アシュターディヤーイー』で、サンスクリット語の音、語形、語の作り方、文法規則を約四千の短い規則にまとめました。例外処理や規則の適用順序まで厳密に示し、限られた記号で複雑な言語を生成する体系を築きました。この文法はヴェーダ聖典の正確な伝承を支え、サンスクリット語の標準化に役立ちました。近代言語学や形式体系との類似でも注目されています。",
        "whatDid": "『アシュターディヤーイー』でサンスクリット語の音・語形・生成規則を厳密な体系にまとめました。",
        "whyImportant": "世界最古級の精密な文法体系を築き、インドの聖典伝承・言語学・論理的分析へ影響したためです。"
      },
      "image": "assets/people/person-e7bd6b387b.webp",
      "imageAlt": "パーニニの画像"
    },
    "ナーガールジュナ": {
      "name": "ナーガールジュナ",
      "kana": "ナーガールジュナ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "2～3世紀ごろのインド仏教思想家で、大乗仏教の中観派の基礎を築きました。主著『中論』で、すべての存在は他との関係や原因によって成り立ち、変わらない本質を持たないという「空」を論じました。空は何も存在しないという意味ではなく、物事を固定的に捉える執着から離れる考えです。また、日常の言葉で成り立つ世俗の真理と、究極の理解を区別する二諦説を示しました。その思想はチベット・中国・日本の仏教へ広く影響しました。",
        "whatDid": "『中論』で縁起と空を論じ、物事に固定した本質があるとする考えを論理的に批判しました。",
        "whyImportant": "大乗仏教の中観思想を体系化し、東アジアとチベットの仏教哲学の基礎を築いたためです。"
      },
      "image": "assets/people/person-df73badf15.webp",
      "imageAlt": "ナーガールジュナの画像"
    },
    "王安石": {
      "name": "王安石",
      "kana": "王安石",
      "era": "交易と王国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "11世紀の北宋の政治家・文学者で、神宗皇帝のもとで「新法」と呼ばれる改革を進めました。農民へ低利で資金を貸す青苗法、物資価格を調整する市易法、兵役を地域で担う保甲法などにより、財政を立て直し、農民と国家を守ろうとしました。しかし役人の強制や運用上の負担が生じ、司馬光ら旧法派と激しく対立しました。改革は本人の退任後に多くが廃止されましたが、国家が経済と社会へ介入する政策論争を引き起こしました。",
        "whatDid": "青苗法・市易法・保甲法などの新法を実施し、農民支援・財政再建・軍事改革を進めました。",
        "whyImportant": "国家が金融・流通・社会保障へ介入する大規模改革を行い、中国政治史の代表的論争を生んだためです。"
      },
      "image": "assets/people/person-5a0f7044b8.webp",
      "imageAlt": "王安石の画像"
    },
    "サラディン": {
      "name": "サラディン",
      "kana": "サラディン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "クルド系の軍人としてエジプトで台頭し、ファーティマ朝を終わらせてアイユーブ朝を開きました。エジプトとシリアを統合し、十字軍国家に対抗しました。1187年のヒッティーンの戦いで十字軍主力を破り、エルサレムを奪回しました。住民の多くを身代金で解放し、1099年の十字軍占領時の大量殺害とは異なる対応を取りました。第3回十字軍ではリチャード1世と戦い、沿岸部を十字軍側に残しつつ巡礼を認める和平を結びました。",
        "whatDid": "エジプトとシリアを統合し、ヒッティーンで十字軍を破ってエルサレムを奪回しました。",
        "whyImportant": "イスラム勢力を再統合して十字軍国家へ反撃し、宗教戦争下の統治と外交の象徴となったためです。"
      },
      "image": "assets/people/person-fdca958aaf.webp",
      "imageAlt": "サラディンの画像"
    },
    "アベル・タスマン": {
      "name": "アベル・タスマン",
      "kana": "アベル・タスマン",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "17世紀オランダ東インド会社の航海者で、東南アジアから南方海域の航路と交易地を探しました。1642年の航海で、現在のタスマニア島へヨーロッパ人として初めて到達し、続いてニュージーランドを記録しました。現地マオリとの衝突で乗組員が死亡し、上陸せずに離れました。その後トンガやフィジー周辺を航海し、オーストラリア大陸北岸も調査しました。航海は南方大陸の全体像を解明しませんでしたが、南太平洋地図の空白を大きく埋めました。",
        "whatDid": "オランダ東インド会社の船で南太平洋を探検し、タスマニアとニュージーランドを欧州の地図へ記録しました。",
        "whyImportant": "南太平洋の地理認識を広げ、後のオーストラリア・ニュージーランド探検の基礎をつくったためです。"
      },
      "image": "assets/people/person-1cc2e1acb2.webp",
      "imageAlt": "アベル・タスマンの画像"
    },
    "ジョン・アダムズ": {
      "name": "ジョン・アダムズ",
      "kana": "ジョン・アダムズ",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ独立運動を担った法律家・政治家で、ボストン虐殺事件では被告となったイギリス兵の弁護を引き受け、法の手続きを守りました。大陸会議では独立を主張し、ヨーロッパ外交でフランスの支援と講和条約締結に関わりました。初代副大統領を経て第2代大統領となり、フランスとの戦争を避けました。一方、政府批判を取り締まる外国人・治安諸法を成立させ、言論の自由をめぐる強い反発を招きました。",
        "whatDid": "独立を支持し、欧州外交と講和に関わり、第2代大統領として対仏全面戦争を回避しました。",
        "whyImportant": "独立国家の外交と大統領制を形づくる一方、国家安全と表現の自由の対立を残したためです。"
      },
      "image": "assets/people/person-a15bda296f.webp",
      "imageAlt": "ジョン・アダムズの画像"
    },
    "ルイ・パスツール": {
      "name": "ルイ・パスツール",
      "kana": "ルイ・パスツール",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀フランスの化学者・微生物学者で、発酵や腐敗が自然に起こるのではなく、微生物の働きによることを実験で示しました。空気中の微生物が入りにくい白鳥の首型フラスコを使い、自然発生説を退けました。ワインや牛乳を低温で加熱して腐敗を防ぐパスツール法を考案し、蚕病の原因も調査しました。さらに炭疽や狂犬病のワクチン開発を進め、感染症予防と衛生管理に大きな道を開きました。",
        "whatDid": "微生物が発酵・腐敗・病気を起こすことを示し、低温殺菌法と炭疽・狂犬病ワクチンを開発しました。",
        "whyImportant": "微生物学と予防医学の基礎を築き、食品衛生・感染症対策・ワクチン研究を大きく進めたためです。"
      },
      "image": "assets/people/person-5a8e297376.webp",
      "imageAlt": "ルイ・パスツールの画像"
    },
    "サイイド・アフマド・ハーン": {
      "name": "サイイド・アフマド・ハーン",
      "kana": "サイイド・アフマド・ハーン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀インドのイスラム教徒知識人・官僚で、1857年のインド大反乱後、ムスリム社会の教育と政治的立場の立て直しに取り組みました。イスラムの信仰と近代科学は両立できると考え、英語・科学・西洋の学問を学ぶ学校をアリーガルに設立しました。この学校は後のアリーガル・ムスリム大学へ発展しました。ヒンドゥー教徒とムスリムの政治的利害は異なると主張したため、後の分離主義へ影響したとの評価もあります。",
        "whatDid": "アリーガルに近代学校を設立し、イスラム信仰と英語・科学教育を両立させる改革を進めました。",
        "whyImportant": "インドのムスリム教育改革を進め、近代化と宗教共同体の政治意識の形成へ影響したためです。"
      },
      "image": "assets/people/person-7e57baa118.webp",
      "imageAlt": "サイイド・アフマド・ハーンの画像"
    },
    "ダグラス・マッカーサー": {
      "name": "ダグラス・マッカーサー",
      "kana": "ダグラス・マッカーサー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカ陸軍の軍人で、第二次世界大戦ではフィリピン防衛に失敗して脱出した後、南西太平洋方面の連合軍を指揮し、島伝いに反攻してフィリピンへ戻りました。日本降伏後は連合国軍最高司令官として占領政策を進め、日本国憲法の制定、軍の解体、農地改革、女性参政権などに関わりました。朝鮮戦争では国連軍を指揮しましたが、中国への戦争拡大を公然と主張し、文民統制に反したとしてトルーマン大統領に解任されました。",
        "whatDid": "太平洋戦争の反攻と日本占領改革を指揮し、朝鮮戦争では戦争拡大を主張して解任されました。",
        "whyImportant": "戦後日本の制度改革に大きく関わり、軍人も文民政府に従う原則を示す事件の中心となったためです。"
      },
      "image": "assets/people/person-7215bb0731.webp",
      "imageAlt": "ダグラス・マッカーサーの画像"
    },
    "エレン・ジョンソン・サーリーフ": {
      "name": "エレン・ジョンソン・サーリーフ",
      "kana": "エレン・ジョンソン・サーリーフ",
      "era": "今につながる時代",
      "genre": [
        "monarch",
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "リベリアの経済官僚・政治家で、軍事政権への批判や亡命を経験した後、内戦終結後の2005年選挙で大統領に選ばれました。アフリカで選挙によって国家元首となった最初の女性です。国家財政の再建、対外債務の削減、教育・保健、女性の政治参加を進めました。女性の平和運動を支えた功績でノーベル平和賞を受賞しました。一方、汚職対策の限界や親族登用への批判も受け、復興と統治責任の両面で評価されています。",
        "whatDid": "内戦後のリベリアで大統領となり、債務削減・国家再建・女性の政治参加を進めました。",
        "whyImportant": "アフリカ初の選挙による女性国家元首として、内戦後復興と女性の政治参加を象徴したためです。"
      },
      "image": "assets/people/person-d76f4801e9.webp",
      "imageAlt": "エレン・ジョンソン・サーリーフの画像"
    },
    "トトメス3世": {
      "name": "トトメス3世",
      "kana": "トトメス3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王で、幼少期には継母ハトシェプストと共同統治し、その死後に単独支配を始めました。メギドの戦いをはじめシリア・パレスチナへ多数の遠征を行い、諸都市から貢納を集めて新王国の支配を広げました。ヌビア方面も統制し、カルナック神殿を増築して戦勝記録を刻ませました。後世に軍事王として称賛されましたが、支配は遠征と人質、貢納に支えられていました。",
        "whatDid": "メギドの戦いなど多数の遠征でシリア・パレスチナを支配し、新王国エジプトの勢力を最大級に広げました。",
        "whyImportant": "エジプトを広域帝国へ成長させ、遠征記録から新王国の軍事・外交・貢納支配を知ることができるためです。"
      },
      "image": "assets/people/person-6279b7538a.webp",
      "imageAlt": "トトメス3世の画像"
    },
    "サルゴン2世": {
      "name": "サルゴン2世",
      "kana": "サルゴン2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前8世紀の新アッシリア王で、王位継承の事情には不明な点がありますが、各地の反乱を鎮圧して帝国支配を立て直しました。イスラエル王国の首都サマリア征服を完成させ、住民の一部を移住させました。ウラルトゥ遠征やバビロニア支配も進め、新都ドゥル・シャルキンを建設しました。宮殿には巨大な人面有翼雄牛像を置き、王の権力を示しました。戦死後、新都は放棄されましたが、帝国拡張の転機をつくりました。",
        "whatDid": "反乱を鎮圧し、サマリア・ウラルトゥ・バビロニアへ勢力を広げ、新都ドゥル・シャルキンを築きました。",
        "whyImportant": "新アッシリア帝国の領土拡大と住民移住政策、巨大宮殿による王権表現を代表するためです。"
      },
      "image": "assets/people/person-fe80e980ec.webp",
      "imageAlt": "サルゴン2世の画像"
    },
    "ヘロドトス": {
      "name": "ヘロドトス",
      "kana": "ヘロドトス",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のギリシャ人著述家で、ペルシャ戦争がなぜ起こったのかを調べ、『歴史』にまとめました。ギリシャだけでなく、エジプト、リディア、ペルシャ、スキタイなど各地の地理・風習・伝承を聞き取り、複数の説を並べて読者に示しました。神話や誇張、確認できない話も含まれますが、原因を探り、異なる社会を比較しようとしました。後世に「歴史の父」と呼ばれ、戦争だけでなく文化の違いを記録した点でも重要です。",
        "whatDid": "ペルシャ戦争の原因を調べ、旅と聞き取りを通じて諸民族の歴史・地理・風習を『歴史』にまとめました。",
        "whyImportant": "出来事の原因を調査し、異文化を比較して記録する歴史叙述の基礎をつくったためです。"
      },
      "image": "assets/people/person-53e377756e.webp",
      "imageAlt": "ヘロドトスの画像"
    },
    "ティベリウス": {
      "name": "ティベリウス",
      "kana": "ティベリウス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ第2代皇帝で、アウグストゥスの養子として帝位を継ぎました。若い頃はゲルマニアやバルカンで軍功を立て、即位後は財政を慎重に管理し、属州統治の安定を重視しました。しかし元老院との関係は悪化し、晩年はカプリ島へ移って近衛長官セイヤヌスへ政治を任せました。反逆罪裁判と密告が増え、恐怖政治の印象を残しました。古代史家の敵意も強いため、私生活の悪評をそのまま史実とすることには注意が必要です。",
        "whatDid": "軍事経験を生かして帝国の財政と属州を安定させましたが、晩年は反逆罪裁判と密告政治を招きました。",
        "whyImportant": "アウグストゥス後の帝政継承を定着させる一方、皇帝と元老院の緊張を深めたためです。"
      },
      "image": "assets/people/person-6a7d9d902d.webp",
      "imageAlt": "ティベリウスの画像"
    },
    "荘子": {
      "name": "荘子",
      "kana": "荘子",
      "era": "古代の大国の時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代の中国で活動したと考えられる思想家で、道家の代表的古典『荘子』と結び付けられます。書物には後世の文章も含まれますが、夢で蝶になった話、巨大な魚が鳥へ変わる話などを通して、人間が決めた善悪・有用無用・生死の区別を絶対視しないよう説きました。自然の変化に逆らわず、物事を一つの立場だけで判断しない自由な生き方を重んじました。その寓話と逆説は中国思想・文学・芸術へ大きな影響を与えました。",
        "whatDid": "寓話と逆説を用いて固定した価値判断を問い直し、自然の変化に従う自由な生き方を説きました。",
        "whyImportant": "道家思想を豊かな文学表現へ発展させ、中国の哲学・詩文・絵画・人生観へ影響したためです。"
      },
      "image": "assets/people/person-906371073c.webp",
      "imageAlt": "荘子の画像"
    },
    "アサンガ": {
      "name": "アサンガ",
      "kana": "アサンガ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "4世紀ごろのインド仏教思想家で、弟ヴァスバンドゥとともに瑜伽行派の形成に関わりました。瞑想修行の体験を重視し、人が認識する世界は心の働きと深く結び付いていると論じました。経験や行為の影響を蓄える基盤として「阿頼耶識」という深層の意識を説明し、迷いから悟りへ至る心の変化を体系化しました。著作の帰属には議論がありますが、その思想は中国の法相宗やチベット仏教へ伝わり、大乗仏教の心の理論を発展させました。",
        "whatDid": "瑜伽行派の思想を整え、阿頼耶識を用いて認識・行為・迷い・悟りの関係を説明しました。",
        "whyImportant": "大乗仏教の心と認識の理論を体系化し、中国・日本・チベットの仏教思想へ影響したためです。"
      },
      "image": "assets/people/person-0090ac93db.webp",
      "imageAlt": "アサンガの画像"
    },
    "司馬光": {
      "name": "司馬光",
      "kana": "司馬光",
      "era": "交易と王国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "11世紀の北宋の政治家・歴史家で、王安石の新法に反対し、急激な国家介入より既存制度の安定を重んじました。長年をかけて編年体の歴史書『資治通鑑』を編集し、戦国時代から五代まで約千三百年の政治・戦争・人物を年代順にまとめました。書名には、過去を統治の参考とする意味があります。哲宗の即位後に宰相となり新法を廃止しましたが、党派対立をさらに深めました。その著作は東アジアの政治と歴史教育に長く用いられました。",
        "whatDid": "『資治通鑑』を編さんし、宰相として王安石の新法を廃止して旧法へ戻しました。",
        "whyImportant": "歴史を政治の教訓として体系化し、東アジアの歴史叙述と統治思想へ長く影響したためです。"
      },
      "image": "assets/people/person-846b68113b.webp",
      "imageAlt": "司馬光の画像"
    },
    "リチャード・ド・ベリー": {
      "name": "リチャード・ド・ベリー",
      "kana": "リチャード・ド・ベリー",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "14世紀イングランドの聖職者・政治家で、王エドワード3世に仕え、財務長官や大法官、ダラム司教を務めました。外交や宮廷勤務のかたわら多数の写本を集め、書物への愛と学問の価値をラテン語著作『フィロビブロン』に記しました。書物を大切に扱う方法、学者を支援する必要、図書を教育へ生かす考えを説きました。計画した図書館がそのまま実現したかには不明点がありますが、中世の書物収集と知識保存を語る代表人物です。",
        "whatDid": "写本を収集し、『フィロビブロン』で書物の保存・利用・学者支援の重要性を論じました。",
        "whyImportant": "印刷以前の中世社会で、書物と図書館が知識を保存し教育へ伝える価値を明確に説いたためです。"
      },
      "image": "assets/people/person-22dedfd93d.webp",
      "imageAlt": "リチャード・ド・ベリーの画像"
    },
    "ジェームズ・クック": {
      "name": "ジェームズ・クック",
      "kana": "ジェームズ・クック",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "18世紀イギリス海軍の航海者で、三度の太平洋航海を指揮しました。金星の太陽面通過観測を目的にタヒチへ向かい、その後ニュージーランド周航とオーストラリア東岸の測量を行いました。二度目の航海では南極圏へ入り、未知の巨大南方大陸が温暖な海域に存在するという説を弱めました。航海では壊血病予防にも成果を上げました。一方、記録と領有宣言はイギリス植民地化に利用され、ハワイで現地住民との衝突により死亡しました。",
        "whatDid": "太平洋を三度航海し、ニュージーランド・オーストラリア東岸・南太平洋を精密に測量しました。",
        "whyImportant": "太平洋地図と長距離航海を発展させる一方、その成果が先住民の土地の植民地化へ利用されたためです。"
      },
      "image": "assets/people/person-80df1afa33.webp",
      "imageAlt": "ジェームズ・クックの画像"
    },
    "アビゲイル・アダムズ": {
      "name": "アビゲイル・アダムズ",
      "kana": "アビゲイル・アダムズ",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ独立期の女性で、第2代大統領ジョン・アダムズの妻、第6代大統領ジョン・クインシー・アダムズの母です。夫が大陸会議や海外外交で不在の間、農場と家計を管理し、政治・戦争・教育について多数の手紙を交わしました。独立時には新しい法律を作る際に「女性たちを忘れないで」と訴え、夫の権力を制限する必要を指摘しました。奴隷制にも批判的でした。公職には就きませんでしたが、書簡は女性の政治意識と日常生活を伝えます。",
        "whatDid": "独立期の政治を手紙で論じ、女性の権利と教育を求め、家庭と農場を自ら管理しました。",
        "whyImportant": "公的政治から女性が排除された時代に、その権利要求と生活経験を具体的な書簡として残したためです。"
      },
      "image": "assets/people/person-a9d2a04956.webp",
      "imageAlt": "アビゲイル・アダムズの画像"
    },
    "エドワード・ジェンナー": {
      "name": "エドワード・ジェンナー",
      "kana": "エドワード・ジェンナー",
      "era": "革命と工場の時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "18～19世紀イギリスの医師で、牛痘にかかった人は天然痘にかかりにくいという農村の経験に注目しました。1796年、牛痘の膿を少年へ接種し、その後に天然痘への抵抗を確かめました。当時の実験方法は現代の倫理基準を満たしませんが、結果を記録して予防接種を広めました。「ワクチン」という言葉は牛を意味するラテン語に由来します。天然痘予防は世界へ広がり、長い取り組みの末、1980年に天然痘根絶が宣言されました。",
        "whatDid": "牛痘を用いた天然痘予防法を実験・記録し、ワクチン接種を広めました。",
        "whyImportant": "人類初の本格的なワクチンを実用化し、感染症を予防し最終的に根絶する道を開いたためです。"
      },
      "image": "assets/people/person-187e9cbe5e.webp",
      "imageAlt": "エドワード・ジェンナーの画像"
    },
    "ラーマクリシュナ": {
      "name": "ラーマクリシュナ",
      "kana": "ラーマクリシュナ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "19世紀ベンガルのヒンドゥー教宗教家で、コルカタ近郊ダクシネーシュワルのカーリー寺院で祭司を務めました。女神カーリーへの信仰を中心に、ヴィシュヌ信仰、タントラ、イスラム教、キリスト教の修行も試み、異なる宗教は同じ究極の真理へ至る道だと説きました。体系的な著作は残さず、弟子たちが言行を記録しました。弟子ヴィヴェーカーナンダはその教えを世界へ紹介し、宗教間対話と近代ヒンドゥー教改革へ影響を与えました。",
        "whatDid": "複数の宗教的修行を実践し、異なる信仰は同じ究極の真理へ至る道だと説きました。",
        "whyImportant": "宗教間の共通性を強調し、近代ヒンドゥー教の改革と世界的な宗教対話へ影響したためです。"
      },
      "image": "assets/people/person-b9fe782cb4.webp",
      "imageAlt": "ラーマクリシュナの画像"
    },
    "ジョージ・マーシャル": {
      "name": "ジョージ・マーシャル",
      "kana": "ジョージ・マーシャル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカ陸軍の軍人・政治家で、第二次世界大戦中に陸軍参謀総長として兵力拡大、将校登用、連合国との作戦調整を進めました。戦後は国務長官となり、1947年にヨーロッパ復興計画を提案しました。後にマーシャル・プランと呼ばれ、食料・機械・資金を供給して西ヨーロッパの経済再建と政治安定を支えました。中国内戦の仲介は失敗しましたが、国防長官も務め、軍事と外交の両面から戦後秩序の形成に関わりました。",
        "whatDid": "米陸軍の戦時動員を統括し、戦後はマーシャル・プランで西ヨーロッパの復興を支援しました。",
        "whyImportant": "連合国の戦争遂行と戦後欧州復興を支え、冷戦初期の西側秩序を安定させたためです。"
      },
      "image": "assets/people/person-19bc08aafd.webp",
      "imageAlt": "ジョージ・マーシャルの画像"
    },
    "ルース・ベイダー・ギンズバーグ": {
      "name": "ルース・ベイダー・ギンズバーグ",
      "kana": "ルース・ベイダー・ギンズバーグ",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの法律家・連邦最高裁判所判事で、女性が法曹界で強い差別を受けた時代に弁護士となりました。女性の権利訴訟では、性別による固定観念が女性だけでなく男性にも不利益を与えることを示し、連邦最高裁で段階的に違憲判決を積み重ねました。1993年に女性として二人目の最高裁判事となり、男女平等、投票権、労働者の権利などで多数意見にも反対意見にも重要な文章を残しました。司法を通じた平等実現の象徴です。",
        "whatDid": "性差別訴訟を通じて男女平等を進め、最高裁判事として権利を守る多数意見・反対意見を残しました。",
        "whyImportant": "性別に基づく法的差別を段階的に崩し、司法による平等と少数意見の役割を示したためです。"
      },
      "image": "assets/people/person-99609838d7.webp",
      "imageAlt": "ルース・ベイダー・ギンズバーグの画像"
    },
    "アメンホテプ3世": {
      "name": "アメンホテプ3世",
      "kana": "アメンホテプ3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "紀元前14世紀のエジプト第18王朝の王で、大規模な征服戦争より外交と建築によって新王国の繁栄を示しました。ミタンニやバビロニアなどの王家と婚姻・贈答を行い、アマルナ文書には各国王との外交交渉が残ります。ルクソール神殿や巨大な葬祭殿を建て、入口の巨像は「メムノンの巨像」として残っています。王妃ティイも強い地位を持ちました。豊かな金と国際外交に支えられた王権は、次代アクエンアテンの宗教改革へつながりました。",
        "whatDid": "婚姻と贈答外交で周辺大国との関係を保ち、ルクソール神殿など巨大建築を進めました。",
        "whyImportant": "新王国エジプトの国際外交・富・建築・王妃の地位が最盛期に達した時代を代表するためです。"
      },
      "image": "assets/people/person-3402a471f3.webp",
      "imageAlt": "アメンホテプ3世の画像"
    },
    "トゥキディデス": {
      "name": "トゥキディデス",
      "kana": "トゥキディデス",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5世紀のアテネ人で、将軍としてペロポネソス戦争に参加しました。アンフィポリス救援に失敗して追放された後、戦争の経過を『戦史』にまとめました。神々の意志より、人間の恐怖・利益・名誉、国家間の力関係を重視して原因を分析しました。演説は実際の言葉をそのまま記録したものではなく、状況にふさわしい内容を再構成したと説明しています。証言を比較し、誇張を避けようとする姿勢から、批判的歴史研究の先駆者とされます。",
        "whatDid": "ペロポネソス戦争を証言比較と因果分析によって記録し、国家の力・恐怖・利益を検討しました。",
        "whyImportant": "神話ではなく人間の行動と政治構造から戦争原因を分析する批判的歴史学の基礎を示したためです。"
      },
      "image": "assets/people/person-66b848de50.webp",
      "imageAlt": "トゥキディデスの画像"
    },
    "老子": {
      "name": "老子",
      "kana": "老子",
      "era": "古代の大国の時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "古代中国の思想家として伝えられ、『老子』または『道徳経』の著者とされますが、実在した一人の人物か、書物が複数時代の思想を集めたものかは確定していません。『老子』は、万物の根源で言葉では捉えきれない「道」に従い、無理に支配せず自然な働きを生かす「無為」を説きました。強さより柔らかさ、多く持つことより足ることを知る姿勢を重んじます。その思想は道家・道教だけでなく、中国の政治論・文学・芸術へ深く影響しました。",
        "whatDid": "『老子』に結び付けられ、万物の根源である道と、無理に支配しない無為の生き方を説きました。",
        "whyImportant": "道家思想の基礎をつくり、中国の政治観・自然観・宗教・文学・芸術へ長く影響したためです。"
      },
      "image": "assets/people/person-99a3a0b6f0.webp",
      "imageAlt": "老子の画像"
    },
    "エルナン・コルテス": {
      "name": "エルナン・コルテス",
      "kana": "エルナン・コルテス",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "スペイン出身の征服者で、1519年にキューバ総督の命令を越えてメキシコへ進みました。アステカに反発するトラスカラなどの先住民勢力と同盟し、通訳マリンチェの協力も得て都テノチティトランへ入りました。一度は追放されましたが、天然痘の流行と包囲戦を利用して1521年に都を陥落させました。その結果、アステカ帝国は滅び、スペイン植民地ヌエバ・エスパーニャが成立しました。征服は大量死・強制労働・文化破壊を伴いました。",
        "whatDid": "先住民勢力との同盟と軍事力を用いてテノチティトランを陥落させ、アステカ帝国を征服しました。",
        "whyImportant": "スペインのアメリカ大陸支配を決定的に進める一方、先住民社会へ大規模な破壊をもたらしたためです。"
      },
      "image": "assets/people/person-0abcdd8016.webp",
      "imageAlt": "エルナン・コルテスの画像"
    },
    "ローザ・パークス": {
      "name": "ローザ・パークス",
      "kana": "ローザ・パークス",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカ南部アラバマ州の裁縫師・公民権活動家で、長く全米黒人地位向上協会に参加していました。1955年、モンゴメリーの市バスで白人へ席を譲るよう命じられましたが拒否し、逮捕されました。この行動は偶然の反発ではなく、差別撤廃運動の蓄積の上にありました。黒人住民は一年以上にわたりバス利用を拒否し、連邦裁判所は人種分離を違憲と判断しました。その後も住宅・雇用・政治参加の平等を求め続けました。",
        "whatDid": "人種分離されたバスで席を譲る命令を拒否し、モンゴメリー・バス・ボイコットのきっかけをつくりました。",
        "whyImportant": "日常生活の差別へ抵抗し、公民権運動を全国的な非暴力運動へ発展させる象徴となったためです。"
      },
      "image": "assets/people/person-6fc044a210.webp",
      "imageAlt": "ローザ・パークスの画像"
    },
    "アクエンアテン": {
      "name": "アクエンアテン",
      "kana": "アクエンアテン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王で、即位当初はアメンホテプ4世と名乗りました。太陽円盤神アテンを唯一に近い特別な神として崇拝し、名をアクエンアテンへ改め、新都アケトアテンを建設しました。アメン神官団の力を弱め、王家がアテンと人々を結ぶ宗教体制を進めました。美術では王族の身体や家庭場面を従来より自然に表しました。しかし死後、改革は急速に否定され、都と王名も破壊されました。宗教改革の目的や範囲には議論があります。",
        "whatDid": "アテン神崇拝を中心とする宗教改革を進め、新都アケトアテンを建設して神官勢力を再編しました。",
        "whyImportant": "古代エジプトで王権主導の大規模宗教改革を行い、宗教・美術・政治の関係を大きく変えたためです。"
      },
      "image": "assets/people/person-5282fde232.webp",
      "imageAlt": "アクエンアテンの画像"
    },
    "ネロ": {
      "name": "ネロ",
      "kana": "ネロ",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ第5代皇帝で、母アグリッピナの支援により若くして即位しました。治世初期は哲学者セネカらの補佐で比較的安定しましたが、のちに母や妻を殺害し、反対派への弾圧を強めました。64年のローマ大火後には復興と都市整備を進めましたが、キリスト教徒を処刑し、豪華な黄金宮殿を建てたことで反感を招きました。火災を自ら起こしたという話は確証がありません。各地の反乱で元老院に敵とされ、自殺しました。",
        "whatDid": "ローマ大火後の復興を進める一方、皇族・反対派・キリスト教徒を弾圧し、反乱の末に自殺しました。",
        "whyImportant": "皇帝独裁の不安定さと、後世の暴君像が史実・宣伝・伝承から形成される過程を示すためです。"
      },
      "image": "assets/people/person-b5cbace420.webp",
      "imageAlt": "ネロの画像"
    },
    "孟子": {
      "name": "孟子",
      "kana": "孟子",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代の中国で諸国を巡り、君主へ儒家の政治を説いた思想家です。人には他人の苦しみを見過ごせない心があり、それを育てれば仁・義・礼・智という徳になると考え、性善説を唱えました。民衆の生活を安定させる「仁政」を行うことが王の務めであり、暴政を行う君主は正当性を失うと主張しました。言行は弟子たちとまとめた『孟子』に伝わります。孔子の教えを政治思想として発展させ、後の儒教に大きな影響を与えました。",
        "whatDid": "性善説と仁政を説き、民衆を苦しめる君主は統治の正当性を失うと主張しました。",
        "whyImportant": "儒教を人間観と政治論の両面で発展させ、東アジアの道徳・教育・統治思想へ影響したためです。"
      },
      "image": "assets/people/person-e106018db8.webp",
      "imageAlt": "孟子の画像"
    },
    "フランシスコ・ピサロ": {
      "name": "フランシスコ・ピサロ",
      "kana": "フランシスコ・ピサロ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "スペイン出身の征服者で、パナマから南アメリカ西岸を探検し、インカ帝国の富を知りました。1532年、内戦直後のインカへ少数の兵で入り、カハマルカで皇帝アタワルパを捕らえました。莫大な身代金を受け取った後も皇帝を処刑し、1533年に都クスコを占領しました。先住民の協力者、鉄製武器、馬、天然痘による混乱を利用して支配を広げました。スペイン植民地を築く一方、略奪・強制労働・内戦を招き、最後は対立するスペイン人に殺されました。",
        "whatDid": "内戦中のインカ帝国へ侵入し、皇帝アタワルパを捕らえて処刑し、クスコを占領しました。",
        "whyImportant": "スペインの南米支配を確立する一方、インカ社会へ略奪・強制労働・人口減少をもたらしたためです。"
      },
      "image": "assets/people/person-1f806de4af.webp",
      "imageAlt": "フランシスコ・ピサロの画像"
    },
    "ロベスピエール": {
      "name": "ロベスピエール",
      "kana": "ロベスピエール",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "フランス革命期の弁護士・政治家で、身分制廃止、普通選挙、奴隷制反対などを主張し、「清廉の人」と呼ばれました。王政廃止後はジャコバン派の指導者となり、対外戦争と国内反乱の危機に対して公安委員会を中心とする非常体制を進めました。革命裁判所と反革命容疑者法のもとで多数が処刑され、この時期は恐怖政治と呼ばれます。徳に基づく共和国を目指しましたが、1794年に政敵から独裁を疑われ、逮捕・処刑されました。",
        "whatDid": "共和政と社会改革を進めましたが、公安委員会を中心に反対派を処刑する恐怖政治を主導しました。",
        "whyImportant": "革命を守る非常措置が権利侵害と政治的暴力へ転じる危険を示す中心人物だからです。"
      },
      "image": "assets/people/person-5fd87cbb63.webp",
      "imageAlt": "ロベスピエールの画像"
    },
    "カール・マルクス": {
      "name": "カール・マルクス",
      "kana": "カール・マルクス",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "political-scholar",
        "society"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "19世紀ドイツ生まれの思想家・経済学者で、政治活動のため各地を追われ、ロンドンで研究を続けました。エンゲルスと『共産党宣言』を著し、歴史は生産手段を持つ階級と働く階級の対立によって動くと論じました。『資本論』では、労働者が生み出した価値の一部を資本家が利潤として得る仕組みや、資本主義が恐慌と格差を生む過程を分析しました。その思想は労働運動と社会主義革命へ影響しましたが、後の国家による実践は本人の構想と同一ではありません。",
        "whatDid": "資本主義の階級対立と利潤の仕組みを分析し、『共産党宣言』『資本論』で社会変革を論じました。",
        "whyImportant": "資本主義批判と社会主義思想を体系化し、世界の労働運動・革命・政治体制へ大きく影響したためです。"
      },
      "image": "assets/people/person-b83304a316.webp",
      "imageAlt": "カール・マルクスの画像"
    },
    "マルコムX": {
      "name": "マルコムX",
      "kana": "マルコムX",
      "era": "今につながる時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの黒人公民権活動家で、犯罪と服役を経験した後、ネーション・オブ・イスラムへ入りました。白人社会の人種差別を激しく批判し、黒人の自尊心、経済的自立、自衛を訴え、非暴力だけを求める運動とは異なる立場を示しました。組織と決別後、メッカ巡礼で多民族のイスラム教徒に接し、人種を越えた連帯へ考えを広げました。アフリカ系アメリカ人の問題を国際的人権問題として訴えましたが、1965年に演説会場で暗殺されました。",
        "whatDid": "黒人の自尊・自立・自衛を訴え、後には人種差別を国際的な人権問題として提起しました。",
        "whyImportant": "公民権運動に急進的な視点を加え、黒人解放・自己決定・国際的人権の議論を広げたためです。"
      },
      "image": "assets/people/person-99c89b30cf.webp",
      "imageAlt": "マルコムXの画像"
    },
    "ネフェルティティ": {
      "name": "ネフェルティティ",
      "kana": "ネフェルティティ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第18王朝の王妃で、アクエンアテンの妻として宗教改革を支えました。アテン神へ祈る場面では夫とほぼ同じ大きさで描かれ、娘たちとともに王家の親密な家庭像を示しました。新都アケトアテンの神殿儀礼にも重要な役割を持ち、単なる王妃以上の政治・宗教的地位を持ったと考えられます。有名な彩色胸像はアマルナ美術を代表します。晩年の記録は少なく、共同統治者や王になったという説もありますが、確定していません。",
        "whatDid": "アクエンアテンの宗教改革を支え、アテン神の儀礼と王権表現で重要な役割を果たしました。",
        "whyImportant": "アマルナ時代の女性王権・宗教改革・美術を示す中心人物であり、王妃の政治的地位を考えられるためです。"
      },
      "image": "assets/people/person-84e02673e2.webp",
      "imageAlt": "ネフェルティティの画像"
    },
    "アッシュルバニパル": {
      "name": "アッシュルバニパル",
      "kana": "アッシュルバニパル",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "紀元前7世紀の新アッシリア王で、父エサルハドンの後継者として帝国を統治しました。エジプトへ遠征し、兄が治めるバビロンの反乱を鎮圧し、エラムを破壊して領土を維持しました。宮殿浮彫には獅子狩りや戦争が精密に描かれています。一方、ニネヴェの宮殿に各地の粘土板を集め、神話・医学・占星術・辞書などを収めた大図書館を築きました。『ギルガメシュ叙事詩』が現代へ伝わったのも、この収集のおかげです。",
        "whatDid": "軍事遠征で帝国を維持し、ニネヴェに膨大な粘土板を集めた王立図書館を築きました。",
        "whyImportant": "新アッシリアの最盛期を支え、古代メソポタミアの文学・科学・宗教文書を後世へ残したためです。"
      },
      "image": "assets/people/person-ed3d36e598.webp",
      "imageAlt": "アッシュルバニパルの画像"
    },
    "荀子": {
      "name": "荀子",
      "kana": "荀子",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代末の中国で活動した儒家の思想家です。人は生まれつき欲望を持つため、そのままでは争いが起こるという「性悪説」を唱えました。ただし人が悪のままでよいとしたのではなく、学問・礼・正しい習慣によって欲望を整え、善へ変わることができると説きました。天を人間社会へ直接罰や恵みを与える存在とは考えず、自然の法則として捉えました。弟子に韓非や李斯がいたとされ、儒家と法家の双方へ影響しました。",
        "whatDid": "性悪説を唱え、学問と礼によって欲望を整え、人は善へ変わることができると説きました。",
        "whyImportant": "儒教に教育と制度を重視する現実的な人間観を加え、法家思想の形成にも影響したためです。"
      },
      "image": "assets/people/person-4a209266db.webp",
      "imageAlt": "荀子の画像"
    },
    "ダントン": {
      "name": "ダントン",
      "kana": "ダントン",
      "era": "革命と工場の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "フランス革命期の弁護士・政治家で、王政打倒を進めたコルドリエ派の指導者でした。1792年に司法大臣となり、外国軍の侵攻に対して国民へ抵抗を呼びかけました。王政廃止後は国民公会で活動し、革命裁判所や公安委員会の成立にも関わりました。しかし戦局が安定すると処刑の縮小と和解を求め、ロベスピエール派と対立しました。汚職の疑いも受け、1794年に仲間とともに処刑されました。革命的暴力を推進した側が、その停止を求めて犠牲となった人物です。",
        "whatDid": "王政打倒と共和国防衛を進め、革命政府の制度を整えましたが、恐怖政治の緩和を求めて処刑されました。",
        "whyImportant": "革命を守る暴力に関わった指導者が、その暴力の拡大によって排除される革命政治の矛盾を示すためです。"
      },
      "image": "assets/people/person-c4428b21b4.webp",
      "imageAlt": "ダントンの画像"
    },
    "山本五十六": {
      "name": "山本五十六",
      "kana": "山本五十六",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本海軍の軍人で、アメリカ駐在経験から同国の工業力を理解し、日独伊三国同盟や対米長期戦に慎重でした。しかし連合艦隊司令長官として開戦が決まると、真珠湾攻撃を立案して太平洋戦争の初期作戦を指揮しました。空母航空戦力を重視しましたが、ミッドウェー海戦では作戦の複雑さと情報判断の失敗により主力空母四隻を失いました。1943年、前線視察中に暗号を解読した米軍機に撃墜され死亡しました。慎戦論と開戦実行の責任を併せ持つ人物です。",
        "whatDid": "真珠湾攻撃と太平洋初期作戦を指揮し、空母航空戦を進めましたが、ミッドウェー海戦で大敗しました。",
        "whyImportant": "対米戦の危険を理解しながら開戦作戦を実行し、日本海軍の初期勝利と戦局転換の双方を担ったためです。"
      },
      "image": "assets/people/person-42eae66dde.webp",
      "imageAlt": "山本五十六の画像"
    },
    "ツタンカーメン": {
      "name": "ツタンカーメン",
      "kana": "ツタンカーメン",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "少年期に即位した古代エジプト第18王朝の王で、アクエンアテンの宗教改革後に統治しました。名をツタンカーテンからツタンカーメンへ改め、都をアケトアテンから移し、アメン神をはじめ従来の神々と神殿を復興しました。十代後半で死去し、政治的実績は大きくありません。しかし1922年にハワード・カーターが王墓を発見し、黄金の棺・仮面・日用品がほぼまとまって残っていたため、新王国の葬祭・工芸・王宮生活を知る重要資料となりました。",
        "whatDid": "アテン中心の宗教改革を終わらせ、アメン神など従来の信仰と神殿を復興しました。",
        "whyImportant": "ほぼ未盗掘の王墓と膨大な副葬品が発見され、新王国エジプトの葬祭と生活を詳しく伝えるためです。"
      },
      "image": "assets/people/person-0d6bdcc9e3.webp",
      "imageAlt": "ツタンカーメンの画像"
    },
    "アルキメデス": {
      "name": "アルキメデス",
      "kana": "アルキメデス",
      "era": "古代の大国の時代",
      "genre": [
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前3世紀、シチリア島の都市シラクサで活動した数学者・技術者です。円周率の範囲、球と円柱の体積、放物線で囲まれた面積などを、図形を細かく分けて考える方法で求めました。物体が液体から受ける浮力を説明する「アルキメデスの原理」や、てこの原理も体系化しました。ローマ軍によるシラクサ包囲では防御機械を考案したと伝えられ、陥落時に殺されました。「エウレカ」の逸話には伝説的要素があります。",
        "whatDid": "浮力とてこの原理を明らかにし、円・球・放物線の面積や体積を高度な数学で求めました。",
        "whyImportant": "数学的証明を力学・流体・工学へ結び付け、近代科学へ続く数理的な自然研究の模範となったためです。"
      },
      "image": "assets/people/person-490f054f0e.webp",
      "imageAlt": "アルキメデスの画像"
    },
    "韓非": {
      "name": "韓非",
      "kana": "韓非",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar",
        "philosopher"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "戦国時代末の韓の王族出身の思想家で、荀子に学んだと伝えられます。人は利益を求めて行動すると考え、君主は道徳的な善意に頼らず、明確な法律である「法」、役職に伴う権力である「勢」、臣下を操る統治技術である「術」を組み合わせるべきだと説きました。著作『韓非子』は秦王政に高く評価されましたが、秦へ使者として赴いた後、李斯らの策で投獄され、自害しました。その思想は秦の中央集権統治へ強く影響しました。",
        "whatDid": "法・勢・術を組み合わせ、君主が法律と制度によって臣下と国家を統制する方法を体系化しました。",
        "whyImportant": "法家思想を完成させ、秦の中央集権国家と後世の中国統治論へ大きな影響を与えたためです。"
      },
      "image": "assets/people/person-a053e7d49e.webp",
      "imageAlt": "韓非の画像"
    },
    "忽必烈": {
      "name": "忽必烈",
      "kana": "忽必烈",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "チンギス・ハンの孫で、モンゴル帝国の大ハンとなり、1271年に中国王朝として元を建てました。南宋を滅ぼして中国全土を統一し、都を大都、現在の北京に置きました。中国式の官僚制度を取り入れながら、モンゴル人を頂点とする身分秩序を設け、駅伝制と紙幣で広域支配と交易を支えました。朝鮮や東南アジアへ影響を広げましたが、日本・ベトナム・ジャワへの遠征は失敗し、戦費と重税が国民へ大きな負担を与えました。",
        "whatDid": "元を建てて南宋を滅ぼし、中国を統一して大都・駅伝制・紙幣を用いた広域統治を進めました。",
        "whyImportant": "モンゴル帝国と中国王朝を結び、ユーラシア交流を活発化させる一方、遠征と支配の負担も残したためです。"
      },
      "image": "assets/people/person-148ecd3c87.webp",
      "imageAlt": "忽必烈の画像"
    },
    "アベラール": {
      "name": "アベラール",
      "kana": "アベラール",
      "era": "交易と王国の時代",
      "genre": [
        "culture",
        "philosopher"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "11～12世紀フランスの哲学者・神学者で、各地の学者と公開討論を行い、パリで多くの学生を集めました。普遍的な概念が実在するかをめぐる問題で、言葉と個物の関係を論理的に考えました。著書『然りと否』では、教父たちの矛盾する文章を並べ、権威ある言葉も理性によって検討する方法を示しました。弟子エロイーズとの恋愛と結婚、処罰、修道生活は往復書簡で知られます。信仰と理性を結ぶスコラ学の発展に重要な役割を果たしました。",
        "whatDid": "討論と『然りと否』を通して、権威ある教説の矛盾を論理的に検討する学問方法を広めました。",
        "whyImportant": "信仰内容を理性と論理で研究するスコラ学を発展させ、中世大学の討論教育へ影響したためです。"
      },
      "image": "assets/people/person-309ec64c9d.webp",
      "imageAlt": "アベラールの画像"
    },
    "ホセ・リサール": {
      "name": "ホセ・リサール",
      "kana": "ホセ・リサール",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "スペイン植民地時代のフィリピンの医師・作家・改革運動家です。ヨーロッパ留学で医学を学び、小説『ノリ・メ・タンヘレ』『エル・フィリブステリスモ』により、修道会の権力、植民地官僚の腐敗、差別を告発しました。武装独立ではなく、法の平等、教育、言論の自由、フィリピン人の政治参加を求めました。秘密結社カティプナンの反乱への直接関与はありませんでしたが、反乱の責任を問われ、1896年に処刑されました。その死は独立運動を強く刺激しました。",
        "whatDid": "小説でスペイン植民地支配と修道会の腐敗を批判し、教育・法の平等・政治参加を求めました。",
        "whyImportant": "言論による改革運動と処刑がフィリピン人の民族意識を高め、独立革命の象徴となったためです。"
      },
      "image": "assets/people/person-4f62e94133.webp",
      "imageAlt": "ホセ・リサールの画像"
    },
    "東条英機": {
      "name": "東条英機",
      "kana": "東条英機",
      "era": "二つの世界大戦の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本陸軍の軍人・政治家で、満州事変後に関東軍参謀長、陸軍大臣などを務め、軍部主導の政治を進めました。1941年に首相となり、対米交渉を続けながらも開戦を決定し、真珠湾攻撃によって太平洋戦争へ入りました。首相・陸相を兼ねて権力を集中しましたが、戦局悪化とサイパン島陥落後に辞任しました。敗戦後、極東国際軍事裁判で侵略戦争を指導した責任を問われ、死刑判決を受けて処刑されました。",
        "whatDid": "首相として対米開戦を決定し、太平洋戦争中に軍事・政治権力を集中して戦争を指導しました。",
        "whyImportant": "日本の対米英開戦と侵略戦争を最高指導部で進め、戦争指導責任を裁かれた中心人物だからです。"
      },
      "image": "assets/people/person-6a7901b7d1.webp",
      "imageAlt": "東条英機の画像"
    },
    "アイ": {
      "name": "アイ",
      "kana": "アイ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "ツタンカーメンの死後に王となった高官です。少年王の時代から宮廷で大きな力を持ち、伝統的な神々への復帰を進めました。王位についた経緯には、資料が少なく不明な点があります。",
        "whatDid": "王権と建築を示す人",
        "whyImportant": "アイを知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ホルエムヘブ": {
      "name": "ホルエムヘブ",
      "kana": "ホルエムヘブ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "war",
        "religion",
        "culture"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第十八王朝末の王です。軍人から王となり、役人の不正を禁じる法令を出しました。アテン信仰の時代を公式の王名表から省き、王朝の歴史を作り直しました。",
        "whatDid": "王権と建築を示す人",
        "whyImportant": "ホルエムヘブを知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "セティ1世": {
      "name": "セティ1世",
      "kana": "セティ1世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第十九王朝の王です。シリア・パレスチナ方面で失った地域の回復を目指し、カルナック神殿を整備しました。王墓の壁画には、神々や死後の世界についての考えが描かれています。",
        "whatDid": "王権と建築を示す人",
        "whyImportant": "セティ1世を知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラムセス2世": {
      "name": "ラムセス2世",
      "kana": "ラムセス2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "war",
        "culture"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "古代エジプト第十九王朝の王です。ヒッタイトとカデシュで戦い、のちに和平を結びました。アブ・シンベル神殿など大建築を残しましたが、記録には王の功績を大きく見せる面もあります。",
        "whatDid": "王権と建築を示す人",
        "whyImportant": "ラムセス2世を知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラムセス3世": {
      "name": "ラムセス3世",
      "kana": "ラムセス3世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "ラムセス3世は古代エジプトでナイル川流域の王権、神殿、墓、文字文化に関わりました。巨大な建築や記録から、都市と国家のしくみが見えてきます。",
        "whatDid": "王権と建築を示す人",
        "whyImportant": "ラムセス3世を知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イムホテプ": {
      "name": "イムホテプ",
      "kana": "イムホテプ",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "古代エジプト",
      "title": "王権と建築を示す人",
      "icon": "👑",
      "modal": {
        "profile": "イムホテプは古代エジプトでナイル川流域の王権、神殿、墓、文字文化に関わりました。巨大な建築や記録から、都市と国家のしくみが見えてきます。",
        "whatDid": "王権と建築を示す人",
        "whyImportant": "イムホテプを知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ネブカドネザル2世": {
      "name": "ネブカドネザル2世",
      "kana": "ネブカドネザル2世",
      "era": "最初の文明の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "新バビロニア王国の王です。エルサレムを攻略し、多くのユダヤ人をバビロンへ移しました。城壁や神殿を整え、バビロンを大都市にしましたが、強制移住は大きな苦しみを生みました。",
        "whatDid": "都市と文字を動かした人",
        "whyImportant": "ネブカドネザル2世を知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ナボニドス": {
      "name": "ナボニドス",
      "kana": "ナボニドス",
      "era": "最初の文明の時代",
      "genre": [
        "other"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "新バビロニア王国最後の王です。古い神殿や碑文に関心をもち、アラビアのテイマに長く滞在しました。その間、息子ベルシャザルがバビロンの政務を担い、国はペルシャに敗れました。",
        "whatDid": "都市と文字を動かした人",
        "whyImportant": "ナボニドスを知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ベルシャザル": {
      "name": "ベルシャザル",
      "kana": "ベルシャザル",
      "era": "最初の文明の時代",
      "genre": [
        "war"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "新バビロニア王国のナボニドス王の息子です。父が不在の間、バビロンで軍や政務を任されました。正式な王ではなく皇太子・摂政だったことが、粘土板などの資料から分かっています。",
        "whatDid": "都市と文字を動かした人",
        "whyImportant": "ベルシャザルを知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヒラム1世": {
      "name": "ヒラム1世",
      "kana": "ヒラム1世",
      "era": "最初の文明の時代",
      "genre": [
        "culture"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "フェニキアの港市ティルスの王です。レバノン杉や職人の技術を用いた交易を進め、海上都市を豊かにしました。後世の記録ではイスラエルのソロモン王との協力でも知られます。",
        "whatDid": "都市と文字を動かした人",
        "whyImportant": "ヒラム1世を知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ディド": {
      "name": "ディド",
      "kana": "ディド",
      "era": "最初の文明の時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "メソポタミアと西アジア",
      "title": "都市と文字を動かした人",
      "icon": "📜",
      "modal": {
        "profile": "ディドはメソポタミアと西アジアで都市国家、法律、楔形文字、軍事、交易に関わりました。川の文明で人々をまとめるしくみが育ったことを示します。",
        "whatDid": "都市と文字を動かした人",
        "whyImportant": "ディドを知ると、最初の文明の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エラトステネス": {
      "name": "エラトステネス",
      "kana": "エラトステネス",
      "era": "古代の大国の時代",
      "genre": [
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "アレクサンドリア図書館の学者です。別の町でできる影の長さを比べ、地球の大きさをかなり正確に計算しました。地図づくりや地理学の基礎を作った人物の一人です。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "エラトステネスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アリスタルコス": {
      "name": "アリスタルコス",
      "kana": "アリスタルコス",
      "era": "古代の大国の時代",
      "genre": [
        "culture",
        "philosopher",
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "古代ギリシャの天文学者です。太陽の周りを地球が回るという考えを早くから示したと伝えられます。当時は広く受け入れられませんでしたが、宇宙を見る別の考え方を残しました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "アリスタルコスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "サッフォー": {
      "name": "サッフォー",
      "kana": "サッフォー",
      "era": "古代の大国の時代",
      "genre": [
        "philosopher"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "エーゲ海のレスボス島出身の詩人です。愛、友情、祭りを歌った詩を残し、古代ギリシャの音楽や人々の感情を伝えました。作品の多くは失われ、現在は短い断片を中心に読まれています。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "サッフォーを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アイスキュロス": {
      "name": "アイスキュロス",
      "kana": "アイスキュロス",
      "era": "古代の大国の時代",
      "genre": [
        "war",
        "philosopher"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "古代アテネの劇作家です。神話を使いながら、戦争、正義、復讐を考える悲劇を書きました。『ペルシア人』では、敵側の悲しみにも目を向け、戦争の被害を描きました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "アイスキュロスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ソフォクレス": {
      "name": "ソフォクレス",
      "kana": "ソフォクレス",
      "era": "古代の大国の時代",
      "genre": [
        "culture",
        "philosopher",
        "science",
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "ソフォクレスは古代ギリシャで都市国家の政治、哲学、数学、医学、文学に関わりました。話し合い、学び、芸術が後の世界へ広がる出発点になりました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "ソフォクレスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エウリピデス": {
      "name": "エウリピデス",
      "kana": "エウリピデス",
      "era": "古代の大国の時代",
      "genre": [
        "culture",
        "philosopher",
        "science",
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "エウリピデスは古代ギリシャで都市国家の政治、哲学、数学、医学、文学に関わりました。話し合い、学び、芸術が後の世界へ広がる出発点になりました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "エウリピデスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アリストファネス": {
      "name": "アリストファネス",
      "kana": "アリストファネス",
      "era": "古代の大国の時代",
      "genre": [
        "culture",
        "philosopher",
        "science",
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "アリストファネスは古代ギリシャで都市国家の政治、哲学、数学、医学、文学に関わりました。話し合い、学び、芸術が後の世界へ広がる出発点になりました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "アリストファネスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フィディアス": {
      "name": "フィディアス",
      "kana": "フィディアス",
      "era": "古代の大国の時代",
      "genre": [
        "culture",
        "philosopher",
        "science",
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "フィディアスは古代ギリシャで都市国家の政治、哲学、数学、医学、文学に関わりました。話し合い、学び、芸術が後の世界へ広がる出発点になりました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "フィディアスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "プトレマイオス": {
      "name": "プトレマイオス",
      "kana": "プトレマイオス",
      "era": "古代の大国の時代",
      "genre": [
        "culture",
        "philosopher",
        "science",
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "プトレマイオスは古代ギリシャで都市国家の政治、哲学、数学、医学、文学に関わりました。話し合い、学び、芸術が後の世界へ広がる出発点になりました。",
        "whatDid": "政治と学問を広げた人",
        "whyImportant": "プトレマイオスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トラヤヌス": {
      "name": "トラヤヌス",
      "kana": "トラヤヌス",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ帝国の皇帝です。ダキアを征服し、帝国の領土を最も広くしました。ローマ市内に市場や記念柱を建て、道路や福祉にも力を入れましたが、拡大は軍事負担も大きくしました。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "トラヤヌスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ハドリアヌス": {
      "name": "ハドリアヌス",
      "kana": "ハドリアヌス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ帝国の皇帝です。領土をさらに広げるより、国境と都市を守る方針を取りました。ブリタンニア北部のハドリアヌスの長城を築き、各地を旅して行政を確かめました。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "ハドリアヌスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マルクス・アウレリウス": {
      "name": "マルクス・アウレリウス",
      "kana": "マルクス・アウレリウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "philosopher",
        "society"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ帝国の皇帝で、哲学書『自省録』を書いた人物です。国境での戦いと疫病に苦しむ中、義務や心のあり方を考えました。五賢帝の一人ですが、平和な時代だけの皇帝ではありません。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "マルクス・アウレリウスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "コンモドゥス": {
      "name": "コンモドゥス",
      "kana": "コンモドゥス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ローマ帝国の皇帝です。父マルクス・アウレリウスの後を継ぎましたが、政治より競技や自分を英雄に見せることへ傾いたと同時代の記録は批判します。暗殺後、帝国は再び混乱しました。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "コンモドゥスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "セプティミウス・セウェルス": {
      "name": "セプティミウス・セウェルス",
      "kana": "セプティミウス・セウェルス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "セプティミウス・セウェルスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "セプティミウス・セウェルスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "カラカラ": {
      "name": "カラカラ",
      "kana": "カラカラ",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "カラカラはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "カラカラを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ディオクレティアヌス": {
      "name": "ディオクレティアヌス",
      "kana": "ディオクレティアヌス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ディオクレティアヌスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "ディオクレティアヌスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ユリアヌス": {
      "name": "ユリアヌス",
      "kana": "ユリアヌス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ユリアヌスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "ユリアヌスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "テオドシウス": {
      "name": "テオドシウス",
      "kana": "テオドシウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "テオドシウスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "テオドシウスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スパルタクス": {
      "name": "スパルタクス",
      "kana": "スパルタクス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "スパルタクスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "スパルタクスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウェルギリウス": {
      "name": "ウェルギリウス",
      "kana": "ウェルギリウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "ウェルギリウスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "ウェルギリウスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "オウィディウス": {
      "name": "オウィディウス",
      "kana": "オウィディウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "オウィディウスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "オウィディウスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "セネカ": {
      "name": "セネカ",
      "kana": "セネカ",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "セネカはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "セネカを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "タキトゥス": {
      "name": "タキトゥス",
      "kana": "タキトゥス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "タキトゥスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "タキトゥスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "プリニウス": {
      "name": "プリニウス",
      "kana": "プリニウス",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "ローマと地中海",
      "title": "法律と帝国を形づくった人",
      "icon": "🏛️",
      "modal": {
        "profile": "プリニウスはローマと地中海で共和政、帝政、法律、道路、都市文化に関わりました。広い地域を治める制度と、その負担の両方を示します。",
        "whatDid": "法律と帝国を形づくった人",
        "whyImportant": "プリニウスを知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "李斯": {
      "name": "李斯",
      "kana": "李斯",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "political-scholar"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "秦の始皇帝に仕えた政治家です。文字や度量衡をそろえる政策を進め、中央集権の国づくりに関わりました。一方で思想を取り締まる政策にも関わり、最後は政争で処刑されました。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "李斯を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "項羽": {
      "name": "項羽",
      "kana": "項羽",
      "era": "古代の大国の時代",
      "genre": [
        "war"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "秦が倒れた後の中国で、強い軍を率いた武将です。劉邦と覇権を争い、一時は広い地域を支配しました。戦いでは勇敢でしたが、味方をまとめきれず垓下で敗れて自害しました。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "項羽を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "劉邦": {
      "name": "劉邦",
      "kana": "劉邦",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "農民出身から秦の滅亡後の争いを勝ち抜き、漢を建てた皇帝です。項羽を破り、役人の仕組みと地方の王を組み合わせて国を治めました。漢は長く中国の基礎となりました。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "劉邦を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "呂后": {
      "name": "呂后",
      "kana": "呂后",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "漢の高祖・劉邦の皇后で、夫の死後に幼い皇帝を支えて実権を持ちました。一族を重用したため後世の史書では厳しく描かれますが、女性が政治を動かした重要な例でもあります。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "呂后を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "武帝": {
      "name": "武帝",
      "kana": "武帝",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "武帝はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "武帝を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "司馬遷": {
      "name": "司馬遷",
      "kana": "司馬遷",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "司馬遷はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "司馬遷を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "張騫": {
      "name": "張騫",
      "kana": "張騫",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "張騫はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "張騫を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "王莽": {
      "name": "王莽",
      "kana": "王莽",
      "era": "古代の大国の時代",
      "genre": [
        "religion"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "王莽はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "王莽を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "班固": {
      "name": "班固",
      "kana": "班固",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "班固はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "班固を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "班昭": {
      "name": "班昭",
      "kana": "班昭",
      "era": "古代の大国の時代",
      "genre": [
        "culture"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "班昭はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "班昭を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "張衡": {
      "name": "張衡",
      "kana": "張衡",
      "era": "古代の大国の時代",
      "genre": [
        "science"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "張衡はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "張衡を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "蔡倫": {
      "name": "蔡倫",
      "kana": "蔡倫",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "蔡倫はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "蔡倫を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "華佗": {
      "name": "華佗",
      "kana": "華佗",
      "era": "古代の大国の時代",
      "genre": [
        "medicine"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "華佗はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "華佗を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "曹操": {
      "name": "曹操",
      "kana": "曹操",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "曹操はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "曹操を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "劉備": {
      "name": "劉備",
      "kana": "劉備",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "war"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "劉備はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "劉備を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "諸葛亮": {
      "name": "諸葛亮",
      "kana": "諸葛亮",
      "era": "古代の大国の時代",
      "genre": [
        "politician",
        "war",
        "political-scholar"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "諸葛亮はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "諸葛亮を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "卑弥呼": {
      "name": "卑弥呼",
      "kana": "卑弥呼",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "religion"
      ],
      "field": "ペルシャ・インド・中国",
      "title": "広い国をまとめた人",
      "icon": "👑",
      "modal": {
        "profile": "卑弥呼はペルシャ・インド・中国で王朝、官僚、思想、宗教、交易に関わりました。広い土地を治める政治と、多様な人々を結ぶしくみを示します。",
        "whatDid": "広い国をまとめた人",
        "whyImportant": "卑弥呼を知ると、古代の大国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "法顕": {
      "name": "法顕",
      "kana": "法顕",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "中国東晋時代の僧です。戒律の経典を求めてインドへ旅し、海路で中国へ帰りました。『仏国記』にインドや東南アジアの社会を記録し、当時の国際交流を伝えました。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "法顕を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "義浄": {
      "name": "義浄",
      "kana": "義浄",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "唐の時代に海路でインドや東南アジアを旅した僧です。ナーランダー僧院で学び、仏典を中国語へ訳しました。港町シュリーヴィジャヤでの学びも記し、海の交流路を伝えました。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "義浄を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "空海": {
      "name": "空海",
      "kana": "空海",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "平安時代に唐へ渡り、長安で密教を学んだ僧です。帰国後に真言宗を開き、高野山を拠点にしました。文字、書、土木にも関心を持ち、日本の仏教文化に大きな影響を残しました。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "空海を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "最澄": {
      "name": "最澄",
      "kana": "最澄",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "平安時代に比叡山延暦寺を開き、天台宗を広めた僧です。唐で学んだ教えをもとに、僧が学ぶ場を作りました。のちの日本仏教の多くの僧が比叡山で学びました。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "最澄を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "鑑真": {
      "name": "鑑真",
      "kana": "鑑真",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "鑑真はユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "鑑真を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "聖徳太子": {
      "name": "聖徳太子",
      "kana": "聖徳太子",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "聖徳太子はユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "聖徳太子を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "推古天皇": {
      "name": "推古天皇",
      "kana": "推古天皇",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "推古天皇はユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "推古天皇を知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アル・フワーリズミー": {
      "name": "アル・フワーリズミー",
      "kana": "アル・フワーリズミー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アル・フワーリズミーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アル・フワーリズミーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アル・キンディー": {
      "name": "アル・キンディー",
      "kana": "アル・キンディー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アル・キンディーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アル・キンディーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アル・ラーズィー": {
      "name": "アル・ラーズィー",
      "kana": "アル・ラーズィー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アル・ラーズィーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アル・ラーズィーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アル・ファーラービー": {
      "name": "アル・ファーラービー",
      "kana": "アル・ファーラービー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アル・ファーラービーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アル・ファーラービーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イブン・シーナー": {
      "name": "イブン・シーナー",
      "kana": "イブン・シーナー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "イブン・シーナーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "イブン・シーナーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イブン・ルシュド": {
      "name": "イブン・ルシュド",
      "kana": "イブン・ルシュド",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "イブン・ルシュドはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "イブン・ルシュドを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アル・ビールーニー": {
      "name": "アル・ビールーニー",
      "kana": "アル・ビールーニー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アル・ビールーニーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アル・ビールーニーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ファーティマ・アル・フィフリーヤ": {
      "name": "ファーティマ・アル・フィフリーヤ",
      "kana": "ファーティマ・アル・フィフリーヤ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ファーティマ・アル・フィフリーヤはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ファーティマ・アル・フィフリーヤを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラービア・アルアダウィーヤ": {
      "name": "ラービア・アルアダウィーヤ",
      "kana": "ラービア・アルアダウィーヤ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ラービア・アルアダウィーヤはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ラービア・アルアダウィーヤを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "バスラのハサン": {
      "name": "バスラのハサン",
      "kana": "バスラのハサン",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "バスラのハサンはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "バスラのハサンを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アリー": {
      "name": "アリー",
      "kana": "アリー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アリーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アリーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アーイシャ": {
      "name": "アーイシャ",
      "kana": "アーイシャ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アーイシャはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アーイシャを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウマル": {
      "name": "ウマル",
      "kana": "ウマル",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ウマルはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ウマルを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウスマーン": {
      "name": "ウスマーン",
      "kana": "ウスマーン",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ウスマーンはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ウスマーンを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アブー・バクル": {
      "name": "アブー・バクル",
      "kana": "アブー・バクル",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "アブー・バクルはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "アブー・バクルを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ハールーン・アッラシード": {
      "name": "ハールーン・アッラシード",
      "kana": "ハールーン・アッラシード",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ハールーン・アッラシードはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ハールーン・アッラシードを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マアムーン": {
      "name": "マアムーン",
      "kana": "マアムーン",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "マアムーンはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "マアムーンを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ビラール": {
      "name": "ビラール",
      "kana": "ビラール",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ビラールはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ビラールを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "バヤジード・バスターミー": {
      "name": "バヤジード・バスターミー",
      "kana": "バヤジード・バスターミー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "バヤジード・バスターミーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "バヤジード・バスターミーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ルーミー": {
      "name": "ルーミー",
      "kana": "ルーミー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ルーミーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ルーミーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イブン・アラビー": {
      "name": "イブン・アラビー",
      "kana": "イブン・アラビー",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "イブン・アラビーはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "イブン・アラビーを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トマス・アクィナス": {
      "name": "トマス・アクィナス",
      "kana": "トマス・アクィナス",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "トマス・アクィナスはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "トマス・アクィナスを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フランチェスコ": {
      "name": "フランチェスコ",
      "kana": "フランチェスコ",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "フランチェスコはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "フランチェスコを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヒルデガルト": {
      "name": "ヒルデガルト",
      "kana": "ヒルデガルト",
      "era": "宗教と文化が広がった時代",
      "genre": [
        "religion",
        "culture",
        "philosopher"
      ],
      "field": "ユーラシアとアフリカ",
      "title": "信仰と学問を伝えた人",
      "icon": "🕊️",
      "modal": {
        "profile": "ヒルデガルトはユーラシアとアフリカで宗教、翻訳、神学、修道院、寺院、学びの場に関わりました。信仰が文化や社会の支えにも支配の力にもなったことを示します。",
        "whatDid": "信仰と学問を伝えた人",
        "whyImportant": "ヒルデガルトを知ると、宗教と文化が広がった時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マルコ・ポーロ": {
      "name": "マルコ・ポーロ",
      "kana": "マルコ・ポーロ",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "イタリアの商人で、父や叔父とともにモンゴル帝国の支配地を旅したと伝えられます。帰国後の話をまとめた本はアジアへの関心を高めましたが、内容には聞き書きや誇張の可能性もあります。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "マルコ・ポーロを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラシードゥッディーン": {
      "name": "ラシードゥッディーン",
      "kana": "ラシードゥッディーン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "war"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "イルハン朝に仕えたペルシャの役人・歴史家です。モンゴル帝国と周辺地域の歴史を『集史』にまとめました。中国、西アジア、ヨーロッパなどの情報を集め、広い地域を結ぶ歴史書を作りました。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "ラシードゥッディーンを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "バトゥ": {
      "name": "バトゥ",
      "kana": "バトゥ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "war"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "チンギス・ハンの孫で、モンゴル軍を率いてロシアや東ヨーロッパへ進んだ指導者です。ボルガ川下流にキプチャク・ハン国を築き、交易を支配しましたが、征服は大きな被害を生みました。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "バトゥを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フレグ": {
      "name": "フレグ",
      "kana": "フレグ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "フレグは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "フレグを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ティムール": {
      "name": "ティムール",
      "kana": "ティムール",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "ティムールは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "ティムールを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウルグ・ベク": {
      "name": "ウルグ・ベク",
      "kana": "ウルグ・ベク",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "ウルグ・ベクは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "ウルグ・ベクを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "バーブル": {
      "name": "バーブル",
      "kana": "バーブル",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "バーブルは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "バーブルを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アクバル": {
      "name": "アクバル",
      "kana": "アクバル",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "アクバルは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "アクバルを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "シャー・ジャハーン": {
      "name": "シャー・ジャハーン",
      "kana": "シャー・ジャハーン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "シャー・ジャハーンは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "シャー・ジャハーンを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アウラングゼーブ": {
      "name": "アウラングゼーブ",
      "kana": "アウラングゼーブ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "アウラングゼーブは東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "アウラングゼーブを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "世宗": {
      "name": "世宗",
      "kana": "世宗",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "世宗は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "世宗を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "李成桂": {
      "name": "李成桂",
      "kana": "李成桂",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "李成桂は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "李成桂を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "新羅の善徳女王": {
      "name": "新羅の善徳女王",
      "kana": "新羅の善徳女王",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "新羅の善徳女王は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "新羅の善徳女王を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "王建": {
      "name": "王建",
      "kana": "王建",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "王建は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "王建を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "北条時宗": {
      "name": "北条時宗",
      "kana": "北条時宗",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "北条時宗は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "北条時宗を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "足利義満": {
      "name": "足利義満",
      "kana": "足利義満",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "足利義満は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "足利義満を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "一休宗純": {
      "name": "一休宗純",
      "kana": "一休宗純",
      "era": "交易と王国の時代",
      "genre": [
        "monarch"
      ],
      "field": "東アジアと中央アジア",
      "title": "交易と王朝を動かした人",
      "icon": "🐫",
      "modal": {
        "profile": "一休宗純は東アジアと中央アジアで王朝政治、草原の交流、紙や印刷、海と陸の交易に関わりました。人と物と考えが広く移動した時代を表します。",
        "whatDid": "交易と王朝を動かした人",
        "whyImportant": "一休宗純を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "クリスティーヌ・ド・ピザン": {
      "name": "クリスティーヌ・ド・ピザン",
      "kana": "クリスティーヌ・ド・ピザン",
      "era": "交易と王国の時代",
      "genre": [
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "中世フランスで、女性が学び働く力を持つと文章で訴えた作家です。『女性の都』で、女性への偏見に反論しました。女性が自分の名で生計を立てた早い例の一人です。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "クリスティーヌ・ド・ピザンを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "メフメト2世": {
      "name": "メフメト2世",
      "kana": "メフメト2世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "war"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "オスマン帝国のスルタンです。1453年にコンスタンティノープルを攻略し、都をイスタンブルとして整えました。東ローマ帝国を終わらせ、地中海と黒海の政治・交易を大きく変えました。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "メフメト2世を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スレイマン1世": {
      "name": "スレイマン1世",
      "kana": "スレイマン1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "スレイマン1世はヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "スレイマン1世を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "オスマン1世": {
      "name": "オスマン1世",
      "kana": "オスマン1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "オスマン1世はヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "オスマン1世を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イブン・ハルドゥーン": {
      "name": "イブン・ハルドゥーン",
      "kana": "イブン・ハルドゥーン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "イブン・ハルドゥーンはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "イブン・ハルドゥーンを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "レオ・アフリカヌス": {
      "name": "レオ・アフリカヌス",
      "kana": "レオ・アフリカヌス",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "レオ・アフリカヌスはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "レオ・アフリカヌスを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スンジャタ": {
      "name": "スンジャタ",
      "kana": "スンジャタ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "スンジャタはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "スンジャタを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イブン・ヤースィーン": {
      "name": "イブン・ヤースィーン",
      "kana": "イブン・ヤースィーン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "イブン・ヤースィーンはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "イブン・ヤースィーンを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ソンニ・アリ": {
      "name": "ソンニ・アリ",
      "kana": "ソンニ・アリ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "ソンニ・アリはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "ソンニ・アリを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アスキア・ムハンマド": {
      "name": "アスキア・ムハンマド",
      "kana": "アスキア・ムハンマド",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "アスキア・ムハンマドはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "アスキア・ムハンマドを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エザナ": {
      "name": "エザナ",
      "kana": "エザナ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "エザナはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "エザナを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラリベラ": {
      "name": "ラリベラ",
      "kana": "ラリベラ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "ラリベラはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "ラリベラを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヤア・アサンテワ": {
      "name": "ヤア・アサンテワ",
      "kana": "ヤア・アサンテワ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "ヤア・アサンテワはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "ヤア・アサンテワを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "パチャクテク": {
      "name": "パチャクテク",
      "kana": "パチャクテク",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "パチャクテクはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "パチャクテクを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トゥパク・インカ・ユパンキ": {
      "name": "トゥパク・インカ・ユパンキ",
      "kana": "トゥパク・インカ・ユパンキ",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "トゥパク・インカ・ユパンキはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "トゥパク・インカ・ユパンキを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ワイナ・カパック": {
      "name": "ワイナ・カパック",
      "kana": "ワイナ・カパック",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "ワイナ・カパックはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "ワイナ・カパックを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "モクテスマ1世": {
      "name": "モクテスマ1世",
      "kana": "モクテスマ1世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "モクテスマ1世はヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "モクテスマ1世を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "モクテスマ2世": {
      "name": "モクテスマ2世",
      "kana": "モクテスマ2世",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "モクテスマ2世はヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "モクテスマ2世を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ネサワルコヨトル": {
      "name": "ネサワルコヨトル",
      "kana": "ネサワルコヨトル",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "ネサワルコヨトルはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "ネサワルコヨトルを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "パカル王": {
      "name": "パカル王",
      "kana": "パカル王",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "パカル王はヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "パカル王を知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イシュ・ワク・チャン": {
      "name": "イシュ・ワク・チャン",
      "kana": "イシュ・ワク・チャン",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "イシュ・ワク・チャンはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "イシュ・ワク・チャンを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "シヤフ・カック": {
      "name": "シヤフ・カック",
      "kana": "シヤフ・カック",
      "era": "交易と王国の時代",
      "genre": [
        "monarch",
        "culture"
      ],
      "field": "ヨーロッパ・アフリカ・アメリカ",
      "title": "王国と都市を支えた人",
      "icon": "🏰",
      "modal": {
        "profile": "シヤフ・カックはヨーロッパ・アフリカ・アメリカで王国、都市、大学、交易、巡礼、建築、文学に関わりました。地域ごとの文化が交流しながら発展したことを示します。",
        "whatDid": "王国と都市を支えた人",
        "whyImportant": "シヤフ・カックを知ると、交易と王国の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トゥパク・アマル2世": {
      "name": "トゥパク・アマル2世",
      "kana": "トゥパク・アマル2世",
      "era": "大航海と出会いの時代",
      "genre": [
        "society"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "18世紀ペルーで、植民地の重い税や強制労働に反対して大きな反乱を率いた人物です。反乱は鎮圧され処刑されましたが、先住民の権利と独立を求める運動の象徴になりました。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "トゥパク・アマル2世を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "クアウテモック": {
      "name": "クアウテモック",
      "kana": "クアウテモック",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "アステカ最後の支配者です。スペイン軍と同盟した先住民勢力に包囲されたテノチティトランを守りました。1521年に捕らえられ、アステカ帝国の終わりと先住民の抵抗を象徴する人物です。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "クアウテモックを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アタワルパ": {
      "name": "アタワルパ",
      "kana": "アタワルパ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "インカ帝国の皇帝です。兄との内戦に勝った直後、ピサロの軍にカハマルカで捕らえられました。身代金を集めても処刑され、スペインによるインカ支配の決定的な転機になりました。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "アタワルパを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ポカホンタス": {
      "name": "ポカホンタス",
      "kana": "ポカホンタス",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ポカホンタスは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ポカホンタスを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ワフンスナコック": {
      "name": "ワフンスナコック",
      "kana": "ワフンスナコック",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ワフンスナコックは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ワフンスナコックを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "メタコメット": {
      "name": "メタコメット",
      "kana": "メタコメット",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "メタコメットは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "メタコメットを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "テカムセ": {
      "name": "テカムセ",
      "kana": "テカムセ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "テカムセは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "テカムセを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "サミュエル・ド・シャンプラン": {
      "name": "サミュエル・ド・シャンプラン",
      "kana": "サミュエル・ド・シャンプラン",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "サミュエル・ド・シャンプランは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "サミュエル・ド・シャンプランを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ピーター・ストイフェサント": {
      "name": "ピーター・ストイフェサント",
      "kana": "ピーター・ストイフェサント",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ピーター・ストイフェサントは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ピーター・ストイフェサントを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジャン・ド・ブレブフ": {
      "name": "ジャン・ド・ブレブフ",
      "kana": "ジャン・ド・ブレブフ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ジャン・ド・ブレブフは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ジャン・ド・ブレブフを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マテオ・リッチ": {
      "name": "マテオ・リッチ",
      "kana": "マテオ・リッチ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "マテオ・リッチは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "マテオ・リッチを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フランシスコ・ザビエル": {
      "name": "フランシスコ・ザビエル",
      "kana": "フランシスコ・ザビエル",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "フランシスコ・ザビエルは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "フランシスコ・ザビエルを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "山田長政": {
      "name": "山田長政",
      "kana": "山田長政",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "山田長政は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "山田長政を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "支倉常長": {
      "name": "支倉常長",
      "kana": "支倉常長",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "支倉常長は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "支倉常長を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウィリアム・アダムス": {
      "name": "ウィリアム・アダムス",
      "kana": "ウィリアム・アダムス",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ウィリアム・アダムスは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ウィリアム・アダムスを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "織田信長": {
      "name": "織田信長",
      "kana": "織田信長",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "織田信長は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "織田信長を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "豊臣秀吉": {
      "name": "豊臣秀吉",
      "kana": "豊臣秀吉",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "豊臣秀吉は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "豊臣秀吉を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "徳川家康": {
      "name": "徳川家康",
      "kana": "徳川家康",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "徳川家康は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "徳川家康を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エリザベス1世": {
      "name": "エリザベス1世",
      "kana": "エリザベス1世",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "エリザベス1世は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "エリザベス1世を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フェリペ2世": {
      "name": "フェリペ2世",
      "kana": "フェリペ2世",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "フェリペ2世は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "フェリペ2世を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イサベル1世": {
      "name": "イサベル1世",
      "kana": "イサベル1世",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "イサベル1世は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "イサベル1世を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フェルナンド2世": {
      "name": "フェルナンド2世",
      "kana": "フェルナンド2世",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "フェルナンド2世は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "フェルナンド2世を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アフォンソ・デ・アルブケルケ": {
      "name": "アフォンソ・デ・アルブケルケ",
      "kana": "アフォンソ・デ・アルブケルケ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "アフォンソ・デ・アルブケルケは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "アフォンソ・デ・アルブケルケを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ザビエル・デ・メネゼス": {
      "name": "ザビエル・デ・メネゼス",
      "kana": "ザビエル・デ・メネゼス",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ザビエル・デ・メネゼスは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ザビエル・デ・メネゼスを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "オラウダ・イクイアーノ": {
      "name": "オラウダ・イクイアーノ",
      "kana": "オラウダ・イクイアーノ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "オラウダ・イクイアーノは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "オラウダ・イクイアーノを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アンソニー・ジョンソン": {
      "name": "アンソニー・ジョンソン",
      "kana": "アンソニー・ジョンソン",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "アンソニー・ジョンソンは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "アンソニー・ジョンソンを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ンジンガ女王": {
      "name": "ンジンガ女王",
      "kana": "ンジンガ女王",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ンジンガ女王は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ンジンガ女王を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アフォンソ1世": {
      "name": "アフォンソ1世",
      "kana": "アフォンソ1世",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "アフォンソ1世は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "アフォンソ1世を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ベンキョウ": {
      "name": "ベンキョウ",
      "kana": "ベンキョウ",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "ベンキョウは海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "ベンキョウを知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "鄭成功": {
      "name": "鄭成功",
      "kana": "鄭成功",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "鄭成功は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "鄭成功を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "林則徐": {
      "name": "林則徐",
      "kana": "林則徐",
      "era": "大航海と出会いの時代",
      "genre": [
        "war"
      ],
      "field": "海をこえた世界",
      "title": "航海と交流に関わった人",
      "icon": "⛵",
      "modal": {
        "profile": "林則徐は海をこえた世界で航海、地図、交易、征服、宣教、抵抗に関わりました。世界が近づく一方で、植民地支配と被害が広がったことを考えます。",
        "whatDid": "航海と交流に関わった人",
        "whyImportant": "林則徐を知ると、大航海と出会いの時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "オランプ・ド・グージュ": {
      "name": "オランプ・ド・グージュ",
      "kana": "オランプ・ド・グージュ",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "フランス革命期の作家です。『女性と女性市民の権利宣言』で、女性にも政治参加や法の平等が必要だと訴えました。革命政府を批判したため逮捕され、ギロチンで処刑されました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "オランプ・ド・グージュを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トゥサン・ルーヴェルチュール": {
      "name": "トゥサン・ルーヴェルチュール",
      "kana": "トゥサン・ルーヴェルチュール",
      "era": "革命と工場の時代",
      "genre": [
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "フランス植民地サン＝ドマングで奴隷身分から解放され、奴隷制廃止を求める革命を率いた指導者です。フランスに捕らえられて死去しましたが、ハイチ独立の道を開きました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "トゥサン・ルーヴェルチュールを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジャン＝ジャック・デサリーヌ": {
      "name": "ジャン＝ジャック・デサリーヌ",
      "kana": "ジャン＝ジャック・デサリーヌ",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "ハイチ革命の指導者です。トゥサンの後を継ぎ、1804年にフランスから独立したハイチを建国しました。奴隷制に抵抗して生まれた最初の黒人共和国の独立に関わりました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ジャン＝ジャック・デサリーヌを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "サン＝マルティン": {
      "name": "サン＝マルティン",
      "kana": "サン＝マルティン",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "南アメリカの独立運動を率いた軍人です。アルゼンチン、チリ、ペルーの独立に関わり、アンデス越えの遠征でスペイン軍を破りました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "サン＝マルティンを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ベルナルド・オヒギンス": {
      "name": "ベルナルド・オヒギンス",
      "kana": "ベルナルド・オヒギンス",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "political-scholar",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "チリ独立運動の指導者です。サン＝マルティンと協力してスペイン軍と戦い、独立後は最高統治者として軍や教育を整えました。強権的な政治への反発で退きました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ベルナルド・オヒギンスを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ミゲル・イダルゴ": {
      "name": "ミゲル・イダルゴ",
      "kana": "ミゲル・イダルゴ",
      "era": "革命と工場の時代",
      "genre": [
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "メキシコ独立運動の始まりを告げた神父です。1810年にドロレスの叫びで民衆に蜂起を呼びかけました。反乱は鎮圧され処刑されましたが、独立運動の象徴となりました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ミゲル・イダルゴを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ホセ・モレーロス": {
      "name": "ホセ・モレーロス",
      "kana": "ホセ・モレーロス",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "メキシコ独立運動を引き継いだ神父・軍事指導者です。独立、身分制の廃止、土地と税の改革を掲げました。議会を開き憲法づくりを進めましたが、捕らえられ処刑されました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ホセ・モレーロスを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アグスティン・デ・イトゥルビデ": {
      "name": "アグスティン・デ・イトゥルビデ",
      "kana": "アグスティン・デ・イトゥルビデ",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "メキシコ独立をまとめた軍人です。1821年に独立を実現しましたが、まもなく皇帝となって権力を集めました。反発で退位し、帰国後に処刑されました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "アグスティン・デ・イトゥルビデを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ペドロ1世": {
      "name": "ペドロ1世",
      "kana": "ペドロ1世",
      "era": "革命と工場の時代",
      "genre": [
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "ポルトガル王家出身で、1822年にブラジル独立を宣言した皇帝です。ポルトガルとの関係を断ち、独立国家の形を作りましたが、政治対立の中で退位しました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ペドロ1世を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エカチェリーナ2世": {
      "name": "エカチェリーナ2世",
      "kana": "エカチェリーナ2世",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "religion",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "ロシア帝国の女帝です。啓蒙思想に関心を示しながら、貴族の力を強め、農奴制を維持しました。黒海方面へ領土を広げ、ポーランド分割にも関わりました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "エカチェリーナ2世を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ピョートル1世": {
      "name": "ピョートル1世",
      "kana": "ピョートル1世",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "ロシア皇帝です。軍隊、官僚制、造船、首都サンクトペテルブルクを整え、西ヨーロッパの技術を取り入れました。強い改革でロシアを大国へ押し上げました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ピョートル1世を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フリードリヒ2世": {
      "name": "フリードリヒ2世",
      "kana": "フリードリヒ2世",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "プロイセン王です。軍隊と官僚制を整え、オーストリア継承戦争や七年戦争で領土を広げました。啓蒙専制君主と呼ばれますが、国家と軍事を強く重視しました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "フリードリヒ2世を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マリア・テレジア": {
      "name": "マリア・テレジア",
      "kana": "マリア・テレジア",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "ハプスブルク家の君主です。オーストリア継承戦争を戦いながら、税制、軍制、教育の改革を進めました。多民族帝国を維持するため、中央集権化を強めました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "マリア・テレジアを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヨーゼフ2世": {
      "name": "ヨーゼフ2世",
      "kana": "ヨーゼフ2世",
      "era": "革命と工場の時代",
      "genre": [
        "religion",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "ハプスブルク家の皇帝です。農奴制の緩和、宗教寛容令、教育改革を進めました。急な改革は貴族や各地域の反発を招き、死後に多くが取り消されました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ヨーゼフ2世を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウィリアム・ウィルバーフォース": {
      "name": "ウィリアム・ウィルバーフォース",
      "kana": "ウィリアム・ウィルバーフォース",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "イギリスの政治家です。議会で奴隷貿易廃止を長く訴え、1807年の奴隷貿易廃止法に結びつけました。奴隷制そのものの廃止運動にも関わり、人道改革を進めました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ウィリアム・ウィルバーフォースを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ハリエット・タブマン": {
      "name": "ハリエット・タブマン",
      "kana": "ハリエット・タブマン",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "medicine",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカで奴隷身分から逃れた女性です。地下鉄道と呼ばれる逃亡支援の道を使い、多くの奴隷を自由州へ導きました。南北戦争では北軍の偵察や看護にも関わりました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ハリエット・タブマンを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フレデリック・ダグラス": {
      "name": "フレデリック・ダグラス",
      "kana": "フレデリック・ダグラス",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカで奴隷身分から逃れた演説家・新聞発行者です。自伝と講演で奴隷制の残酷さを訴え、南北戦争中は黒人兵の参加と奴隷制廃止を強く求めました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "フレデリック・ダグラスを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ソジャーナ・トゥルース": {
      "name": "ソジャーナ・トゥルース",
      "kana": "ソジャーナ・トゥルース",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカの黒人女性活動家です。奴隷制廃止と女性の権利を訴え、演説で黒人女性も自由と平等の主体だと示しました。南北戦争中は解放された人々の支援にも関わりました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "ソジャーナ・トゥルースを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エイブラハム・リンカーン": {
      "name": "エイブラハム・リンカーン",
      "kana": "エイブラハム・リンカーン",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "political-scholar",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカ第16代大統領です。南北戦争で連邦の分裂を防ぎ、1863年に奴隷解放宣言を出しました。ゲティスバーグ演説で民主政治の意味を示し、奴隷制廃止を進めました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "エイブラハム・リンカーンを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "クララ・バートン": {
      "name": "クララ・バートン",
      "kana": "クララ・バートン",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "medicine",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカの看護師・人道活動家です。南北戦争で負傷兵を助け、戦後は行方不明兵の確認に努めました。1881年にアメリカ赤十字を設立し、災害救援の仕組みを広げました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "クララ・バートンを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エリザベス・キャディ・スタントン": {
      "name": "エリザベス・キャディ・スタントン",
      "kana": "エリザベス・キャディ・スタントン",
      "era": "革命と工場の時代",
      "genre": [
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカの女性権利運動家です。1848年のセネカフォールズ会議で女性の参政権を求める宣言をまとめました。奴隷制廃止運動とも関わり、法的平等を訴えました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "エリザベス・キャディ・スタントンを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スーザン・B・アンソニー": {
      "name": "スーザン・B・アンソニー",
      "kana": "スーザン・B・アンソニー",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "ヨーロッパと大西洋世界",
      "title": "革命と権利を動かした人",
      "icon": "🗽",
      "modal": {
        "profile": "アメリカの女性参政権運動家です。女性にも投票権が必要だと各地で演説し、組織づくりを進めました。生前に実現はしませんでしたが、運動は憲法修正第19条へつながりました。",
        "whatDid": "革命と権利を動かした人",
        "whyImportant": "スーザン・B・アンソニーを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ナポレオン3世": {
      "name": "ナポレオン3世",
      "kana": "ナポレオン3世",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "フランス第二帝政の皇帝です。鉄道、銀行、パリ改造を進め、産業化を後押ししました。一方で対外戦争を行い、普仏戦争で敗れて退位しました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "ナポレオン3世を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ビスマルク": {
      "name": "ビスマルク",
      "kana": "ビスマルク",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "プロイセンの首相です。デンマーク、オーストリア、フランスとの戦争を利用してドイツ統一を進めました。社会保険制度も作りましたが、強い国家と軍事を重視する政治家でした。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "ビスマルクを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ガリバルディ": {
      "name": "ガリバルディ",
      "kana": "ガリバルディ",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "イタリア統一運動の軍人です。1860年に「千人隊」を率いてシチリアとナポリを制圧し、統一への流れを作りました。民衆の支持を集めましたが、統一後も地域の差は大きく残りました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "ガリバルディを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "カヴール": {
      "name": "カヴール",
      "kana": "カヴール",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "サルデーニャ王国の首相です。外交と近代化でイタリア統一を進め、フランスの支援を得てオーストリアと戦いました。ガリバルディの動きも利用し、統一王国成立へ導きました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "カヴールを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マッツィーニ": {
      "name": "マッツィーニ",
      "kana": "マッツィーニ",
      "era": "革命と工場の時代",
      "genre": [
        "religion",
        "philosopher",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "イタリア統一をめざした思想家・革命家です。青年イタリアを作り、共和政と民族統一を訴えました。蜂起は失敗も多かったものの、統一運動の理想を広げました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "マッツィーニを知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヴィクトリア女王": {
      "name": "ヴィクトリア女王",
      "kana": "ヴィクトリア女王",
      "era": "革命と工場の時代",
      "genre": [
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "イギリス女王です。在位中に産業化と帝国拡大が進み、カナダ、インド、アフリカなどへ影響が広がりました。繁栄の象徴である一方、植民地支配の時代とも重なります。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "ヴィクトリア女王を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "伊藤博文": {
      "name": "伊藤博文",
      "kana": "伊藤博文",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "明治日本の政治家です。初代内閣総理大臣となり、大日本帝国憲法の制定に中心的に関わりました。韓国統監も務め、朝鮮半島への支配拡大にも関わりました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "伊藤博文を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "大久保利通": {
      "name": "大久保利通",
      "kana": "大久保利通",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "明治維新の中心人物です。新政府で版籍奉還、廃藩置県、殖産興業を進めました。士族反乱を抑えて中央集権国家を作りましたが、強い改革への反発も受けました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "大久保利通を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "木戸孝允": {
      "name": "木戸孝允",
      "kana": "木戸孝允",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "長州藩出身の明治維新の政治家です。五箇条の御誓文や版籍奉還に関わり、藩をなくして中央政府に力を集める改革を進めました。岩倉使節団にも参加しました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "木戸孝允を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "福沢諭吉": {
      "name": "福沢諭吉",
      "kana": "福沢諭吉",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "religion",
        "philosopher",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "明治時代の思想家・教育者です。『学問のすゝめ』で学びによる自立を説き、慶應義塾を開きました。西洋の制度や考えを紹介し、近代日本の教育に大きく影響しました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "福沢諭吉を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "渋沢栄一": {
      "name": "渋沢栄一",
      "kana": "渋沢栄一",
      "era": "革命と工場の時代",
      "genre": [
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "明治から大正の実業家です。第一国立銀行など多くの会社設立に関わり、近代的な企業経営を広げました。利益だけでなく道徳も大切にする考えを唱えました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "渋沢栄一を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "西郷隆盛": {
      "name": "西郷隆盛",
      "kana": "西郷隆盛",
      "era": "革命と工場の時代",
      "genre": [
        "politician",
        "war",
        "science"
      ],
      "field": "工業化する世界",
      "title": "科学と産業を変えた人",
      "icon": "⚙️",
      "modal": {
        "profile": "明治維新の中心人物です。江戸城無血開城に関わり、新政府の成立を助けました。のちに政府と対立し、西南戦争で士族反乱を率いて敗れました。",
        "whatDid": "科学と産業を変えた人",
        "whyImportant": "西郷隆盛を知ると、革命と工場の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エミリオ・アギナルド": {
      "name": "エミリオ・アギナルド",
      "kana": "エミリオ・アギナルド",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "フィリピン独立運動の軍人・政治家です。スペイン支配に対する革命を率い、1898年に独立を宣言しました。その後アメリカとの戦争で敗れ、植民地支配が続きました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "エミリオ・アギナルドを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スカルノ": {
      "name": "スカルノ",
      "kana": "スカルノ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "インドネシアの独立運動家で、初代大統領です。第二次世界大戦後の1945年に独立を宣言しました。多民族国家をまとめようとしましたが、政治の対立と経済の混乱も深まりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "スカルノを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ホー・チ・ミン": {
      "name": "ホー・チ・ミン",
      "kana": "ホー・チ・ミン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ベトナムの独立運動家です。フランス植民地支配に反対し、1945年に独立を宣言しました。その後の戦争を経て北ベトナムを率い、ベトナムの統一へつながる運動の中心となりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ホー・チ・ミンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "袁世凱": {
      "name": "袁世凱",
      "kana": "袁世凱",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "清末から中華民国初期の軍人・政治家です。辛亥革命後に臨時大総統となりましたが、議会政治を弱め、皇帝即位を試みて強い反発を受けました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "袁世凱を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "康有為": {
      "name": "康有為",
      "kana": "康有為",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "religion",
        "political-scholar",
        "philosopher"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "清末の改革思想家です。光緒帝を支えて戊戌の変法を進め、制度改革と立憲政治を訴えました。改革は西太后らにより短期間で失敗し、亡命しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "康有為を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "梁啓超": {
      "name": "梁啓超",
      "kana": "梁啓超",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "political-scholar",
        "philosopher"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "清末から民国期の思想家・ジャーナリストです。戊戌の変法に参加し、亡命後は新聞や著作で立憲政治、国民意識、近代教育の必要を広めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "梁啓超を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "西太后": {
      "name": "西太后",
      "kana": "西太后",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "清朝後期の実力者です。同治帝・光緒帝の時代に政治を動かし、戊戌の変法を抑えました。義和団事件後には改革も進めましたが、清の危機は深まりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "西太后を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "光緒帝": {
      "name": "光緒帝",
      "kana": "光緒帝",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "清朝の皇帝です。康有為らとともに1898年の戊戌の変法を進め、政治や教育の改革をめざしました。しかし西太后らに抑えられ、実権を失いました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "光緒帝を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "溥儀": {
      "name": "溥儀",
      "kana": "溥儀",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "清朝最後の皇帝です。辛亥革命で退位し、のちに日本の支援で満州国皇帝となりました。戦後は戦犯として収容され、晩年は一般市民として暮らしました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "溥儀を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "毛沢東": {
      "name": "毛沢東",
      "kana": "毛沢東",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "war",
        "culture"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "中国共産党の指導者です。長征と日中戦争を経て国共内戦に勝ち、1949年に中華人民共和国を建国しました。大躍進や文化大革命は社会に大きな混乱と被害をもたらしました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "毛沢東を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "周恩来": {
      "name": "周恩来",
      "kana": "周恩来",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "culture",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "中華人民共和国の首相です。建国後の外交と行政を支え、インドとの平和五原則や米中接近に関わりました。文化大革命期にも国家運営の調整役を務めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "周恩来を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "蒋介石": {
      "name": "蒋介石",
      "kana": "蒋介石",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "war",
        "political-scholar"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "中国国民党の指導者です。北伐で中国統一を進め、日中戦争では国民政府を率いました。国共内戦に敗れて台湾へ移り、長く権威主義的な統治を行いました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "蒋介石を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "宋慶齢": {
      "name": "宋慶齢",
      "kana": "宋慶齢",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "孫文の妻で、中国の政治家です。国民党右派と距離を置き、のちに中華人民共和国で国家副主席などを務めました。革命、女性、子どもの福祉に関わりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "宋慶齢を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "魯迅": {
      "name": "魯迅",
      "kana": "魯迅",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion",
        "culture"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "中国近代文学を代表する作家です。『狂人日記』『阿Q正伝』などで古い社会や人々の弱さを鋭く批判しました。中国の近代化と思想運動に大きな影響を与えました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "魯迅を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ドストエフスキー": {
      "name": "ドストエフスキー",
      "kana": "ドストエフスキー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "religion"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシアの小説家です。『罪と罰』『カラマーゾフの兄弟』などで信仰、罪、自由、貧困を描きました。近代社会の不安と人間の内面を深く表した作家です。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ドストエフスキーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トルストイ": {
      "name": "トルストイ",
      "kana": "トルストイ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war",
        "religion",
        "philosopher",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシアの小説家です。『戦争と平和』『アンナ・カレーニナ』で貴族社会、戦争、家族を描きました。晩年は非暴力や質素な生活を説き、世界の思想家にも影響しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "トルストイを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "チェーホフ": {
      "name": "チェーホフ",
      "kana": "チェーホフ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "medicine"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシアの作家・医師です。短編小説と戯曲で、地方社会の停滞や人々の小さな希望を描きました。『桜の園』などは近代演劇の表現に大きな影響を与えました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "チェーホフを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "チャイコフスキー": {
      "name": "チャイコフスキー",
      "kana": "チャイコフスキー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシアの作曲家です。『白鳥の湖』『くるみ割り人形』『悲愴』などで知られ、バレエ音楽と交響曲を発展させました。ロシア音楽を世界に広めた人物です。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "チャイコフスキーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マリー・キュリー": {
      "name": "マリー・キュリー",
      "kana": "マリー・キュリー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ポーランド出身の科学者です。夫ピエールと放射能を研究し、ポロニウムとラジウムを発見しました。物理学賞と化学賞を受け、女性科学者の道を開きました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "マリー・キュリーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ピエール・キュリー": {
      "name": "ピエール・キュリー",
      "kana": "ピエール・キュリー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "フランスの物理学者です。マリー・キュリーとともに放射能を研究し、ラジウムとポロニウムの発見に関わりました。結晶や磁性の研究でも科学に貢献しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ピエール・キュリーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "メンデル": {
      "name": "メンデル",
      "kana": "メンデル",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "オーストリアの修道士・科学者です。エンドウの交配実験から、親の性質が一定の規則で子に伝わることを示しました。遺伝学の出発点となりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "メンデルを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マクスウェル": {
      "name": "マクスウェル",
      "kana": "マクスウェル",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "スコットランドの物理学者です。電気と磁気を一つの理論としてまとめ、光も電磁波だと示しました。電波、通信、現代物理学の基礎を作りました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "マクスウェルを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "メンデレーエフ": {
      "name": "メンデレーエフ",
      "kana": "メンデレーエフ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシアの化学者です。元素を性質と重さで並べ、周期表を作りました。まだ見つかっていない元素の存在や性質を予測し、化学の整理に大きく貢献しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "メンデレーエフを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ロベルト・コッホ": {
      "name": "ロベルト・コッホ",
      "kana": "ロベルト・コッホ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ドイツの医師・細菌学者です。炭疽菌、結核菌、コレラ菌の研究で、特定の細菌が特定の病気を起こすことを示しました。近代医学と公衆衛生を進めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ロベルト・コッホを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジョゼフ・リスター": {
      "name": "ジョゼフ・リスター",
      "kana": "ジョゼフ・リスター",
      "era": "世界が深く結びついた時代",
      "genre": [
        "medicine"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "イギリスの外科医です。手術で石炭酸を使う消毒法を導入し、感染による死亡を減らしました。無菌手術へつながる考えを広め、外科医療を大きく変えました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ジョゼフ・リスターを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "グラハム・ベル": {
      "name": "グラハム・ベル",
      "kana": "グラハム・ベル",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "スコットランド生まれの発明家です。音を電気信号に変えて送る電話の実用化に関わりました。遠くの人と声で話せる通信は、仕事と生活の距離を縮めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "グラハム・ベルを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トーマス・エジソン": {
      "name": "トーマス・エジソン",
      "kana": "トーマス・エジソン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの発明家・事業家です。白熱電球、蓄音機、映画技術、電力供給の仕組みに関わりました。研究所で多くの技術者を組織し、発明を産業へ結びつけました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "トーマス・エジソンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ニコラ・テスラ": {
      "name": "ニコラ・テスラ",
      "kana": "ニコラ・テスラ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "セルビア系の発明家です。交流電流の送電やモーターの研究で知られ、電気を遠くへ送る技術に大きく貢献しました。無線や高電圧の実験も行いました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ニコラ・テスラを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ライト兄弟": {
      "name": "ライト兄弟",
      "kana": "ライト兄弟",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの兄弟発明家です。1903年に動力付き飛行機の飛行に成功しました。翼の制御を工夫し、人が空を移動する航空時代の出発点を作りました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ライト兄弟を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヘンリー・フォード": {
      "name": "ヘンリー・フォード",
      "kana": "ヘンリー・フォード",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの実業家です。流れ作業による大量生産でT型フォードを安く作り、自動車を一般家庭へ広げました。労働と消費の形を大きく変えました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ヘンリー・フォードを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フレデリック・テイラー": {
      "name": "フレデリック・テイラー",
      "kana": "フレデリック・テイラー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war",
        "culture",
        "science",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "フレデリック・テイラーは19世紀から20世紀初めの世界で帝国主義、科学、芸術、教育、労働運動、独立運動に関わりました。世界が速く結びつく中で生まれた変化を示します。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "フレデリック・テイラーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アンドリュー・カーネギー": {
      "name": "アンドリュー・カーネギー",
      "kana": "アンドリュー・カーネギー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの鉄鋼王です。製鉄業を大きく発展させ、鉄道や都市建設を支えました。巨額の富を得た後、図書館や教育へ寄付しましたが、労働争議も起きました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "アンドリュー・カーネギーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジョン・ロックフェラー": {
      "name": "ジョン・ロックフェラー",
      "kana": "ジョン・ロックフェラー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの実業家です。スタンダード石油を築き、石油精製と販売を支配しました。巨大企業の力と独占への批判を生み、反トラスト法の議論を強めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ジョン・ロックフェラーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "J・P・モルガン": {
      "name": "J・P・モルガン",
      "kana": "J・P・モルガン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの銀行家です。鉄道、鉄鋼、金融を結びつけ、大企業の再編を進めました。恐慌時には金融界をまとめましたが、少数の資本家に力が集まる問題も示しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "J・P・モルガンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジェーン・アダムズ": {
      "name": "ジェーン・アダムズ",
      "kana": "ジェーン・アダムズ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの社会改革家です。シカゴにハルハウスを作り、移民や貧しい人々の教育、保育、労働改善を支えました。平和運動にも関わり、ノーベル平和賞を受けました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ジェーン・アダムズを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エメリン・パンクハースト": {
      "name": "エメリン・パンクハースト",
      "kana": "エメリン・パンクハースト",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "イギリスの女性参政権運動家です。女性社会政治同盟を率い、投票権を求めて集会や抗議行動を行いました。激しい行動は議論を呼びましたが、参政権実現を後押ししました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "エメリン・パンクハーストを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ミリセント・フォーセット": {
      "name": "ミリセント・フォーセット",
      "kana": "ミリセント・フォーセット",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "イギリスの女性参政権運動家です。合法的な請願、演説、組織活動を重視し、女性の投票権を求めました。第一次世界大戦後の女性参政権実現に大きく貢献しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ミリセント・フォーセットを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "クララ・ツェトキン": {
      "name": "クララ・ツェトキン",
      "kana": "クララ・ツェトキン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ドイツの社会主義者・女性運動家です。働く女性の権利と参政権を訴え、国際女性デーの提案に関わりました。労働運動と女性運動を結びつけました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "クララ・ツェトキンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ローザ・ルクセンブルク": {
      "name": "ローザ・ルクセンブルク",
      "kana": "ローザ・ルクセンブルク",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ポーランド出身の社会主義者です。ドイツで反戦と労働者の権利を訴え、第一次世界大戦に反対しました。ドイツ革命後にスパルタクス団の蜂起に関わり殺害されました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ローザ・ルクセンブルクを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "レーニン": {
      "name": "レーニン",
      "kana": "レーニン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシア革命の指導者です。1917年にボリシェヴィキを率いて政権を取り、ソビエト政権を作りました。土地と平和を掲げましたが、一党支配と内戦の体制も始まりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "レーニンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "トロツキー": {
      "name": "トロツキー",
      "kana": "トロツキー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシア革命の指導者です。十月革命と赤軍の組織に大きく関わりました。スターリンとの権力争いに敗れて国外追放され、メキシコで暗殺されました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "トロツキーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スターリン": {
      "name": "スターリン",
      "kana": "スターリン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ソ連の指導者です。急速な工業化と農業集団化を進めましたが、大粛清や強制収容所で多くの人が犠牲になりました。第二次世界大戦後は東欧支配と冷戦に関わりました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "スターリンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ニコライ2世": {
      "name": "ニコライ2世",
      "kana": "ニコライ2世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "ロシア帝国最後の皇帝です。日露戦争、第一次世界大戦、国内の不満に対応できず、1917年の二月革命で退位しました。のちに家族とともに処刑されました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ニコライ2世を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ムスタファ・ケマル": {
      "name": "ムスタファ・ケマル",
      "kana": "ムスタファ・ケマル",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "トルコ共和国の建国者です。第一次世界大戦後、独立戦争を率いて共和国を成立させました。政教分離、文字改革、教育改革を進め、近代国家づくりを行いました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ムスタファ・ケマルを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "レザー・シャー": {
      "name": "レザー・シャー",
      "kana": "レザー・シャー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "イランのパフラヴィー朝を開いた国王です。軍と行政を強め、鉄道、教育、服装などの近代化を進めました。一方で反対派を抑え、第二次世界大戦中に退位させられました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "レザー・シャーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "セオドア・ルーズベルト": {
      "name": "セオドア・ルーズベルト",
      "kana": "セオドア・ルーズベルト",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカ第26代大統領です。企業独占を規制し、自然保護区を広げました。パナマ運河建設や海軍強化も進め、アメリカの海外進出を強めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "セオドア・ルーズベルトを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウッドロウ・ウィルソン": {
      "name": "ウッドロウ・ウィルソン",
      "kana": "ウッドロウ・ウィルソン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカ第28代大統領です。第一次世界大戦後に十四か条を示し、国際連盟の設立を提案しました。民族自決を掲げましたが、国内外で限界もありました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ウッドロウ・ウィルソンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フランクリン・ルーズベルト": {
      "name": "フランクリン・ルーズベルト",
      "kana": "フランクリン・ルーズベルト",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカ第32代大統領です。世界恐慌に対してニューディール政策を進め、第二次世界大戦では連合国側を支えました。長期政権で現代アメリカ政治に大きな影響を残しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "フランクリン・ルーズベルトを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エレノア・ルーズベルト": {
      "name": "エレノア・ルーズベルト",
      "kana": "エレノア・ルーズベルト",
      "era": "世界が深く結びついた時代",
      "genre": [
        "politician"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの政治活動家です。大統領夫人として社会的弱者の支援を訴え、戦後は国連で世界人権宣言の作成を主導しました。人権を国際的な基準にする仕事を進めました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "エレノア・ルーズベルトを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヘレン・ケラー": {
      "name": "ヘレン・ケラー",
      "kana": "ヘレン・ケラー",
      "era": "世界が深く結びついた時代",
      "genre": [
        "philosopher",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの作家・社会活動家です。視覚と聴覚に障害を持ちながら学び、障害者教育と福祉を訴えました。労働者、女性、平和の問題にも発言しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ヘレン・ケラーを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マーク・トウェイン": {
      "name": "マーク・トウェイン",
      "kana": "マーク・トウェイン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "アメリカの作家です。『トム・ソーヤーの冒険』『ハックルベリー・フィンの冒険』で知られます。ユーモアを使い、人種差別や社会の矛盾を描きました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "マーク・トウェインを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ゴッホ": {
      "name": "ゴッホ",
      "kana": "ゴッホ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "オランダの画家です。強い色と筆づかいで、人物や風景に感情をこめて描きました。生前は評価が低かったものの、後の表現主義や近代美術に大きな影響を与えました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ゴッホを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "モネ": {
      "name": "モネ",
      "kana": "モネ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "フランスの画家です。光や空気の変化を短い筆づかいで描き、印象派を代表しました。『睡蓮』の連作などは、見た瞬間の印象を重視する新しい絵画を広げました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "モネを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ピカソ": {
      "name": "ピカソ",
      "kana": "ピカソ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "war"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "スペイン出身の画家です。キュビスムを生み、形を分解して複数の視点から描きました。『ゲルニカ』では戦争の暴力を強く表現し、20世紀美術を変えました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ピカソを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ロダン": {
      "name": "ロダン",
      "kana": "ロダン",
      "era": "世界が深く結びついた時代",
      "genre": [
        "other"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "フランスの彫刻家です。『考える人』『カレーの市民』などで、人間の動きや苦しみを力強く表しました。近代彫刻を代表する人物です。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ロダンを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ガウディ": {
      "name": "ガウディ",
      "kana": "ガウディ",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "スペインの建築家です。バルセロナでサグラダ・ファミリアなどを設計しました。自然の形や曲線を取り入れ、石、鉄、陶器を使った独自の建築を残しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ガウディを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ココ・シャネル": {
      "name": "ココ・シャネル",
      "kana": "ココ・シャネル",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture",
        "society"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "フランスの服飾デザイナーです。動きやすい服、短い髪型、黒いドレスなどを広め、女性の服装を大きく変えました。20世紀の生活文化とファッションに影響しました。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "ココ・シャネルを知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "夏目漱石": {
      "name": "夏目漱石",
      "kana": "夏目漱石",
      "era": "世界が深く結びついた時代",
      "genre": [
        "culture"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "日本の小説家です。『吾輩は猫である』『坊っちゃん』『こころ』などで、近代化する社会の中の個人の悩みを描きました。近代日本文学を代表する人物です。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "夏目漱石を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "野口英世": {
      "name": "野口英世",
      "kana": "野口英世",
      "era": "世界が深く結びついた時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "19世紀から20世紀初めの世界",
      "title": "近代社会を動かした人",
      "icon": "🚂",
      "modal": {
        "profile": "日本の細菌学者です。黄熱病や梅毒などの研究に取り組み、アメリカや中南米でも活動しました。研究には後に修正された点もありますが、国際的に働いた医学者です。",
        "whatDid": "近代社会を動かした人",
        "whyImportant": "野口英世を知ると、世界が深く結びついた時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "昭和天皇": {
      "name": "昭和天皇",
      "kana": "昭和天皇",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本の天皇です。戦時中の意思決定への関わりや責任は歴史研究で議論が続いています。1945年の終戦後は象徴天皇制のもとで在位し、戦後日本の歩みと長く重なりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "昭和天皇を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "近衛文麿": {
      "name": "近衛文麿",
      "kana": "近衛文麿",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "戦前日本の首相です。日中戦争の拡大期と、太平洋戦争直前に内閣を率いました。外交で戦争を避ける交渉に失敗し、国内の政治体制を戦時向けにまとめる動きにも関わりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "近衛文麿を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "石原莞爾": {
      "name": "石原莞爾",
      "kana": "石原莞爾",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本陸軍の軍人です。1931年の満州事変の計画に深く関わり、日本の中国東北部への軍事進出を進めました。後に軍部と対立もしましたが、侵略の責任から切り離しては考えられません。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "石原莞爾を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "金日成": {
      "name": "金日成",
      "kana": "金日成",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "北朝鮮の建国指導者です。朝鮮民主主義人民共和国を成立させ、朝鮮戦争を始める中心となりました。戦後は個人崇拝と一党支配を強めました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "金日成を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "李承晩": {
      "name": "李承晩",
      "kana": "李承晩",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "大韓民国の初代大統領です。反共を掲げて韓国政府を作り、朝鮮戦争期に政権を率いました。長期政権と不正選挙への反発で1960年に退陣しました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "李承晩を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アンネ・フランク": {
      "name": "アンネ・フランク",
      "kana": "アンネ・フランク",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ユダヤ系ドイツ人の少女です。ナチス占領下のオランダで隠れ家生活を送り、日記に恐怖と希望を書き残しました。強制収容所で亡くなり、迫害の記憶を伝えています。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "アンネ・フランクを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "シモーヌ・ヴェイユ": {
      "name": "シモーヌ・ヴェイユ",
      "kana": "シモーヌ・ヴェイユ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "religion",
        "philosopher",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの思想家です。工場労働やスペイン内戦を経験し、抑圧、労働、信仰について考えました。第二次世界大戦中は自由フランスに関わり、亡命先で亡くなりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "シモーヌ・ヴェイユを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ハンナ・アーレント": {
      "name": "ハンナ・アーレント",
      "kana": "ハンナ・アーレント",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "religion",
        "political-scholar",
        "philosopher",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ドイツ出身の政治思想家です。ナチスの迫害を逃れ、全体主義や責任について研究しました。アイヒマン裁判の報告で、命令に従うだけの危うさを論じました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ハンナ・アーレントを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラウル・ワレンバーグ": {
      "name": "ラウル・ワレンバーグ",
      "kana": "ラウル・ワレンバーグ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "スウェーデンの外交官です。第二次世界大戦末期のハンガリーで保護証を発行し、多くのユダヤ人を救いました。戦後にソ連に拘束され、消息不明となりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ラウル・ワレンバーグを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "オスカー・シンドラー": {
      "name": "オスカー・シンドラー",
      "kana": "オスカー・シンドラー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ドイツの実業家です。ナチス支配下で工場労働者としてユダヤ人を雇い、強制収容所送りから救いました。戦時下で人命救助に動いた人物として知られます。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "オスカー・シンドラーを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "杉原千畝": {
      "name": "杉原千畝",
      "kana": "杉原千畝",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本の外交官です。1940年にリトアニアで、迫害から逃れるユダヤ人に通過ビザを発給しました。多くの命を救った行動は、戦時下の人道的判断として記憶されています。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "杉原千畝を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヤヌシュ・コルチャック": {
      "name": "ヤヌシュ・コルチャック",
      "kana": "ヤヌシュ・コルチャック",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "medicine",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ポーランドの医師・教育者です。孤児院で子どもの権利と自治を重んじました。ナチスにより子どもたちが移送される際、彼らとともにトレブリンカへ向かい殺害されました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ヤヌシュ・コルチャックを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヴィクトール・フランクル": {
      "name": "ヴィクトール・フランクル",
      "kana": "ヴィクトール・フランクル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "オーストリアの精神科医です。強制収容所を生き延び、『夜と霧』で極限状況の中でも意味を探す人間の姿を記しました。心理療法にも影響を与えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ヴィクトール・フランクルを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エディット・シュタイン": {
      "name": "エディット・シュタイン",
      "kana": "エディット・シュタイン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "religion",
        "philosopher",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ユダヤ系ドイツ人の哲学者・修道女です。ナチスの迫害でアウシュヴィッツに送られ殺害されました。学問、信仰、迫害の歴史が重なる人物です。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "エディット・シュタインを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アラン・チューリング": {
      "name": "アラン・チューリング",
      "kana": "アラン・チューリング",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリスの数学者です。第二次世界大戦中に暗号解読に関わり、戦後は計算機科学の基礎を築きました。同性愛を理由に処罰され、後に名誉回復されました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "アラン・チューリングを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジョン・フォン・ノイマン": {
      "name": "ジョン・フォン・ノイマン",
      "kana": "ジョン・フォン・ノイマン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ハンガリー出身の数学者です。ゲーム理論、量子力学、コンピューター設計に貢献しました。戦時中は原爆開発にも関わり、科学と軍事の結びつきを示します。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ジョン・フォン・ノイマンを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ロバート・オッペンハイマー": {
      "name": "ロバート・オッペンハイマー",
      "kana": "ロバート・オッペンハイマー",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの物理学者です。マンハッタン計画で原子爆弾開発を指導しました。戦後は核兵器管理を訴えましたが、冷戦下で公職から排除されました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ロバート・オッペンハイマーを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "エンリコ・フェルミ": {
      "name": "エンリコ・フェルミ",
      "kana": "エンリコ・フェルミ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イタリア出身の物理学者です。原子核と中性子の研究を進め、世界初の原子炉の実験に成功しました。原爆開発にも関わり、核時代の始まりを支えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "エンリコ・フェルミを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ニールス・ボーア": {
      "name": "ニールス・ボーア",
      "kana": "ニールス・ボーア",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "デンマークの物理学者です。原子模型を提案し、量子論の発展に大きく貢献しました。第二次世界大戦中はナチスから逃れ、核兵器の国際管理も訴えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ニールス・ボーアを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "リチャード・ファインマン": {
      "name": "リチャード・ファインマン",
      "kana": "リチャード・ファインマン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの物理学者です。量子電磁力学を発展させ、わかりやすい講義でも知られます。原爆開発に参加し、後に科学教育と自由な発想を重視しました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "リチャード・ファインマンを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "湯川秀樹": {
      "name": "湯川秀樹",
      "kana": "湯川秀樹",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本の物理学者です。中間子の存在を予言し、日本人初のノーベル賞を受けました。戦後は科学者として平和運動にも関わりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "湯川秀樹を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "朝永振一郎": {
      "name": "朝永振一郎",
      "kana": "朝永振一郎",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "日本の物理学者です。量子電磁力学の研究でノーベル賞を受けました。戦後日本の科学研究を支え、科学と平和についても発言しました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "朝永振一郎を知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "レイチェル・カーソン": {
      "name": "レイチェル・カーソン",
      "kana": "レイチェル・カーソン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの海洋生物学者・作家です。『沈黙の春』で農薬が鳥や人の環境に与える影響を示しました。科学の情報を市民に伝え、環境保護の運動と制度づくりを後押ししました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "レイチェル・カーソンを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジョージ・オーウェル": {
      "name": "ジョージ・オーウェル",
      "kana": "ジョージ・オーウェル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリスの作家です。『動物農場』『一九八四年』で独裁、監視、言葉の支配を批判しました。全体主義への警告として、現代政治を考える作品を残しました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ジョージ・オーウェルを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アーネスト・ヘミングウェイ": {
      "name": "アーネスト・ヘミングウェイ",
      "kana": "アーネスト・ヘミングウェイ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの作家です。第一次世界大戦やスペイン内戦を経験し、簡潔な文体で戦争と人間の孤独を描きました。『老人と海』などで知られます。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "アーネスト・ヘミングウェイを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "チャップリン": {
      "name": "チャップリン",
      "kana": "チャップリン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イギリス出身の映画俳優・監督です。喜劇映画で貧困や機械化社会を描き、『独裁者』ではヒトラーを風刺しました。映画を通じて社会批判を広げました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "チャップリンを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ベルトルト・ブレヒト": {
      "name": "ベルトルト・ブレヒト",
      "kana": "ベルトルト・ブレヒト",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ドイツの劇作家です。観客に社会の仕組みを考えさせる叙事的演劇を作りました。ナチスを逃れて亡命し、戦争と権力への批判を作品にこめました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ベルトルト・ブレヒトを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "パール・バック": {
      "name": "パール・バック",
      "kana": "パール・バック",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの作家です。中国で育った経験をもとに『大地』を書き、中国農村の生活を世界に紹介しました。人種差別や孤児支援にも関わりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "パール・バックを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マーガレット・ミード": {
      "name": "マーガレット・ミード",
      "kana": "マーガレット・ミード",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "culture",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "アメリカの文化人類学者です。サモアなどの調査を通じて、性別役割や成長のあり方は文化によって違うと示しました。社会の常識を見直す研究を広げました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "マーガレット・ミードを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フリーダ・カーロ": {
      "name": "フリーダ・カーロ",
      "kana": "フリーダ・カーロ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "culture",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "メキシコの画家です。自画像に身体の痛み、先住民文化、政治的な思いを描きました。女性、民族、障害をめぐる表現として後世に大きな影響を与えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "フリーダ・カーロを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ディエゴ・リベラ": {
      "name": "ディエゴ・リベラ",
      "kana": "ディエゴ・リベラ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "religion",
        "culture",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "メキシコの画家です。壁画で労働者、革命、先住民の歴史を描き、公共の場所に芸術を広げました。社会主義思想とも結びつきました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ディエゴ・リベラを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マリア・モンテッソーリ": {
      "name": "マリア・モンテッソーリ",
      "kana": "マリア・モンテッソーリ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "medicine",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "イタリアの医師・教育者です。子どもが自分で学ぶ力を重視する教育法を作りました。教材や環境を整え、世界各地の幼児教育に影響を与えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "マリア・モンテッソーリを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジャン・モネ": {
      "name": "ジャン・モネ",
      "kana": "ジャン・モネ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの実務家です。戦後ヨーロッパの石炭と鉄鋼を共同管理する構想を進めました。欧州石炭鉄鋼共同体からEUへつながる統合の基礎を作りました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ジャン・モネを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ロベール・シューマン": {
      "name": "ロベール・シューマン",
      "kana": "ロベール・シューマン",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの政治家です。1950年にシューマン宣言を出し、フランスと西ドイツの石炭・鉄鋼共同管理を提案しました。ヨーロッパ統合の出発点を作りました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ロベール・シューマンを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "クワメ・エンクルマ": {
      "name": "クワメ・エンクルマ",
      "kana": "クワメ・エンクルマ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ガーナ独立運動の指導者で、初代首相・大統領です。1957年にサハラ以南アフリカで早い独立を実現し、アフリカ統一を訴えましたが、のちに独裁化しました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "クワメ・エンクルマを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジョモ・ケニヤッタ": {
      "name": "ジョモ・ケニヤッタ",
      "kana": "ジョモ・ケニヤッタ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ケニア独立運動の指導者で、独立後の初代大統領です。イギリス植民地支配からの独立を進めましたが、政権は強い与党支配と土地問題を抱えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ジョモ・ケニヤッタを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ホセ・マルティ": {
      "name": "ホセ・マルティ",
      "kana": "ホセ・マルティ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "religion",
        "philosopher",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "キューバ独立運動の思想家・詩人です。スペイン支配からの独立を訴え、1895年の独立戦争に参加して戦死しました。ラテンアメリカの自由と反植民地主義の象徴です。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ホセ・マルティを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "レフ・ランダウ": {
      "name": "レフ・ランダウ",
      "kana": "レフ・ランダウ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ソ連の物理学者です。低温物理や量子論を発展させ、理論物理学に大きく貢献しました。スターリン体制下で逮捕された経験もあり、科学と政治の関係を示します。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "レフ・ランダウを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "アンドレイ・サハロフ": {
      "name": "アンドレイ・サハロフ",
      "kana": "アンドレイ・サハロフ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "politician",
        "war",
        "science",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "ソ連の物理学者です。水爆開発に関わった後、核軍縮と人権を訴える活動家になりました。政府批判により流刑されましたが、ノーベル平和賞を受けました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "アンドレイ・サハロフを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "シモーヌ・ド・ボーヴォワ": {
      "name": "シモーヌ・ド・ボーヴォワ",
      "kana": "シモーヌ・ド・ボーヴォワ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "philosopher",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの哲学者・作家です。『第二の性』で、女性らしさは社会によって作られる面があると論じました。現代フェミニズムに大きな影響を与えました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "シモーヌ・ド・ボーヴォワを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジャン＝ポール・サルトル": {
      "name": "ジャン＝ポール・サルトル",
      "kana": "ジャン＝ポール・サルトル",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "philosopher",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "フランスの哲学者・作家です。実存主義を代表し、人は自分の選択に責任を持つと論じました。戦後の政治運動や植民地支配への批判にも関わりました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ジャン＝ポール・サルトルを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ガブリエラ・ミストラ": {
      "name": "ガブリエラ・ミストラ",
      "kana": "ガブリエラ・ミストラ",
      "era": "二つの世界大戦の時代",
      "genre": [
        "war",
        "culture",
        "society"
      ],
      "field": "20世紀前半の世界",
      "title": "戦争と平和を動かした人",
      "icon": "🕯️",
      "modal": {
        "profile": "チリの詩人・教育者です。ラテンアメリカ女性として初めてノーベル文学賞を受けました。教育、子ども、母性、貧しさをテーマにした詩を残しました。",
        "whatDid": "戦争と平和を動かした人",
        "whyImportant": "ガブリエラ・ミストラを知ると、二つの世界大戦の時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ハーヴェイ・ミルク": {
      "name": "ハーヴェイ・ミルク",
      "kana": "ハーヴェイ・ミルク",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカのサンフランシスコ市議です。性的少数者への差別をなくす条例を進め、地域の住民を政治参加へ招きました。1978年に暗殺され、平等を求める運動の象徴となりました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ハーヴェイ・ミルクを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジェーン・グドール": {
      "name": "ジェーン・グドール",
      "kana": "ジェーン・グドール",
      "era": "今につながる時代",
      "genre": [
        "culture",
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの霊長類学者です。タンザニアのゴンベでチンパンジーを長年観察し、道具を使うことや社会関係を記録しました。研究を通じて森林と野生動物を守る活動を世界へ広げました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ジェーン・グドールを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "シルビア・アール": {
      "name": "シルビア・アール",
      "kana": "シルビア・アール",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの海洋学者です。深海調査を行い、海の生態系を守る必要を世界に訴えました。海洋保護区づくりや環境教育を進めています。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "シルビア・アールを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "グレタ・トゥーンベリ": {
      "name": "グレタ・トゥーンベリ",
      "kana": "グレタ・トゥーンベリ",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "スウェーデンの環境活動家です。学校ストライキを始め、気候変動への対策を政治家に求めました。若者の国際的な気候運動を広げました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "グレタ・トゥーンベリを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マザー・テレサ": {
      "name": "マザー・テレサ",
      "kana": "マザー・テレサ",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "カトリック修道女です。インドのコルカタで貧しい人や病人を助ける活動を行い、神の愛の宣教者会を広げました。ノーベル平和賞を受けました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "マザー・テレサを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ダニエル・エルズバーグ": {
      "name": "ダニエル・エルズバーグ",
      "kana": "ダニエル・エルズバーグ",
      "era": "今につながる時代",
      "genre": [
        "politician",
        "war"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの元政府分析官です。ベトナム戦争の実態を示すペンタゴン・ペーパーズを公表し、政府の説明への疑問を広げました。内部告発と報道の自由をめぐる議論を動かしました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ダニエル・エルズバーグを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スティーブ・ジョブズ": {
      "name": "スティーブ・ジョブズ",
      "kana": "スティーブ・ジョブズ",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの実業家です。Appleを共同創業し、Mac、iPod、iPhoneなどを通じて個人用コンピューターとスマートフォンの使い方を大きく変えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "スティーブ・ジョブズを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ビル・ゲイツ": {
      "name": "ビル・ゲイツ",
      "kana": "ビル・ゲイツ",
      "era": "今につながる時代",
      "genre": [
        "medicine"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの実業家です。Microsoftを共同創業し、Windowsなどのソフトウェアを広めました。個人用コンピューターの普及に大きく関わり、後に感染症対策などの慈善活動も行いました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ビル・ゲイツを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ティム・バーナーズ＝リー": {
      "name": "ティム・バーナーズ＝リー",
      "kana": "ティム・バーナーズ＝リー",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの計算機科学者です。1989年にWorld Wide Webを考案し、URL、HTTP、HTMLの仕組みを整えました。情報を世界で結びつける基盤を作りました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ティム・バーナーズ＝リーを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "リーナス・トーバルズ": {
      "name": "リーナス・トーバルズ",
      "kana": "リーナス・トーバルズ",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "フィンランド出身の技術者です。Linuxカーネルを公開し、世界中の開発者が協力して改良するオープンソース開発を広げました。サーバーやスマートフォンにも影響しました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "リーナス・トーバルズを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "セルゲイ・ブリン": {
      "name": "セルゲイ・ブリン",
      "kana": "セルゲイ・ブリン",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "Googleの共同創業者です。ラリー・ペイジと検索エンジンを開発し、膨大なウェブ情報を順位づけて探しやすくしました。インターネット利用の形を大きく変えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "セルゲイ・ブリンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ラリー・ペイジ": {
      "name": "ラリー・ペイジ",
      "kana": "ラリー・ペイジ",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "Googleの共同創業者です。ウェブページのつながりを使って検索結果を並べるPageRankを考案しました。検索、広告、情報サービスの広がりに大きく関わりました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ラリー・ペイジを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マーク・ザッカーバーグ": {
      "name": "マーク・ザッカーバーグ",
      "kana": "マーク・ザッカーバーグ",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "Facebookの創業者です。大学内の交流サイトから始め、世界的なSNSへ広げました。人々のつながり方を変えた一方、個人情報や情報操作の問題も生みました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "マーク・ザッカーバーグを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "イーロン・マスク": {
      "name": "イーロン・マスク",
      "kana": "イーロン・マスク",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "実業家です。Teslaで電気自動車、SpaceXで民間宇宙開発を広げました。Xなど情報サービスにも関わり、技術革新と巨大企業の影響力をめぐる議論を生んでいます。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "イーロン・マスクを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "キャサリン・ジョンソン": {
      "name": "キャサリン・ジョンソン",
      "kana": "キャサリン・ジョンソン",
      "era": "今につながる時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの数学者です。NASAで宇宙船の軌道計算を担当し、マーキュリー計画やアポロ計画を支えました。黒人女性科学者の貢献を示す人物です。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "キャサリン・ジョンソンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "メイ・ジェミソン": {
      "name": "メイ・ジェミソン",
      "kana": "メイ・ジェミソン",
      "era": "今につながる時代",
      "genre": [
        "science",
        "medicine",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの医師・宇宙飛行士です。1992年に黒人女性として初めて宇宙へ行きました。科学教育や多様な人が宇宙開発に参加することの大切さを伝えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "メイ・ジェミソンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "サリー・ライド": {
      "name": "サリー・ライド",
      "kana": "サリー・ライド",
      "era": "今につながる時代",
      "genre": [
        "science",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの宇宙飛行士です。1983年にアメリカ人女性として初めて宇宙へ行きました。退役後は科学教育を進め、子どもや女性の理科教育を支えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "サリー・ライドを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ワレンチナ・テレシコワ": {
      "name": "ワレンチナ・テレシコワ",
      "kana": "ワレンチナ・テレシコワ",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ソ連の宇宙飛行士です。1963年に女性として初めて宇宙へ行きました。冷戦下の宇宙開発競争を象徴し、女性の宇宙参加の道を開きました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ワレンチナ・テレシコワを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ユーリイ・ガガーリン": {
      "name": "ユーリイ・ガガーリン",
      "kana": "ユーリイ・ガガーリン",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ソ連の宇宙飛行士です。1961年に人類で初めて宇宙飛行を行い、地球を一周しました。冷戦時代の宇宙開発競争でソ連の技術力を示しました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ユーリイ・ガガーリンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ニール・アームストロング": {
      "name": "ニール・アームストロング",
      "kana": "ニール・アームストロング",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの宇宙飛行士です。1969年のアポロ11号で月面に降り立った最初の人です。月面着陸は冷戦下の宇宙開発と科学技術の象徴となりました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ニール・アームストロングを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "スティーブン・ホーキング": {
      "name": "スティーブン・ホーキング",
      "kana": "スティーブン・ホーキング",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの理論物理学者です。ブラックホールと宇宙の始まりを研究し、一般向けの本で宇宙論を広めました。難病とともに研究を続けた科学者としても知られます。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "スティーブン・ホーキングを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ドロシー・ホジキン": {
      "name": "ドロシー・ホジキン",
      "kana": "ドロシー・ホジキン",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの化学者です。X線結晶解析でペニシリン、ビタミンB12、インスリンの構造を解明しました。薬や生命科学の発展に大きく貢献しました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ドロシー・ホジキンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ロザリンド・フランクリン": {
      "name": "ロザリンド・フランクリン",
      "kana": "ロザリンド・フランクリン",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの科学者です。X線写真でDNAの二重らせん構造解明に重要な証拠を示しました。生前は十分な評価を受けにくく、科学史で再評価されています。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ロザリンド・フランクリンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "チエン・シゥン・ウー": {
      "name": "チエン・シゥン・ウー",
      "kana": "チエン・シゥン・ウー",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "中国出身の物理学者です。弱い力の実験で、自然界の左右対称性が破れることを示しました。ノーベル賞の対象から外れたことも、科学界の評価の問題として語られます。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "チエン・シゥン・ウーを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジェームズ・ワトソン": {
      "name": "ジェームズ・ワトソン",
      "kana": "ジェームズ・ワトソン",
      "era": "今につながる時代",
      "genre": [
        "philosopher",
        "science",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの生物学者です。フランシス・クリックとDNAの二重らせん構造を提案しました。生命科学を大きく進めましたが、研究倫理や差別的発言への批判もあります。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ジェームズ・ワトソンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "フランシス・クリック": {
      "name": "フランシス・クリック",
      "kana": "フランシス・クリック",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの生物学者です。ワトソンとDNAの二重らせん構造を提案し、遺伝情報の仕組みを理解する道を開きました。分子生物学の発展に大きく関わりました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "フランシス・クリックを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マーシャル・ニーレンバーグ": {
      "name": "マーシャル・ニーレンバーグ",
      "kana": "マーシャル・ニーレンバーグ",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの生化学者です。遺伝暗号の解読に成功し、DNAの情報がどのようにタンパク質へ変わるかを明らかにしました。生命科学の基礎を築きました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "マーシャル・ニーレンバーグを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "山中伸弥": {
      "name": "山中伸弥",
      "kana": "山中伸弥",
      "era": "今につながる時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本の医学者です。体の細胞をiPS細胞へ変える方法を発見し、再生医療研究を大きく進めました。病気の研究や治療法開発への応用が期待されています。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "山中伸弥を知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "大隅良典": {
      "name": "大隅良典",
      "kana": "大隅良典",
      "era": "今につながる時代",
      "genre": [
        "science"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本の生物学者です。細胞が不要なものを分解して再利用するオートファジーの仕組みを研究しました。生命維持や病気の理解に大きく貢献しました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "大隅良典を知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "本庶佑": {
      "name": "本庶佑",
      "kana": "本庶佑",
      "era": "今につながる時代",
      "genre": [
        "science",
        "medicine"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本の医学者です。免疫の働きを抑えるPD-1を発見し、がん免疫療法の開発につなげました。がん治療の新しい道を開いた研究者です。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "本庶佑を知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "黒澤明": {
      "name": "黒澤明",
      "kana": "黒澤明",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本の映画監督です。『羅生門』『七人の侍』などで国際的に高く評価されました。映像表現と物語の作り方は世界の映画監督に影響を与えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "黒澤明を知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "宮崎駿": {
      "name": "宮崎駿",
      "kana": "宮崎駿",
      "era": "今につながる時代",
      "genre": [
        "war"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本のアニメーション映画監督です。『風の谷のナウシカ』『千と千尋の神隠し』などで、自然、戦争、成長を描きました。日本アニメを世界に広げました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "宮崎駿を知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "手塚治虫": {
      "name": "手塚治虫",
      "kana": "手塚治虫",
      "era": "今につながる時代",
      "genre": [
        "culture"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本の漫画家・アニメ作家です。『鉄腕アトム』『火の鳥』などで、物語性のある漫画とテレビアニメを発展させました。戦後日本の大衆文化に大きな影響を与えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "手塚治虫を知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ボブ・ディラン": {
      "name": "ボブ・ディラン",
      "kana": "ボブ・ディラン",
      "era": "今につながる時代",
      "genre": [
        "culture"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの歌手・作詞家です。公民権運動や反戦運動の時代に、社会への問いをこめた歌を発表しました。歌詞の文学性が評価されノーベル文学賞を受けました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ボブ・ディランを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ジョン・レノン": {
      "name": "ジョン・レノン",
      "kana": "ジョン・レノン",
      "era": "今につながる時代",
      "genre": [
        "war",
        "culture",
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "イギリスの音楽家です。ビートルズの一員として世界の音楽文化を変え、解散後は平和運動にも関わりました。『イマジン』などで戦争のない世界を訴えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ジョン・レノンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マイケル・ジャクソン": {
      "name": "マイケル・ジャクソン",
      "kana": "マイケル・ジャクソン",
      "era": "今につながる時代",
      "genre": [
        "culture"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカの歌手・ダンサーです。『スリラー』などで音楽、ダンス、映像表現を結びつけ、ポップ音楽を世界化しました。人種をこえた大衆文化の象徴にもなりました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "マイケル・ジャクソンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ペレ": {
      "name": "ペレ",
      "kana": "ペレ",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ブラジルのサッカー選手です。ワールドカップで三度優勝し、サッカーを世界的な人気スポーツへ押し上げました。ブラジルの国民的英雄としても知られます。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ペレを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "モハメド・アリ": {
      "name": "モハメド・アリ",
      "kana": "モハメド・アリ",
      "era": "今につながる時代",
      "genre": [
        "war",
        "religion"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカのボクサーです。世界ヘビー級王者となり、ベトナム戦争への徴兵を拒否しました。黒人の誇り、信仰、反戦を公に語ったスポーツ選手です。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "モハメド・アリを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "セリーナ・ウィリアムズ": {
      "name": "セリーナ・ウィリアムズ",
      "kana": "セリーナ・ウィリアムズ",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカのテニス選手です。多くの四大大会で優勝し、女子テニスの競技水準を押し上げました。人種差別や女性選手への評価の問題にも発言しました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "セリーナ・ウィリアムズを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "大坂なおみ": {
      "name": "大坂なおみ",
      "kana": "大坂なおみ",
      "era": "今につながる時代",
      "genre": [
        "society"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "日本のテニス選手です。四大大会で優勝し、国際的に活躍しました。人種差別への抗議やメンタルヘルスについて発言し、スポーツ選手の社会的発信を広げました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "大坂なおみを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ウサイン・ボルト": {
      "name": "ウサイン・ボルト",
      "kana": "ウサイン・ボルト",
      "era": "今につながる時代",
      "genre": [
        "culture"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "ジャマイカの陸上選手です。100メートルと200メートルで世界記録を出し、オリンピックで何度も優勝しました。短距離走の歴史を大きく変えました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "ウサイン・ボルトを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "リオネル・メッシ": {
      "name": "リオネル・メッシ",
      "kana": "リオネル・メッシ",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アルゼンチンのサッカー選手です。クラブと代表で多くの得点とタイトルを重ね、2022年にはワールドカップ優勝に貢献しました。現代サッカーを代表する選手です。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "リオネル・メッシを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "マイケル・ジョーダン": {
      "name": "マイケル・ジョーダン",
      "kana": "マイケル・ジョーダン",
      "era": "今につながる時代",
      "genre": [
        "other"
      ],
      "field": "戦後から現代の世界",
      "title": "現代社会を動かした人",
      "icon": "🌐",
      "modal": {
        "profile": "アメリカのバスケットボール選手です。シカゴ・ブルズでNBA優勝を重ね、世界的なスポーツスターとなりました。競技、広告、ファッションを結びつける存在になりました。",
        "whatDid": "現代社会を動かした人",
        "whyImportant": "マイケル・ジョーダンを知ると、今につながる時代の大きな流れを具体的な人物の行動から考えられます。"
      }
    },
    "ヒポクラテス": {
      "name": "ヒポクラテス",
      "kana": "ヒポクラテス",
      "era": "古代の大国の時代",
      "genre": [
        "medicine"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前5～4世紀ごろのギリシャの医師で、コス島で活動したと伝えられます。病気を神罰だけで説明せず、患者の食事、生活、気候、症状の変化を観察して治療する姿勢と結び付けられています。『ヒポクラテス全集』は一人の著作ではなく、複数の医師による文書集です。体液説など現代医学では誤りとなった考えも含みますが、診察記録、予後判断、医師の倫理を重視しました。後世の「ヒポクラテスの誓い」は医療倫理の象徴となりました。",
        "whatDid": "病気を自然現象として観察し、症状・生活・環境の記録と医師の倫理を重んじる医学を発展させました。",
        "whyImportant": "病気を超自然的説明から切り離し、観察と倫理に基づく西洋医学の伝統を築いたためです。"
      },
      "image": "assets/people/person-fcd6694e3d.webp",
      "imageAlt": "ヒポクラテスの画像"
    },
    "ユークリッド": {
      "name": "ユークリッド",
      "kana": "ユークリッド",
      "era": "古代の大国の時代",
      "genre": [
        "science"
      ],
      "field": "古代ギリシャ",
      "title": "政治と学問を広げた人",
      "icon": "🏛️",
      "modal": {
        "profile": "紀元前3世紀ごろ、エジプトのアレクサンドリアで活動した数学者です。著書『原論』で、点・線・角などの定義と、証明せずに出発点とする公理・公準を置き、そこから命題を論理的に証明しました。平面幾何だけでなく、比例、数論、無理数、立体も扱いました。内容のすべてを本人が発見したわけではなく、先行するギリシャ数学を整理したものです。それでも、少数の前提から体系を築く方法は二千年以上にわたり数学教育と論理的思考の模範となりました。",
        "whatDid": "『原論』で定義・公理・公準から命題を証明する形に、幾何学と数論を体系化しました。",
        "whyImportant": "数学を論理的証明の体系として整え、二千年以上にわたり科学と数学教育の基礎となったためです。"
      },
      "image": "assets/people/person-838a2f0c28.webp",
      "imageAlt": "ユークリッドの画像"
    },
    "コンスタンティヌス帝": {
      "name": "コンスタンティヌス帝",
      "kana": "コンスタンティヌスてい",
      "era": "古代の大国の時代",
      "genre": [
        "monarch",
        "politician",
        "religion"
      ],
      "field": "ローマと地中海",
      "title": "キリスト教を公認したローマ皇帝",
      "icon": "🏛️",
      "aliases": [
        "コンスタンティヌス1世"
      ],
      "modal": {
        "profile": "ローマ帝国の皇帝で、313年のミラノ勅令によってキリスト教を公認しました。内戦を勝ち抜いて帝国の統一を進め、325年にはニカイア公会議を開いて、キリスト教の教義をめぐる対立を整理しようとしました。また、ビザンティウムを整備してコンスタンティノープルとし、後の東ローマ帝国の中心となる都市の基礎を築きました。",
        "whatDid": "ミラノ勅令でキリスト教を公認し、ニカイア公会議を開いて教会のまとまりを支えました。コンスタンティノープルも整備しました。",
        "whyImportant": "ローマ帝国とキリスト教の関係を大きく変え、ヨーロッパの宗教・政治・都市の歴史に長く影響したためです。"
      },
      "image": "assets/people/person-8efd9d5df5.webp",
      "imageAlt": "コンスタンティヌス帝の画像"
    }
  },
  "genreGroups": [
    {
      "id": "monarch",
      "label": "国王・権力者"
    },
    {
      "id": "politician",
      "label": "政治家"
    },
    {
      "id": "war",
      "label": "軍事・戦争"
    },
    {
      "id": "religion",
      "label": "宗教・思想"
    },
    {
      "id": "culture",
      "label": "文化・芸術"
    },
    {
      "id": "political-scholar",
      "label": "政治学者"
    },
    {
      "id": "philosopher",
      "label": "哲学者"
    },
    {
      "id": "science",
      "label": "科学者"
    },
    {
      "id": "medicine",
      "label": "医療・看護"
    },
    {
      "id": "society",
      "label": "産業・社会"
    },
    {
      "id": "other",
      "label": "そのほか"
    }
  ],
  "inlineAliases": {
    "コロンブス": "クリストファー・コロンブス",
    "マゼラン": "フェルディナンド・マゼラン",
    "ダ・ガマ": "ヴァスコ・ダ・ガマ",
    "ディアス": "バルトロメウ・ディアス",
    "ルター": "マルティン・ルター",
    "ワシントン": "ジョージ・ワシントン",
    "リンカーン": "エイブラハム・リンカーン",
    "ガンディー": "マハトマ・ガンディー",
    "マンデラ": "ネルソン・マンデラ",
    "ケネディ": "ジョン・F・ケネディ",
    "ダーウィン": "チャールズ・ダーウィン",
    "エジソン": "トーマス・エジソン",
    "アインシュタイン": "アルベルト・アインシュタイン",
    "アームストロング": "ニール・アームストロング",
    "コンスタンティヌス1世": "コンスタンティヌス帝"
  }
}
